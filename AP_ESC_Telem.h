...
private:
    ...
    //@yu Ti params
    AP_Int8 error_count_max;//To avoid esc error count too large
    //AP_Int8 servo2esc;
    AP_Int16 max_acceptable_temperature;//To avoid esc error count too large
    AP_Int32 esc_timeout;//if esc lost over this value, then alarm gcs
    //@yu overheat alarm
    int16_t over_heat_esc_index[ESC_TELEM_MAX_ESCS] = {0};
    bool _have_over_heat;
    uint32_t over_heat_alarm_delay_count = 0;
    //@yu timeout alarm
    uint32_t esc_lost_alarm_ms = 0;
    uint32_t esc_lost_need_alarm_mask = 0;
    //uint32_t esc_log_neglect_mask = 0;
