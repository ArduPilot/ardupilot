...
void ModeAuto::auto_terrain_recover_run()
{
    ...
    if (!motors.armed()) {
        ...
        position_control->relax_D_controller(motors.get_throttle_hover());  // Reset z axis controller
        return;
    }
    ...
    case RangeFinder::Status::Good: // exit on success (recovered rangefinder data)
        ...
        if (sub.rangefinder_state.alt_healthy) {
            ...
            if (rangefinder_recovery_ms == 0) {
                ...
                position_control->relax_D_controller(motors.get_throttle_hover()); // Reset alt hold targets
            }
