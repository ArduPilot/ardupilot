...
void Sub::failsafe_gcs_check()
{
    ...
    // Check if we have gotten a GCS heartbeat recently (GCS sysid must match MAV_MYGCS_SYSID parameter)
    ...
    if (tnow - failsafe.last_gcs_warn_ms > 30000) {
        ...
        gcs().send_text(MAV_SEVERITY_WARNING, "MYGCS: %u, heartbeat lost", unsigned(gcs().sysid_mygcs()));
    }
