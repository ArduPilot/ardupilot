...
void Sub::mainloop_failsafe_disable() { failsafe_enabled = false; }
...
void Sub::mainloop_failsafe_check() {
  ...
  if (!in_failsafe && failsafe_enabled &&
    ...
    // disarm the motors.
    in_failsafe = true;
    // reduce motors to minimum (we do not immediately disarm because we want to
    // log the failure)
    if (motors.armed()) {
      motors.output_min();
    }
...
void Sub::failsafe_sensors_check() {
  ...
  if (control_mode == Mode::Number::ALT_HOLD ||
      control_mode == Mode::Number::SURFACE || sub.flightmode->requires_GPS()) {
...
void Sub::failsafe_pilot_input_check() {
  ...
  if (AP_HAL::millis() <
      failsafe.last_pilot_input_ms + g.failsafe_pilot_input_timeout * 1000.0f) {
    failsafe.pilot_input = false; // We've received an update from the pilot
...
// External pressure failsafe check
// Check if the external pressure (depth) has exceeded the maximum specified by
// the FS_PRESS_EXT_MAX parameter
void Sub::failsafe_external_pressure_check() {
  // Check if enabled
  if (g.failsafe_pressure_ext == FS_PRESS_EXT_DISABLED) {
    failsafe.external_pressure = false;
    return;
  }

  // Find water barometer
  AP_Baro &baro = AP::baro();
  int8_t baro_idx = -1;

  // Check primary first
  if (baro.get_type(baro.get_primary()) == AP_Baro::BARO_TYPE_WATER) {
    baro_idx = baro.get_primary();
  } else {
    // Find first water baro
    for (uint8_t i = 0; i < baro.num_instances(); i++) {
      if (baro.get_type(i) == AP_Baro::BARO_TYPE_WATER) {
        baro_idx = i;
        break;
      }
    }
  ...
  if (baro_idx == -1) {
    return; // No water barometer found
  }
  ...
  // Handle actions
  if (g.failsafe_pressure_ext == FS_PRESS_EXT_SURFACE) {
    if (control_mode != Mode::Number::SURFACE) {
      set_mode(Mode::Number::SURFACE, ModeReason::EXTERNAL_PRESSURE_FAILSAFE);
    }
  } else if (g.failsafe_pressure_ext == FS_PRESS_EXT_DEPTH_HOLD) {
    if (control_mode != Mode::Number::ALT_HOLD &&
        control_mode != Mode::Number::POSHOLD) {
      set_mode(Mode::Number::ALT_HOLD, ModeReason::EXTERNAL_PRESSURE_FAILSAFE);
    }
  }
}
...
void Sub::failsafe_leak_check() {
  ...
  // Do nothing if we have already triggered the failsafe action, or if the
  // motors are disarmed
  if (failsafe.leak) {
    return;
  }
...
void Sub::failsafe_gcs_check() {
  // return immediately if we have never had contact with a gcs, or if gcs
  // failsafe action is disabled this also checks to see if we have a GCS
  // failsafe active, if we do, then must continue to process the logic for
  ...
  // Check if we have gotten a GCS heartbeat recently (GCS sysid must match
  // MAV_GCS_SYSID parameter)
  const uint32_t gcs_timeout_ms = uint32_t(
      constrain_float(g.failsafe_gcs_timeout * 1000.0f, 0.0f, UINT32_MAX));
...
#define CRASH_CHECK_ANGLE_DEVIATION_DEG                                        \
  30.0f // 30 degrees beyond angle max is signal we are inverted

// Check for a crash
// The vehicle is considered crashed if the angle error exceeds a specified
// limit for more than 2 seconds
...
//  missing_data should be set to true if the vehicle failed to navigate
//  because of missing data, false if navigation is proceeding successfully
...
// Terrain failure occurs when terrain data is not found, or rangefinder is
// not enabled or healthy set terrain data status (found or not found)
...
void Sub::failsafe_radio_off_event() {
  ...
  // user can now override roll, pitch, yaw and throttle and even use flight
  // mode switch to restore previous flight mode
  LOGGER_WRITE_ERROR(LogErrorSubsystem::FAILSAFE_RADIO,
                     LogErrorCode::FAILSAFE_RESOLVED);
