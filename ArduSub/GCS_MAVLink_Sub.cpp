...
bool GCS_MAVLINK_Sub::send_info()
{
    ...
    if (sub.control_mode == Mode::Number::SURFTRAK) {
        CHECK_PAYLOAD_SIZE(NAMED_VALUE_FLOAT);
        send_named_float("RFTarget", sub.mode_surftrak.get_rangefinder_target_cm() * 0.01f);
    } else if (sub.control_mode == Mode::Number::GUIDED) {
        CHECK_PAYLOAD_SIZE(NAMED_VALUE_FLOAT);
        // Sends 0 in unsupported sub-modes
        send_named_float("RFTarget", sub.mode_guided.get_rangefinder_target_cm() * 0.01f);
    }
