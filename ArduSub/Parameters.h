...
public:
  ...
  enum {
    ...
    k_param_failsafe_pressure_max, // maximum internal pressure in pascal before
    ...
    k_param_xtrack_angle_limit, // Angle limit for crosstrack correction in Auto
                                // modes (degrees)
    ...
    k_param_acro_yaw_p =
        220, // Used in all modes for get_pilot_desired_yaw_rate
  ...
  AP_Int8 rangefinder_signal_min; // minimum signal quality for good rangefinder
                                  // readings
  ...
  AP_Int8 wp_yaw_behavior; // controls how the autopilot controls yaw during
                           // missions
  AP_Int8 rc_feel_rp; // controls vehicle response to user input with 0 being
                      // extremely soft and 100 begin extremely crisp

  // Waypoints
  //
  AP_Int16 pilot_speed_up; // maximum vertical ascending velocity the pilot may
                           // request
  AP_Int16 pilot_speed_dn; // maximum vertical descending velocity the pilot may
                           // request
  AP_Int16
      pilot_speed; // maximum horizontal (xy) velocity the pilot may request
...
static const struct AP_Param::defaults_table_struct defaults_table[] = {
    {"BRD_SAFETY_DEFLT", 0},
    {"ARMING_SKIPCHK",
     (~(uint32_t(AP_Arming::Check::RC) | uint32_t(AP_Arming::Check::VOLTAGE) |
