...
Sub::Sub()
    :
...
#endif
      motors(MAIN_LOOP_RATE), auto_yaw_mode(AUTO_YAW_LOOK_AT_NEXT_WP),
      ...
      circle_nav(ahrs_view, pos_control), param_loader(var_info),
      flightmode(&mode_manual), auto_mode(Auto_WP), guided_mode(Guided_WP) {
...
#define SCHED_TASK(func, rate_hz, max_time_micros, priority)                   \
  SCHED_TASK_CLASS(Sub, &sub, func, rate_hz, max_time_micros, priority)
...
#if AP_SUB_RC_ENABLED
    SCHED_TASK_CLASS(RC_Channels, (RC_Channels *)&sub.g2.rc_channels,
...
void Sub::run_rate_controller() {
  ...
  if (control_mode != Mode::Number::MANUAL &&
    ...
    // run low level rate controllers that only require IMU data and set loop
    // time
...
void Sub::ten_hz_logging_loop() {
  ...
  if (should_log(MASK_LOG_ATTITUDE_MED) &&
    ...
    if (should_log(MASK_LOG_PID)) {
      logger.Write_PID(LOG_PIDR_MSG,
                       attitude_control.get_rate_roll_pid().get_pid_info());
      logger.Write_PID(LOG_PIDP_MSG,
                       attitude_control.get_rate_pitch_pid().get_pid_info());
      logger.Write_PID(LOG_PIDY_MSG,
                       attitude_control.get_rate_yaw_pid().get_pid_info());
      logger.Write_PID(LOG_PIDA_MSG,
                       pos_control.D_get_accel_pid().get_pid_info());
    }
  }
  ...
  if (should_log(MASK_LOG_NTUN) &&
      (sub.flightmode->requires_GPS() || sub.flightmode->requires_altitude())) {
...
void Sub::three_hz_loop() {
  ...
  failsafe_external_pressure_check();
...
void Sub::one_hz_loop() {
  ...
  pos_control.D_get_accel_pid().set_notch_sample_rate(
      AP::scheduler().get_filtered_loop_rate_hz());
}
...
void Sub::read_AHRS() {
  ...
  //-----------------------------------------------
  // <true> tells AHRS to skip INS update as we have already done it in
  // fast_loop()
  ahrs.update(true);
  ahrs_view.update();
}
...
bool Sub::get_wp_crosstrack_error_m(float &xtrack_error) const {
  // no crosstrack error reported, see
  // GCS_MAVLINK_Sub::send_nav_controller_output()
  xtrack_error = 0;
  return true;
}
...
/*
  update AP_Stats
*/
void Sub::stats_update(void) { AP::stats()->set_flying(motors.armed()); }
#endif
...
bool Sub::ensure_ekf_origin() {
  ...
  auto backup_origin =
      Location(static_cast<int32_t>(sub.g2.backup_origin_lat * 1e7),
  ...
  if (backup_origin.lat == 0 || backup_origin.lng == 0) {
    gcs().send_text(MAV_SEVERITY_WARNING,
                    "Backup location parameters are missing or zero");
    return false;
  }

  if (!check_latlng(backup_origin.lat, backup_origin.lng)) {
    gcs().send_text(MAV_SEVERITY_WARNING,
                    "Backup location parameters are not valid");
    return false;
  }
  ...
  if (!ahrs.set_origin(backup_origin)) {
    ...
    gcs().send_text(MAV_SEVERITY_WARNING,
                    "Failed to set origin, check EK3_SRC parameters");
    return false;
  }
