...
void Sub::update_surface_and_bottom_detector()
{
    ...
    if (ap.depth_sensor_present && sensor_health.depth) { // we can use the external pressure sensor for a very accurate and current measure of our z axis position
        ...
        if (ap.at_surface) {
            set_surfaced(current_depth > g.surface_depth*0.01f - 0.05); // add a 5cm buffer so it doesn't trigger too often
        } else {
            set_surfaced(current_depth > g.surface_depth*0.01f); // If we are above surface depth, we are surfaced
        }
