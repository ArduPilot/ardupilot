...
static Vector3p posvelaccel_pos_target_cm;
static Vector3f posvelaccel_vel_target_cms;
static Vector3f posvelaccel_accel_target_cmss;
...
bool ModeGuided::guided_set_destination(const Vector3f& destination)
{
    ...
    // log target
    sub.Log_Write_GuidedTarget(sub.guided_mode, destination, Vector3f(), Vector3f());
#endif
...
bool ModeGuided::guided_set_posvelaccel(const Vector3f& destination, const Vector3f& velocity, const Vector3f& accel)
{
#if AP_FENCE_ENABLED
    // reject destination if outside the fence
    const Location dest_loc(destination, Location::AltFrame::ABOVE_ORIGIN);
    ...
    update_time_ms = AP_HAL::millis();
    posvelaccel_pos_target_cm = destination.topostype();
    posvelaccel_vel_target_cms = velocity;
    posvelaccel_accel_target_cmss = accel;

    position_control->input_pos_vel_accel_NE_cm(posvelaccel_pos_target_cm.xy(), posvelaccel_vel_target_cms.xy(), posvelaccel_accel_target_cmss.xy());
    float dz = posvelaccel_pos_target_cm.z;
    position_control->input_pos_vel_accel_U_cm(dz, posvelaccel_vel_target_cms.z, posvelaccel_accel_target_cmss.z);
    posvelaccel_pos_target_cm.z = dz;

#if HAL_LOGGING_ENABLED
    // log target
    sub.Log_Write_GuidedTarget(sub.guided_mode, destination, velocity, accel);
#endif
...
// set guided mode posvelaccel target
bool ModeGuided::guided_set_posvelaccel(const Vector3f& destination, const Vector3f& velocity, const Vector3f& accel, bool use_yaw, float yaw_cd, bool use_yaw_rate, float yaw_rate_cds, bool relative_yaw)
{
    #if AP_FENCE_ENABLED
    // reject destination if outside the fence
    const Location dest_loc(destination, Location::AltFrame::ABOVE_ORIGIN);
...
void ModeGuided::guided_posvelaccel_control_run()
{
    ...
    // set velocity to zero if no updates received for 3 seconds
    uint32_t tnow = AP_HAL::millis();
    if (tnow - update_time_ms > GUIDED_VEL_TIMEOUT_MS && !posvelaccel_vel_target_cms.is_zero()) {
        posvelaccel_vel_target_cms.zero();
        posvelaccel_accel_target_cmss.zero();
    }

    // send position and velocity targets to position controller
    position_control->input_pos_vel_accel_NE_cm(posvelaccel_pos_target_cm.xy(), posvelaccel_vel_target_cms.xy(), posvelaccel_accel_target_cmss.xy());
    float pz = posvelaccel_pos_target_cm.z;
    position_control->input_pos_vel_accel_U_cm(pz, posvelaccel_vel_target_cms.z, posvelaccel_accel_target_cmss.z);
    posvelaccel_pos_target_cm.z = pz;
