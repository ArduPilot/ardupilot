...
void Sub::do_set_roi(const AP_Mission::Mission_Command& cmd)
{
    ...
    const uint8_t instance = camera_mount.get_primary_instance();
    sub.mode_auto.set_auto_yaw_roi(instance, cmd.content.location);
}

// MAV_CMD_DO_SET_ROI_LOCATION (195)
void Sub::do_set_roi_location(const AP_Mission::Mission_Command& cmd)
{
    sub.mode_auto.set_auto_yaw_roi(camera_mount.get_specified_instance(cmd.p1), cmd.content.location);
}
#endif // HAL_MOUNT_ENABLED
...
void Sub::do_mount_control(const AP_Mission::Mission_Command& cmd)
{
    ...
    // Because MAVLink does not specify which sensor(s) is contolled by this command, the decision is made here.
    const uint8_t instance = camera_mount.get_primary_instance();
    camera_mount.set_angle_target(instance, cmd.content.mount_control.roll, cmd.content.mount_control.pitch, cmd.content.mount_control.yaw, false);
#endif
}
