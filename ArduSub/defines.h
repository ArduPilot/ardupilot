...
enum autopilot_yaw_mode {
  ...
  AUTO_YAW_ROI =
      2, // point towards a location held in roi_WP (no pilot input accepted)
  AUTO_YAW_LOOK_AT_HEADING =
      3, // point towards a particular angle (not pilot input accepted)
  ...
  AUTO_YAW_CORRECT_XTRACK = 6, // steer the sub in order to correct for
                               // crosstrack error during line following
...
// Yaw behaviours during missions - possible values for WP_YAW_BEHAVIOR
// parameter
#define WP_YAW_BEHAVIOR_NONE                                                   \
  0 // auto pilot will never control yaw during missions or rtl (except for
    // DO_CONDITIONAL_YAW command received)
#define WP_YAW_BEHAVIOR_LOOK_AT_NEXT_WP                                        \
  1 // auto pilot will face next waypoint or home during rtl
#define WP_YAW_BEHAVIOR_LOOK_AT_NEXT_WP_EXCEPT_RTL                             \
  2 // auto pilot will face next waypoint except when doing RTL at which time it
    // will stay in it's last
#define WP_YAW_BEHAVIOR_LOOK_AHEAD                                             \
  3 // auto pilot will look ahead during missions and rtl (primarily meant for
    // traditional helicopters)
#define WP_YAW_BEHAVIOR_CORRECT_XTRACK                                         \
  4 // point towards intermediate position target during line following
...
#define FS_GCS_TIMEOUT_S                                                       \
  5.0 // gcs failsafe triggers after this number of seconds with no GCS
      // heartbeat
#endif
...
#define FS_TERRAIN_TIMEOUT_MS                                                  \
  1000 // 1 second of unhealthy rangefinder and/or missing terrain data will
       // trigger failsafe
#endif
...
#define FS_EKF_THRESHOLD_DEFAULT                                               \
  0.8f // EKF failsafe's default compass and velocity variance threshold above
       // which the EKF failsafe will be triggered
#endif

// GCS failsafe definitions (FS_GCS_ENABLE parameter)
#define FS_GCS_DISABLED 0 // Disabled
#define FS_GCS_WARN_ONLY                                                       \
  1 // Only send warning to gcs (only useful with multiple gcs links)
...
#define FS_PRESS_MAX_DEFAULT                                                   \
  105000 // Maximum internal pressure in pascal before failsafe is triggered
// Internal pressure failsafe definitions (FS_PRESS_ENABLE parameter)
#define FS_PRESS_DISABLED 0
#define FS_PRESS_WARN_ONLY 1

// External pressure failsafe definitions (FS_PRESS_EXT_ENABLE parameter)
#define FS_PRESS_EXT_DISABLED 0
#define FS_PRESS_EXT_WARN_ONLY 1
