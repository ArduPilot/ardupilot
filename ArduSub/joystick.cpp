            ...
            int16_t aux6)
{
    ...
    if (roll_pitch_flag == 0) {
        ...
        channel_forward->set_override(constrain_float((xTot)*rpyScale+rpyCenter,1100,1900), tnow);
        channel_lateral->set_override(constrain_float((yTot)*rpyScale+rpyCenter,1100,1900), tnow);
    } else {
        // neutralize forward and lateral input while we are adjusting roll and pitch
        channel_forward->set_override(constrain_float(xTrim*rpyScale+rpyCenter,1100,1900), tnow);
        channel_lateral->set_override(constrain_float(yTrim*rpyScale+rpyCenter,1100,1900), tnow);
    }
