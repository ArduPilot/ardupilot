...
private:
  ...
  // Mission library
  AP_Mission mission{FUNCTOR_BIND_MEMBER(&Sub::start_command, bool,
                                         const AP_Mission::Mission_Command &),
                     FUNCTOR_BIND_MEMBER(&Sub::verify_command_callback, bool,
                                         const AP_Mission::Mission_Command &),
...
#include USERHOOK_VARIABLES
  ...
  union {
    struct {
      uint8_t pre_arm_check : 1;   // true if all pre-arm checks (rc, accel
                                   // calibration, gps lock) have been performed
      uint8_t logging_started : 1; // true if logging has started
      uint8_t compass_mot : 1; // true if we are currently performing compassmot
                               // calibration
      uint8_t
          motor_test : 1; // true if we are currently performing the motors test
      uint8_t initialised : 1; // true once the init_ardupilot function has
                               // completed.  Extended status to GCS is not sent
                               // until this completes
      uint8_t gps_base_pos_set : 1; // true when the gps base position has been
                                    // set (used for RTK gps only)
      uint8_t at_bottom : 1;        // true if we are at the bottom
      uint8_t at_surface : 1;       // true if we are at the surface
      uint8_t depth_sensor_present : 1; // true if there is a depth sensor
                                        // detected at boot
      uint8_t unused1 : 1; // was compass_init_location; true when the compass's
                           // initial location has been set
    };
    uint32_t value;
  } ap;
  ...
  struct {
    ...
    uint32_t
        last_pilot_input_ms; // last time we received pilot input in the form of
                             // MANUAL_CONTROL or RC_CHANNELS_OVERRIDE messages
    uint32_t terrain_first_failure_ms; // the first time terrain data access
                                       // failed - used to calculate the
    ...
    uint8_t pilot_input : 1; // true if pilot input failsafe is active, handles
                             // things like joystick being disconnected during
                             // operation
    uint8_t gcs : 1;         // A status flag for the ground station failsafe
    uint8_t ekf : 1;         // true if ekf failsafe has occurred
    uint8_t
        terrain : 1;  // true if the missing terrain data failsafe has occurred
    uint8_t leak : 1; // true if leak recently detected
    uint8_t
        external_pressure : 1; // true if external pressure is over threshold
    uint8_t
        internal_pressure : 1; // true if internal pressure is over threshold
    uint8_t internal_temperature : 1; // true if temperature is over threshold
    uint8_t crash : 1;                // true if we are crashed
    uint8_t sensor_health : 1; // true if at least one sensor has triggered a
                               // failsafe (currently only used for depth in
                               // depth enabled modes)
  } failsafe;

  bool any_failsafe_triggered() const {
    return (failsafe.pilot_input || battery.has_failsafed() || failsafe.gcs ||
            failsafe.ekf || failsafe.terrain || failsafe.leak ||
            failsafe.external_pressure || failsafe.internal_pressure ||
            failsafe.internal_temperature || failsafe.crash ||
  ...
  // Loiter control
  uint16_t loiter_time_max; // How long we should stay in Loiter Mode for
                            // mission scripting (time in seconds)
  uint32_t
      loiter_time; // How long have we been loitering - The start time in millis
  ...
  AP_BattMonitor battery{MASK_LOG_CURRENT,
                         FUNCTOR_BIND_MEMBER(&Sub::handle_battery_failsafe,
  ...
  // Flag indicating if we are currently controlling Pitch and Roll instead of
  // forward/lateral
  ...
  int32_t condition_value; // used in condition commands (eg delay, change alt,
                           // etc.)
  ...
  void get_pilot_desired_lean_angles(float roll_in, float pitch_in,
  ...
  void transform_manual_control_to_rc_override(
      ...
      uint16_t buttons2, uint8_t enabled_extensions, int16_t s, int16_t t,
      int16_t aux1, int16_t aux2, int16_t aux3, int16_t aux4, int16_t aux5,
      int16_t aux6);
  ...
  void failsafe_external_pressure_check();
  ...
  static constexpr int8_t _failsafe_priorities[] = {
      Failsafe_Action_Disarm, Failsafe_Action_Surface, Failsafe_Action_Warn,
...
public:
  ...
  static Sub *get_singleton() { return _singleton; }
  ...
  float get_rangefinder_target_cm() const WARN_IF_UNUSED {
    return mode_surftrak.get_rangefinder_target_cm();
  }
  bool set_rangefinder_target_cm(float new_target_cm) {
    return mode_surftrak.set_rangefinder_target_cm(new_target_cm);
  }
