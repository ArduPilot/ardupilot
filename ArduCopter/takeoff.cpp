...
void Mode::_TakeOff::do_pilot_takeoff_ms(float& pilot_climb_rate_ms)
{
    ...
    if (copter.ap.land_complete) {
        ...
        copter.pos_control->init_D_controller();
        if (throttle_norm >= MIN(copter.g2.takeoff_throttle_max, 0.9) || 
            (copter.pos_control->get_estimated_accel_U_mss() >= 0.5 * copter.pos_control->get_max_accel_D_mss()) ||
...
void _AutoTakeoff::run()
{
    ...
    const float stop_distance_m = 0.5 * sq(vel_threshold_fraction * copter.pos_control->get_max_speed_up_ms()) / copter.pos_control->get_max_accel_D_mss();
