...
void Copter::set_simple_mode(uint8_t b)
{
    if(ap.simple_mode != b){
        if(b == 0){
            ...
            gcs().send_text_active(MAV_SEVERITY_INFO, "SIMPLE mode off");
        }else if(b == 1){
            Log_Write_Event(DATA_SET_SIMPLE_ON);
            gcs().send_text_active(MAV_SEVERITY_INFO, "SIMPLE mode on");
        }else{
            // initialise super simple heading
            update_super_simple_bearing(true);
            Log_Write_Event(DATA_SET_SUPERSIMPLE_ON);
            gcs().send_text_active(MAV_SEVERITY_INFO, "SUPERSIMPLE mode on");
        }
        ap.simple_mode = b;
    }
}
