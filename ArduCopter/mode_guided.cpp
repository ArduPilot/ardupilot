...
bool ModeGuided::set_destination(const Location& dest_loc, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw)
{
    ...
    // log target
#if HAL_LOGGING_ENABLED
    float dest_loc_alt_m = 0;
    UNUSED_RESULT(dest_loc.get_alt_m(dest_loc.get_alt_frame(), dest_loc_alt_m));
    copter.Log_Write_Guided_Position_Target(
        guided_mode,
        Vector3f(dest_loc.lat, dest_loc.lng, dest_loc_alt_m),
        guided_is_terrain_alt,
        guided_vel_target_neu_ms,
