...
static bool guided_is_terrain_alt;              // true if guided_pos_target_ned_m.z is an alt above terrain
...
struct Guided_Limit {
    ...
    Vector3p start_pos_ned_m;   // start position as a distance from home in m.  used for checking horiz_max limit
} static guided_limit;
...
{
    // initialize vertical speeds and acceleration
    pos_control->set_max_speed_accel_D_m(wp_nav->get_default_speed_down_ms(), speed_up_ms, wp_nav->get_accel_D_mss());
    pos_control->set_correction_speed_accel_D_m(wp_nav->get_default_speed_down_ms(), speed_up_ms, wp_nav->get_accel_D_mss());
    return true;
}
...
{
    // initialize vertical speeds and acceleration
    pos_control->set_max_speed_accel_D_m(speed_down_ms, wp_nav->get_default_speed_up_ms(), wp_nav->get_accel_D_mss());
    pos_control->set_correction_speed_accel_D_m(speed_down_ms, wp_nav->get_default_speed_up_ms(), wp_nav->get_accel_D_mss());
    return true;
}
...
// set_pos_vel_NED_m - set guided mode position and velocity target
...
void ModeGuided::velaccel_control_run()
{
    ...
    copter.avoid.adjust_velocity_m(guided_vel_target_ned_ms, pos_control->get_pos_NE_p().kP(), pos_control->get_max_accel_NE_mss(), pos_control->get_pos_D_p().kP(), pos_control->get_max_accel_D_mss(), G_Dt);
...
void ModeGuided::angle_control_run()
{
    ...
    if (guided_angle_state.use_thrust) {
        ...
        pos_control->set_pos_target_D_from_climb_rate_ms(climb_rate_ms);
        pos_control->update_D_controller();
    }
}
