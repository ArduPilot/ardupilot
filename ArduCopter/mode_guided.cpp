...
struct Guided_Limit {
    ...
    Vector3f start_pos_neu_m;     // start position as a distance from home in m.  used for checking horiz_max limit
} static guided_limit;
...
bool ModeGuided::do_user_takeoff_start(float takeoff_alt_m)
{
...
void ModeGuided::wp_control_start()
{
    ...
    wp_nav->get_wp_stopping_point_NEU_m(stopping_point);
    if (!wp_nav->set_wp_destination_NEU_m(stopping_point, false)) {
...
bool ModeGuided::set_speed_xy_ms(float speed_xy_ms)
{
    // initialise horizontal speed, acceleration
...
bool ModeGuided::set_pos_neu_m(const Vector3f& pos_neu_m, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw, bool is_terrain_alt)
{
    ...
    const Location dest_loc(pos_neu_m, is_terrain_alt ? Location::AltFrame::ABOVE_TERRAIN : Location::AltFrame::ABOVE_ORIGIN);
    ...
    guided_pos_target_neu_m = pos_neu_m.topostype();
...
#if HAL_LOGGING_ENABLED
    // log target
    copter.Log_Write_Guided_Position_Target(guided_mode, guided_pos_target_neu_m.tofloat(), guided_is_terrain_alt, guided_vel_target_neu_ms, guided_accel_target_neu_mss);
#endif
...
bool ModeGuided::set_destination(const Location& dest_loc, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw)
{
    ...
    guided_pos_target_neu_m = pos_target_neu_m.topostype();
    guided_is_terrain_alt = is_terrain_alt;
    guided_vel_target_neu_ms.zero();
    guided_accel_target_neu_mss.zero();
    update_time_ms = millis();
    ...
    // log target
#if HAL_LOGGING_ENABLED
    copter.Log_Write_Guided_Position_Target(guided_mode, Vector3f(dest_loc.lat, dest_loc.lng, dest_loc.alt), guided_is_terrain_alt, guided_vel_target_neu_ms, guided_accel_target_neu_mss);
#endif
...
void ModeGuided::set_accel_neu_mss(const Vector3f& accel_neu_m, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw, bool log_request)
{
    ...
    guided_is_terrain_alt = false;
    guided_vel_target_neu_ms.zero();
    guided_accel_target_neu_mss = accel_neu_m;
    update_time_ms = millis();
...
void ModeGuided::set_vel_neu_ms(const Vector3f& vel_neu_m, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw, bool log_request)
{
    set_vel_accel_neu_m(vel_neu_m, Vector3f(), use_yaw, yaw_rad, use_yaw_rate, yaw_rate_rads, relative_yaw, log_request);
}

// set_vel_accel_neu_m - sets guided mode's target velocity and acceleration
void ModeGuided::set_vel_accel_neu_m(const Vector3f& vel_neu_m, const Vector3f& accel_neu_m, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw, bool log_request)
{
    ...
    guided_pos_target_neu_m.zero();
    guided_is_terrain_alt = false;
    guided_vel_target_neu_ms = vel_neu_m;
    guided_accel_target_neu_mss = accel_neu_m;
    update_time_ms = millis();
...
bool ModeGuided::set_pos_vel_neu_m(const Vector3f& pos_neu_m, const Vector3f& vel_neu_m, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw)
{
    return set_pos_vel_accel_neu_m(pos_neu_m, vel_neu_m, Vector3f(), use_yaw, yaw_rad, use_yaw_rate, yaw_rate_rads, relative_yaw);
}

// set_pos_vel_accel_neu_m - set guided mode position, velocity and acceleration target
bool ModeGuided::set_pos_vel_accel_neu_m(const Vector3f& pos_neu_m, const Vector3f& vel_neu_m, const Vector3f& accel_neu_m, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw)
{
#if AP_FENCE_ENABLED
    // reject destination if outside the fence
...
void ModeGuided::limit_init_time_and_pos()
{
    ...
    guided_limit.start_pos_neu_m = pos_control->get_pos_estimate_NEU_m().tofloat();
}
...
bool ModeGuided::limit_check()
{
    ...
    const Vector3f& curr_pos_neu_m = pos_control->get_pos_estimate_NEU_m().tofloat();
