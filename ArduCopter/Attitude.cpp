...
float Copter::get_surface_tracking_climb_rate(int16_t target_rate, float current_alt_target, float dt)
{
    ...
    if (now - last_call_ms > RANGEFINDER_TIMEOUT_MS) {
        target_rangefinder_alt = rangefinder_height_above_terrain_cm + current_alt_target - current_alt;
    }
    last_call_ms = now;
    ...
    /*
      ...
      reading
     */
    int32_t glitch_cm = rangefinder_height_above_terrain_cm - target_rangefinder_alt;
    ...
    if (abs(rangefinder_state.glitch_count) >= RANGEFINDER_GLITCH_NUM_SAMPLES) {
        ...
        target_rangefinder_alt = rangefinder_height_above_terrain_cm;
        rangefinder_state.glitch_count = 0;
    }
    ...
    distance_error = (target_rangefinder_alt - rangefinder_height_above_terrain_cm) - (current_alt_target - current_alt);
