...
bool AP_Arming_Copter::parameter_checks(bool display_failure)
{
    ...
    if ((checks_to_perform == ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_PARAMETERS)) {
        ...
        if (!is_positive(copter.pos_control->get_pos_z_p().kP())) {
            parameter_checks_pid_warning_message(display_failure, "PSC_POSZ_P");
            return false;
        }
        if (!is_positive(copter.pos_control->get_vel_z_p().kP())) {
            parameter_checks_pid_warning_message(display_failure, "PSC_VELZ_P");
            return false;
        }
        if (!is_positive(copter.pos_control->get_accel_z_pid().kP())) {
            parameter_checks_pid_warning_message(display_failure, "PSC_ACCZ_P");
            return false;
        }
        if (!is_positive(copter.pos_control->get_accel_z_pid().kI())) {
            parameter_checks_pid_warning_message(display_failure, "PSC_ACCZ_I");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_rate_roll_pid().kP()) &&
            !is_positive(copter.attitude_control->get_rate_roll_pid().ff())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_RAT_RLL_P");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_rate_roll_pid().kI()) &&
            !is_positive(copter.attitude_control->get_rate_roll_pid().ff())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_RAT_RLL_I");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_rate_roll_pid().kD()) &&
            !is_positive(copter.attitude_control->get_rate_roll_pid().ff())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_RAT_RLL_D");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_rate_pitch_pid().kP()) &&
            !is_positive(copter.attitude_control->get_rate_pitch_pid().ff())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_RAT_PIT_P");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_rate_pitch_pid().kI()) &&
            !is_positive(copter.attitude_control->get_rate_pitch_pid().ff())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_RAT_PIT_I");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_rate_pitch_pid().kD()) &&
            !is_positive(copter.attitude_control->get_rate_pitch_pid().ff())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_RAT_PIT_D");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_rate_yaw_pid().kP()) &&
            !is_positive(copter.attitude_control->get_rate_yaw_pid().ff())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_RAT_YAW_P");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_rate_yaw_pid().kI()) &&
            !is_positive(copter.attitude_control->get_rate_yaw_pid().ff())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_RAT_YAW_I");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_angle_pitch_p().kP())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_ANG_PIT_P");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_angle_roll_p().kP())) {
            parameter_checks_pid_warning_message(display_failure, "ATC_ANG_RLL_P");
            return false;
        }
        if (!is_positive(copter.attitude_control->get_angle_yaw_p().kP())) {
