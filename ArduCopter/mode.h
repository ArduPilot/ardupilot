...
private:
    ...
    struct {
        uint8_t time_updated_roll   : 1;        // true if braking timeout on roll axis has been re-estimated
        uint8_t time_updated_pitch  : 1;        // true if braking timeout on pitch axis has been re-estimated
        ...
        uint32_t start_time_roll;               // system time (ms) when braking on roll axis began
        uint32_t start_time_pitch;              // system time (ms) when braking on pitch axis began
        ...
        uint32_t loiter_transition_start_time;  // system time (ms) when transition from brake to loiter started
    } brake;


    // loiter related variables
    uint32_t controller_to_pilot_start_time_roll;   // time (ms) when transition from controller to pilot roll input began
    uint32_t controller_to_pilot_start_time_pitch;  // time (ms) when transition from controller to pilot pitch input began
    ...
    uint32_t wind_comp_start_timer;  // system time (ms) when wind compensation updates are allowed to begin
