...
bool ModeAuto::set_speed_xy_ms(float speed_xy_ms)
{
...
void PayloadPlace::run()
{
    ...
    case State::Descent_Start:
        ...
        descent_speed_ms = MIN((is_positive(g2.pldp_descent_speed_ms)) ? g2.pldp_descent_speed_ms : abs(g.land_speed) * 0.01, wp_nav->get_default_speed_down_ms());
...
void ModeAuto::do_change_speed(const AP_Mission::Mission_Command& cmd)
{
    if (cmd.content.speed.target_ms > 0) {
        ...
        case SPEED_TYPE_GROUNDSPEED:
            copter.wp_nav->set_speed_NE_cms(cmd.content.speed.target_ms);
