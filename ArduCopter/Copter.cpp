...
{
    // try to set origin from params if option enabled (only possible when disarmed)
    if (!motors->armed() &&
        arming.option_enabled(AP_Arming::Option::SET_ORIGIN_FROM_AHRS_PARAMS) &&
        arming.option_enabled(AP_Arming::Option::SET_ORIGIN_AT_STARTUP)) {
        ahrs.set_origin_from_params_maybe();
    }
