...
bool Copter::ModeZigZag::has_arr_at_dest()
{
    ...
    if ((dist_x*dist_x + dist_y*dist_y) < ZIGZAG_WP_RADIUS_SQUARED) {
        return true;
    }
    zigzag_judge_moving.last_judge_pos_time = AP_HAL::millis();
    zigzag_judge_moving.last_pos = inertial_nav.get_position();
...
bool Copter::ModeZigZag::calculate_next_dest(Vector3f& next_dest, RC_Channel::aux_switch_pos_t next_A_or_B) const
{
    ...
    if (next_A_or_B == zigzag_waypoint.switch_pos_B) {
        ...
        float dis_CB_squared = (cur_pos_2d - zigzag_waypoint.B_pos).length_squared();
        float dis_BE = sqrtf(dis_CB_squared - dis_from_AB_squared);
        float dis_ratio = dis_BE / dis_AB;
    ...
    float dis_CA_squared = (cur_pos_2d - zigzag_waypoint.A_pos).length_squared();
    float dis_AE = sqrtf(dis_CA_squared - dis_from_AB_squared);
    float dis_ratio = dis_AE / dis_AB;
    next_dest.x = cur_pos.x - next_dir*dis_ratio*pos_diff.x;
    next_dest.y = cur_pos.y - next_dir*dis_ratio*pos_diff.y;
    return true;
}
