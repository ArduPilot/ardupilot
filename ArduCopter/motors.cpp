...
static void auto_disarm_check()
{
    ...
    //BEV do not disarm if it's likely we're in the air
    if(!is_on_ground_maybe()) {
        return;
    }

    //BEV do not allow auto disarming when in plane mode. Too likely to trigger when gliding
...
static bool init_arm_motors()
{
    ...
    read_radio();
    //BEV simplified check below
    if (g.rc_3.control_in > 100) {
        motors.output_min();
        failsafe_enable();
        AP_Notify::flags.armed = false;
        //BEV alert the user why it's not arming
...
static void pre_arm_checks(bool display_failure)
{
    ...
    //BEV make certain 2 compass are plugged in. Do this if any checks are enabled
    if (g.arming_check) {
        //BEV make certain both compass are present
...
static bool arm_checks(bool display_failure)
{
    ...
    if (hal.util->safety_switch_state() == AP_HAL::Util::SAFETY_DISARMED) {
        if (display_failure) {
            gcs_send_text_P(SEVERITY_HIGH,PSTR("Arm: Safety Button"));
        }
        return false;
    }
