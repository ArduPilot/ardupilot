        ...
        if (throttle_pct <= 0.0f) {
            ...
            if (comp_type == AP_COMPASS_MOT_COMP_THROTTLE) {
                for (uint8_t i=0; i<compass.get_count(); i++) {
                    ...
                    interference_pct[i] = motor_compensation[i].length() / (float)compass.get_expected_magfield() * 100.0f;
                }
            }else{
                for (uint8_t i=0; i<compass.get_count(); i++) {
                    // interference is impact/amp * (max current seen / max throttle seen) / mag field * 100
                    interference_pct[i] = motor_compensation[i].length() * (current_amps_max/throttle_pct_max) / (float)compass.get_expected_magfield() * 100.0f;
                }
            }
