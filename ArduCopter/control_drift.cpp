...
void Copter::drift_run()
{
    ...
    float roll_vel =  current_vel.y * ahrs.cos_yaw() - current_vel.x * ahrs.sin_yaw(); // body roll vel
    float pitch_vel = current_vel.y * ahrs.sin_yaw() + current_vel.x * ahrs.cos_yaw(); // body pitch vel
    ...
    attitude_control->set_throttle_out(get_throttle_assist(current_vel.z, pilot_throttle_scaled), true, g.throttle_filt);
}
