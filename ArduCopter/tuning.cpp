...
void Copter::tuning(const RC_Channel *tuning_ch, int8_t tuning_param, float tuning_min, float tuning_max)
{
    ...
    if (!rc().has_valid_input()) {
        // no valid transmitter values to use
        return;
    }

    const float control_in = tuning_ch->norm_input_ignore_trim();
    const float tuning_value = linear_interpolate(tuning_min, tuning_max, control_in, -1, 1);

#if HAL_LOGGING_ENABLED
    Log_Write_PTUN(tuning_param, tuning_value, tuning_min, tuning_max, control_in);
#endif
