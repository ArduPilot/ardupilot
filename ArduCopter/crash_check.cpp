...
void Copter::crash_check()
{
    ...
    if (crash_counter >= (CRASH_CHECK_TRIGGER_SEC * MAIN_LOOP_RATE)) {
        ...
        // send message to gcs
        gcs_frontend.send_text(MAV_SEVERITY_EMERGENCY,"Crash: Disarming");
        // disarm motors
        init_disarm_motors();
    }
}
...
void Copter::parachute_release()
{
    ...
    gcs_frontend.send_text(MAV_SEVERITY_INFO,"Parachute: Released");
...
void Copter::parachute_manual_release()
{
    ...
    if (ap.land_complete) {
        ...
        gcs_frontend.send_text(MAV_SEVERITY_INFO,"Parachute: Landed");
        // log an error in the dataflash
    ...
    if ((parachute.alt_min() != 0 && (current_loc.alt < (int32_t)parachute.alt_min() * 100))) {
        // warn user of reason for failure
        gcs_frontend.send_text(MAV_SEVERITY_ALERT,"Parachute: Too low");
        // log an error in the dataflash
