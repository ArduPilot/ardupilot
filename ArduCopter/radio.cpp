...
void Copter::read_radio()
{
    ...
    if (hal.rcin->new_input()) {
        ...
        if (((!failsafe.rc_override_active && (elapsed >= FS_RADIO_TIMEOUT_MS)) || (failsafe.rc_override_active && (elapsed >= FS_RADIO_RC_OVERRIDE_TIMEOUT_MS))) &&
            (g.failsafe_throttle && (ap.rc_receiver_present||hal.util->get_soft_arm_state()!=AP_HAL::Util::SOFT_ARM_STATE_DISARMED) && !failsafe.radio)) {
...
void Copter::set_throttle_and_failsafe(uint16_t throttle_pwm)
{
    ...
    if (throttle_pwm < (uint16_t)g.failsafe_throttle_value) {
        ...
        if (failsafe.radio || !(ap.rc_receiver_present || hal.util->get_soft_arm_state() != AP_HAL::Util::SOFT_ARM_STATE_DISARMED)) {
            channel_throttle->set_pwm(throttle_pwm);
            return;
        }
