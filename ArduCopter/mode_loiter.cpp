...
bool ModeLoiter::init(bool ignore_checks)
{
    ...
    pos_control->set_max_speed_accel_D_m(get_pilot_speed_dn_ms(), get_pilot_speed_up_ms(), get_pilot_accel_D_mss());
    pos_control->set_correction_speed_accel_D_m(get_pilot_speed_dn_ms(), get_pilot_speed_up_ms(), get_pilot_accel_D_mss());
...
void ModeLoiter::run()
{
    ...
    case AltHoldModeState::MotorStopped:
        ...
        pos_control->relax_D_controller(0.0f);   // forces throttle output to decay to zero
        loiter_nav->init_target();
        break;
