...
#define POSHOLD_BRAKE_TIME_ESTIMATE_MAX_MS      6000    // Maxiumum time in ms that the brake will be applied before we switch to loiter
#define POSHOLD_BRAKE_TO_LOITER_TIME_MS         1500    // Time in ms to transition from brake mode to loiter mode.
#define POSHOLD_WIND_COMP_START_TIME_MS         1500    // Time in ms to start wind compensation update after loiter is engaged
#define POSHOLD_CONTROLLER_TO_PILOT_MIX_TIME_MS 500     // Time in ms loiter and manual commands are mixed to make a smooth transition.
...
void ModePosHold::run()
{
    ...
    switch (roll_mode) {
        ...
        case RPMode::PILOT_OVERRIDE:
            ...
            if (is_zero(target_roll_cd) && (fabsf(pilot_roll_cd) < 2 * g.poshold_brake_rate)) {
                ...
                brake.start_time_roll = now_ms;           // the time that the brake will be applied, updated during braking mode.
        ...
        case RPMode::BRAKE_READY_TO_LOITER:
            ...
            if (!brake.time_updated_roll) {
                ...
                if (fabsf(brake.roll_cd) >= brake.angle_max_roll_cd) {
                    ...
                    brake.start_time_roll = now_ms;
                    brake.time_updated_roll = true;
                }
            } else {
                const uint32_t break_timeout_roll = MIN(POSHOLD_BRAKE_TIME_ESTIMATE_MAX_MS, (1.5 * 1000 * 0.01) * brake.angle_max_roll_cd / g.poshold_brake_rate);  // the 1.5 times the time taken to level the aircraft in ms.

                // if velocity is very low reduce braking time to 0.5seconds
                if ((fabsf(vel_right_cms) <= POSHOLD_SPEED_0) && (now_ms - brake.start_time_roll > 500)) {
                    brake.start_time_roll = now_ms - break_timeout_roll + 500;
                }

                // reduce braking timer
                if (now_ms - brake.start_time_roll > break_timeout_roll) {
                    // indicate that we are ready to move to Loiter.
                    // Loiter will only actually be engaged once both roll_mode and roll_mode are changed to RPMode::BRAKE_READY_TO_LOITER
                    //  logic for engaging loiter is handled below the roll and roll mode switch statements
        ...
        case RPMode::CONTROLLER_TO_PILOT_OVERRIDE:
            ...
            if (now_ms - controller_to_pilot_start_time_roll > POSHOLD_CONTROLLER_TO_PILOT_MIX_TIME_MS) {
                // when timer runs out switch to full pilot override for next iteration
                roll_mode = RPMode::PILOT_OVERRIDE;
            }

            // calculate controller_to_pilot mix ratio
            controller_to_pilot_roll_mix = (float)(now_ms - controller_to_pilot_start_time_roll) / (float)POSHOLD_CONTROLLER_TO_PILOT_MIX_TIME_MS;
    ...
    switch (pitch_mode) {
        ...
        case RPMode::PILOT_OVERRIDE:
            ...
            // switch to BRAKE mode for next iteration if no pilot input
            if (is_zero(target_pitch_cd) && (fabsf(pilot_pitch_cd) < 2 * g.poshold_brake_rate)) {
                // initialise BRAKE mode
                ...
                brake.start_time_pitch = now_ms;          // the time that the brake will be applied, updated during braking mode.
        ...
        case RPMode::BRAKE_READY_TO_LOITER:
            ...
            if (!brake.time_updated_pitch) {
                ...
                if (fabsf(brake.pitch_cd) >= brake.angle_max_pitch_cd) {
                    ...
                    // braking angle has started decreasing so re-estimate braking time
                    brake.start_time_pitch = now_ms;
                    brake.time_updated_pitch = true;
                }
            } else {
                const uint32_t break_timeout_pitch = MIN(POSHOLD_BRAKE_TIME_ESTIMATE_MAX_MS, (1.5 * 1000 * 0.01) * brake.angle_max_pitch_cd / g.poshold_brake_rate);  // the 1.5 times the time taken to level the aircraft in ms.

                // if velocity is very low reduce braking time to 0.5seconds
                if ((fabsf(vel_fw_cms) <= POSHOLD_SPEED_0) && (now_ms - brake.start_time_pitch > 500)) {
                    brake.start_time_pitch = now_ms - break_timeout_pitch + 500;
                }

                // reduce braking timer
                if (now_ms - brake.start_time_pitch > break_timeout_pitch) {
                    // indicate that we are ready to move to Loiter.
        ...
        case RPMode::CONTROLLER_TO_PILOT_OVERRIDE:
            ...
            // count-down loiter to pilot timer
            if (now_ms - controller_to_pilot_start_time_pitch > POSHOLD_CONTROLLER_TO_PILOT_MIX_TIME_MS) {
                // when timer runs out switch to full pilot override for next iteration
                pitch_mode = RPMode::PILOT_OVERRIDE;
            }

            // calculate controller_to_pilot mix ratio
            controller_to_pilot_pitch_mix = (float)(now_ms - controller_to_pilot_start_time_pitch) / (float)POSHOLD_CONTROLLER_TO_PILOT_MIX_TIME_MS;
    ...
    if (roll_mode == RPMode::BRAKE_READY_TO_LOITER && pitch_mode == RPMode::BRAKE_READY_TO_LOITER) {
        ...
        brake.loiter_transition_start_time = now_ms;
        ...
        wind_comp_start_timer = now_ms + POSHOLD_WIND_COMP_START_TIME_MS;
    }
    ...
    if (roll_mode == RPMode::BRAKE_TO_LOITER || roll_mode == RPMode::LOITER) {
        ...
        switch (roll_mode) {
            case RPMode::BRAKE_TO_LOITER: {
                ...
                if (now_ms - brake.loiter_transition_start_time > POSHOLD_BRAKE_TO_LOITER_TIME_MS) {
                ...
                const float brake_to_loiter_mix = (float)(now_ms - brake.loiter_transition_start_time) / (float)POSHOLD_BRAKE_TO_LOITER_TIME_MS;
...
void ModePosHold::update_pilot_lean_angle_cd(float &lean_angle_filtered_cd, float &lean_angle_raw_cd)
{
    ...
    if ((lean_angle_filtered_cd > 0 && lean_angle_raw_cd < 0) || (lean_angle_filtered_cd < 0 && lean_angle_raw_cd > 0) || (fabsf(lean_angle_raw_cd) > POSHOLD_STICK_RELEASE_SMOOTH_ANGLE)) {
        ...
        if (lean_angle_filtered_cd > 0) {
            ...
            lean_angle_filtered_cd -= MAX(lean_angle_filtered_cd * POSHOLD_SMOOTH_RATE_FACTOR, 100.0 * g.poshold_brake_rate * G_Dt);
...
//  brake_angle is slewed with the wpnav.poshold_brake_rate_degs and constrained by the wpnav.poshold_braking_angle_max
...
{
    float lean_angle;
    float brake_delta_cd = g.poshold_brake_rate * G_Dt;
...
void ModePosHold::update_wind_comp_estimate()
{
    ...
    if (wind_comp_start_timer - now_ms > 0) {
        return;
    }
...
void ModePosHold::roll_controller_to_pilot_override()
{
    ...
    controller_to_pilot_start_time_roll = now_ms;
...
void ModePosHold::pitch_controller_to_pilot_override()
{
    ...
    controller_to_pilot_start_time_pitch = now_ms;
