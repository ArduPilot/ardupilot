        ...
        case RPMode::BRAKE_READY_TO_LOITER:
            ...
            if (!brake.time_updated_roll) {
                ...
                if (fabsf(brake.roll_cd) >= brake.angle_max_roll_cd) {
                    ...
                    brake.timeout_roll = 1+(uint16_t)(LOOP_RATE_FACTOR*15L*(int32_t)(fabsf(brake.roll_cd))/(10L*(int32_t)g.poshold_brake_rate));  // the 1.2 (12/10) factor has to be tuned in flight, here it means 120% of the "normal" time.
                    brake.time_updated_roll = true;
                }
            }

            // if velocity is very low reduce braking time to 0.5seconds
            if ((fabsf(vel_right_cms) <= POSHOLD_SPEED_0) && (brake.timeout_roll > 50*LOOP_RATE_FACTOR)) {
        ...
        case RPMode::BRAKE_READY_TO_LOITER:
            ...
            if (!brake.time_updated_pitch) {
                ...
                if (fabsf(brake.pitch_cd) >= brake.angle_max_pitch_cd) {
                    ...
                    // braking angle has started decreasing so re-estimate braking time
                    brake.timeout_pitch = 1+(uint16_t)(LOOP_RATE_FACTOR*15L*(int32_t)(fabsf(brake.pitch_cd))/(10L*(int32_t)g.poshold_brake_rate));  // the 1.2 (12/10) factor has to be tuned in flight, here it means 120% of the "normal" time.
                    brake.time_updated_pitch = true;
                }
            }

            // if velocity is very low reduce braking time to 0.5seconds
            if ((fabsf(vel_fw_cms) <= POSHOLD_SPEED_0) && (brake.timeout_pitch > 50*LOOP_RATE_FACTOR)) {
...
void ModePosHold::update_pilot_lean_angle_cd(float &lean_angle_filtered_cd, float &lean_angle_raw_cd)
{
    ...
    if ((lean_angle_filtered_cd > 0 && lean_angle_raw_cd < 0) || (lean_angle_filtered_cd < 0 && lean_angle_raw_cd > 0) || (fabsf(lean_angle_raw_cd) > POSHOLD_STICK_RELEASE_SMOOTH_ANGLE)) {
        ...
        if (lean_angle_filtered_cd > 0) {
            ...
            lean_angle_filtered_cd -= MAX(lean_angle_filtered_cd * POSHOLD_SMOOTH_RATE_FACTOR, MAX(1.0f, g.poshold_brake_rate/(float)LOOP_RATE_FACTOR));
...
void ModePosHold::update_brake_angle_from_velocity(float &brake_angle_cd, float velocity_cms)
{
    ...
    brake_angle_cd = constrain_float(lean_angle, brake_angle_cd - brake_rate, brake_angle_cd + brake_rate);
