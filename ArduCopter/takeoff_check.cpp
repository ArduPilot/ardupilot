...
void Copter::takeoff_check()
{
    ...
    const bool rpm_adequate = AP::esc_telem().are_motors_running(motor_mask, g2.takeoff_rpm_min, g2.takeoff_rpm_max, 1.0f);
    ...
    if (now_ms - takeoff_check_warning_ms > 5000) {
        ...
        if (!telem_active) {
            ...
            gcs().send_text(MAV_SEVERITY_CRITICAL, "%s ESC RPM or error rate out of range", prefix_str);
