...
void GCS_MAVLINK_Copter::handleMessage(mavlink_message_t* msg)
{
    ...
    case MAVLINK_MSG_ID_SET_POSITION_TARGET_LOCAL_NED:     // MAV ID: 84
    {
        ...
        if (!pos_ignore && !vel_ignore && acc_ignore) {
            ...
            copter.flightmode_guided.set_velocity(vel_vector);
        } else if (!pos_ignore && vel_ignore && acc_ignore) {
            if (!copter.flightmode_guided.set_destination(pos_vector)) {
                result = MAV_RESULT_FAILED;
            }
        } else {
            result = MAV_RESULT_FAILED;
    ...
    case MAVLINK_MSG_ID_SET_POSITION_TARGET_GLOBAL_INT:    // MAV ID: 86
    {
        ...
        if (!pos_ignore && !vel_ignore && acc_ignore) {
            copter.flightmode_guided.set_destination_posvel(pos_ned, Vector3f(packet.vx * 100.0f, packet.vy * 100.0f, -packet.vz * 100.0f));
        } else if (pos_ignore && !vel_ignore && acc_ignore) {
            copter.flightmode_guided.set_velocity(Vector3f(packet.vx * 100.0f, packet.vy * 100.0f, -packet.vz * 100.0f));
        } else if (!pos_ignore && vel_ignore && acc_ignore) {
            if (!copter.flightmode_guided.set_destination(pos_ned)) {
                result = MAV_RESULT_FAILED;
            }
        } else {
            result = MAV_RESULT_FAILED;
