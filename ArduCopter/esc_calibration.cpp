    ...
    switch (g.esc_calibrate) {
        case ESCCAL_NONE:
            ...
            if (channel_throttle->control_in >= ESC_CALIBRATION_HIGH_THROTTLE) {
                ...
                // send message to gcs
                gcs_frontend.send_text(MAV_SEVERITY_CRITICAL,"ESC calibration: Restart board");
...
void Copter::esc_calibration_passthrough()
{
    ...
    // send message to GCS
    gcs_frontend.send_text(MAV_SEVERITY_INFO,"ESC calibration: Passing pilot throttle to ESCs");
...
void Copter::esc_calibration_auto()
{
    ...
    // send message to GCS
    gcs_frontend.send_text(MAV_SEVERITY_INFO,"ESC calibration: Auto calibration");
    ...
    while (hal.util->safety_switch_state() == AP_HAL::Util::SAFETY_DISARMED) {
        if (!printed_msg) {
            gcs_frontend.send_text(MAV_SEVERITY_INFO,"ESC calibration: Push safety switch");
            printed_msg = true;
        }
        delay(10);
    }
