...
void ModeRTL::build_path()
{
    ...
    // set land flag
    rtl_path.land = (g.rtl_alt_final_m <= 0.0f);
}
...
void ModeRTL::compute_return_target()
{
    ...
    if (alt_type == ReturnTargetAltType::RANGEFINDER) {
        if (copter.get_rangefinder_height_interpolated_cm(curr_alt)) {
            ...
            rtl_path.return_target.set_alt_cm(MAX(curr_alt + MAX(0, g.rtl_climb_min()), int32_t(MAX(g.rtl_altitude(), RTL_ALT_MIN))), Location::AltFrame::ABOVE_TERRAIN);
        } else {
    ...
    const int32_t min_rtl_alt = MAX(RTL_ALT_MIN, curr_alt + MAX(0, g.rtl_climb_min()));
