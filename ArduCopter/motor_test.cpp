...
bool Copter::mavlink_motor_test_check(GCS_MAVLINK *_gcs, bool check_rc)
{
    // check rc has been calibrated
    pre_arm_rc_checks();
    if(check_rc && !ap.pre_arm_rc_check) {
        _gcs->send_text(MAV_SEVERITY_CRITICAL,"Motor Test: RC not calibrated");
        return false;
    }

    // ensure we are landed
    if (!ap.land_complete) {
        _gcs->send_text(MAV_SEVERITY_CRITICAL,"Motor Test: vehicle not landed");
        return false;
    }

    // check if safety switch has been pushed
    if (hal.util->safety_switch_state() == AP_HAL::Util::SAFETY_DISARMED) {
        _gcs->send_text(MAV_SEVERITY_CRITICAL,"Motor Test: Safety switch");
        return false;
    }
...
uint8_t Copter::mavlink_motor_test_start(GCS_MAVLINK *_gcs, uint8_t motor_seq, uint8_t throttle_type, uint16_t throttle_value, float timeout_sec)
{
    ...
    if (!ap.motor_test) {
        /* perform checks that it is ok to start test
           ...
           supplied
        */
        if (!mavlink_motor_test_check(_gcs, throttle_type != 1)) {
