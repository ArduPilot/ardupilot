    ...
    */
    ...
    if (!motors->armed()) {
        ...
        if (g2.throw_type == ThrowType::Drop) {
            pos_control->set_pos_desired_z_cm(inertial_nav.get_position_z_up_cm() - g.throw_altitude_descent * 100.0f);
        } else {
            pos_control->set_pos_desired_z_cm(inertial_nav.get_position_z_up_cm() + g.throw_altitude_ascent * 100.0f);
        }
