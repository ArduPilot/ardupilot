...
void AP_DroneCAN::init(uint8_t driver_index, bool enable_filters)
{
    ...
    _options.set_and_save(int16_t(_options.get() + 1));//@yu 每次開機都重置dronecan dna
}
...
void AP_DroneCAN::handle_actuator_status(const CanardRxTransfer& transfer, const uavcan_equipment_actuator_Status& msg)
{
    ...
    // log as CSRV message
    if(is_zero(save_voltage[msg.actuator_id]) && is_zero(save_current[msg.actuator_id]) && is_zero(save_temperature[msg.actuator_id])){
        ...
        AP::logger().Write_ServoStatus(AP_HAL::micros64(),
                                   ...
                                   msg.power_rating_pct,
                                   0, 
                                   save_voltage[msg.actuator_id], 
                                   save_current[msg.actuator_id], 
                                   save_temperature[msg.actuator_id], 
                                   0, 
                                   save_servo_error[msg.actuator_id]);
    }
#endif

/**/
//@yu 將servo的資料以esc的架構回傳到mavlink
#if HAL_WITH_ESC_TELEM
    const uint8_t esc_index = msg.actuator_id - 1;
    ...
    TelemetryData t {
        .temperature_cdeg = int16_t(save_temperature[msg.actuator_id]),
        .voltage = save_voltage[msg.actuator_id],
        .current = save_current[msg.actuator_id],
        .motor_temp_cdeg = int16_t(426),//servo2esc確認碼
    };

    update_rpm(esc_index, msg.position, save_servo_error[msg.actuator_id]);
    update_telem_data(esc_index, t,
        (isnan(1) ? 0 : AP_ESC_Telem_Backend::TelemetryType::CURRENT)
            | (isnan(1) ? 0 : AP_ESC_Telem_Backend::TelemetryType::VOLTAGE)
            | (isnan(1) ? 0 : AP_ESC_Telem_Backend::TelemetryType::TEMPERATURE)
            | (isnan(1) ? 0 : AP_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE));            
#endif
...
{
    if(msg.circuit_id < 16){
        save_voltage[msg.circuit_id] = msg.voltage;
        save_current[msg.circuit_id] = msg.current;
        save_servo_error[msg.circuit_id] = msg.error_flags;
    }
}
...
{
    if(msg.device_id < 16){
        save_temperature[msg.device_id] = msg.temperature;
    }
}
...
void AP_DroneCAN::handle_ESC_status(const CanardRxTransfer& transfer, const uavcan_equipment_esc_Status& msg)
{
    ...
    TelemetryData t {
        ...
        .voltage = msg.voltage,
        .current = msg.current,
        .motor_temp_cdeg = (int16_t)(save_temperature[msg.esc_index] * 100 - 273),//新增的vertiq控制器溫度(凱氏  //因為esc id的關係，此讀值會與servo的讀值互相覆蓋，若想存取，須將esc id區隔
    ...
    update_telem_data(esc_index, t,
        (isnan(msg.current) ? 0 : AP_ESC_Telem_Backend::TelemetryType::CURRENT)
            ...
            | (isnan(save_temperature[msg.esc_index]) ? 0 : AP_ESC_Telem_Backend::TelemetryType::MOTOR_TEMPERATURE)//新增的vertiq控制器溫度
