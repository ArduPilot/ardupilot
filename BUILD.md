...
* **Build ArduCopter**
    ...
    switch from one board to another one. For example we could switch Pixhawk and
    build again:
...
* **Clean the build**
    ...
    the build. The first keeps the configure information while the second cleans
    everything.
...
* **Upload or install**
    ...
    Pixhawk board has and upload option so it uploads to the firmware built to
    a connected board. In order to use it, pass the option `--upload` to build
    command:
    ...
    Currently Linux boards don't support the upload option, but there's a install
    command, which will install to a certain directory. This can be used by
    distributors to create .deb, .rpm or other package types:
...
* **Use different targets**
    ...
    The build commands in the items above use `copter` as the target. This
    ...
    ./waf --targets tests/test_own_ptr
...
* **Other options**
    ...
    It's possible to see all available commands with the following option:
...
and how you can use more advanced commands.
...
`./waf`. The disadvantage is that then you can only have one checkout of the
ardupilot tree.  Add this to your `.bashrc` if you want the alias approach:
