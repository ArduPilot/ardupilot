...
//@yu check rc failsafe result
int check_rc_fs_flage = 0;
//這鬼地方用不了gcs().send_text，連他媽的queue_message發到哪去也不曉得，反正gcs是看不到
/*
...
void AP_RCTelemetry::check_sensor_status_flags(void)
{
    ...
    if ((now - check_sensor_status_timer) >= 5000) { // prevent repeating any system_status messages unless 5 seconds have passed
        ...
        if ((_sensor_status_flags & MAV_SYS_STATUS_SENSOR_GPS) > 0) {
            ...
            if(check_rc_fs_flage == 1){//@yu 這應該能讓我們確定是哪邊故障，如果有發的話
                queue_message(MAV_SEVERITY_CRITICAL, "No RC Receiver FC,P");
            }else if(check_rc_fs_flage == 2){
                queue_message(MAV_SEVERITY_CRITICAL, "No RC Receiver FC,E");
            }else if(check_rc_fs_flage == 3){
                queue_message(MAV_SEVERITY_CRITICAL, "No RC Receiver FC,P,E");
            }else if(check_rc_fs_flage == 4){
                queue_message(MAV_SEVERITY_CRITICAL, "No RC Receiver FC,H");
            }else if(check_rc_fs_flage == 5){
                queue_message(MAV_SEVERITY_CRITICAL, "No RC Receiver FC,P,H");
            }else if(check_rc_fs_flage == 6){
                queue_message(MAV_SEVERITY_CRITICAL, "No RC Receiver FC,E,H");
            }else if(check_rc_fs_flage == 7){
                queue_message(MAV_SEVERITY_CRITICAL, "No RC Receiver FC,P,E,H");
            }
            check_rc_fs_flage = 0;
            check_sensor_status_timer = now;
