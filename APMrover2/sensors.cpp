...
{
    return rover.initialised;
}

bool GCS_Rover::compass_enabled() const
{
    return rover.g.compass_enabled;
}
...
void GCS_Rover::update_sensor_status_flags(void)
{
    ...
    // update control-mode-specific sensors:
    const Mode *control_mode = rover.control_mode;
    uint32_t control_mode_sensors = 0;
    if (control_mode->attitude_stabilized()) {
        control_mode_sensors |= MAV_SYS_STATUS_SENSOR_ANGULAR_RATE_CONTROL;
        control_mode_sensors |= MAV_SYS_STATUS_SENSOR_ATTITUDE_STABILIZATION;
    }
    if (control_mode->is_autopilot_mode()) {
        control_mode_sensors |= MAV_SYS_STATUS_SENSOR_YAW_POSITION;
    ...
    // update MAV_SYS_STATUS_SENSOR_LASER_POSITION
    const RangeFinder &rangefinder = rover.rangefinder;
    ...
    // update MAV_SYS_STATUS_SENSOR_PROXIMITY (Proximity)
    const AP_Proximity &proximity = rover.g2.proximity;
    ...
    if (millis() - rover.failsafe.last_valid_rc_ms < 200) {
