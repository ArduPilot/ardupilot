...
float Rover::sailboat_calc_heading(float desired_heading)
{
    ...
    /*
        ...
        Tack can be trigerd by geo fence, aux switch, rudder imput and max cross track error
    ...
    */
    ...
    // Are we due to tack?
    // TODO: double check that we did tack if we ment to
    // TODO: do_tack routine that takes the current and target heading and gives target rates depending on conditons, ie in light wind tack slowly to conserve momentum
    if (_sailboat_tack) {
    ...
    // If were in the process of a tack we should not change the target heading, not sure if this is a good idea or not, the target shouldent change but too much while tacking, except if the vane provides poor readings as we are tacking
...
float Rover::sailboat_update_rate_max(float rate_max_degs)
{
    ...
    // if were travling in a 'straight line' on a single tack reduce the maximum allowed rate to smooth out heading response to wind changes, use normal max rate for tacking
