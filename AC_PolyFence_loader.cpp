...
uint16_t AC_PolyFence_loader::max_items() const
{
    ...
    return fence_storage.size() / sizeof(Vector2l);//@yu數量修正
}
...
//        Debug("fence load requires origin");
    ...
    { // allocate storage for inclusion polyfences:
        const auto count = index_fence_count(AC_PolyFenceType::POLYGON_INCLUSION);//@yu數量修正
    ...
    { // allocate storage for exclusion polyfences:
        const auto count = index_fence_count(AC_PolyFenceType::POLYGON_EXCLUSION);//@yu數量修正
    ...
    { // allocate storage for circular inclusion fences:
        uint32_t count = index_fence_count(AC_PolyFenceType::CIRCLE_INCLUSION);//@yu數量修正
    ...
    { // allocate storage for circular exclusion fences:
        uint32_t count = index_fence_count(AC_PolyFenceType::CIRCLE_EXCLUSION);//@yu數量修正
