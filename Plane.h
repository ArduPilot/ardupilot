...
private:
    ...
    */
    ...
    // @yu ti function
    float get_target_speed(float asp,float origin_up_speed,float min_up_speed);
    Location last_vehical_location;//前一次的位置
    uint32_t time_last_update_in_auto_air = 0;
    // for ship takeoff
    Vector3f takeoff_vel;
    Vector3f base_takeoff_vel;//備份初速用
    Location takeoff_pos;
    bool side_shift_done;
    Vector3f shift_start_vel;
