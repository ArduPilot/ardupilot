...
/*
 * Init and run calls for semi_auto flight mode
 * Flight mode developed by Aerospace Scannig Technologies AERSOCANTECH.
 * Programmed by Systems Engineer ANDRES ARDILA SEDANO nick name a2sAndres.
 * This flight mode is based on the Auto and Alt_hold flight modes, which allows manual altitude control. 
 * While entering the height control the aircraft stops.
 * it re-starts the forward advance by giving it pitch input and continuing with the flight plan.
 */
...
bool Copter::ModeSemiAuto::init(bool ignore_checks)
{
    if ((copter.position_ok() && copter.mission.num_commands() > 1) || ignore_checks) {
        _mode = SemiAuto_Loiter;
...
void Copter::ModeSemiAuto::run()
{
    // call the correct auto controller
    switch (_mode) {

    case SemiAuto_TakeOff:
        takeoff_run();
        break;

    case SemiAuto_WP:
    case SemiAuto_CircleMoveToEdge:
        wp_run();
        break;
    ...
    case SemiAuto_Circle:
        circle_run();
        break;

    case SemiAuto_Spline:
        spline_run();
        break;

    case SemiAuto_NavGuided:
#if NAV_GUIDED == ENABLED
        nav_guided_run();
#endif
        break;

    case SemiAuto_Loiter:
        loiter_run();
        break;

    case SemiAuto_NavPayloadPlace:
        payload_place_run();
        break;
    }
}
...
bool Copter::ModeSemiAuto::loiter_start()
{
...
void Copter::ModeSemiAuto::rtl_start()
{
    _mode = SemiAuto_RTL;
...
void Copter::ModeSemiAuto::takeoff_start(const Location& dest_loc)
{
    _mode = SemiAuto_TakeOff;
...
void Copter::ModeSemiAuto::wp_start(const Vector3f& destination)
{
    _mode = SemiAuto_WP;
...
// auto_wp_start - initialises waypoint controller to implement flying to a particular destination
void Copter::ModeSemiAuto::wp_start(const Location_Class& dest_loc)
{
    _mode = SemiAuto_WP;
...
void Copter::ModeSemiAuto::land_start()
{
...
// auto_land_start - initialises controller to implement a landing
void Copter::ModeSemiAuto::land_start(const Vector3f& destination)
{
    _mode = SemiAuto_Land;
...
void Copter::ModeSemiAuto::circle_movetoedge_start(const Location_Class &circle_center, float radius_m)
{
    ...
    if (dist_to_edge > 300.0f) {
        ...
        _mode = SemiAuto_CircleMoveToEdge;
...
void Copter::ModeSemiAuto::circle_start()
{
    _mode = SemiAuto_Circle;
...
void Copter::ModeSemiAuto::spline_start(const Location_Class& destination, bool stopped_at_start,
    AC_WPNav::spline_segment_end_type seg_end_type,
    const Location_Class& next_destination)
{
    _mode = SemiAuto_Spline;
...
void Copter::ModeSemiAuto::nav_guided_start()
{
    _mode = SemiAuto_NavGuided;
...
bool Copter::ModeSemiAuto::landing_gear_should_be_deployed() const
{
    switch (_mode) {
    case SemiAuto_Land:
        return true;
...
void Copter::ModeSemiAuto::payload_place_start()
{
    // set target to stopping point
    Vector3f stopping_point;
    loiter_nav->get_stopping_point_xy(stopping_point);
...
bool Copter::ModeSemiAuto::start_command(const AP_Mission::Mission_Command& cmd)
{
...
bool Copter::ModeSemiAuto::verify_command_callback(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::exit_mission()
{
    // play a tone
...
bool Copter::ModeSemiAuto::do_guided(const AP_Mission::Mission_Command& cmd)
{
    // only process guided waypoint if we are in guided mode
    if (copter.control_mode != GUIDED && !(copter.control_mode == AUTO && mode() == SemiAuto_NavGuided)) {
        return false;
    }
...
uint32_t Copter::ModeSemiAuto::wp_distance() const
{
    return wp_nav->get_wp_distance_to_destination();
}

int32_t Copter::ModeSemiAuto::wp_bearing() const
{
    return wp_nav->get_wp_bearing_to_destination();
}

// update mission
void Copter::ModeSemiAuto::run_autopilot()
{
...
bool Copter::ModeSemiAuto::verify_command(const AP_Mission::Mission_Command& cmd)
{
    switch (cmd.id) {
        //
        // navigation commands
...
void Copter::ModeSemiAuto::takeoff_run()
{
...
// auto_wp_run_semi - runs the auto waypoint controller 
// for flight mode SEMI_AUTO add a2s_Andres
//      called by auto_run at 100hz or more
void Copter::ModeSemiAuto::wp_run()
{
    // if not auto armed or motor interlock not enabled set throttle to zero and exit immediately
    if (!motors->armed() || !ap.auto_armed || !motors->get_interlock()) {
...
//      called by auto_run at 100hz or more
void Copter::ModeSemiAuto::spline_run()
{
    // if not auto armed or motor interlock not enabled set throttle to zero and exit immediately
    if (!motors->armed() || !ap.auto_armed || !motors->get_interlock()) {
        // To-Do: reset waypoint origin to current location because copter is probably on the ground so we don't want it lurching left or right on take-off
...
//      called by auto_run at 100hz or more
void Copter::ModeSemiAuto::land_run()
{
...
//      called by auto_run at 100hz or more
void Copter::ModeSemiAuto::rtl_run()
{
...
//      called by auto_run at 100hz or more
void Copter::ModeSemiAuto::circle_run()
{
...
//      called by auto_run at 100hz or more
void Copter::ModeSemiAuto::nav_guided_run()
{
...
//      called by auto_run at 100hz or more
void Copter::ModeSemiAuto::loiter_run()
{
    // if not auto armed or motor interlock not enabled set throttle to zero and exit immediately
    if (!motors->armed() || !ap.auto_armed || ap.land_complete || !motors->get_interlock()) {
        zero_throttle_and_relax_ac();
...
void Copter::ModeSemiAuto::payload_place_start(const Vector3f& destination)
{
    _mode = SemiAuto_NavPayloadPlace;
...
//      called by auto_run at 100hz or more
void Copter::ModeSemiAuto::payload_place_run()
{
...
bool Copter::ModeSemiAuto::payload_place_run_should_run()
{
    // muts be armed
    if (!motors->armed()) {
        return false;
...
void Copter::ModeSemiAuto::payload_place_run_loiter()
{
    // loiter...
    land_run_horizontal_control();
...
void Copter::ModeSemiAuto::payload_place_run_descend()
{
    land_run_horizontal_control();
    land_run_vertical_control();
}
...
Location_Class Copter::ModeSemiAuto::terrain_adjusted_location(const AP_Mission::Mission_Command& cmd) const
{
...
void Copter::ModeSemiAuto::do_takeoff(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    Location_Class target_loc(cmd.content.location);
    const Location_Class &current_loc = copter.current_loc;
...
void Copter::ModeSemiAuto::do_land(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_loiter_unlimited(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_circle(const AP_Mission::Mission_Command& cmd)
{
...
// note: caller should set yaw_mode
void Copter::ModeSemiAuto::do_loiter_time(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_spline_wp(const AP_Mission::Mission_Command& cmd)
{
    Location_Class target_loc(cmd.content.location);
    const Location_Class &current_loc = copter.current_loc;
...
void Copter::ModeSemiAuto::do_nav_guided_enable(const AP_Mission::Mission_Command& cmd)
{
    if (cmd.p1 > 0) {
...
void Copter::ModeSemiAuto::do_guided_limits(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_nav_delay(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_wait_delay(const AP_Mission::Mission_Command& cmd)
{
    condition_start = millis();
    condition_value = cmd.content.delay.seconds * 1000;     // convert seconds to milliseconds
}

void Copter::ModeSemiAuto::do_within_distance(const AP_Mission::Mission_Command& cmd)
{
    condition_value = cmd.content.distance.meters * 100;
}

void Copter::ModeSemiAuto::do_yaw(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_change_speed(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_set_home(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_roi(const AP_Mission::Mission_Command& cmd)
{
    auto_yaw.set_roi(cmd.content.location);
}

// point the camera to a specified angle
void Copter::ModeSemiAuto::do_mount_control(const AP_Mission::Mission_Command& cmd)
{
#if MOUNT == ENABLED
...
void Copter::ModeSemiAuto::do_digicam_configure(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_digicam_control(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_parachute(const AP_Mission::Mission_Command& cmd)
{
    switch (cmd.p1) {
...
void Copter::ModeSemiAuto::do_gripper(const AP_Mission::Mission_Command& cmd)
{
...
void Copter::ModeSemiAuto::do_winch(const AP_Mission::Mission_Command& cmd)
{
    // Note: we ignore the gripper num parameter because we only support one gripper
...
void Copter::ModeSemiAuto::do_payload_place(const AP_Mission::Mission_Command& cmd)
{
    // if location provided we fly to that location at current altitude
    if (cmd.content.location.lat != 0 || cmd.content.location.lng != 0) {
        // set state to fly to location
...
void Copter::ModeSemiAuto::do_RTL(void)
{
...
bool Copter::ModeSemiAuto::verify_takeoff()
{
...
bool Copter::ModeSemiAuto::verify_land()
{
    bool retval = false;
...
bool Copter::ModeSemiAuto::verify_payload_place()
{
...
bool Copter::ModeSemiAuto::verify_loiter_unlimited()
{
    return false;
}

// verify_loiter_time - check if we have loitered long enough
bool Copter::ModeSemiAuto::verify_loiter_time()
{
...
bool Copter::ModeSemiAuto::verify_RTL()
{
...
bool Copter::ModeSemiAuto::verify_wait_delay()
{
...
bool Copter::ModeSemiAuto::verify_within_distance()
{
...
bool Copter::ModeSemiAuto::verify_yaw()
{
...
bool Copter::ModeSemiAuto::verify_nav_wp(const AP_Mission::Mission_Command& cmd)
{
...
bool Copter::ModeSemiAuto::verify_circle(const AP_Mission::Mission_Command& cmd)
{
    // check if we've reached the edge
    if (mode() == SemiAuto_CircleMoveToEdge) {
        if (copter.wp_nav->reached_wp_destination()) {
...
bool Copter::ModeSemiAuto::verify_spline_wp(const AP_Mission::Mission_Command& cmd)
{
    // check if we have reached the waypoint
    if (!copter.wp_nav->reached_wp_destination()) {
        return false;
...
bool Copter::ModeSemiAuto::verify_nav_guided_enable(const AP_Mission::Mission_Command& cmd)
{
...
bool Copter::ModeSemiAuto::verify_nav_delay(const AP_Mission::Mission_Command& cmd)
{
