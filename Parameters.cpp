...
const AP_Param::Info Plane::var_info[] = {
    ...
    // @Param: TI_HOLD_RC2LONG
    // @DisplayName: Hold rc input untill trigger fs_long_timeout.
    // @Description: Enable when only use GCS FS ,this will hold rc input nutill trigger fs_long_timeout instead of 1 sec.
    // @Values: 0:Disabled,1:Enabled
    // @User: Advanced
    GSCALAR(ti_hold_thr2long, "TI_HOLD_RC2LONG", 0),

    // @Param: TI_AS2QUP_MIN
    // @DisplayName: The min up speed  during vtol takeoff if airspeed too high.
    // @Description: If not zero ,reduce vtol takeoff climb speed to this value based on wind speed.
    // @Units: m/s
    // @Range: 0 10
    // @User: Advanced
    GSCALAR(ti_arspd2up_min, "TI_AS2QUP_MIN", 0),

    // @Param: TI_AS_SAFE_MAX
    // @DisplayName: The max airspeed that vehicale can hold position without decreas up speed.
    // @Description: If airspeed higher than this value and lower than q_assist_speed ,vtol takeoff climb speed will be decreas.
    // @Units: m/s
    // @Range: 0 10
    // @User: Advanced
    GSCALAR(ti_max_safe_arspd, "TI_AS_SAFE_MAX", 5),

    // @Param: TI_RTL2QRTL_ALT
    // @DisplayName: Auto change qrtl_alt to rtl_alt.
    // @Description: This will change q_rtl_alt to rtl_alt - 5 to avoid down before arrive home.
    // @Values: 0:Disabled, 1:enable
    // @User: Advanced
    GSCALAR(ti_auto_fix_alt, "TI_RTL2QRTL_ALT", 0),

    // @Param: TI_WP_RLL_RATE
    // @DisplayName: Auto change roll_rate_ff in auto plane mode.
    // @Description: This will change roll_rate_ff to this value just in auto mode.
    // @Range: 0 3.000
    // @Increment: 0.001
    // @User: Advanced
    GSCALAR(ti_wp_roll_rate_ff, "TI_WP_RLL_RATE", 0.0),

    // @Param: TI_GPS_FS_TYPE
    // @DisplayName: ti gps failsafe use
    // @Description: Use gps failsafe ,description by @yu ,to avoid flying without gps signal ,change to rtl and circle at home ,until gps signal restore ,if value = 2,will change to qhover mode when loss gps signal,and change to circle mode after ti_gps_fs_overtime sec
    // @Values: 0:Disabled,1:Enabled,2:just qhover
    // @User: Advanced
    GSCALAR(ti_gps_fs, "TI_GPS_FS_USE", 0),

    // @Param: TI_GPS_FS_TIME
    // @DisplayName: ti gps failsafe overtime
    // @Description: gps signal loss overtime;if ti_gps_fs = 2 ,mean the time before mode change to circle,-1 will keep qhover;if ti_gps_fs = 1 ,mean the time how long we allow gps signal loss
    // @Range: 0 100
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_gps_fs_overtime, "TI_GPS_FS_TIME", 5),

    // @Param: TI_GPS_FS_OPT
    // @DisplayName: ti gps failsafe option
    // @Description: Ti gps failsafe function's option
    // @Bitmask: 0: only in auto mode, 1: use FW wait at home,
    // @User: Advanced
    GSCALAR(ti_gps_fs_option, "TI_GPS_FS_OPT", 0),

    // @Param: TI_RTL_ASPD_DN
    // @DisplayName: Speed down when RTL near home
    // @Description: Will chenge target airspeed to airspeed_min + 2 if arspd is use
    // @Values: 0:Disabled,1:Enabled
    // @User: Advanced
    GSCALAR(ti_rtl_spd_dn, "TI_RTL_ASPD_DN", 0),

    // @Param: TI_FOLL_TRI_LEN
    // @DisplayName: Planes triangle formation's vertical length 
    // @Description: Triangle formation's vertical length
    // @Range: 0 1000
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_tri_v, "TI_FOLL_TRI_LEN", 0),

    // @Param: TI_FOLL_TRI_WID
    // @DisplayName: Planes triangle formation's horizontal length 
    // @Description: Triangle formation's horizontal length
    // @Range: 0 1000
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_tri_h, "TI_FOLL_TRI_WID", 0),

    // @Param: TI_FOLL_AIRSPD
    // @DisplayName: Planes triangle formation's airspeed m/s
    // @Description: Triangle formation's airspeed  .If zero ,use origin airspeed_cruise 
    // @Range: 0 100
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_spd_m, "TI_FOLL_AIRSPD", 0.0),

    // @Param: TI_FOLL_SPD_P
    // @DisplayName: Planes triangle formation's speed PID control's P gain 
    // @Description: Triangle formation's airspeed
    // @Range: 0.0 5
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_spd_p, "TI_FOLL_SPD_P", 0.1),

    // @Param: TI_FOLL_SPD_I
    // @DisplayName: Planes triangle formation's speed PID control's I gain
    // @Description: Triangle formation's airspeed
    // @Range: 0.0 0.5
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_spd_i, "TI_FOLL_SPD_I", 0.0),

    // @Param: TI_FOLL_SPD_D
    // @DisplayName: Planes triangle formation's speed PID control's D gain
    // @Description: Triangle formation's airspeed
    // @Range: 0.0 0.5
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_spd_d, "TI_FOLL_SPD_D", 0.05),

    // @Param: TI_FOLL_SP_GAIN
    // @DisplayName: Planes triangle formation's speed PID control's total gain
    // @Description: Triangle formation's speed contrallor's total gain
    // @Range: 1 10
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_spd_gain, "TI_FOLL_SP_GAIN", 1.0),

    // @Param: TI_FOLL_SPD_D
    // @DisplayName: Planes triangle formation's speed PD control's D gain
    // @Description: Triangle formation's airspeed
    // @Values: 0:back to original cruise airspeed, 1:wait for original leader, 2:change to follow next member(not available now)
    // @User: Advanced
    GSCALAR(ti_group_type, "TI_FOLL_TYPE", 1),

    // @Param: TI_FOLL_FF_GAIN
    // @DisplayName: The gain for the feed forward airspeed control
    // @Description: The gain for the feed forward airspeed control,0:disable
    // @Range: 0 2  
    // @Increment: 1
    // @User: Advanced
    //GSCALAR(ti_group_ff_pid_gain, "TI_FOLL_FF_GAIN", 0.0),

    // @Param: TI_FOLL_FF_P
    // @DisplayName: The P gain for the feed forward airspeed control
    // @Description: The P gain for the feed forward airspeed control,0:disable
    // @Range: 0 2
    // @Increment: 0.01
    // @User: Advanced
    //GSCALAR(ti_group_ff_pid_p, "TI_FOLL_FF_P", 0.01),

    // @Param: TI_FOLL_FF_D
    // @DisplayName: The D gain for the feed forward airspeed control
    // @Description: The D gain for the feed forward airspeed control,0:disable
    // @Range: 0 2
    // @Increment: 0.01
    // @User: Advanced
    //GSCALAR(ti_group_ff_pid_d, "TI_FOLL_FF_D", 0.1),

    // @Param: TI_STATE_REPORT
    // @DisplayName: Report the ahrs status from mavlink
    // @Description: Report the ahrs status from mavlink every 10 second
    // @Bitmask: 0: altitude, 1: airspeed, 2: battsum/batt1 voltage
    // @User: Advanced
    GSCALAR(ti_status_report, "TI_STAT_REP_TYP", 0),

    // @Param: TI_STAT_REP_CYC
    // @DisplayName: The cycle for the ahrs status f Report
    // @Description: The cycle for the ahrs status f Report in second
    // @Range: 1 127
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_status_report_cycle, "TI_STAT_REP_CYC", 20),

    // @Param: TI_STAT_REP_BAT
    // @DisplayName: The index of batt that status report use
    // @Description: The index of batt that status report use
    // @Range: 1 16
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_status_report_batt_index, "TI_STAT_REP_BAT", 1),

    // @Param: TI_FOLL_STATUS
    // @DisplayName: Follow control status report
    // @Description: This will report follow control result by once a second
    // @Values: 0:disable, 1:enable
    // @User: Advanced
    GSCALAR(ti_group_status_report, "TI_FOLL_REPORT", 0),

    // @Param: TI_FOLL_ER_ALT
    // @DisplayName: The max altitude error accept in follow control
    // @Description: The max altitude error accept in follow control, if over, seem as target lost control, 0 disable
    // @Range: 0 127
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_error_alt_max, "TI_FOLL_ER_ALT", 0),

    // @Param: TI_FOLL_SPD_RAT
    // @DisplayName: The rate between up speed gain qnd down speed gain
    // @Description: If increase, the rate between up speed gain and down speed gain will increase
    // @Range: 0 1
    // @Increment: 0.1
    // @User: Advanced
    GSCALAR(ti_group_speed_rate, "TI_FOLL_F_L_RAT", 0.5),

    // @Param: TI_FOLL_UPD_CYC
    // @DisplayName: The cycle that follow control will update follow speed
    // @Description: This will average the sum of needed airspeed calculate consult, in second.0 disable.
    // @Range: 0 10
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_speed_update_cycle, "TI_FOLL_UPD_CYC", 3),

    // @Param: TI_FOLL_THR_TYP
    // @DisplayName: The follower's throttle control update type
    // @Description: If TI_FOLL_UPD_CYC over one, this will decide target speed will change in once or TI_FOLL_UPD_CYC time(smooth)
    // @Values: 0:once a cycle(sec), 1: smooth with TI_FOLL_UPD_CYC time(update 1Hz), 2:MAX increment 0.05m/s (update 10Hz)
    // @User: Advanced
    GSCALAR(ti_group_throttle_type, "TI_FOLL_THR_TYP", 0),

    // @Param: TI_FOLL_LNK_TES
    // @DisplayName: Link test between target and this mav
    // @Description: If true, this will triger follow link report in 5 sec when disarming
    // @Values: 0:disable, 1:enable, 2:always in disarm
    // @User: Advanced
    GSCALAR(ti_group_target_link_test, "TI_FOLL_LNK_TES", 0),

    // @Param: TI_FOLL_MAX_DIS
    // @DisplayName: The max distance that will continue follow
    // @Description: The max distance that will continue follow, if over, then will seem as lost target, 0 disable
    // @Range: 0 10000
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_target_lost_distance, "TI_FOLL_MAX_DIS", 0),

    // @Param: TI_FOLL_RNG_LIM
    // @DisplayName: The change limit of follow speed 
    // @Description: The change limit of follow speed, -1:disable limit, 0:nevigate speed control
    // @Range: 0 10
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_speed_range_limit, "TI_FOLL_RNG_LIM", -1),

    // @Param: TI_FOLL_WAIT_S
    // @DisplayName: The time that before follwer change to leader 
    // @Description: If follow type is 2, that the follower will wait as long as this parameter, 0 for wait ultimate
    // @Range: 0 1000
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_wait_long_time, "TI_FOLL_WAIT_S", 0),

    // @Param: TI_FOLL_ENABLE
    // @DisplayName: Enable TI follow mode (a.k.a formation) 
    // @Description: If is true ,then enable TI follow mode
    // @Values: 0:disable, 1:enable
    // @User: Advanced
    GSCALAR(ti_group_enable, "TI_FOLL_ENABLE", 0),

    // @Param: TI_Q_T_DECEL_WP
    // @DisplayName: Transition deceleration value in WP(ex: loiter_time)
    // @Description: If q_guided is enable, this will change vtol transition deceleration value in loiter mission
    // @Range: 0.2 5
    // @Increment: 0.1
    // @User: Advanced
    GSCALAR(ti_q_transition_decel_in_mission, "TI_WP_Q_T_DECEL", 0),

    // @Param: TI_FOLL_MIN_ALT
    // @DisplayName: The minimun altitude that Ti follow will be enable
    // @Description: This will disable Ti follow function until approach altitude as this value,  to avoid motor over heat when climb altitude in Ti follow mode. 0 unlimite.
    // @Range: 0 1000
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_group_min_safe_altitude, "TI_FOLL_MIN_ALT", 0),

    // @Param: TI_VOYAGE_CAL
    // @DisplayName: Auto calculate voyage in auto
    // @Description: If enable, will calculate the voyage in auto in 3 Hz, and finish when disarm.
    // @Values: 0:disable, 1:enable
    // @User: Advanced
    GSCALAR(ti_calculate_voyage_in_auto, "TI_VOYAGE_LOG", 1),

#if HAL_QUADPLANE_ENABLED
    // @Param: TI_SHIP_MATCH
    // @DisplayName: Match velocity of the ship
    // @Description: If enable, will match the velocity is ground speed is not zero when VTOL takeoff. Source by ship_vel_match.lua
    // @Values: 0:disable, 1:enable
    // @User: Advanced
    GSCALAR(ti_ship_vel_match, "TI_SHIP_MATCH", 0),

    // @Param: TI_SHIP_M_ALT
    // @DisplayName: Min altitude that will begin descend match speed
    // @Description: If not zero, the match speed will smoothly descend to 0 when altitude higher then this value in meter. -1 will not descend.
    // @Units: m
    // @Values: 0:disable, 1:enable
    // @User: Advanced
    GSCALAR(ti_ship_vel_match_min_alt, "TI_SHIP_M_ALT", -1),

    // @Param: TI_SHIP_M_LSPD
    // @DisplayName: The lowest speed that will make velocity match work
    // @Description: If initial speed lower than this value, the velocity match will not work
    // @Units: m/s
    // @Range: 0.0 1.0
    // @Increment: 0.1
    // @User: Advanced
    GSCALAR(ti_ship_vel_match_low_speed, "TI_SHIP_M_LSPD", 0.1),

    // @Param: TI_SHIP_M_DECEL
    // @DisplayName: The value that match speed will be descend per second
    // @Description: If TI_SHIP_MATCH_H not zero, the match speed will smoothly descend to 0 by this value per second.
    // @Units: m/s/s
    // @Range: 0.1 5
    // @Increment: 0.1
    // @User: Advanced
    GSCALAR(ti_ship_vel_descend_per_second, "TI_SHIP_M_DECEL", 0.5),

    // @Param: TI_SHIP_M_S_DIS
    // @DisplayName: The distance that vehicle will shift during the speed match
    // @Description: The distance that vehicle will shift by angle of TI_SHIP_M_S_POS relatively the vehicle in meter
    // @Units: m
    // @Range: 0 20
    // @Increment: 0.1
    // @User: Advanced
    GSCALAR(ti_ship_vel_side_shift_dist, "TI_SHIP_M_S_DIS", 0.0),

    // @Param: TI_SHIP_M_S_POS
    // @DisplayName: The position that vehicle will shift to
    // @Description: If is zero, side shift will not work, if > 0, will shift to right hand side, if < 0, will shift left hand side
    // @Values: -1:left side, t0:disable, 1:right side
    // @User: Advanced
    GSCALAR(ti_ship_vel_side_shift_position, "TI_SHIP_M_S_POS", 0.0),

    // @Param: TI_SHIP_M_S_ALT
    // @DisplayName: The min altitude(meter) that vehicle will begin shift
    // @Description: If altitude is higher than this value, vehicle will start do side shift. If TI_SHIP_M_ALT > -1, speed match will descend after this done.
    // @Units: m
    // @Range: 0 100
    // @Increment: 1
    // @User: Advanced
    GSCALAR(ti_ship_vel_side_shift_min_alt, "TI_SHIP_M_S_ALT", 1),
#endif
