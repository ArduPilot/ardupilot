...
MAVLINK_HELPER bool mavlink_encrypt_message(mavlink_message_t* msg) {
	...
	if (cipher_text_len != msg->len) {
		fprintf(stderr, "cipher_text_len != msg->len\n");
		return false;
	}
...
MAVLINK_HELPER bool mavlink_decrypt_message (mavlink_message_t* msg) {
	...
	status = mavlink_chacha20_256_decrypt(_MAV_PAYLOAD_UCHAR(msg), msg->len, 
		_MAV_PAYLOAD_UCHAR_NON_CONST(msg), plain_text_len);

	if (plain_text_len != msg->len) {
		fprintf(stderr, "plain_text_len != msg->len\n");
		return false;
	}
