    ...
    if (!crashed) {
        ...
        if (g.crash_detection_enable == CRASH_DETECT_ACTION_BITMASK_DISABLED) {
            if (crashed_near_land_waypoint) {
                gcs_frontend.send_text(MAV_SEVERITY_CRITICAL, "Hard landing detected. No action taken");
            } else {
                gcs_frontend.send_text(MAV_SEVERITY_EMERGENCY, "Crash detected. No action taken");
            }
        }
        else {
            ...
            if (crashed_near_land_waypoint) {
                gcs_frontend.send_text(MAV_SEVERITY_CRITICAL, "Hard landing detected");
            } else {
                gcs_frontend.send_text(MAV_SEVERITY_EMERGENCY, "Crash detected");
            }
        }
    }
}
