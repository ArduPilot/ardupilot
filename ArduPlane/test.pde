...
static int8_t test_range_finder(uint8_t argc, const Menu::arg *argv)
{
	...
	float range_finder_voltage = 0;
	int range_finder_distance = 0;
	...
	while (1) 
	{
		...
		range_finder_voltage = rangeFinder->_analog_source->voltage_average();
		range_finder_distance = rangeFinder->convert_raw_to_distance(range_finder_voltage);

		cliSerial->printf_P(PSTR("cur_dist: %d cm | volts: %0.2f v | health: %d | read: %d cm \n"),
			range_finder_distance,
			range_finder_voltage,
