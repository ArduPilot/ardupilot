...
void Plane::do_takeoff(const AP_Mission::Mission_Command& cmd)
{
	...
	if ((g.takeoff_throttle_min_speed>0) || (g.takeoff_throttle_min_accel>0)) {
		auto_state.takeoff_type=0; //Assume free hand or bungee launch
	} else {
		auto_state.takeoff_type=1; //Assume rolling runway takeoff with fixed runway direction
...
bool Plane::verify_takeoff()
{
    if (ahrs.yaw_initialised() && steer_state.hold_course_cd == -1) {
		if (auto_state.takeoff_type==0) //Free hand or bungee launch
		{
		...
		} else //Fixed direction runway takeoff
		{
			...
			//So instead, calculate & display deviation / correction angle from/to Centerline, feed L1 controller with additional info if extractable
			//Leave correction to the nav / L1 controller towards Runway Centerline Projected Waypoint (set in do_takeoff())
			if (millis()-auto_state.runway_takeoff_centerline_deviation_analysed_ms>1000) {
				auto_state.runway_takeoff_centerline_deviation_analysed_ms=millis();
    ...
    if ((steer_state.hold_course_cd != -1) && (auto_state.takeoff_type==0)) {
