...
void Plane::do_set_roi_location(const AP_Mission::Mission_Command& cmd)
{
    ...
    const uint8_t instance = camera_mount.get_specified_instance(gimbal_device_id);
    do_set_roi(instance, cmd.content.location);
}

void Plane::do_set_roi(const uint8_t instance, const Location &loc) {
    if (loc.initialised()) {
        camera_mount.set_roi_target(instance, loc);
        return;
    }

    // switch off the camera tracking if enabled
    if (camera_mount.get_mode(instance) == MAV_MOUNT_MODE_GPS_POINT) {
        camera_mount.set_mode_to_default(instance);
    }
}
