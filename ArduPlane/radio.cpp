...
int16_t Plane::rudder_input(void)
{
    ...
    if ((g2.flight_options & FlightOptions::DIRECT_RUDDER_ONLY) &&
        !(control_mode == &mode_manual || control_mode == &plane.mode_stallrecovery || control_mode == &mode_stabilize || control_mode == &mode_acro)) {
...
bool Plane::trim_radio()
{
    ...
    if (is_zero(control_flap_auto.scaled_output) && is_zero(control_flap.scaled_output)) {
