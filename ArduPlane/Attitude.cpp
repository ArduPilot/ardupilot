...
bool Plane::suppress_throttle(void)
{
    ...
    if (relative_altitude_abs_cm() >= 1000) {
        ...
        throttle_suppressed = false;
        gcs_frontend.send_text_fmt(MAV_SEVERITY_INFO, "Throttle enabled. Altitude %.2f",
    ...
    if (gps_movement) {
        ...
        if ((!ahrs.airspeed_sensor_enabled()) || airspeed.get_airspeed() >= 5) {
            ...
            gcs_frontend.send_text_fmt(MAV_SEVERITY_INFO, "Throttle enabled. Speed %.2f airspeed %.2f",
    ...
    if (quadplane.is_flying()) {
        gcs_frontend.send_text_fmt(MAV_SEVERITY_INFO, "Throttle enabled VTOL");
        throttle_suppressed = false;
    }
...
{
    while(i > 0) {
        gcs_frontend.send_text(MAV_SEVERITY_INFO,"Demo servos");
        demoing_servos = true;
        servo_write(1, 1400);
