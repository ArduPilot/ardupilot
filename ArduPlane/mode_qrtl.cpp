...
bool ModeQRTL::_enter()
{
    ...
    float next_WP_loc_alt = 0;
    UNUSED_RESULT(plane.next_WP_loc.get_alt_m(plane.next_WP_loc.get_alt_frame(), next_WP_loc_alt));
    poscontrol.slow_descent = (plane.current_loc.get_alt_m() > next_WP_loc_alt);
    return true;
}
...
void ModeQRTL::run()
{
    ...
    switch (submode) {
        case SubMode::climb: {
            ...
            Location stopping_loc = Location(stopping_point_neu_m.tofloat() * 100.0, Location::AltFrame::ABOVE_ORIGIN);
