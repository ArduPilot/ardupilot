...
void GCS_MAVLINK_Plane::handle_set_position_target_local_ned(const mavlink_message_t &msg)
    {
        ...
        float next_WP_loc_relalt = NaNf;
        UNUSED_RESULT(plane.next_WP_loc.get_alt_m(Location::AltFrame::ABOVE_HOME, next_WP_loc_relalt));
        gcs().send_text(MAV_SEVERITY_INFO, "Change alt to %.1f",
                        double(next_WP_loc_relalt));
    }
...
int16_t GCS_MAVLINK_Plane::high_latency_target_altitude() const
{
    ...
    if (quadplane.show_vtol_view()) {
        return (plane.control_mode != &plane.mode_qstabilize) ? (global_position_current.get_alt_m() + quadplane.pos_control->get_pos_error_U_m()) : 0;
    }
#endif
