...
void Plane::failsafe_short_on_event(enum failsafe_state fstype)
{
    ...
    gcs_frontend.send_text(MAV_SEVERITY_WARNING, "Failsafe. Short event on, ");
    switch(control_mode)
    {
    case MANUAL:
    case STABILIZE:
    ...
    gcs_frontend.send_text_fmt(MAV_SEVERITY_INFO, "Flight mode = %u", (unsigned)control_mode);
}

void Plane::failsafe_long_on_event(enum failsafe_state fstype)
{
    // This is how to handle a long loss of control signal failsafe.
    gcs_frontend.send_text(MAV_SEVERITY_WARNING, "Failsafe. Long event on, ");
    ...
    if (fstype == FAILSAFE_GCS) {
        gcs_frontend.send_text(MAV_SEVERITY_CRITICAL, "No GCS heartbeat");
    }
    gcs_frontend.send_text_fmt(MAV_SEVERITY_INFO, "Flight mode = %u", (unsigned)control_mode);
}
...
void Plane::failsafe_short_off_event()
{
    ...
    gcs_frontend.send_text(MAV_SEVERITY_WARNING, "Failsafe. Short event off");
...
void Plane::low_battery_event(void)
{
    ...
    gcs_frontend.send_text_fmt(MAV_SEVERITY_WARNING, "Low battery %.2fV used %.0f mAh",
