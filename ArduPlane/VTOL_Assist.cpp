    ...
    if (angle <= 0) {
        ...
        */
        ...
        const bool inside_envelope =
            (labs(ahrs_roll_deg) <= (plane.aparm.roll_limit + allowed_envelope_error_deg)) &&
        ...
        const bool inside_angle_error =
            (labs(ahrs_roll_deg - plane.nav_roll_cd*0.01) < angle) &&
            (labs(ahrs_pitch_deg - plane.nav_pitch_cd*0.01) < angle);
