...
bool Plane::auto_takeoff_check(void)
{
    ...
    if (last_check_ms && (now - last_check_ms) > 200) {
        gcs_frontend.send_text_fmt(MAV_SEVERITY_WARNING, "Timer interrupted AUTO");
    ...
    if (!launchTimerStarted) {
        ...
        gcs_frontend.send_text_fmt(MAV_SEVERITY_INFO, "Armed AUTO, xaccel = %.1f m/s/s, waiting %.1f sec",
    ...
    if ((now - last_tkoff_arm_time) > wait_time_ms+100U) {
        gcs_frontend.send_text_fmt(MAV_SEVERITY_WARNING, "Timeout AUTO");
        goto no_launch;
    }
    ...
    if (ahrs.pitch_sensor <= -3000 ||
        ...
        gcs_frontend.send_text_fmt(MAV_SEVERITY_WARNING, "Bad launch AUTO");
        goto no_launch;
    }
    ...
    if (((gps.ground_speed() > g.takeoff_throttle_min_speed || is_zero(g.takeoff_throttle_min_speed))) &&
        ...
        gcs_frontend.send_text_fmt(MAV_SEVERITY_INFO, "Triggered AUTO. GPS speed = %.1f", (double)gps.ground_speed());
        launchTimerStarted = false;
        last_tkoff_arm_time = 0;
...
return_zero:
    if (auto_state.fbwa_tdrag_takeoff_mode) {
        gcs_frontend.send_text(MAV_SEVERITY_NOTICE, "FBWA tdrag off");
