...
void Plane::parachute_release()
{
    ...
    gcs_frontend.send_text(MAV_SEVERITY_CRITICAL,"Parachute: Released");
...
bool Plane::parachute_manual_release()
{
    ...
    if (!is_flying()) {
        ...
        gcs_frontend.send_text(MAV_SEVERITY_WARNING,"Parachute: Not flying");
        return false;
    }

    if (relative_altitude() < parachute.alt_min()) {
        gcs_frontend.send_text_fmt(MAV_SEVERITY_WARNING, "Parachute: Too low");
        return false;
    }
