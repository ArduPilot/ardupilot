...
const AP_Param::Info Plane::var_info[] = {
    ...
    // @Description: NOTE, this ONLY APPLIES TO GCS failsafe at the moment, but the long term intention is to have ALL failure modes use it.  The action to take on a failsafe event. If the aircraft was in a stabilization or manual mode when failsafe started and a failsafe occurs then it will change to RTL mode if FS_ACTION is 0 or 1, and will change to FBWA if FS_ACTION is set to 2. If the aircraft was in an auto mode (such as AUTO or GUIDED) when the failsafe started then it will continue in the auto mode if FS_ACTION is set to 0, will change to RTL mode if FS_ACTION is set to 1 and will change to FBWA mode if FS_ACTION is set to 2. If FS_ACTION is set to 3 the aircraft exeutes the landing sequesnce (which should have been determined preflight).  If FS_ACTION is set to 4 the aircraft executes the predetermined quadplane landing.  If FS_ACTION is set to 5 the aircraft executes the terminate flight behavior defined in the Advanced Failsafe. If FS_ACTION is set to 6, the parachute will be deployed (make sure the chute is configured and enabled).
    
    // @Values: 0:Continue,1:ReturnToLaunch,2:Glide,3:Land,4:QLand,5:Terminate,6:Deploy Parachute
    // @User: Standard
