...
float Plane::lookahead_adjustment(void)
{
    ...
    // we need to know the climb ratio. We use PTCH_LKAHD_CLMB % of the maximum
    // climb rate so we are not constantly at 100% throttle and to
    // give a bit more margin on terrain
    float climb_ratio = g2.pitch_lookahead_climb_ratio * 0.01 * TECS_controller.get_max_climbrate() / groundspeed;
