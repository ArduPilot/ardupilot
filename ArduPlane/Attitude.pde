...
static bool auto_takeoff_check(void)
{
    ...
    if (SpdHgt_Controller->get_VXdot() >= g.takeoff_throttle_min_accel || 
        ...
        launchTimerStarted)
    {
        if (!launchTimerStarted)
        {
            ...
            gcs_send_text_fmt(PSTR("Armed AUTO, xaccel = %.1f m/s/s, waiting %.1f sec"), SpdHgt_Controller->get_VXdot(), 0.1f*float(min(g.takeoff_throttle_delay,25)));
        }
 		...
 		if ((now - last_tkoff_arm_time) <= 2500)
		{
                ...
                abs(ahrs.roll_sensor) < 3000)
            {
			    ...
			    if (((g_gps->ground_speed_cm > g.takeoff_throttle_min_speed*100.0f || g.takeoff_throttle_min_speed == 0.0)) && 
                    ((now -last_tkoff_arm_time) >= min(uint32_t(g.takeoff_throttle_delay*100),2500)))
                {
