...
bool Plane::verify_land()
{
    ...
    */
    ...
    if (height <= g.land_flare_alt ||
        ...
        if (!auto_state.land_complete) {
            ...
            if (!is_flying() && (millis()-auto_state.last_flying_ms) > 3000) {
                gcs_frontend.send_text_fmt(MAV_SEVERITY_CRITICAL, "Flare crash detected: speed=%.1f", (double)gps.ground_speed());
            } else {
                gcs_frontend.send_text_fmt(MAV_SEVERITY_INFO, "Flare %.1fm sink=%.2f speed=%.1f dist=%.1f",
    ...
    if (auto_state.post_landing_stats && !arming.is_armed()) {
        ...
        gcs_frontend.send_text_fmt(MAV_SEVERITY_INFO, "Distance from LAND point=%.2fm", (double)get_distance(current_loc, next_WP_loc));
    }
...
void Plane::disarm_if_autoland_complete()
{
    if (g.land_disarm_delay > 0 && 
        ...
        if (millis() - auto_state.last_flying_ms >= g.land_disarm_delay*1000UL) {
            if (disarm_motors()) {
                gcs_frontend.send_text(MAV_SEVERITY_INFO,"Auto disarmed");
            }
        }
    }
}
...
bool Plane::restart_landing_sequence()
{
            ...
            mission.set_current_cmd(current_index+1))
    {
        ...
        gcs_frontend.send_text_fmt(MAV_SEVERITY_NOTICE, "Restarted landing sequence. Climbing to %dm", cmd.content.location.alt/100);
        success =  true;
    }
            ...
            mission.set_current_cmd(do_land_start_index))
    {
        ...
        gcs_frontend.send_text_fmt(MAV_SEVERITY_NOTICE, "Restarted landing via DO_LAND_START: %d",do_land_start_index);
        success =  true;
    }
               ...
               mission.set_current_cmd(prev_cmd_with_wp_index))
    {
        ...
        gcs_frontend.send_text_fmt(MAV_SEVERITY_NOTICE, "Restarted landing sequence at waypoint %d", prev_cmd_with_wp_index);
        success =  true;
    } else {
        gcs_frontend.send_text_fmt(MAV_SEVERITY_WARNING, "Unable to restart landing sequence");
        success =  false;
    }
    return success;
}
...
bool Plane::jump_to_landing_sequence(void) 
{
    ...
    if (land_idx != 0) {
        if (mission.set_current_cmd(land_idx)) {
            ...
            gcs_frontend.send_text(MAV_SEVERITY_INFO, "Landing sequence start");
            return true;
        }            
    }

    gcs_frontend.send_text(MAV_SEVERITY_WARNING, "Unable to start landing sequence");
    return false;
}
