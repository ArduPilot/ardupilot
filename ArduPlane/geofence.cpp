...
void Plane::geofence_load(void)
{
    ...
    gcs_frontend.send_text(MAV_SEVERITY_INFO,"Geofence loaded");
    gcs_frontend.send_message(MSG_FENCE_STATUS);
    return;

failed:
    g.fence_action.set(FENCE_ACTION_NONE);
    gcs_frontend.send_text(MAV_SEVERITY_WARNING,"Geofence setup error");
}
...
void Plane::geofence_check(bool altitude_check_only)
{
    ...
    if (!outside) {
        if (geofence_state->fence_triggered && !altitude_check_only) {
            ...
            geofence_state->fence_triggered = false;
            gcs_frontend.send_text(MAV_SEVERITY_INFO,"Geofence OK");
    ...
    gcs_frontend.send_text(MAV_SEVERITY_NOTICE,"Geofence triggered");
    gcs_frontend.send_message(MSG_FENCE_STATUS);
