...
void QuadPlane::relax_roll_and_yaw_controllers(void)
{
    ...
    Quaternion attitude_target_quat = attitude_control->get_attitude_target_quat();
    attitude_target_quat = current_attitude * pitch_rotation;
    attitude_target_quat.normalize();
    Vector3f target_eulers;
    attitude_target_quat.to_euler(target_eulers.x, target_eulers.y, target_eulers.z);
    attitude_control->set_attitude_target_euler_angle(target_eulers);
    attitude_control->set_attitude_ang_error(current_attitude.inverse() * attitude_target_quat);

    // Initialize the roll and yaw angular rate variables to the current rate
    attitude_control->set_attitude_target_ang_vel(ahrs.get_gyro());
    attitude_control->set_rate_target_ang_vel(Vector3f(ahrs.get_gyro().x, 0, ahrs.get_gyro().z));
...
void QuadPlane::input_euler_rate_yaw_euler_angle_pitch_bf_roll(bool plane_controls, float body_roll_cd, float euler_pitch_cd, float euler_yaw_rate_cds)
{
    ...
    ahrs_view->get_quat_body_to_ned(attitude_vehicle_quat);
    Quaternion attitude_target_quat = attitude_control->get_attitude_target_quat();
    error_quat = attitude_vehicle_quat.inverse() * attitude_target_quat;
    Vector3f att_error;
    ...
    Vector3f attitude_target_euler_angle = attitude_control->get_attitude_target_euler_angle();
    attitude_target_euler_angle.z = wrap_PI(attitude_target_euler_angle.z + yaw_rate * loop_delta_t);
    attitude_control->set_attitude_target_euler_angle(attitude_target_euler_angle);

    // init attitude target to desired euler yaw and pitch with zero roll
    attitude_target_quat.from_euler(0, euler_pitch, attitude_target_euler_angle.z);
    ...
    attitude_control->set_attitude_target_quat(attitude_target_quat);
    ...
    attitude_control->set_attitude_target_ang_vel(Vector3f(0.0f, 0.0f, 0.0f));
    ...
    attitude_control->set_rate_target_ang_vel(attitude_control->update_ang_vel_target_from_att_error(att_error));
}
