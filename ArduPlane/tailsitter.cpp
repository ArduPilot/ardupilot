...
{
    return plane.quadplane.frame_class == AP_Motors::MOTOR_FRAME_TAILSITTER
...
bool Tailsitter::active(void)
{
    ...
    if (plane.quadplane.transition_state == QuadPlane::TRANSITION_ANGLE_WAIT_FW) {
        return true;
    }
    return false;
}
...
{
    if (!enabled() || plane.quadplane.motor_test.running) {
    ...
    if (!active() || in_vtol_transition()) {
        ...
        if (hal.util->get_soft_armed() && in_vtol_transition() && !plane.quadplane.throttle_wait && plane.is_flying()) {
            /*
            ...
            */
            throttle = plane.quadplane.motors->thrust_to_actuator(plane.quadplane.motors->get_throttle_hover()) * 100;
            throttle = MAX(throttle,plane.aparm.throttle_cruise.get());

            SRV_Channels::set_output_scaled(SRV_Channel::k_rudder, 0);
            plane.quadplane.pos_control->get_accel_z_pid().set_integrator(throttle*10);
            ...
            uint16_t throttle_pwm = plane.quadplane.motors->get_pwm_output_min() + (plane.quadplane.motors->get_pwm_output_max() - plane.quadplane.motors->get_pwm_output_min()) * throttle * 0.01f;
        ...
        if (!plane.quadplane.assisted_flight) {
            // set AP_MotorsMatrix throttles for forward flight
            plane.quadplane.motors->output_motor_mask(throttle * 0.01f, motor_mask, plane.rudder_dt);

            // in forward flight: set motor tilt servos and throttles using FW controller
            if (vectored_forward_gain > 0) {
                // remove scaling from surface speed scaling and apply throttle scaling
                const float scaler = plane.control_mode == &plane.mode_manual?1:(plane.quadplane.tilt_throttle_scaling() / plane.get_speed_scaler());
    ...
    if (plane.quadplane.assisted_flight && (plane.quadplane.transition_state != QuadPlane::TRANSITION_ANGLE_WAIT_FW)) {
        plane.quadplane.hold_stabilize(SRV_Channels::get_output_scaled(SRV_Channel::k_throttle) * 0.01f);
        plane.quadplane.motors_output(true);

        if ((plane.quadplane.options & QuadPlane::OPTION_TAILSIT_Q_ASSIST_MOTORS_ONLY) != 0) {
            // only use motors for Q assist, control surfaces remain under plane control
            // zero copter I terms and use plane
            plane.quadplane.attitude_control->reset_rate_controller_I_terms();

            // output tilt motors
            tilt_left = 0.0f;
            tilt_right = 0.0f;
            if (vectored_hover_gain > 0) {
                const float hover_throttle = plane.quadplane.motors->get_throttle_hover();
                const float throttle = plane.quadplane.motors->get_throttle();
        ...
        plane.quadplane.motors_output(false);
    }
    ...
    SRV_Channels::set_output_scaled(SRV_Channel::k_aileron, (plane.quadplane.motors->get_yaw() + plane.quadplane.motors->get_yaw_ff())*-SERVO_MAX);
    SRV_Channels::set_output_scaled(SRV_Channel::k_elevator, (plane.quadplane.motors->get_pitch() + plane.quadplane.motors->get_pitch_ff())*SERVO_MAX);
    SRV_Channels::set_output_scaled(SRV_Channel::k_rudder, (plane.quadplane.motors->get_roll() + plane.quadplane.motors->get_roll_ff())*SERVO_MAX);
    ...
    if (vectored_hover_gain > 0) {
         ...
         */
        float des_pitch_cd = plane.quadplane.attitude_control->get_att_target_euler_cd().y;
        int32_t pitch_error_cd = (des_pitch_cd - plane.quadplane.ahrs_view->pitch_sensor) * 0.5;
        float extra_pitch = constrain_float(pitch_error_cd, -SERVO_MAX, SERVO_MAX) / SERVO_MAX;
        float extra_sign = extra_pitch > 0?1:-1;
        float extra_elevator = 0;
        if (!is_zero(extra_pitch) && plane.quadplane.in_vtol_mode()) {
    ...
    if (roll_lim) {
        plane.quadplane.motors->limit.roll = true;
    }
    if (pitch_lim || tilt_lim) {
        plane.quadplane.motors->limit.pitch = true;
    }
    if (yaw_lim || tilt_lim) {
        plane.quadplane.motors->limit.yaw = true;
    }
...
bool Tailsitter::transition_fw_complete(void)
{
    ...
    if (labs(plane.quadplane.ahrs_view->pitch_sensor) > transition_angle_fw*100) {
        gcs().send_text(MAV_SEVERITY_INFO, "Transition FW done");
        return true;
    }
    if (labs(plane.quadplane.ahrs_view->roll_sensor) > MAX(4500, plane.roll_limit_cd + 500)) {
        gcs().send_text(MAV_SEVERITY_WARNING, "Transition FW done, roll error");
        return true;
    }
    if (AP_HAL::millis() - plane.quadplane.transition_start_ms > ((transition_angle_fw+(plane.quadplane.transition_initial_pitch*0.01f))/transition_rate_fw)*1500) {
...
bool Tailsitter::transition_vtol_complete(void) const
{
    ...
    if (AP_HAL::millis() - plane.quadplane.transition_start_ms >  ((trans_angle-(plane.quadplane.transition_initial_pitch*0.01f))/transition_rate_vtol)*1500) {
...
{
    if (!enabled() || !plane.quadplane.in_vtol_mode()) {
        return false;
    }
    if (plane.quadplane.transition_state == QuadPlane::TRANSITION_ANGLE_WAIT_VTOL) {
        return true;
    }
    if ((now != 0) && ((now - plane.quadplane.last_vtol_mode_ms) > 1000)) {
...
{
    return enabled() && !plane.quadplane.in_vtol_mode() && plane.quadplane.transition_state == QuadPlane::TRANSITION_DONE;
}
...
void Tailsitter::speed_scaling(void)
{
    ...
    if ((gain_scaling_mask & TAILSITTER_GSCL_ATT_THR) != 0) {
        ...
        const float c_tilt = plane.quadplane.ahrs_view->get_rotation_body_to_ned().c.z;
        ...
        float airspeed;
        if (!plane.quadplane.ahrs.airspeed_estimate(airspeed)) {
