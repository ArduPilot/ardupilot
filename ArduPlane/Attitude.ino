...
static bool demoing_servos_test;

static void demo_servos_test(uint8_t i)
{
    while(i > 0){
        demoing_servos_test = true;
        servo_write(0, 900);
        servo_write(1, 900);
        servo_write(3, 900);
        mavlink_delay(300);
        ...
        servo_write(3, 1900);
        mavlink_delay(300);
        demoing_servos_test = false;
        mavlink_delay(300);
        i--;
    }
}

static bool dance_now;

static void dance_test(uint8_t i)
{
  while(i > 0){
    dance_now = true;
    for(unsigned x = 900; x < 1900; x++){
      servo_write(0, x);
        servo_write(1, x);
        servo_write(3, x);
        //mavlink_delay(10);
    }
    for(unsigned x = 1900; x > 900; x--){
      servo_write(0, x);
        servo_write(1, x);
        servo_write(3, x);
        //mavlink_delay(10);
