...
bool Plane::adsb_evasion_start(void)
{
    ...
    default:
        gcs_frontend.send_text(MAV_SEVERITY_CRITICAL, "ADS-B threat found, no action taken");
        break;

    case AP_ADSB::ADSB_BEHAVIOR_GUIDED:
        gcs_frontend.send_text(MAV_SEVERITY_CRITICAL, "ADS-B threat found, switching to GUIDED mode");
        set_mode(GUIDED);
    ...
    case AP_ADSB::ADSB_BEHAVIOR_LOITER_AND_DESCEND:
        gcs_frontend.send_text(MAV_SEVERITY_CRITICAL, "ADS-B threat found, performing LOITER");
        set_mode(LOITER);
        adsb_state.is_evading = true; // must be done after set_mode()
        break;
    } // switch behavior
...
{
    gcs_frontend.send_text(MAV_SEVERITY_CRITICAL, "ADS-B threat gone, continuing mission");
