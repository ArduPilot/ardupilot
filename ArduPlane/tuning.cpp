...
AP_Float *AP_Tuning_Plane::get_param_pointer(uint8_t parm)
{
    ...
    case TUNING_PZ_P:
        return &plane.quadplane.pos_control->get_pos_D_p().kP();
    ...
    case TUNING_VZ_P:
        return &plane.quadplane.pos_control->get_vel_D_pid().kP();

    case TUNING_AZ_P:
        return &plane.quadplane.pos_control->get_accel_D_pid().kP();

    case TUNING_AZ_I:
        return &plane.quadplane.pos_control->get_accel_D_pid().kI();

    case TUNING_AZ_D:
        return &plane.quadplane.pos_control->get_accel_D_pid().kD();
