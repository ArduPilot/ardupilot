...
// agmatthews USERHOOKS
    ...
    if (check_startup_for_CLI()) {
        digitalWriteFast(A_LED_PIN, LED_ON);                        // turn on setup-mode LED
        cliSerial->printf_P(PSTR("\nCLI:\n\n"));
        run_cli(cliSerial);
    }
#else
...
{
    digitalWriteFast(A_LED_PIN, on ? LED_OFF : LED_ON);
    digitalWriteFast(C_LED_PIN, on ? LED_ON : LED_OFF);
}
