...
static void NOINLINE send_nav_controller_output(mavlink_channel_t chan)
{
    mavlink_msg_nav_controller_output_send(
        ...
        wp_bearing / 1.0e2,
        wp_distance / 1.0e2,
        wp_distance, //altitude_error / 1.0e2,
        0,
...
static void NOINLINE send_raw_imu2(mavlink_channel_t chan)
{
    mavlink_msg_scaled_pressure_send(
        chan,
        millis(),
        0, //(float)barometer.get_pressure()/100.0,
        0, //(float)(barometer.get_pressure() - barometer.get_ground_pressure())/100.0,
        0); //(int)(barometer.get_temperature()*10));
}
