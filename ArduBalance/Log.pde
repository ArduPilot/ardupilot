...
static void Log_Write_Motors()
{
    ...
    DataFlash.WriteInt(motor_out[0]);
    DataFlash.WriteInt(motor_out[1]);
    DataFlash.WriteByte(END_BYTE);
}
...
static void Log_Read_Motors()
{
    ...
    cliSerial->printf_P(PSTR("MOT, %d, %d\n"),temp1,temp2);
}
...
static void Log_Write_Nav_Tuning()
{
    ...
    DataFlash.WriteInt(nav_bearing/100);             // 3
    DataFlash.WriteInt(long_error);                  // 4

    DataFlash.WriteInt(lat_error);                   // 5
    DataFlash.WriteInt(nav_pitch);                   // 6
    DataFlash.WriteInt((int16_t)ground_speed);       // 7
    DataFlash.WriteInt(desired_nav_speed);               // 8
...
static void Log_Write_Control_Tuning()
{
    ...
    DataFlash.WriteInt(0);          // 1
    DataFlash.WriteInt(0);          // 2
    DataFlash.WriteInt(0);          // 3
    DataFlash.WriteInt(0);          // 4
    DataFlash.WriteInt(0);          // 5
    DataFlash.WriteInt(0);          // 6
    DataFlash.WriteInt(0);          // 7
    DataFlash.WriteInt(0);          // 8
    DataFlash.WriteInt(0);          // 9
...
static void Log_Write_Iterm()
{
    ...
    DataFlash.WriteInt((int16_t)g.pid_balance.get_integrator());    	 	// 1
    DataFlash.WriteInt((int16_t)g.pid_yaw.get_integrator());    	 		// 2
	DataFlash.WriteInt((int16_t)g.pid_nav.get_integrator());         	// 4
...
static void Log_Write_Attitude()
{
    ...
    DataFlash.WriteInt(wheel.right_speed_output);   // 1
    DataFlash.WriteInt(wheel.left_speed_output);    // 2
    DataFlash.WriteInt(wheel.right_speed); 		    // 3
    DataFlash.WriteInt(wheel.left_speed);    		// 4
    DataFlash.WriteInt(wheel.speed);	    		// 5
    DataFlash.WriteInt(pitch_speed);        			// 6
    DataFlash.WriteInt((int16_t)ahrs.pitch_sensor);	// 7
    DataFlash.WriteByte(END_BYTE);
}
...
static void Log_Read_Attitude()
{
    ...
    int16_t temp6 	= DataFlash.ReadInt();
                        	...
                        	// 1   2   3    4
    cliSerial->printf_P(PSTR("ATT, %d, %d, %d, %d, %d, %d\n"),
			temp1,
			temp2,
			temp3,
			temp4,
...
static void Log_Write_INAV(float delta_t)
{
    ...
    DataFlash.WriteInt(0);                                  // 1 barometer altitude
    DataFlash.WriteInt(0);               // 2 accel + baro filtered altitude
    DataFlash.WriteInt(0);                         // 3 barometer based climb rate
    DataFlash.WriteInt(0);             // 4 accel + baro based climb rate
