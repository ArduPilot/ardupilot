...
ASSERT_STORAGE_SIZE(GCS::statustext_t, 58);
    ...
    //@yu
    /**/
    uint8_t check_rc_fs_flage = 0;
    if(!(present & MAV_SYS_STATUS_SENSOR_RC_RECEIVER)){
        check_rc_fs_flage += 1;
    }
    if(!(enabled & MAV_SYS_STATUS_SENSOR_RC_RECEIVER)){
        check_rc_fs_flage += 2;
    }
    if(!(health & MAV_SYS_STATUS_SENSOR_RC_RECEIVER)){
        check_rc_fs_flage += 4;
    }
    if(check_rc_fs_flage != 0){
        if(last_rc_error_flage != check_rc_fs_flage){
            if(check_rc_fs_flage == 1){//@yu 這應該能讓我們確定是哪邊故障，如果有開RC FAILSAFE的話
                gcs().send_text(MAV_SEVERITY_CRITICAL, "No RC Receiver,P");
            }else if(check_rc_fs_flage == 2){
                gcs().send_text(MAV_SEVERITY_CRITICAL, "No RC Receiver,E");
            }else if(check_rc_fs_flage == 3){
                gcs().send_text(MAV_SEVERITY_CRITICAL, "No RC Receiver,P,E");
            }else if(check_rc_fs_flage == 4){
                gcs().send_text(MAV_SEVERITY_CRITICAL, "No RC Receiver,H");
            }else if(check_rc_fs_flage == 5){
                gcs().send_text(MAV_SEVERITY_CRITICAL, "No RC Receiver,P,H");
            }else if(check_rc_fs_flage == 6){
                gcs().send_text(MAV_SEVERITY_CRITICAL, "No RC Receiver,E,H");
            }else if(check_rc_fs_flage == 7){
                gcs().send_text(MAV_SEVERITY_CRITICAL, "No RC Receiver,P,E,H");
            }
            last_rc_error_flage = check_rc_fs_flage;
        }
    }else{
        if(last_rc_error_flage != 0){
            gcs().send_text(MAV_SEVERITY_CRITICAL, "RC ERROR Clear");
        }
        last_rc_error_flage = 0;
    }
}
