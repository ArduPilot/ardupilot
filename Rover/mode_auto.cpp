...
void ModeAuto::do_set_roi(const AP_Mission::Mission_Command& cmd)
{
    ...
    const uint8_t instance = rover.camera_mount.get_primary_instance();
    do_set_roi(instance, cmd.content.location);
}

// MAV_CMD_DO_SET_ROI_LOCATION (195)
void ModeAuto::do_set_roi_location(const AP_Mission::Mission_Command& cmd)
{
    const uint8_t instance = rover.camera_mount.get_specified_instance(cmd.p1);
    do_set_roi(instance, cmd.content.location);
}

void ModeAuto::do_set_roi(const uint8_t instance, const Location &loc) {
    if (loc.initialised()) {
        rover.camera_mount.set_roi_target(instance, loc);
        return;
    }

    // switch off the camera tracking if enabled
    if (rover.camera_mount.get_mode(instance) == MAV_MOUNT_MODE_GPS_POINT) {
        rover.camera_mount.set_mode_to_default(instance);
    }
}
#endif // HAL_MOUNT_ENABLED
