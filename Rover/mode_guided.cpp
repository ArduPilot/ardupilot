...
void ModeGuided::update()
{
    switch (_guided_mode) {
        ...
        case GuidedMode::HeadingAndSpeed:
        {
        ...
        case GuidedMode::TurnRateAndSpeed:
        {
            // stop vehicle if target not updated within 3 seconds
            if (have_attitude_target && (millis() - _des_att_time_ms) > 3000) {
                gcs().send_text(MAV_SEVERITY_WARNING, "target not received last 3secs, stopping");
        ...
        case GuidedMode::Loiter:
        {
            rover.mode_loiter.update();
            break;
        }

        case GuidedMode::SteeringAndThrottle:
        {
            // handle timeout
                                      ...
                                      float next_leg_bearing_cd)
{
    if (g2.wp_nav.set_desired_location(destination, next_leg_bearing_cd)) {
        ...
        _guided_mode = ModeGuided::GuidedMode::WP;
        send_notification = true;
        rover.Log_Write_GuidedTarget(uint8_t(_guided_mode), Vector3f(destination.lat, destination.lng, 0), Vector3f(g2.wp_nav.get_desired_speed(), 0.0f, 0.0f));
        return true;
    }
    return false;
}
...
void ModeGuided::set_desired_heading_and_speed(float yaw_angle_cd, float target_speed)
{
    ...
    _guided_mode = ModeGuided::GuidedMode::HeadingAndSpeed;
...
void ModeGuided::set_desired_heading_delta_and_speed(float yaw_delta_cd, float target_speed)
{
    ...
    if (_guided_mode != ModeGuided::GuidedMode::HeadingAndSpeed) {
        _guided_mode = ModeGuided::GuidedMode::HeadingAndSpeed;
...
{
    // handle initialisation
    _guided_mode = ModeGuided::GuidedMode::TurnRateAndSpeed;
    _des_att_time_ms = AP_HAL::millis();
    _reached_destination = false;
...
{
    _guided_mode = ModeGuided::GuidedMode::SteeringAndThrottle;
...
bool ModeGuided::start_loiter()
{
    if (rover.mode_loiter.enter()) {
        _guided_mode = GuidedMode::Loiter;
        return true;
    }
    return false;
}
