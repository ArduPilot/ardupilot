...
void ModeGuided::update()
{
    switch (_guided_mode) {
        ...
        case SubMode::TurnRateAndHeading:
        {
            ...
            if (have_attitude_target) {
                // run steering and throttle controllers
                calc_steering_to_heading(_desired_yaw_cd, _desired_yaw_rate_cds * 0.01f, true);
                break;
            }
            // we have reached the destination so stay here
            if (rover.is_boat()) {
...
void ModeGuided::set_desired_turn_rate_and_heading_delta(float yaw_delta_cd, float turn_rate_cds, int16_t direction)
{
    ...
    set_desired_turn_rate_and_heading(desired_yaw_cd + yaw_delta_cd, turn_rate_cds, direction);
}
