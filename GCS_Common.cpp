...
void GCS_MAVLINK::handle_mission_item(const mavlink_message_t &msg)
{
    ...
    bool ti_param_limite = true;//@yu 參數限制開關
    if (msg.msgid == MAVLINK_MSG_ID_MISSION_ITEM) {
        mavlink_mission_item_t mission_item;
        mavlink_msg_mission_item_decode(&msg, &mission_item);
        if(ti_param_limite == true){//@yu
            mission_item.frame = 3;//將高度來源強置改寫為相對高
            if(mission_item.command == 85){//vtol land 自動將高度歸0
                mission_item.z = 0;
            }
            if(mission_item.z < 12000){//限定高度設定最高120，要超過120，須將高度*100
                //mission_item.z = constrain_value(mission_item.z,0.0f,120.0f);
            }else{
                mission_item.z = mission_item.z/100;
            }
        }//
        ...
        if(ti_param_limite == true){//@yu
            mission_item_int.frame = 3;
            if(mission_item_int.command == 85){//vtol land 自動將高度歸0
                mission_item_int.z = 0;
            }
            if(mission_item_int.z < 12000){
                mission_item_int.z = constrain_value(mission_item_int.z,0.0f,120.0f);
            }else{
                mission_item_int.z = mission_item_int.z/100;
            }
        }//
    }
