...
/*
  update synthetic battery state
 */
static void battery_update(float dt_s)
{
    if (battery.voltage <= 0) {
        // initial values
        battery.voltage = 14*4.2;
        battery.current = 50.0;
        battery.temperature_K = 273.0 + 25.0;
        battery.remaining_capacity = 100;
        battery.total_capacity_Ah = 1000;
    }

    battery.consumed_Ah += battery.current * dt_s;
}
