...
#define BMP280_RED_DATA      0xF7

AP_Baro_BMP280::AP_Baro_BMP280(AP_Baro &baro, AP_HAL::OwnPtr<AP_HAL::I2CDevice> dev)
    ...
    , _dev(std::move(dev))
{
    ...
    if (!sem->take(HAL_SEMAPHORE_BLOCK_FOREVER)) {
        AP_HAL::panic("BMP280: unable to get semaphore");
    }

    // Check BMP280 ID
    uint8_t val = 0;
    
    if (!_dev->read_registers(BMP280_REG_ID, &val, 1)) {
        AP_HAL::panic("BMP280: can't read ID");
    }
    if (val != BMP280_ID) {
        AP_HAL::panic("BMP280: wrong ID");
    }

    // Read the calibration data registers
    if (!_dev->read_registers(0x88, buf, sizeof(buf))) {
        AP_HAL::panic("BMP280: bad calibration registers");
    }
    ...
    _dev->write_register(BMP280_REG_CTRL_MEAS, (BMP280_OVERSAMPLING_T << 5) + (BMP280_OVERSAMPLING_P << 2) + BMP280_MODE);
    _dev->write_register(BMP280_REG_CONFIG, BMP280_FILTER_COEFFICIENT << 2);
...
bool AP_Baro_BMP280::_timer(void)
{
    ...
    if (!_dev->read_registers(BMP280_RED_DATA, buf, sizeof(buf))) {
...
void AP_Baro_BMP280::_update_temperature(int32_t temp_raw)
{
    ...
    // See datasheet page 22 for this formulas
