...
AC_ADRC::AC_ADRC(const AC_ADRC::Defaults &defaults) :
		default_adrc_type(defaults.adrc_type),       //adrc mode type
		default_b0(defaults.b0),                     //b0
		...
		default_eso_h_gain(defaults.eso_h_gain),     //ESO-h
		...
		default_limit_u_max(defaults.limit_u_max),   //output
		default_filt_hz(defaults.fil_hz) ,           //filter 10hz
		default_wc(defaults.wc) ,                    //control bandwidth
		default_wo(defaults.wo)                     //observation bandwidth(3-5)*wc
...
float AC_ADRC::update_axis(float target, float measure, float dt,float& v1, float& v2,float& z1, float& z2, float& z3,float& last_u,bool limit,uint8_t aix)
{
    ...
    static uint8_t log_loop_delay1=0;
    static uint8_t log_loop_delay2=0;
    static uint8_t log_loop_delay3=0;
	...
	if(_adrc_type==0)
	{
        ...
        //get temp paramer
	    const float b0   = MAX(_b0.get(),1.0f);
    ...
    else if(_adrc_type==1) ////Second-order LADRC
	{
	    if (!is_valid_data(z1) || !is_valid_data(z2) || !is_valid_data(z3)
	    	|| !is_valid_data(v1) || !is_valid_data(v2))
	    {
	    ...
	    const float b0   = MAX(_b0.get(),1.0f);
	    const float output_max  = MAX(_limit_u_max.get(),0.1f);
	...
	else if(_adrc_type==2)//extreme point configuration for 2nd order -ladrc
	{
	    if (!is_valid_data(z1) || !is_valid_data(z2) || !is_valid_data(z3)
	    	|| !is_valid_data(v1) || !is_valid_data(v2))
	    {
	...
	else
	{
	    ...
	    const float b0   = MAX(_b0.get(),0.1);
	    const float r    = MAX(_td_r.get(),1.0f);
	    const float h0 = MAX(_td_h0.get(), dt);
	    const float nlsef_a1   = MAX(_nlsef_alpha1.get(),0.001f);
	    const float nlsef_a2   = MAX(_nlsef_alpha2.get(),0.001f);
	    const float nlsef_delt  = MAX(_nlsef_delta.get(),0.001f);
	    const float eso_delt  = MAX(_nlsef_delta.get(),0.001f);
	    const float output_max  = MAX(_limit_u_max.get(),1.0f);
...
float AC_ADRC::fal(float e, float alpha, float delta)
{
    float abs_e = fabsf(e);
    if (abs_e > delta) {
        // nonlinear segment：|e|^alpha * sign(e)
        return powf(abs_e, alpha) * (e > 0.0f ? 1.0f : -1.0f);
    } else {
        // linear segment：e / delta^(1-alpha)
...
float AC_ADRC::fhan(float x1, float x2, float r, float h)
{
    ...
    if (fabsf(y) > d)
    {
        // Large error: saturation region
        a = a0 + (y > 0.0f ? 1.0f : -1.0f) * (a1 - d) * 0.5f;
    } else
    {
        // Small error: linear region
        a = a0 + y;
    }
