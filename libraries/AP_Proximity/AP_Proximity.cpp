...
const AP_Param::GroupInfo AP_Proximity::var_info[] = {
    ...
    // @Description: Ignore proximity data that is within 1 meter of the ground
    // below the vehicle. This requires a downward facing rangefinder
    // @Values: 0:Disabled, 1:Enabled
    // @User: Standard
    AP_GROUPINFO_FRAME("_IGN_GND", 16, AP_Proximity, _ign_gnd_enable, 0,
                       AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_HELI |
                           AP_PARAM_FRAME_TRICOPTER),

    // @Param: _LOG_RAW
    // @DisplayName: Proximity raw distances log
    // @Description: Set this parameter to one if logging unfiltered(raw)
    // distances from sensor should be enabled
    // @Values: 0:Off, 1:On
    // @User: Advanced
    ...
    // @Description: Cutoff frequency for low pass filter applied to each face
    ...
    // @Range: 0 10
    // @User: Advanced
    AP_GROUPINFO_FRAME("_ALT_MIN", 25, AP_Proximity, _alt_min, 1.0f,
                       AP_PARAM_FRAME_COPTER | AP_PARAM_FRAME_HELI |
                           AP_PARAM_FRAME_TRICOPTER),
...
const AP_Param::GroupInfo
    *AP_Proximity::backend_var_info[AP_PROXIMITY_MAX_INSTANCES];
...
void AP_Proximity::init() {
  ...
  for (uint8_t instance = 0; instance < AP_PROXIMITY_MAX_INSTANCES;
       instance++) {
    ...
    case Type::RPLidarA2:
      if (AP_Proximity_RPLidarA2::detect(serial_instance)) {
        ...
        drivers[instance] = NEW_NOTHROW AP_Proximity_RPLidarA2(
            *this, state[instance], params[instance], serial_instance);
        serial_instance++;
      }
      break;
#endif
    ...
    case Type::MAV:
      state[instance].instance = instance;
      drivers[instance] = NEW_NOTHROW AP_Proximity_MAV(*this, state[instance],
                                                       params[instance]);
      break;
#endif
    ...
    case Type::TRTOWER:
      if (AP_Proximity_TeraRangerTower::detect(serial_instance)) {
        state[instance].instance = instance;
        drivers[instance] = NEW_NOTHROW AP_Proximity_TeraRangerTower(
            *this, state[instance], params[instance], serial_instance);
        serial_instance++;
      }
      break;
    ...
    case Type::TRTOWEREVO:
      if (AP_Proximity_TeraRangerHub::detect(serial_instance)) {
        state[instance].instance = instance;
        drivers[instance] = NEW_NOTHROW AP_Proximity_TeraRangerHub(
            *this, state[instance], params[instance], serial_instance);
        serial_instance++;
      }
      break;
    ...
    case Type::SF40C:
      if (AP_Proximity_LightWareSF40C::detect(serial_instance)) {
        state[instance].instance = instance;
        drivers[instance] = NEW_NOTHROW AP_Proximity_LightWareSF40C(
            *this, state[instance], params[instance], serial_instance);
        serial_instance++;
      }
      break;
    ...
    case Type::SF45B:
      if (AP_Proximity_LightWareSF45B::detect(serial_instance)) {
        state[instance].instance = instance;
        drivers[instance] = NEW_NOTHROW AP_Proximity_LightWareSF45B(
            *this, state[instance], params[instance], serial_instance);
        serial_instance++;
      }
      break;
    ...
    case Type::CYGBOT_D1:
      if (AP_Proximity_Cygbot_D1::detect(serial_instance)) {
        state[instance].instance = instance;
        drivers[instance] = NEW_NOTHROW AP_Proximity_Cygbot_D1(
            *this, state[instance], params[instance], serial_instance);
        serial_instance++;
      }
      break;
    ...
    case Type::Scripting:
      state[instance].instance = instance;
      drivers[instance] = NEW_NOTHROW AP_Proximity_Scripting(
          *this, state[instance], params[instance]);
      break;
#endif
    ...
    case Type::SITL:
      state[instance].instance = instance;
      drivers[instance] = NEW_NOTHROW AP_Proximity_SITL(*this, state[instance],
                                                        params[instance]);
      break;
#endif
    ...
    case Type::AirSimSITL:
      state[instance].instance = instance;
      drivers[instance] = NEW_NOTHROW AP_Proximity_AirSimSITL(
          *this, state[instance], params[instance]);
      break;
#endif
    ...
    case Type::LD06:
      if (AP_Proximity_LD06::detect(serial_instance)) {
        state[instance].instance = instance;
        drivers[instance] = NEW_NOTHROW AP_Proximity_LD06(
            *this, state[instance], params[instance], serial_instance);
        serial_instance++;
      }
      break;
    ...
    // if the backend has some local parameters then make those available in the
    // tree
    if (drivers[instance] && state[instance].var_info) {
      backend_var_info[instance] = state[instance].var_info;
      AP_Param::load_object_from_eeprom(drivers[instance],
                                        backend_var_info[instance]);
    }
  }
}

// update Proximity state for all instances. This should be called at a high
// rate by the main loop
...
// Returns status of first good sensor. If no good sensor found, returns status
// of last instance sensor
...
// prearm checks
bool AP_Proximity::prearm_healthy(char *failure_msg,
  ...
  for (uint8_t i = 0; i < num_instances; i++) {
    ...
    case Status::NotConnected:
      hal.util->snprintf(failure_msg, failure_msg_len, "PRX%d: Not Connected",
                         i + 1);
      return false;
    case Status::Good:
      break;
...
bool AP_Proximity::get_horizontal_distances(
    Proximity_Distance_Array &prx_dist_array) const {
  Proximity_Distance_Array prx_filt_dist_array; // unused
  return boundary.get_layer_distances(PROXIMITY_MIDDLE_LAYER, distance_max(),
                                      prx_dist_array, prx_filt_dist_array);
}
...
// get vector to obstacle based on obstacle_num passed, used in GPS based Simple
// Avoidance
bool AP_Proximity::get_obstacle(uint8_t obstacle_num,
                                Vector3f &vec_to_obstacle) const {
  return boundary.get_obstacle(obstacle_num, vec_to_obstacle);
}

// returns shortest distance to "obstacle_num" obstacle, from a line segment
// formed between "seg_start" and "seg_end" returns FLT_MAX if it's an invalid
// instance.
bool AP_Proximity::closest_point_from_segment_to_obstacle(
    uint8_t obstacle_num, const Vector3f &seg_start, const Vector3f &seg_end,
    Vector3f &closest_point) const {
  return boundary.closest_point_from_segment_to_obstacle(
      obstacle_num, seg_start, seg_end, closest_point);
}
...
bool AP_Proximity::get_object_angle_and_distance(uint8_t object_number,
  ...
  return boundary.get_horizontal_object_angle_and_distance(object_number,
                                                           angle_deg, distance);
}

// get obstacle pitch and angle for a particular obstacle num
bool AP_Proximity::get_obstacle_info(uint8_t obstacle_num, float &angle_deg,
                                     float &pitch, float &distance) const {
...
void AP_Proximity::log() {
  ...
  for (uint8_t i = 0; i < boundary.get_num_layers(); i++) {
    const bool active = boundary.get_layer_distances(
        i, distance_max(), dist_array, filt_dist_array);
    if (!active) {
...
AP_Proximity *proximity() { return AP_Proximity::get_singleton(); }
