...
bool AP_Proximity_Lua::set_distance_min_max(float min, float max)
{
    if (min >= max) {
        return false;
    }
...
bool AP_Proximity_Lua::handle_script_distance_msg(float dist_m, float yaw_deg, float pitch_deg, bool push_to_boundary)
{
...
bool AP_Proximity_Lua::handle_script_3d_msg(Vector3f vec_to_obstacle, bool push_to_boundary)
{
    // convert to FRU
