...
volatile uint32_t YUNEECRCInput::_last_input_interrupt_time = 0; // Last time the input interrupt ran

// Pin 6 is connected to timer 1 channel 1
#define YUNEEC_RC_INPUT_PIN 6
...
#define YUNEEC_TIMER_RELOAD 60000
...
void YUNEECRCInput::_timer_capt_cb(void)
{
    ...
    timer_dev *tdev = PIN_MAP[YUNEEC_RC_INPUT_PIN].timer_device;
    uint8 timer_channel = PIN_MAP[YUNEEC_RC_INPUT_PIN].timer_channel;
    ...
    if (current_count < previous_count) {
        pulse_width = current_count + YUNEEC_TIMER_RELOAD - previous_count;
    } else {
    ...
    if (pulse_width > 8000) { // 4ms
	    ...
	    while (channel_ctr < YUNEEC_RC_INPUT_NUM_CHANNELS)
...
uint16_t YUNEECRCInput::read(uint8_t ch) {
    ...
    /* constrain ch */
    if (ch >= YUNEEC_RC_INPUT_NUM_CHANNELS)
	return 0;
...
{
    for (uint8_t i = 0; i < YUNEEC_RC_INPUT_NUM_CHANNELS; i++) {
        _override[i] = 0;
    }
}
