...
// parameters
const AP_Param::GroupInfo AP_Torqeedo_TQBus::var_info[] = {
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("ONOFF_PIN", 2, AP_Torqeedo_TQBus, _pin_onoff, -1),
    ...
    // @User: Standard
    // @RebootRequired: True
    AP_GROUPINFO("DE_PIN", 3, AP_Torqeedo_TQBus, _pin_de, -1),
    ...
    // @User: Advanced
    AP_GROUPINFO("OPTIONS", 4, AP_Torqeedo_TQBus, _options, (int8_t)options::LOG),
    ...
    // @Increment: 1
    // @User: Advanced
    AP_GROUPINFO("POWER", 5, AP_Torqeedo_TQBus, _motor_power, 100),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("SLEW_TIME", 6, AP_Torqeedo_TQBus, _slew_time, 2.0),
    ...
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("DIR_DELAY", 7, AP_Torqeedo_TQBus, _dir_delay, 1.0),

    // @Param: SRV_FUNC
    // @DisplayName: Torqeedo motor servo function
    // @Description: Torqeedo motor servo function. Default is kthrottle
    AP_GROUPINFO("SRV_FUNC", 8, AP_Torqeedo_TQBus, _channel_function, 70),

    AP_GROUPEND
};

AP_Torqeedo_TQBus::AP_Torqeedo_TQBus(AP_Torqeedo &_frontend,
                                     AP_Torqeedo::Torqeedo_State &_state,
                                     AP_Torqeedo_Params& _params,
                                     uint8_t serial_instance) :
    AP_Torqeedo_Backend_Serial(_frontend, _state, _params, serial_instance)
{
...
// initialise driver
void AP_Torqeedo_TQBus::init(void)
{
    ...
    char name[32];
    snprintf(name, sizeof(name), "torqeedo-TQ-%u", (uint8_t)(size_t)&_serial_instance);

    //create background thread to process serial input and output
    if (!hal.scheduler->thread_create(FUNCTOR_BIND_MEMBER(&AP_Torqeedo_TQBus::thread_main, void), name, 2048, AP_HAL::Scheduler::PRIORITY_RCOUT, 1)) {
        return;
    }
}
...
// update driver state
void AP_Torqeedo_TQBus::update()
{
    if (!healthy()) {
        set_status(AP_Torqeedo::Status::NoData);
    } else {
        set_status(AP_Torqeedo::Status::Good);
    }
}
...
bool AP_Torqeedo_TQBus::enabled() const
{
...
bool AP_Torqeedo_TQBus::pre_arm_checks(char *failure_msg, uint8_t failure_msg_len)
{
    // exit immediately if not enabled
    if (!enabled()) {
        return true;
...
void AP_Torqeedo_TQBus::send_motor_speed_cmd()
{
    ...
    if (!hal.util->get_soft_armed()) {
        ...
        _motor_speed_desired = constrain_int16(SRV_Channels::get_output_norm(SRV_Channels::channel_function(_channel_function)) * 1000.0, -1000, 1000);
    }
