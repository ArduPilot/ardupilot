...
bool AP_MotorsHeli_Compound::init_outputs()
{
    if (!_flags.initialised_ok) {
        ...
        if (_tail_type == AP_MOTORS_HELI_SINGLE_TAILTYPE_DIRECTDRIVE_VARPITCH) {
            if (!_swash_servo_1 || !_swash_servo_2 || !_swash_servo_3 || !_yaw_servo_1 || !_yaw_servo_2) {
                return false;
            }
        } else {
    ...
    if (_tail_type == AP_MOTORS_HELI_SINGLE_TAILTYPE_DIRECTDRIVE_VARPITCH) {
        ...
        _tail_rotor._pwm_min = 1000;
        _tail_rotor._pwm_max = 2000;
        _tail_rotor._pwm_rev = 1;
    }
...
void AP_MotorsHeli_Compound::move_yaw(float yaw_out)
{
    ...
    if (_tail_type == AP_MOTORS_HELI_SINGLE_TAILTYPE_DIRECTDRIVE_FIXEDPITCH){
        if (_main_rotor.get_desired_speed() > 0.0f && hal.util->get_soft_armed()) {
            ...
            yaw_servo_1 = constrain_float(yaw_servo_1, -0.9f, 1.0f);
            yaw_servo_2 = constrain_float(yaw_servo_2, -0.9f, 1.0f);
            ...
            rc_write(AP_MOTORS_MOT_4, calc_pwm_output_1to1(-1.0f, _yaw_servo_1));
            rc_write(AP_MOTORS_MOT_5, calc_pwm_output_1to1(-1.0f, _yaw_servo_2));
        }
    } else {
        rc_write(AP_MOTORS_MOT_4, calc_pwm_output_1to1(yaw_servo_1, _yaw_servo_1));
        rc_write(AP_MOTORS_MOT_5, calc_pwm_output_1to1(yaw_servo_2, _yaw_servo_2));
    ...
    if (_tail_type == AP_MOTORS_HELI_SINGLE_TAILTYPE_SERVO_EXTGYRO) {
        ...
        write_aux(yaw_servo_1*0.5f+1.0f);
        write_aux(yaw_servo_2*0.5f+1.0f);
    }
}
