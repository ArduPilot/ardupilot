...
public:
    ...
    AP_MotorsHeli_Dual(uint16_t loop_rate,
        ...
        _rotor(RC_Channel_aux::k_heli_rsc, AP_MOTORS_HELI_DUAL_RSC),
        _swash_servo_1(CH_NONE),
        _swash_servo_2(CH_NONE),
        _swash_servo_3(CH_NONE),
        _swash_servo_4(CH_NONE),
        _swash_servo_5(CH_NONE),
        _swash_servo_6(CH_NONE)
    {
    ...
    float get_main_rotor_speed() const { return _rotor.get_rotor_speed(); }

    // get_desired_rotor_speed - gets target rotor speed as a number from 0 ~ 1000
    float get_desired_rotor_speed() const { return _rotor.get_rotor_speed(); }
