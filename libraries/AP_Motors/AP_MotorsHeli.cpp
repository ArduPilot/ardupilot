...
void AP_MotorsHeli::init_swash()
{
    ...
    collective_mid = constrain<int16_t>(collective_mid, collective_min, collective_max);
...
void AP_MotorsHeli::move_swash(int16_t roll_out, int16_t pitch_out, int16_t coll_in, int16_t yaw_out)
{
    ...
    if( servo_manual == 1 ) {      // are we in manual servo mode? (i.e. swash set-up mode)?
        ...
        roll_out = constrain<int16_t>(roll_out, -roll_max, roll_max);

        pitch_out = pitch_out * _pitch_scaler;
        pitch_out = constrain<int16_t>(pitch_out, -pitch_max, pitch_max);

        // scale collective pitch
        coll_out = constrain<int16_t>(coll_in, 0, 1000);
		if (stab_throttle){
