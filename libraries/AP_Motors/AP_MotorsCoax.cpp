...
{
    // intilisation with voltage and air pressure compensation
    ...
    const float actuator_allowed = 2.0f * (1.0f - rp_scale * rp_thrust_max);  // amount of yaw we can fit in
    ...
    const float thrust_min_rpy = MAX(fabsf(rp_scale * rp_thrust_max), fabsf(yaw_thrust));  // the minimum throttle setting that will not limit the roll and pitch output
    ...
    const float thrust_out = _throttle_avg_max + thr_adj;
    ...
    const float thrust_out_actuator = constrain_float(MAX(_throttle_hover * 0.5f, thrust_out), 0.1f, 1.0f);
