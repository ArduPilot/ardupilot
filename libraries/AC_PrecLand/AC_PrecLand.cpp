...
const AP_Param::GroupInfo AC_PrecLand::var_info[] = {
    ...
    // @Values: 0: Land Vertically (Not strict), 1: Retry Landing(Normal Strictness), 2: Do not land/Hover(Very Strict)
    AP_GROUPINFO("STRICT", 10, AC_PrecLand, _strict, 1),
    ...
    // @Range: 0 10
    // @Increment: 1
    AP_GROUPINFO("RT_MAX", 11, AC_PrecLand, _max_retry_num, 4),
    ...
    // @Range: 0 20
    // @Units: s
    AP_GROUPINFO("RT_TIMEOUT", 12, AC_PrecLand, _retry_timeout_sec, 4),
    ...
    AP_GROUPINFO("RT_BEHAVE", 13, AC_PrecLand, _retry_behave, 0),

    // @Param: MIN_HEIGHT
    // @DisplayName: PrecLand minimum height for retry
    ...
    // @Range: 0 5
    // @Units: m
    AP_GROUPINFO("MIN_HEIGHT", 14, AC_PrecLand, _sensor_min_alt, 0.75),

    // @Param: MAX_HEIGHT
    // @DisplayName: PrecLand maximum height for retry
    ...
    // @Range: 0 50
    // @Units: m
    AP_GROUPINFO("MAX_HEIGHT", 15, AC_PrecLand, _sensor_max_alt, 8),
...
void AC_PrecLand::init(uint16_t update_rate_hz)
{
    ...
    // init as target lost, we will update this later
    _current_status = PldState::TARGET_NEVER_SEEN;
...
void AC_PrecLand::check_target_status(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
    if (target_acquired()) {
        // target in sight
        _current_status = PldState::TARGET_FOUND;
        // early return because we already know the status
        return;
    } else if (_current_status == PldState::TARGET_FOUND ||
               _current_status == PldState::TARGET_RECENTLY_LOST) {
    ...
    // We definelty do not have the target in sight
    // check if the sensor is supposed to be in range
    ...
    if (!check_if_sensor_in_range(rangefinder_alt_m, rangefinder_alt_valid)) {
        ...
        _current_status = PldState::TARGET_OUT_OF_RANGE;
        return;
    }

    if (_current_status == PldState::TARGET_RECENTLY_LOST) {
        ...
        if (AP::ahrs().get_relative_position_NE_origin(curr_pos)) {
            const float dist_to_last_target_loc_xy = (curr_pos - Vector2f{_last_target_loc_rel_origin_NED.x, _last_target_loc_rel_origin_NED.y} * 0.01f).length();
            const float dist_to_last_loc_xy = (curr_pos - Vector2f{_last_vehicle_pos_NED.x, _last_vehicle_pos_NED.y} * 0.01f).length();
            if ((AP_HAL::millis() - _last_precland_output_ms) > LANDING_TARGET_LOST_TIMEOUT_MS) {
...
// This needs a valid rangefinder to work
bool AC_PrecLand::check_if_sensor_in_range(float rangefinder_alt_m, bool rangefinder_alt_valid)
{
...
bool AC_PrecLand::target_acquired()
{
    if ((AP_HAL::millis()-_last_update_ms) > LANDING_TARGET_TIMEOUT_MS) {
        if (_target_acquired) {
            ...
            gcs().send_text(MAV_SEVERITY_CRITICAL, "Precision Landing Target Lost");
        }
        ...
        case EstimatorType::RAW_SENSOR: {
            ...
            if (construct_pos_meas_using_rangefinder(rangefinder_alt_m, rangefinder_alt_valid)) {
                if (!_estimator_initialized) {
                    gcs().send_text(MAV_SEVERITY_INFO, "Precision Landing Target Found");
...
void AC_PrecLand::check_ekf_init_timeout()
{
    if (!target_acquired() && _estimator_initialized) {
        ...
        if (AP_HAL::millis()-_last_update_ms > EKF_INIT_SENSOR_MIN_UPDATE_MS) {
            ...
            _estimator_initialized = false;
            gcs().send_text(MAV_SEVERITY_CRITICAL, "Precision Landing Initialization Failed");
            ...
            _target_acquired = true;
            gcs().send_text(MAV_SEVERITY_INFO, "Precision Landing Initialization Complete");
        }
    }
}
    ...
    if (retrieve_los_meas(target_vec_unit_body)) {
        ...
        if (target_vec_valid && alt_valid) {
            ...
            if (_ahrs.get_relative_position_NED_origin(pos_NED)) {
                pos_NED = pos_NED * 100.0f; // m to cm
                _last_target_loc_rel_origin_NED.z = pos_NED.z;
...
void AC_PrecLand::run_output_prediction()
{
    ...
    _last_target_loc_rel_origin_NED.x = last_target_loc_rel_origin_2d.x;
    _last_target_loc_rel_origin_NED.y = last_target_loc_rel_origin_2d.y;

    // record the last time there was a target output
    _last_precland_output_ms = AP_HAL::millis();
}
