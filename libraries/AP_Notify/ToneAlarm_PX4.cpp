...
void ToneAlarm_PX4::play_tune(const uint8_t tone_index)
{
...
void ToneAlarm_PX4::update()
{
    ...
    if (AP_Notify::events.user_mode_change_failed) {
        if (AP_Notify::flags.armed) {
            ...
            play_tune(AP_NOTIFY_PX4_TONE_QUIET_NEG_FEEDBACK);
        }
    }

    if(AP_Notify::events.failsafe_action_taken) {
        play_tune(AP_NOTIFY_PX4_TONE_LOUD_ATTENTION_NEEDED);
    }
    ...
    if (flags.failsafe_radio != AP_Notify::flags.failsafe_radio) {
        ...
        if (flags.failsafe_radio) {
            ...
            if (AP_Notify::flags.armed) {
                play_tune(AP_NOTIFY_PX4_TONE_LOUD_POS_FEEDBACK);
            } else {
                play_tune(AP_NOTIFY_PX4_TONE_QUIET_POS_FEEDBACK);
            }
        }
    }
    ...
    if (flags.pre_arm_check != AP_Notify::flags.pre_arm_check) {
        ...
        if (flags.pre_arm_check) {
            play_tune(AP_NOTIFY_PX4_TONE_QUIET_READY_OR_FINISHED);
        }
    }
    ...
    if (flags.armed != AP_Notify::flags.armed) {
        ...
        if (flags.armed) {
            // arming tune
            play_tune(AP_NOTIFY_PX4_TONE_QUIET_ARMING_WARNING);
        }else{
            // disarming tune
            play_tune(AP_NOTIFY_PX4_TONE_QUIET_NEU_FEEDBACK);
            stop_cont_tone();
    ...
    if (flags.failsafe_battery != AP_Notify::flags.failsafe_battery) {
        ...
        if (flags.failsafe_battery) {
            ...
            play_tune(AP_NOTIFY_PX4_TONE_LOUD_BATTERY_ALERT_CTS);
        }
    }
