...
if (use_full_telemetry) {
    if (telem_avail == 1) {
        if (mask & _telem_req_type) {
            ...
            //(_telem_req_type,requested_telemetry[5],0);
            const float tx_err_rate = calc_tx_crc_error_perc(_telem_req_type-1,requested_telemetry[5],0); // _frontend->_telem_data[i].count == 0 ? 0.0f : requested_telemetry[5] / _frontend->_telem_data[i].count * 100.0f;
...
float AP_FETtecOneWire::calc_tx_crc_error_perc(uint8_t esc_id, uint16_t current_error_count, uint8_t increment_only){
    ...
    if (increment_only==1){ //Only increment
        send_msg_count++;
        if (send_msg_count>400){ //with 400hz its resets every second      
            send_msg_count=0; //reset the counter
            for (int i=0;i<MOTOR_COUNT_MAX;i++){
                error_count_since_overflow[i]=error_count[i]; //save the current error state from thge ESCs
            }
        }
    }
