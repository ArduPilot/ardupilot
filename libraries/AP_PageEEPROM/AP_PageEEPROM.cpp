...
bool AP_PageEEPROM::read(uint32_t offset, uint8_t *buf, uint32_t size)
{
    ...
    // is reached, so even if we get data back, it won't be what we expect.
    if ((size > get_size()) ||
...
{
    // Don't allow writes outside of the EEPROM Page memory
    // Warning: The EEPROM Page devices will go back to address 0x0000 if the limit
    // is reached; if this happens we will overwrite existing data.
    if ((size > get_size()) ||
        (offset > (get_size() - size))) {
        return false;
    }
