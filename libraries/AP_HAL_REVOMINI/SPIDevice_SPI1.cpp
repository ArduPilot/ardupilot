...
REVOMINISemaphore REVOMINISPI1DeviceDriver::_semaphore;

void REVOMINISPI1DeviceDriver::init() {
    _dev = _SPI1;
...
AP_HAL::Semaphore* REVOMINISPI1DeviceDriver::get_semaphore() {
    return &_semaphore;
}

inline void REVOMINISPI1DeviceDriver::_cs_assert() {
    hal.gpio->write(_cs_pin, LOW);
}

inline void REVOMINISPI1DeviceDriver::_cs_release() {
    hal.gpio->write(_cs_pin, HIGH);
}

inline uint8_t REVOMINISPI1DeviceDriver::_transfer(uint8_t data) {
    uint8_t buf[1];
...
uint8_t REVOMINISPI1DeviceDriver::transfer(uint8_t data) {
    return _transfer(data);
}


bool REVOMINISPI1DeviceDriver::transaction(const uint8_t *tx, uint8_t *rx, uint16_t len) {
...
void REVOMINISPI1DeviceDriver::transfer(const uint8_t *tx, uint16_t len) {
    for (uint16_t i = 0; i < len; i++) {
            _transfer(tx[i]);
    }
}
...
void REVOMINISPI1DeviceDriver::cs_release() {
    _cs_release();
}


const spi_pins* REVOMINISPI1DeviceDriver::dev_to_spi_pins(spi_dev *dev) {
    if (_dev->SPIx == SPI1)
...
void REVOMINISPI1DeviceDriver::configure_gpios(spi_dev *dev, bool as_master) {
...
const spi_baud_rate REVOMINISPI1DeviceDriver::determine_baud_rate(SPIFrequency freq)
{
