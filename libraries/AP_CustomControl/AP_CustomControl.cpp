...
const struct AP_Param::GroupInfo *AP_CustomControl::_backend_var_info[AP_PLANE_CUSTOMCONTROL_MAX_TYPES];
...
void AP_CustomControl::set_output_pwm_chan(uint8_t chan, uint16_t value) {
    if ((chan < 1 || chan > NUM_SERVO_CHANNELS)) {
        ...
        if (now_ms - _last_warning_ms < 5000) {
            return;
        }
        GCS_SEND_TEXT(MAV_SEVERITY_ERROR, "Channel %d is out of range.", chan);
...
// Returns true if Plane should act on the needs_reset_*() flags, either because the custom
// controller ran this loop, or because it has just disengaged and the stock controllers it
// was overriding need their integrators cleared.
bool AP_CustomControl::update(void)
{
    // Engagement is governed by is_safe_to_run(), not by the aux switch alone: an RC
    // failsafe, a mode change or a CP_TYPE edit suspends and resumes us with no switch event.
    const bool safe_to_run = is_safe_to_run();
    const bool was_running = _is_running;
    _is_running = safe_to_run;
    ...
    if (safe_to_run) {
        ...
        if (!was_running) {
            // Engaging. The integrators and filters have been frozen at their pre-suspend
            // values while we were not running, so clear them before they are applied.
            _backend->reset();
        ...
        // Disengaging. Leave the flags raised by the final active loop: they name exactly
        // the axes we were overriding, so Plane resets those stock controllers and no others.
        if (_custom_controller_active) {
            // Still switched on, so the backend or the vehicle state stopped us rather than
            // the pilot. Warn, because the stock controller is now flying instead.
            GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "Custom controller suspended");
        } else {
...
// Clear the pending stock controller reset requests.
void AP_CustomControl::clear_reset_flags(void)
{
    _reset_roll = false;
    _reset_pitch = false;
    _reset_yaw = false;
...
void AP_CustomControl::set_controls()
{
    ...
    for (uint8_t idx = 0; idx < AP_PLANE_CUSTOMCONTROL_FUNCTIONS_MAX; idx++) {
...
void AP_CustomControl::set_custom_controller(bool enabled)
{
    ...
    if (_controller_type > AP_PLANE_CUSTOMCONTROL_MAX_TYPES) {
    ...
    // update() announces, logs and resets on the edge where the controller actually starts
    // or stops running, so that suspends without a switch event are reported too. The only
    // case it cannot report is a request that never takes effect, so cover that here.
    if (enabled && !is_safe_to_run()) {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Custom controller enabled, not yet running");
    }
...
// Log the custom controller state. Called on each engage and disengage edge, and twice on
// each switch event, so that the state change is unmistakable in the log.
void AP_CustomControl::log_switch(void) {
    ...
    // @Field: Act: true if the controller is switched on by the pilot
    // @Field: Run: true if the controller is actually running. Act can be true while Run is false, for example during an RC failsafe or in a flight mode the backend refuses to run in.
    AP::logger().Write("CP", "TimeUS,Type,Act,Run","QBBB",
                       AP_HAL::micros64(),
                       (uint8_t)_controller_type.get(),
...
int8_t AP_CustomControl::get_function_index(SRV_Channel::Function function) {
    for ( uint8_t i = 0; i < AP_PLANE_CUSTOMCONTROL_FUNCTIONS_MAX; i++) {
...
int8_t AP_CustomControl::allocate_function() {
    uint8_t i;
    for ( i = 0; i < AP_PLANE_CUSTOMCONTROL_FUNCTIONS_MAX; i++) {
        if (!functions[i].used) {
            break;
        }
    }
    if (i == AP_PLANE_CUSTOMCONTROL_FUNCTIONS_MAX) {
        ...
        i = AP_PLANE_CUSTOMCONTROL_FUNCTIONS_MAX-1;
    }
