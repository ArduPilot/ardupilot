...
void AP_CustomControl::update(void)
{
...
bool AP_CustomControl::is_safe_to_run(void) {
    if (!_custom_controller_active) { return false; }
    if (_controller_type == CustomControlType::CONT_NONE) { return false; }
    if (_controller_type > CUSTOMCONTROL_MAX_TYPES) { return false; }
    if (_backend == nullptr) { return false; }
    if (!_backend->can_run()) { return false; }
