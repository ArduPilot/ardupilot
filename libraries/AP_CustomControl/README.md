...
Only the axes that the custom controller was actually overriding are reset, so the stock controllers on any axes you left alone keep their state.

The custom controller is not only engaged and disengaged by the aux switch. It is also suspended and resumed without a switch event, whenever
`can_run()` or the other conditions in `is_safe_to_run()` change: an RC failsafe, a mode change, or a `CP_TYPE` edit in flight. The library treats
these the same as a switch event: your backend's `reset()` is called on every engage, so it never resumes with integrators and filters that froze
at their pre-suspend values, and the stock controllers are reset on every disengage.
...
3. Register your controller with the library:
    - Add an new entry in `AP_CustomControl.h` in `enum class CustomControlType` and increment the define `AP_PLANE_CUSTOMCONTROL_MAX_TYPES`.
...
```C++
./waf configure --board CubeOrange plane --enable-PLANE_CUSTOM_CONTROL
```
...
Switching in and out of custom controller logged under the `CP` in log tree.
`CP.Act` is what the pilot asked for with the aux switch, and `CP.Run` is whether the controller was actually running.
These differ whenever the controller is suspended without a switch event, so read `CP.Run` for the time and duration the custom controller was really in charge.
A suspension is also reported to the GCS, so it is not silent in flight.
