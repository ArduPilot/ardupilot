...
#ifndef AP_PLANE_CUSTOMCONTROL_MAX_TYPES
#define AP_PLANE_CUSTOMCONTROL_MAX_TYPES 2
#endif

#ifndef AP_PLANE_CUSTOMCONTROL_FUNCTIONS_MAX
#define AP_PLANE_CUSTOMCONTROL_FUNCTIONS_MAX 10
#endif
...
public:
    ...
    uint8_t flight_stage; // The flight stage we are currently in.
    ...
    static const struct AP_Param::GroupInfo *_backend_var_info[AP_PLANE_CUSTOMCONTROL_MAX_TYPES];
...
private:
    ...
    FunctionInfo functions[AP_PLANE_CUSTOMCONTROL_FUNCTIONS_MAX];
    ...
    bool _is_running;               // True if the backend ran on the most recent update(). Drives engage/disengage edge detection and CP.Run.
    ...
    // Return the index in functions where a function is found, or -1 if not found.
    int8_t get_function_index(SRV_Channel::Function);
    int8_t allocate_function(); // Find an empty slot. Return FUNCTIONS_MAX-1 if all slots are full.
