...
bool AP_CustomControl_PID::is_inverted() const
{
...
void AP_CustomControl_PID::update()
{
    ...
    float ch_7_input = 1500;
    if (channel_7 != nullptr) {
        ch_7_input = channel_7->get_radio_in();
    }

    // Run attitude controller.
    const float roll_error = wrap_180(get_roll_target_deg() - AP::ahrs().get_roll_deg()); // Ensure the shortest path is taken.
    const float target_roll_rate = _p_angle_roll.kP() * roll_error;
    const float pitch_error = wrap_180(get_pitch_target_deg() - AP::ahrs().get_pitch_deg());
    float target_pitch_rate = _p_angle_pitch.kP() * pitch_error;
    ...
    // The rate PIDs operate in radians, so that the usual AC_PID gain and IMAX
    // ranges apply, but their output is in degrees of surface deflection.
    ...
    if (_frontend.get_mask() & 1 << 0) {
        aileron = _pid_rate_roll.update_all(radians(target_roll_rate), gyro_latest[0], _dt, false);
        aileron += _pid_rate_roll.get_ff();
        aileron = degrees(aileron);
    } else {
    ...
    if (_frontend.get_mask() & 1 << 1) {
        elevator = _pid_rate_pitch.update_all(radians(target_pitch_rate), gyro_latest[1], _dt, false);
        elevator += _pid_rate_pitch.get_ff();
        elevator = degrees(elevator);
    } else {
