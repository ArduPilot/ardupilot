...
AP_CustomControl_PID::AP_CustomControl_PID(AP_CustomControl& frontend, float dt) :
    ...
    // Initialize gains at slightly more relax that the defaults.
    _p_angle_roll(1.8f),
    _p_angle_pitch(1.8f),
...
bool AP_CustomControl_PID::can_run()
{
    ...
    if (!rc().has_valid_input()) { return false; }
...
void AP_CustomControl_PID::update()
{
    ...
    const RC_Channel *channel_flap_custom = find_channel_for_option(RC_Channel::AUX_FUNC::FLAP);
    float flap_input{0};
    if (channel_flap_custom != nullptr) {
        flap_input = channel_flap_custom->norm_input_dz();
    }
    const RC_Channel *channel_7 = channel(7);
    ...
    if (_frontend.get_mask() & 1 << 3) {
        _frontend.set_output_scaled(SRV_Channel::Function::k_flap, flap_input); // Write scaled value on a function.
    }
