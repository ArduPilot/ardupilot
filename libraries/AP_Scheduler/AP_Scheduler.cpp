...
{
    // check whether the scheduler can perform a tick
    // a tick is one execution cycle every "_loop_period_us"
    static uint32_t task_time_buf = 0;
    if(AP_HAL::micros() - task_time_buf < _loop_period_us) return;
    _tick_counter++;    
    task_time_buf = AP_HAL::micros();
    
    for (uint8_t i = 0; i < _num_tasks; i++) {
        const uint16_t dticks = _tick_counter - _last_run[i];
        const uint16_t ratio = _loop_rate_hz / _tasks[i].rate_hz;
        const uint16_t interval_ticks = ratio < 1 ? 1 : ratio;
        
        //printf("%d %d %d %d %d %d\n", i, _loop_rate_hz, (int)_tasks[i].rate_hz, dticks, ratio, interval_ticks);
        ...
        if (_task_time_allowed <= time_available) {
            _task_time_started = task_time_buf;
            current_task = i;
            ...
            task_time_buf = AP_HAL::micros();
            const uint32_t time_taken = task_time_buf - _task_time_started;
    ...
    // ternary creates smaller executable than if
    const bool check = _spare_ticks == 32;
    _spare_ticks = check ? _spare_ticks / 2 : _spare_ticks;
    _spare_micros = check ? _spare_micros / 2 : _spare_micros;
}
