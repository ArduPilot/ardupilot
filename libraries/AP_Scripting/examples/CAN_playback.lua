--[[
   ...
   Then set CAN_D1_PROTOCOL2 = 10
   and (if 2nd bus is wanted) CAN_D2_PROTOCOL2 = 12

   The playback will loop when the end of the log is reached

   This example replays ArrayCommand for playing back CAN servo commands. Edit below for
...
local function check_frame()
   ...
   local h0, h1, mtype, t1, t2, bus, id, DLC = string.unpack('<BBBIIBIB', string.sub(frame, 1, 18))
...
function read_log()
   for i = 1, 100 do
      ...
      if next_frame then
         ...
         if driver:write_frame(msg, 10000) then
            next_frame = nil
         end
      end
   end
