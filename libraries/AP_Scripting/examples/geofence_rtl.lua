...
--[[
  ...
  // @Description: Terrain brake altitude. The altitude above the ground above which RTL mode will be engaged if in AUTO mode.
  // @Range: 1 150
  // @Units: m
  // @User: Standard
--]]
local TERR_BRK_ALT = bind_add_param('ALT', 2, 140)

--[[
  // @Param: TERR_BRK_HDIST
  // @DisplayName: Terrain brake home distance
  // @Description: Terrain brake home distance. The distance from home past which the rtl failsafe will be enabled. When within this distance of home the script will not activate.
  // @Range: 0 1000
  // @Units: m
  // @User: Standard
--]]
local TERR_BRK_HDIST = bind_add_param('HDIST', 3, 1000)
...
local function run_checks()
   ...
   if hagl <= TERR_BRK_ALT:get() then
      -- Height is less than TERR_BRK_ALT, so don't activate
      return
   end
