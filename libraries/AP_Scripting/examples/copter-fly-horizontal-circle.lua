  ...
  if not arming:is_armed() then -- reset state when disarmed
    ...
    if pwm6 and pwm6 > 1800 then    -- check if RC6 input has moved high
      ...
      elseif (stage == 3 ) then   -- Stage3: fly a horizontal circle
        ...
        local vel_x = math.cos(math.rad(circle_angle)) * circle_speed
        local vel_y = yaw_cos * math.sin(math.rad(circle_angle)) * circle_speed
        local vel_z = yaw_sin * math.sin(math.rad(circle_angle)) * circle_speed
