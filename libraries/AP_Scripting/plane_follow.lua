...
local function update()
   ...
   if target_attitude ~= nil and target_airspeed ~= nil and target_airspeed > airspeed_min then
      if (now - (target_attitude.timestamp_ms * 0.001) < LOST_TARGET_TIMEOUT) and
         ...
         local turn_radius = target_turn_radius
         if tight_turn then
            turn_radius = target_turn_radius - foll_ofs_y
         elseif use_wide_turns then
            turn_radius = target_turn_radius + foll_ofs_y
         end
         ...
         if my_airspeed > 0 and my_airspeed > airspeed_min and target_airspeed > 0 then 
            ...
            -- gcs:send_text(MAV_SEVERITY.INFO, string.format("turning ratio theta %.1f target asp %.0f my asp %.0f ratio %.1f", theta, target_airspeed, my_airspeed, turning_airspeed_ratio))
         end
      end
   end
   ...
   -- reproject the distance error based on the turning angle
   -- if (turning and (tight_turn and turn_starting)) and math.abs(offset_angle) > TURNING_ANGLE  and turning_airspeed_ratio > 0 then
   if turning_airspeed_ratio > 0 and turning_airspeed_ratio < 2.0 and turning_airspeed_ratio ~= 1.0 then
      local dv_error_save = dv_error
      dv_error = dv_error * turning_airspeed_ratio
      -- gcs:send_text(MAV_SEVERITY.INFO, string.format("turning slowdown ratio %.1f angle %.0f dist new/old %.1f / %.1f", turning_airspeed_ratio, offset_angle, dv_error, dv_error_save))
   end
   ...
   logger:write("ZPF1",'Dst,DstP,DstE,AspT,Asp,AspD,AspO,AspR,Mech,Cls,ClsF,OSht','ffffffffBBBB','mmmnnnnn----','------------',
                  xy_dist,
                  projected_distance,
                  dv_error,
                  target_airspeed,
