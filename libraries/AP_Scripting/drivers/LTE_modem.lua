...
local supports_routing = networking and networking.add_route
...
local function step_BAUD()
    if LTE_BAUD:get() ~= LTE_IBAUD:get() then
...
local function step_CONNECTED()
    ...
    if supports_routing and now_ms - last_route_ms > 1000 then
        ...
        if dest ~= uint32_t(0) then
            networking:add_route(0, 1, dest, math.floor(LTE_ROUTE_MASK:get()))
        end
    end
end
