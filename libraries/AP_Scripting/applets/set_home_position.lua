...
my_home_location = Location() my_home_location:lat(448060084) my_home_location:lng(116100275) my_home_location:alt(0000)

function update()
  -- Get switch position (0, 1, or 2)
  local sw_pos = channel:get_aux_switch_pos()
  -- Check if switch position changed or is in option 2
  if sw_state ~= sw_pos or sw_pos == 2 then
    if sw_pos == 0 then
      -- No action
    elseif sw_pos == 1 then
      -- Set home to preset location
    ...
    elseif sw_pos == 2 then
      if ahrs:home_is_set() then
        -- Set home to current location, maintaining the original height
...
-- Return the update function and interval (2 seconds)
return update, 2000
end
