...
Once the aircraft is within `RTL_RADIUS` of Home, SoarNav returns control
to the standard RTL mode to complete the loiter or landing sequence. The
override is automatically suspended if the pilot manually changes flight
modes. This feature is enabled by default via an internal toggle.