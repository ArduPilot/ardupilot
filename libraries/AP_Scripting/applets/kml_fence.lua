...
local function run_checks()
    ...
    if boundary_ok == false then
        if not triggered then
            gcs:send_text(MAV_SEVERITY.ALERT, string.format("Breached boundary %s", breach_name))
            triggered = true
            -- switch to RTL
            vehicle:set_mode(MODE_RTL)
        end
    end
    if inside_one and not within_limits then
        if not triggered then
            gcs:send_text(MAV_SEVERITY.ALERT, string.format("Breached %s %.0f %.0f", breach_name, alt_agl_ft, alt_amsl_ft))
            triggered = true
            -- switch to RTL
            vehicle:set_mode(MODE_RTL)
        end
