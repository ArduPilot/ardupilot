...
-- The claim strength we last put on the air. Contests are settled with this rather
-- than a freshly measured one: a peer can only compare against what we published, so
-- if we judged ourselves on a newer number than they can see, every contender would
-- rate itself the strongest and nobody would ever yield.
local my_pub_prio = 0
...
-- from the mesh position stream. Claims are bookkeeping and can be lost, contradicted or
-- left behind by a vehicle that never made it; a peer sitting on a cell is ground truth.
...
local function negotiate(now)
  if REPLICATED_ASSIGNMENT then
    ...
    -- configuration, not live assignments from the leader. This avoids the fragile
    -- full-peer-table gate used by the first spelling attempt: a missed heartbeat can
    -- no longer change ranks or prevent a vehicle from engaging.
    for i = 0, swarm:count() - 1 do
      local sid = swarm:get_peer_sysid(i)
      if sid > 0 and sid ~= leader_sysid and sid ~= my_sysid then
        local cs = swarm:get_peer_coord_state(sid)
        if cs and cs:task_id() == my_task then
          local peer_rank = cs:formation_slot()
          if peer_rank > 0 then
    ...
    local assigned = my_rank > 0 and my_rank <= cohort_state.active_count and my_rank or 0
    ...
    local here = ahrs:get_location()
    local distance = my_slot > 0 and here and here:get_distance(slot_location(my_slot)) or 255
    my_pub_prio = priority_for(distance)
  ...
  -- unclaimed: take the nearest cell nobody is holding.
  -- Ranked with a small deterministic per-vehicle offset, not on distance alone: two
