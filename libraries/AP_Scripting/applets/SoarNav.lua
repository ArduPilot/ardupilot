...
function SoarNav:update_navigation_controller(loc, dt_s)
	...
	-- NEW: If in TE evasion mode with target_heading set, use that directly
	if G.TE.evasion_active and G.TE.target_heading then
		target_heading_deg = G.TE.target_heading
	else
		-- Normal mode: calculate heading towards waypoint
	...
	if dt_s > 0.01 then
		error_derivative = wrap180(heading_error - G.last_heading_error) / dt_s
	end
	G.last_heading_error = heading_error
	...
	local p_term = heading_error * effective_p_gain
	local d_term = error_derivative * Cache.nav_d
