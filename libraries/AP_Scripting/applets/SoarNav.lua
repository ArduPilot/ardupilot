...
local function wind_vector_to_bearing_deg(w)
	...
	local ok, ang = pcall(function() return (math.deg(w:xy():angle()) + 360) % 360 end)
...
local function predict_thermal_drift(loc, wind, age_sec)
	...
	local ok_len, Wc = pcall(function() return wind:length() end)
	...
	local ok, drift_loc = pcall(function()
		...
		local k_age = select(1, lifecycle_coeff(age, pget(P.tmem_life, 1200)))
...
local function get_cell_index_from_location(loc)
	...
	if lat < b.min_lat then lat = b.min_lat end
	if lat > b.max_lat then lat = b.max_lat end
	if lon < b.min_lon then lon = b.min_lon end
	if lon > b.max_lon then lon = b.max_lon end
	local lat_frac = (lat - b.min_lat) / lat_span
	local lon_frac = (lon - b.min_lon) / lon_span
	if lat_frac < 0 then lat_frac = 0 elseif lat_frac > 1 then lat_frac = 1 end
	if lon_frac < 0 then lon_frac = 0 elseif lon_frac > 1 then lon_frac = 1 end
...
function update_polar_learning()
    if not SoarNavGlobals.polar_learn then return end
...
local function search_for_new_waypoint()
	...
	if SoarNavGlobals.thermal_to_retry and not throttle_on then
		...
		if hotspot_to_retry and hotspot_to_retry.loc and current_loc then
			...
			local predicted = predict_thermal_drift(hotspot_to_retry.loc, wind, age_s) or hotspot_to_retry.loc
			...
			if SoarNavGlobals.is_in_focus_mode then
				if SoarNavGlobals.focus_wp_counter >= (SoarNavGlobals.focus_wp_timeout or 3) then
					...
					local drifted_focus_center =
						predict_thermal_drift(SoarNavGlobals.focus_area_center, wind, age_seconds) or SoarNavGlobals.focus_area_center
...
update_body = function()
	...
	then
        if SoarNavGlobals.polar_learn then
