...
- **Dual Area System (Radius or Polygon)**:
  Supports both a circular area and a custom polygon loaded from an SD card.
...
- SNAV_ENABLE: Master switch. 0:Disabled. >0:Enabled. If RADIUS_M is 0,
  this value (1-9) also selects the polygon file (snavX.poly).
...
- SoarNav: SNav:Poly A=2.45km2;RP=5;MD=2150m
  Displayed on initialization for a polygon area. Shows total Area (A),
  Reference Points (RP, vertices), and Maximum Distance (MD).

- SoarNav: SNav:Radius A=0.79km2;RD=500m
  Displayed on initialization for a circular area. Shows total Area (A)
  and the defined Radius (RD).
...
local function validate_params()
    ...
    if radius_m < 0 then
        log_gcs(MAV_SEVERITY.ERROR, 0, "SNAV_RADIUS_M cannot be negative.")
        ok = false
    end
    if p_reroute_min and p_reroute_max then
        ...
        if rmin > rmax then
            log_gcs(MAV_SEVERITY.ERROR, 0, "SNAV REROUTE_MIN ≤ REROUTE_MAX.")
            ok = false
        end
    end
...
local function update_dynamic_soar_alt_min()
    ...
    if not polar_b or polar_b <= 0 or polar_b ~= polar_b
        ...
        if not SoarNavGlobals.gcone_param_warning_sent then
            log_gcs(MAV_SEVERITY.WARNING, 1, "GCone off: set POLAR_CD0/B & AIRSPEED.")
...
function announce_polygon_area()
    ...
    log_gcs(MAV_SEVERITY.INFO, 1, string.format("SNav:Poly A=%.2fkm2;RP=%u;MD=%.0fm", area_km2, m, md_m))
...
function announce_radius_area()
    ...
    if rad_m > 0 then
        ...
        log_gcs(MAV_SEVERITY.INFO, 1, string.format("SNav:Radius A=%.2fkm2;RD=%.0fm", area_km2, rad_m))
        SoarNavGlobals.area_announced = true
    end
end
...
local function check_pitch_gesture()
    ...
    if SoarNavGlobals.pitch_gesture_count >= SoarNavConstants.PITCH_GESTURE_COUNT_TARGET then
        ...
        if SoarNavGlobals.use_polygon_area then
            log_gcs(MAV_SEVERITY.NOTICE, 1, "Stick CMD: Polygon re-centered.")
            ...
            log_gcs(MAV_SEVERITY.NOTICE, 1, "Stick CMD: Radius Area re-centered.")
...
local function check_roll_gesture()
    ...
    if SoarNavGlobals.roll_gesture_count >= SoarNavConstants.ROLL_GESTURE_COUNT_TARGET then
        ...
        if SoarNavGlobals.script_state == SCRIPT_STATE.WAITING_FOR_ACTIVATION then
            ...
            log_gcs(MAV_SEVERITY.NOTICE, 1, "Activation gesture received.")
...
local function read_polygon_file(filename)
    ...
    if count > 0 then
        ...
        SoarNavGlobals.polygon_centroid = home:copy()
        SoarNavGlobals.polygon_centroid:lat(center_lat)
        SoarNavGlobals.polygon_centroid:lng(center_lon)

        SoarNavGlobals.polygon_vertex_offsets = {}
        for _, point in ipairs(SoarNavGlobals.polygon_points) do
            local offset = {
                lat = point:lat() - center_lat,
                lon = point:lng() - center_lon
            }
            table.insert(SoarNavGlobals.polygon_vertex_offsets, offset)
        end
    end
    ...
    if first_pt:get_distance(last_pt) > 1 then
        ...
        local offset = {
            lat = first_pt:lat() - SoarNavGlobals.polygon_centroid:lat(),
            lon = first_pt:lng() - SoarNavGlobals.polygon_centroid:lng()
        }
        table.insert(SoarNavGlobals.polygon_vertex_offsets, offset)
    end
...
local function generate_target_around_point(target_loc, radius_m)
    ...
    if not SoarNavGlobals.use_polygon_area and rand_radius_m > p_radius_m:get() then
        return nil
    end
...
function log_new_waypoint(dist_to_wp)
    ...
    if current_loc and SoarNavGlobals.target_loc then
        local __bearing_rad = current_loc:get_bearing(SoarNavGlobals.target_loc)
        bearing_deg = math.deg(__bearing_rad)
    end
    bearing_deg = (bearing_deg + 360) % 360
    local brg_rad = math.rad(bearing_deg)
    local t_n = math.cos(brg_rad)
    local t_e = math.sin(brg_rad)
    local wind = get_wind_vector()
    local w_n = 0
    local w_e = 0
    if wind then w_n = wind:x(); w_e = wind:y() end
    local Va = p_airspeed_cruise and p_airspeed_cruise:get() or 0
    if Va <= 0 then Va = 15 end
    ...
    local a_n = s * t_n - w_n
    local a_e = s * t_e - w_e
    local function __atan2(y, x) return math.atan(y, x) end
    local hdg_deg = (math.deg(__atan2(a_e, a_n)) + 360) % 360
    ...
    if __show_cells then
        log_gcs(MAV_SEVERITY.INFO, 1, string.format("🎯 [%s] %.0f°%s %.0fm %d/%d", src, hdg_deg, hdg_dir, d, visited_cells, total_valid_cells))
    else
        log_gcs(MAV_SEVERITY.INFO, 1, string.format("🎯 [%s] %.0f°%s %.0fm", src, hdg_deg, hdg_dir, d))
    end
...
local function search_for_new_waypoint()
    ...
    if not new_wp_found then
        log_gcs(MAV_SEVERITY.WARNING, 1, "All strategies failed. Using guaranteed fallback.")
    ...
    if new_wp_found then
        ...
        log_gcs(MAV_SEVERITY.CRITICAL, 0, "FATAL: Could not set any valid waypoint.")
    end
end
...
local function log_navigation_status(current_time_ms, heading_error, cached_params)
    if not SoarNavGlobals.last_status_log_ms or safe_time_diff(current_time_ms, SoarNavGlobals.last_status_log_ms) > SoarNavConstants.STATUS_LOG_INTERVAL_MS then
        ...
        if (cached_params.log_lvl >= 2) then
            local total_cells = #SoarNavGlobals.valid_cell_indices
            local visited = total_cells - #SoarNavGlobals.unvisited_cell_indices
            local grid_percent = total_cells > 0 and (visited * 100 / total_cells) or 0
            ...
            if tmem_count > 0 then
                ...
                for i = 2, tmem_count do
                    if (valid_hotspots_log[i].avg_strength or 0) > (best_hotspot.avg_strength or 0) then
                        best_hotspot = valid_hotspots_log[i]
                    end
                end
                best_strength = best_hotspot.avg_strength or 0
            end

            local log_msg = string.format("D:%.0fm HdgErr:%+.0f R:%+.1f | Grid:%d/%d(%.0f%%) C:%d | TMem:%d(%.1fm/s)",
                SoarNavGlobals.distance_to_wp,
                heading_error,
                SoarNavGlobals.last_commanded_roll_deg,
                visited,
                total_cells,
                grid_percent,
                SoarNavGlobals.last_cell_index or 0,
                tmem_count,
                best_strength)

            gcs:send_text(MAV_SEVERITY.INFO, "SNav Status: " .. log_msg)
        end
    end
end
    ...
    if can_navigate then
        if not SoarNavGlobals.has_been_activated then
            if not SoarNavGlobals.activation_wait_notified then
                if SoarNavGlobals.script_state ~= SCRIPT_STATE.WAITING_FOR_ACTIVATION then set_script_state(SCRIPT_STATE.WAITING_FOR_ACTIVATION, "Awaiting pilot activation (ROLL gesture).") end
...
handle_navigating = function(current_time_ms, loc, can_navigate, is_in_thermal_mode, cached_params, autotune_active, dt_s)
    if SoarNavGlobals.force_new_search then
        ...
        log_gcs(MAV_SEVERITY.INFO, 1, "Forcing new target search...")
    end
        ...
        if SoarNavGlobals.is_repositioning then
            ...
            if SoarNavGlobals.g_waypoint_source_info == SoarNavWaypointSources.REENGAGE_ENTRY then
                ...
                if SoarNavGlobals.reengage_hold_active and now_ms < SoarNavGlobals.reengage_hold_until_ms and not is_in_thermal_mode then
                    ...
                    -- NON FARE NULLA QUI, CONTINUA A NAVIGARE VERSO IL TARGET ESISTENTE
                    return
                else
    ...
    elseif wp_status == "REROUTE" then
        ...
        if __loc and SoarNavGlobals.target_loc then
            ...
            __b = __loc:get_bearing(SoarNavGlobals.target_loc)
        end
        __b = (__b + 360) % 360
        local __dirs = { "N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE", "S", "SSW", "SW", "WSW", "W", "WNW", "NW", "NNW" }
        ...
        log_gcs(MAV_SEVERITY.NOTICE, 1, string.format("🎯 [Re-route] %.0f°%s %.0fm %d/%d", __b, __dir, __d, __v, __t))
        SoarNavGlobals.target_loc = nil
        goto restart_search
    end
...
handle_waiting = function(can_navigate)
    if SoarNavGlobals.has_been_activated and can_navigate then set_script_state(SCRIPT_STATE.NAVIGATING, "Navigation conditions met, starting."); return end
    if not arming:is_armed() then
        set_script_state(SCRIPT_STATE.IDLE, "Disarmed.")
        return
    end
    if can_navigate then
        check_roll_gesture()
        if SoarNavGlobals.activation_requested then
            ...
            set_script_state(SCRIPT_STATE.NAVIGATING, "Pilot activation received.")
        end
    end
end
...
handle_pilot_override = function(current_time_ms, is_outside_pitch_dz, is_outside_yaw_dz, is_outside_roll_dz, can_navigate)
    ...
    if not SoarNavGlobals.manual_override_active then
        ...
        if not pilot_is_holding_input and SoarNavGlobals.last_pilot_input_ms and resume_delay_passed then
            if can_navigate or SoarNavGlobals.is_initializing or (not SoarNavGlobals.grid_initialized) then
...
update_body = function()
    ...
    if (not is_armed_now) and SoarNavGlobals.last_armed_state == true and not SoarNavGlobals.restored_on_disarm then
        ...
        SoarNavGlobals.has_been_activated = false
        SoarNavGlobals.activation_requested = false
    ...
    if (current_snav_radius_m ~= SoarNavGlobals.last_snav_radius_m_value) or (current_snav_enable ~= SoarNavGlobals.last_snav_enable_value) then
        ...
        if current_snav_radius_m > 0 then
            log_gcs(MAV_SEVERITY.INFO, 1, string.format("Radius Mode activated: %.0fm.", current_snav_radius_m))
    ...
    if not SoarNavGlobals.rc_limits_read and arming:is_armed() then
        ...
        if not (SoarNavGlobals.rc_roll_min < SoarNavGlobals.rc_roll_trim and SoarNavGlobals.rc_roll_trim < SoarNavGlobals.rc_roll_max) then
            log_gcs(MAV_SEVERITY.WARNING, 1, string.format("RC R lim: %d/%d/%d",
        ...
        if not (rc_pitch_min < rc_pitch_trim and rc_pitch_trim < rc_pitch_max) then
            log_gcs(MAV_SEVERITY.WARNING, 1, string.format("RC P lim: %d/%d/%d",
                rc_pitch_min, rc_pitch_trim, rc_pitch_max))
        end
        if not (rc_yaw_min < rc_yaw_trim and rc_yaw_trim < rc_yaw_max) then
            log_gcs(MAV_SEVERITY.WARNING, 1, string.format("RC Y lim: %d/%d/%d",
        ...
        if cached_params.radius_m > 0 then
            ...
            if not SoarNavGlobals.polygon_load_attempted then
                ...
                if poly_loaded then
                    ...
                    log_gcs(MAV_SEVERITY.CRITICAL, 0, "No polygon file; mode off.")
                    SoarNavGlobals.use_polygon_area = false
                end
    ...
    if not gesture_just_completed and SoarNavGlobals.script_state == SCRIPT_STATE.NAVIGATING and (math.abs(pitch_in) > 0.05 or math.abs(yaw_in) > 0.05) and not (SoarNavGlobals.activation_grace_start_ms and safe_time_diff(current_time_ms, SoarNavGlobals.activation_grace_start_ms) < 3000) then
...
log_gcs(MAV_SEVERITY.NOTICE, 1, "Script V1.0.0 initialized.")
