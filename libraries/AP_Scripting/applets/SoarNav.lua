...
local MSG_ID = {
    AREA_INFO_POLY = 1, PARAM_OUT_OF_RANGE = 2, GESTURE_ACTIVATED = 3, ALL_STRAT_FAILED = 4,
    AUTO_START = 5, AWAITING_ACTIVATION = 6, BASE_ALTS_INFO = 7, NO_THERMAL_LOC = 8,
    CELL_INDEX_FAIL = 9, DISARMED = 10, ERR_ROLL_NIL = 11, EFF_CLUSTER_RADIUS = 12,
    GRID_READY = 13, FATAL_NO_WP = 14, FOCUS_ON_DENSITY = 15, FORCING_NEW_TARGET = 16,
    RPS_NEED_3 = 17, GCONE_RST_OVERRIDE = 18, GCONE_OFF_NO_PARAMS = 19, GCONE_ALT_UPDATE = 20,
    GRID_TOO_SMALL = 21, GRID_INIT_FAIL_NO_CENTER = 22, GRID_INIT_STARTED = 23, GRID_VALIDATED = 24,
    GRID_SCAN_INFO = 25, INVALID_RC_MAPPING = 26, INVALID_SNAV_PARAMS = 27, LOADED_POLY = 28,
    MOTOR_FAIL_RTL = 29, MOTOR_FAIL_REASON = 30, NAV_COND_MET = 31, NAV_COND_NOT_MET = 32,
    NO_RPM_FALLBACK = 33, NO_POLY_DISABLED = 34, NO_PROG_WARN = 35, PARAM_CHANGED_REINIT = 36,
    PERSISTENT_LOC_ERROR = 37, PILOT_ACTIVATION = 38, PILOT_OVERRIDE = 39, POLY_INVALID_POINTS = 40,
    POLY_MAX_DIST = 41, RC_P_LIMITS = 42, RC_R_LIMITS = 43, RC_Y_LIMITS = 44, RC_LIMITS_READ = 45,
    RPM_SENSOR_SET = 46, RADIUS_MODE_ACTIVATED = 47, RALLY_CHANGED_REINIT = 48, REROUTE_SKIP_HEADING = 49,
    REPOSITIONED = 50, REROUTE_SKIP_NO_TARGET = 51, RESUMING_NAV = 52, SNAV_REROUTE_MINMAX = 53,
    SNAV_RADIUS_NEGATIVE = 54, AREA_INFO_RADIUS = 55, SCRIPT_INITIALIZED = 56, SCRIPT_DISABLED = 57,
    SET_CRUISE_FBWB = 58, GCS_PREFIX = 59, MISSING_PARAM = 60, PARAM_SKIP = 61, STICK_CMD_MANUAL_OFF = 62,
    STICK_CMD_MANUAL_ON = 63, STICK_CMD_POLY_RECENTER = 64, STICK_CMD_RADIUS_RECENTER = 65, STICK_CMD_STUCK = 66,
    THERMAL_EXIT_NO_DATA = 67, THERMAL_FOUND_EXIT_FOCUS = 68, THERMAL_IGNORED_OOB = 69, POLY_FAILED_FALLBACK = 70,
    NAV_TARGET_SIMPLE = 71, NAV_TARGET_CELLS = 72, NAV_TARGET_REROUTE = 73, RTL_STALL_OVERRIDE = 74, RTL_OVERRIDE_RESUME = 75,
    NAV_TARGET_RIDGE = 76, POLAR_LEARN_UPDATE = 77,
}
...
local function pget(ph, default)
    if ph and ph.get then
        local ok, v = pcall(function() return ph:get() end)
...
local SoarNavGlobals = {
    ...
    polar_est = { s11 = 0, s22 = 0, s12 = 0, y1 = 0, y2 = 0, n = 0, last_fit_ms = 0, last_sample_ms = 0, last_saved_cd0 = nil, last_saved_b = nil, last_commit_ms = 0, stable_count = 0, sink_bias = 0, err_ema = 0 },
...
local function safe_time_diff(newer, older)
    ...
    local function to_ms(x)
        ...
        local ok, v = pcall(function() return x:toint() end)
...
function SoarNavGlobals.ridge_score_at_loc(loc)
    ...
    local h0 = terrain:height_amsl(loc, true); if not h0 then return 0 end
    local pE = loc:copy(); pE:offset_bearing(90, dx)
    local pN = loc:copy(); pN:offset_bearing(0,  dx)
    local hE = terrain:height_amsl(pE, true); if not hE then return 0 end
    local hN = terrain:height_amsl(pN, true); if not hN then return 0 end
...
local function get_agl_m()
    ...
    if not ned then return 100 end
...
local function loc_to_xy(loc)
    local origin = SoarNavGlobals.polygon_origin
    if not origin or not loc then return nil, nil end
...
local function clamp_inside_polygon(loc, pad_m)
    if not SoarNavGlobals.use_polygon_area then return loc end
    if not loc then return loc end
    if in_flight_area(loc) then return loc end
    local px, py = loc_to_xy(loc)
    if not px then return loc end
    local poly = SoarNavGlobals.polygon_xy
    if not poly or #poly < 3 then return loc end
    ...
    for i=1, n do
        ...
        local d = dx*dx + dy*dy
        if d < best_d then best_d = d; best_dx, best_dy = dx, dy end
    end
    local len = math.sqrt(best_d)
    local nx, ny = 0, 0
    if len > 1e-6 then nx, ny = best_dx/len, best_dy/len end
    local pad = math.max(0, pad_m or 0)
    local rx, ry = px + nx*(pad + 1.0), py + ny*(pad + 1.0)
    local res = xy_to_loc(rx, ry)
    if res and in_flight_area(res) then return res end
    return loc
end
...
local function segment_stays_inside(current, target)
    if not SoarNavGlobals.use_polygon_area then return true end
    if not (current and target) then return true end
    local dist = current:get_distance(target)
    if dist < 120 then return true end
    local cache = SoarNavGlobals._seg_cache or {}
    local now = millis()
    local clat = current:lat(); local clon = current:lng()
    local tlat = target:lat();  local tlon = target:lng()
    if cache.clat == clat and cache.clon == clon and cache.tlat == tlat and cache.tlon == tlon and cache.ms and (now - cache.ms) < 400 then
        return cache.res
    end
    ...
    for i=1, #edges do
        ...
        if not ( (math.max(cx or 0, tx or 0) < (e.minx or 0)) or (math.min(cx or 0, tx or 0) > (e.maxx or 0)) or (math.max(cy or 0, ty or 0) < (e.miny or 0)) or (math.min(cy or 0, ty or 0) > (e.maxy or 0)) ) then
...
local function adjust_target_segment(current, target)
    if segment_stays_inside(current, target) then return target end
    local cx, cy = loc_to_xy(current)
    local tx, ty = loc_to_xy(target)
    if not cx then return target end
    ...
    if segment_stays_inside(current, clamped) then return clamped end
    return target
end
...
set_target_safely = function(candidate_loc)
    if not candidate_loc then return false end
...
local function update_dynamic_soar_alt_min()
    ...
    if override_active then
        ...
        if not SoarNavGlobals.override_reset_done then
            ...
            if init_min and init_cutoff and init_max and P.soar_alt_min and P.soar_alt_cutoff and P.soar_alt_max then
                ...
                local changed = false
                if cur_min ~= init_min then P.soar_alt_min:set(init_min); changed = true end
                if cur_cut ~= init_cutoff then P.soar_alt_cutoff:set(init_cutoff); changed = true end
                if cur_max ~= init_max then P.soar_alt_max:set(init_max); changed = true end
                if changed then
    ...
    if not P.dyn_soalt or not P.soar_polar_b or not P.soar_polar_cd0 or not P.airspeed_cruise or
        not P.soar_alt_min or not P.soar_alt_max or not P.soar_alt_cutoff or not P.gc_margin or not P.gc_pad then
        return
    end
...
function update_polar_learning()
    ...
    if not pe then
        pe = { s11=0, s22=0, s12=0, y1=0, y2=0, n=0, last_fit_ms=0, last_sample_ms=0, last_commit_ms=0, last_saved_cd0=nil, last_saved_b=nil, sink_bias=0, err_ema=0, stable_count=0, first_sample_ms=0, v_ema=0, v2_ema=0, s_mag_log_ema=0 }
        SoarNavGlobals.polar_est = pe
    end

    local throttle = SRV_Channels:get_output_scaled(SoarNavConstants.THROTTLE_SRV_CHANNEL) or 0
    if throttle > SoarNavConstants.THROTTLE_ACTIVE_THRESHOLD then return end

    local roll_deg = math.abs(ahrs:get_roll_rad() * (180/math.pi))
    if roll_deg > 12 then return end

    local vned = ahrs:get_velocity_NED()
    if not vned then return end
    ...
    if not v or v <= 8 then return end

    local sink_raw = vned:z()
    if sink_raw < 0.10 then return end

    local now = millis()
    if not pe.first_sample_ms or pe.first_sample_ms == 0 then pe.first_sample_ms = now end
    ...
    if safe_time_diff(now, pe.first_sample_ms or 0) < 60000 then return end
    if (pe.n or 0) < 150 then return end
    if pe.last_fit_ms and safe_time_diff(now, pe.last_fit_ms) < 8000 then return end
    pe.last_fit_ms = now

    local cd0 = P.soar_polar_cd0:get()
    local bb  = P.soar_polar_b:get()
    if not pe.last_saved_cd0 then pe.last_saved_cd0 = cd0 end
    if not pe.last_saved_b   then pe.last_saved_b   = bb  end
    ...
    if not vc or vc <= 0 then vc = 15 end

    if v_std < 0.8 then
        local sink_model = (cd0*v*v + bb) / v
        if not sink_model or sink_model <= 0 then return end
        local s_mag_inst = sink / sink_model
        if s_mag_inst < 0.90 then s_mag_inst = 0.90 end
        if s_mag_inst > 1.10 then s_mag_inst = 1.10 end
        ...
        if new_b < 0.005 then new_b = 0.005 end
        if new_b > 0.060 then new_b = 0.060 end
        if new_cd0 < 0.005 then new_cd0 = 0.005 end
        if new_cd0 > 0.500 then new_cd0 = 0.500 end
    ...
    if not det or det <= 1e-9 then return end
    ...
    if not a_est or not b_est or a_est <= 0 or b_est <= 0 or a_est ~= a_est or b_est ~= b_est then return end

    local pred = a_est*(v^3) + b_est*(1/v)
    local e = math.abs(sink - pred)
    pe.err_ema = (pe.err_ema == 0) and e or (0.9*pe.err_ema + 0.1*e)
    if pe.err_ema > 0.6 then return end

    local r_est = b_est / a_est
    local r_cur = bb / cd0
    if not r_est or not r_cur or r_est <= 0 or r_cur <= 0 then return end

    local h = math.sqrt(r_est / r_cur)
    if h < 0.90 then h = 0.90 end
    if h > 1.10 then h = 1.10 end
    local cd0_shape = cd0 / h
    local b_shape   = bb  * h

    local denom = a_est * vc * vc + b_est / (vc * vc)
    if not denom or denom <= 0 or denom ~= denom then return end
    local eff = 1 / denom
    if not eff or eff <= 0 or eff ~= eff then return end

    local target_prod = 1 / (4 * eff * eff)
    local prod_shape  = cd0_shape * b_shape
    if not prod_shape or prod_shape <= 0 or not target_prod or target_prod <= 0 then return end

    local s_prod = math.sqrt(target_prod / prod_shape)
    if s_prod < 0.90 then s_prod = 0.90 end
    if s_prod > 1.15 then s_prod = 1.15 end
...
local function restore_boot_alts(_)
    if SoarNavGlobals and SoarNavGlobals.initial_soar_alt_min and SoarNavGlobals.initial_soar_alt_max and SoarNavGlobals.initial_soar_alt_cutoff then
        if P.soar_alt_min then P.soar_alt_min:set(SoarNavGlobals.initial_soar_alt_min) end
        if P.soar_alt_max then P.soar_alt_max:set(SoarNavGlobals.initial_soar_alt_max) end
        if P.soar_alt_cutoff then P.soar_alt_cutoff:set(SoarNavGlobals.initial_soar_alt_cutoff) end
...
local function check_pitch_gesture()
    ...
    if SoarNavGlobals.script_state ~= SCRIPT_STATE.PILOT_OVERRIDE
       or not soaring_switch_high
       or not in_fw_nav_mode then
        SoarNavGlobals.pitch_gesture_state = "idle"
        SoarNavGlobals.pitch_gesture_count = 0
        SoarNavGlobals.pitch_gesture_start_ms = nil
        SoarNavGlobals.pitch_gesture_triggered_this_override = false
...
if SoarNavGlobals.script_state ~= SCRIPT_STATE.PILOT_OVERRIDE then
    ...
    if SoarNavGlobals.pitch_gesture_count >= SoarNavConstants.PITCH_GESTURE_COUNT_TARGET then
        ...
        if SoarNavGlobals.use_polygon_area then
            ...
            if (P.radius_m and pget(P.radius_m, 0) or 0) <= 0 then return end
...
local function stop_and_record_thermal()
    ...
    for _, hotspot in ipairs(SoarNavGlobals.thermal_hotspots) do
        if hotspot.timestamp:toint() == new_hotspot.timestamp:toint() then
            if hotspot.cluster_density and hotspot.cluster_density >= pget(P.focus_thr, 1.0) and not SoarNavGlobals.is_in_focus_mode then
...
function read_rally_points_as_polygon()
    ...
    if #initial_points < 3 then return nil end
...
function log_new_waypoint(dist_to_wp)
    ...
    local current_loc = ahrs:get_location()
    if not current_loc or not SoarNavGlobals.target_loc then return end
    ...
    if __show_cells then
        log_gcs(MAV_SEVERITY.INFO, 1, MSG_ID.NAV_TARGET_CELLS, src, hdg_deg, hdg_dir, d, visited_cells, total_valid_cells)
    else
...
local function select_best_thermal_waypoint()
    ...
    table.sort(final_hotspot_list, function(a, b)
        ...
        local score_a = (a.avg_strength or 0) * strength_weight + ((a.avg_strength or 0) * (a.consistency_score or 0.5)) * quality_weight
        local score_b = (b.avg_strength or 0) * strength_weight + ((b.avg_strength or 0) * (b.consistency_score or 0.5)) * quality_weight
...
local function search_for_new_waypoint()
    ...
    if SoarNavGlobals.thermal_to_retry and not throttle_on then
        ...
        if hotspot_to_retry and hotspot_to_retry.loc and current_loc then
            ...
            local candidates = { (bearing_to_thermal + 180) % 360, (bearing_to_thermal + 215) % 360, (bearing_to_thermal + 145) % 360, (bearing_to_thermal + 250) % 360, (bearing_to_thermal + 110) % 360 }
            local best_p = nil
            local best_s = -1e9
            for i=1,#candidates do
                ...
                if SoarNavGlobals.use_polygon_area then
                    if not in_flight_area(p) then
                        p = clamp_inside_polygon(p, (P and P.wp_radius and P.wp_radius:get()) or 50)
                    end
                    if not segment_stays_inside(current_loc, p) then
                        p = adjust_target_segment(current_loc, p)
                    end
                    if not in_flight_area(p) then goto continue end
                end
                ...
                if score > best_s then best_s = score; best_p = p end
                ::continue::
            end
            local flyout_point = best_p or (function()
                local fp = current_loc:copy(); fp:offset_bearing(((bearing_to_thermal + 180) % 360), reengage_dist); return fp
            end)()
        ...
        if not force_grid_search then
            if SoarNavGlobals.is_in_focus_mode then
                if SoarNavGlobals.focus_wp_counter >= (SoarNavGlobals.focus_wp_timeout or 3) then
                    ...
                    local wind = get_wind_vector()
                    local drifted_focus_center = predict_thermal_drift(SoarNavGlobals.focus_area_center, wind, age_seconds)
    ...
    if not new_wp_found then
        if SoarNavGlobals.grid_initialized and #SoarNavGlobals.valid_cell_indices > 0 then
            ...
            local now_ms = millis()
            for _, hotspot in ipairs(SoarNavGlobals.thermal_hotspots) do
                if hotspot.entry_time and safe_time_diff(now_ms, hotspot.entry_time) <= (pget(P.strat_hist, 900) * 1000) then
            ...
            if chosen_cell_index and SoarNavGlobals.grid_cell_centers[chosen_cell_index] then
                ...
                for _ = 1, 5 do
                    ...
                    if in_flight_area(new_target_loc) and current_loc and current_loc:get_distance(new_target_loc) > (pget(P.wp_radius, 50)) then
                ...
                if success then
                    ...
                    if in_flight_area(precalculated_center) and current_loc and current_loc:get_distance(precalculated_center) > (pget(P.wp_radius, 50)) then
    ...
    if not new_wp_found then
        ...
        if center_point then
            ...
            if radius_m <= 0 then radius_m = (P.radius_m and pget(P.radius_m, 0) or 500) end
...
local function manage_anti_stuck(loc, current_time_ms)
    ...
    local time_on_current_wp = safe_time_diff(current_time_ms, SoarNavGlobals.waypoint_start_time_ms)
    if time_on_current_wp < (pget(P.stuck_time, 30) * 1000) then return false end
        ...
        if SoarNavGlobals.distance_at_last_check > 0 and SoarNavGlobals.loc_at_last_check then
            ...
            if progress_made < min_prog or efficiency_ratio < min_eff then
                ...
                if wind_vec_info then
                    ...
                    log_gcs(MAV_SEVERITY.WARNING, 1, MSG_ID.NO_PROG_WARN, efficiency_ratio, wind_vec_info:length(), wind_bearing_info)
                end
            else
                SoarNavGlobals.stuck_counter = 0
            end
...
handle_waiting = function(can_navigate)
    if SoarNavGlobals.has_been_activated and can_navigate then set_script_state(SCRIPT_STATE.NAVIGATING, MSG_ID.NAV_COND_MET); return end
...
handle_pilot_override = function(current_time_ms, is_outside_pitch_dz, is_outside_yaw_dz, is_outside_roll_dz, can_navigate)
    ...
    local __now = millis()
    if SoarNavGlobals.last_stick_tick_ms
       and safe_time_diff(__now, SoarNavGlobals.last_stick_tick_ms) < 200 then
        return
    end
...
function SoarNav_RTLH_update(now_ms)
    ...
    if SoarNavGlobals.RTLH.last_mode ~= nil and SoarNavGlobals.RTLH.engaged_guided and SoarNavGlobals.RTLH.last_mode == SoarNavConstants.GUIDED_MODE_CODE and SoarNavGlobals.RTLH._mode ~= SoarNavConstants.GUIDED_MODE_CODE then
        ...
        if log_gcs then log_gcs(MAV_SEVERITY.NOTICE, 1, MSG_ID.RTL_OVERRIDE_RESUME) end
    end
    ...
    if not SoarNavGlobals.RTLH.engaged_guided and (now_ms - SoarNavGlobals.RTLH.t0_ms) >= SoarNavConstants.RTLH_CHECK_DELAY_MS then
        ...
        if not home or not here then return end

        local d_now = here:get_distance(home)
        
        local rtl_radius = param:get("RTL_RADIUS") or 0
        if d_now <= (rtl_radius * 1.5) then return end
        ...
        if (SoarNavGlobals.RTLH.d0_m - d_now) < needed_gain then
            ...
            if log_gcs then log_gcs(MAV_SEVERITY.NOTICE, 1, MSG_ID.RTL_STALL_OVERRIDE, math.floor(alt_m + 0.5)) end
        end
    end
end
...
do
    ...
    if not SoarNavGlobals.rc_limits_read and arming:is_armed() then
        ...
        if not (SoarNavGlobals.rc_roll_min < SoarNavGlobals.rc_roll_trim and SoarNavGlobals.rc_roll_trim < SoarNavGlobals.rc_roll_max) then
            log_gcs(MAV_SEVERITY.WARNING, 1, MSG_ID.RC_R_LIMITS, SoarNavGlobals.rc_roll_min, SoarNavGlobals.rc_roll_trim, SoarNavGlobals.rc_roll_max)
        end
    ...
    if arming:is_armed() and
       SoarNavGlobals.script_state == SCRIPT_STATE.NAVIGATING and
       (SRV_Channels:get_output_scaled(SoarNavConstants.THROTTLE_SRV_CHANNEL) or 0) < SoarNavConstants.THROTTLE_ACTIVE_THRESHOLD and
       current_mode ~= SoarNavConstants.mode_thermal then
