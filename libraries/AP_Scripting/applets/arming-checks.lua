...
function Arming_Check:state()
    ...
    if self.passed == passed_new then
        ...
        --gcs:send_text(MAV_SEVERITY.ERROR, string.format('ARMING: %s %s new/pass CHANGED', passed_new, self.passed))
...
if Q_ENABLE:get() == 1 then
    q_options = Parameter("Q_OPTIONS"):get()
end
...
local function sysid_not_set()
    local sysid_thismav = SYSID_THISMAV:get()
    if sysid_thismav == 1 then
        return false
    end
    return true
end
...
local function foll_sysid_not_thismav()
    ...
    if foll_enable == 1 then
        ...
        if foll_sysid > 0 and sysid_thismav == foll_sysid then
            return false
        end
    end
    return true
    ...
    if foll_enable == 1 and (mnt1_enable or mnt2_enable) then
        ...
        if mnt1_enable then
            ...
            if mnt1_sysid > 0 and foll_sysid ~= mnt1_sysid then
                --gcs:send_text(MAV_SEVERITY.ERROR, string.format('ARMING: MNT1 SYSID %d FOLL %d', mnt1_sysid, foll_sysid))
                return false
            end
        end
        if mnt2_enable then
            ...
            if mnt2_sysid > 0 and foll_sysid ~= mnt2_sysid then
                --gcs:send_text(MAV_SEVERITY.ERROR, string.format('ARMING: MNT2 SYSID %d FOLL %d', mnt2_sysid, foll_sysid))
                return false
            end
        end
    end
...
local function rtl_climb_legal()
    if (RTL_CLIMB_MIN ~= nil and RTL_CLIMB_MIN:get() > 120) then
        return false
    end
    return true
end
...
local function qland_warning()
    if Q_ENABLE:get() == 1 then
        if q_options ~= nil then
            --for i = 0, 32 do
            --    if q_options & (1 << i) > 0  then
            --        gcs:send_text(MAV_SEVERITY.ERROR, string.format('QLAND: %d bit: %d = %d', q_options, i, (1 << i)) )
            --    end
            --end
            ...
            if (not q_land_option) and (not q_RTL_option) then
                --gcs:send_text(MAV_SEVERITY.ERROR, string.format('LAND: %d land: %s rtl: %s', q_options, q_land_option, q_RTL_option) )
                return false
            end
        end
    end
    ...
    if Q_ENABLE:get() > 0 then
        if q_options ~= nil then
            ...
            if q_land_option or q_RTL_option then
                -- gcs:send_text(MAV_SEVERITY.ERROR, string.format('QRTL: %d land: %s rtl: %s', q_options, q_land_option, q_RTL_option) )
                return false
            end
        end
    end
...
local arming_checks = {
    ...
    SYSID_NotSet = Arming_Check(sysid_not_set, "SYSID",
                            "SYSID_THISMAV not set", MAV_SEVERITY.ERROR, false, false ),
    ...
    FOLL_SYSID_not_THISMAV = Arming_Check(foll_sysid_not_thismav, "FOLL_SYSID_X",
                            "FOLL_SYSID == SYSID_THISMAV", MAV_SEVERITY.ERROR, true, false ),
    ...
    RTLAltitudeLegal = Arming_Check(rtl_altitude_legal, "RTL_ALT",
                            "RTL_ALT too high", MAV_SEVERITY.ERROR, false, false ),
    ...
    QLandWarning = Arming_Check(qland_warning, "Q_FS_LAND",
                            "Q will land on failsafe", MAV_SEVERITY.INFO, true, false ),
    QRTLWarning = Arming_Check(qrtl_warning, "Q_FS_RTL",
                            "Q will RTL on failsafe", MAV_SEVERITY.INFO, true, false )
}
...
local function initialize()
    for _key, check in pairs(arming_checks) do
        --[[
        // @Param: ZAR_xxx
...
validate = function() -- this is the loop which periodically runs
    ...
    for _key, check in pairs(arming_checks) do
        ...
        local severity  = parameter:get() or MAV_SEVERITY.NONE

        if severity ~= MAV_SEVERITY.NONE then -- ignore if NONE
            local check_passed = check:state() -- side effect - sets check.passed and check.changed

            if check.changed then
                if check_passed then
                    gcs:send_text(MAV_SEVERITY.INFO, string.format('%s: Cleared: %s', SCRIPT_NAME_SHORT, check.text))
                else
                    if severity <= MAV_SEVERITY.ERROR then
                        arming:set_aux_auth_failed(arm_auth_id, string.format('%s: fail: %s', SCRIPT_NAME_SHORT, check.text))
                    else
                        gcs:send_text(severity, string.format('%s: %s', SCRIPT_NAME_SHORT, check.text))
                        -- display the message, but this is actually "passed" - it should not block arming
                        check_passed = true
                    end
                end
    ...
    if validated then
        -- gcs:send_text(MAV_SEVERITY.NOTICE, string.format('ARMING: PASSED'))
...
if FWVersion:type() == 3 then -- 3 is Plane 
