...
local function rtl_altitude_legal()
    ...
    if (RTL_ALTITUDE ~= nil and (RTL_ALTITUDE:get() or 0) > ALTITUDE_LEGAL_MAX) then
        return false
    end
    -- copter uses RTL_ALT in cm
    if (RTL_ALT ~= nil and (RTL_ALT:get() or 0) > ALTITUDE_LEGAL_MAX * 100.0) then
        return false
    end
