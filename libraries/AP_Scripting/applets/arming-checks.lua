...
-- create parameter table for ZAR_ parameters that will be created below. Watch out that ALT_LEGAL is using 25!
...
-- check that if the FOLL_SYSID is set it's not pointing to itself
...
local function rtl_altitude_legal()
    ...
    if (RTL_ALTITUDE ~= nil and (RTL_ALTITUDE:get() or 0) > alt_legal_max) then
        return false
    end
    -- copter uses RTL_ALT in cm
    if (RTL_ALT ~= nil and (RTL_ALT:get() or 0) > alt_legal_max * 100.0) then
        return false
    end

    if FWVersion:type() == 3 then -- Plane specific Parameters
...
if FWVersion:type() == 3 then -- add Plane specific Parameters
    -- Display warnings for either Q_OPTIONS bit 5 = QLand AND QRTL which demonstrates boot time notifications
--[[
    // @Param: ZAR_Q_FS_LAND
...
--]]
    ...
    table.insert(arming_checks, EmergencyStop)
end
            ...
            if check.changed then
                if check_passed then
                    ...
                    if param_severity <= MAV_SEVERITY.ERROR then
                        ...
                        gcs:send_text(param_severity, string.format('%s: warn: %s', SCRIPT_NAME_SHORT, check.text))
