...
void RC_Channel::init_aux_function(const aux_func_t ch_option, const AuxSwitchPos ch_flag)
{
    ...
    case AUX_FUNC::COMPASS_NORTH_CAL:
    case AUX_FUNC::ACCEL_CAL_SIMPLE:
...
#if !HAL_MINIMIZE_FEATURES
    case AUX_FUNC::COMPASS_NORTH_CAL:
        if (ch_flag == AuxSwitchPos::HIGH && !hal.util->get_soft_armed()) {
            Compass &compass = AP::compass();
            compass.mag_cal_fixed_yaw(0 /*NORTH*/, 0 /*ALL COMPASSES*/, 0, 0);
        }
        break;

    case AUX_FUNC::ACCEL_CAL_SIMPLE:
        if (ch_flag == AuxSwitchPos::HIGH && !hal.util->get_soft_armed()) {
            AP::ins().simple_accel_cal();
        }
        break;
#endif // !HAL_MINIMIZE_FEATURES
