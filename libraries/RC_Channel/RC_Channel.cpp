...
bool RC_Channel::do_aux_function(const aux_func_t ch_option, const AuxSwitchPos ch_flag)
{
    ...
    case AUX_FUNC::EKF_POS_SOURCE:
        ...
        case AuxSwitchPos::LOW:
            ...
            AP::ahrs().set_posvelyaw_source_set(AP_NavEKF_Source::SOURCE_SET_SWITCH_PRIMARY);
            break;
        case AuxSwitchPos::MIDDLE:
            // middle switches to secondary source
            AP::ahrs().set_posvelyaw_source_set(AP_NavEKF_Source::SOURCE_SET_SWITCH_SECONDARY);
            break;
        case AuxSwitchPos::HIGH:
            // high switches to tertiary source
            AP::ahrs().set_posvelyaw_source_set(AP_NavEKF_Source::SOURCE_SET_SWITCH_TERTIARY);
            break;
        }
        break;
