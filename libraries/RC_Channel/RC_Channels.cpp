...
/*
  ...
  already run those functions, so that the position the vehicle booted with
  is applied once the backend which acts on it exists.  A function whose
  switch cannot be read, or whose dispatch does not take, is left to the
  first debounced read_aux() exactly as it is without this.
 */
