...
void RC_Channels::apply_aux_switch_positions(const RC_Channel::AUX_FUNC *functions, uint8_t count)
{
    for (uint8_t i=0; i<count; i++) {
        ...
        // recording the position below suppresses the announcement the
        // first debounced read_aux() would have made, so make it here
        const char *aux_string = c->string_for_aux_function(aux_function);
        if (aux_string != nullptr) {
            GCS_SEND_TEXT(MAV_SEVERITY_INFO, "RC%i: %s %s", c->ch(), aux_string, c->string_for_aux_pos(pos));
        }
