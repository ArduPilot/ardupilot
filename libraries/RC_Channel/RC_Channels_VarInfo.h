...
const AP_Param::GroupInfo RC_Channels::var_info[] = {
    ...
    // @User: Advanced
    // @Bitmask: 0:Ignore RC Receiver, 1:Ignore MAVLink Overrides, 2:Ignore Receiver Failsafe bit but allow other RC failsafes if setup, 3:FPort Pad, 4:Log RC input bytes, 5:Arming check throttle for 0 input, 6:Skip the arming check for neutral Roll/Pitch/Yaw sticks, 7:Allow Switch reverse, 8:Use passthrough for CRSF telemetry
    // @Values: 0: None,1:Ignore RC Receiver, 2:Ignore MAVLink Overrides, 4:Ignore Receiver Failsafe bit but allow other RC failsafes if setup, 8:FPort Pad, 16:Log RC input bytes, 32:Arming check throttle for 0 input, 64:Skip the arming check for neutral Roll/Pitch/Yaw sticks, 128:Allow Switch reverse, 256:Use passthrough for CRSF telemetry
