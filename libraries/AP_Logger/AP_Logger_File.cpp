...
{
    // not CardInserted(): that is false after an out-of-space stop,
    // which is exactly when the in-flight caller needs to free space
    if (!_initialised) {
        return false;
    }
...
void AP_Logger_File::io_timer(void)
{
    ...
    if (_initialised && tnow - _free_space_last_check_time > _free_space_check_interval) {
        ...
        // don't unlink under a MAVLink log download, which resolves
        // list entries against the oldest log on every packet, or
        // under MAVFTP, which may have any log open
        bool transfer_busy = _front.log_transfer_active();
#if AP_MAVLINK_FTP_ENABLED
        transfer_busy |= GCS_FTP::file_open();
#endif  // AP_MAVLINK_FTP_ENABLED
        if (_low_space_trim_backoff &&
            tnow - _low_space_trim_fail_ms >= _low_space_trim_backoff_ms) {
            _low_space_trim_backoff = false;
        }
        if (target_free > 0 && avail >= 0 && avail < trim_threshold &&
            !transfer_busy && !_low_space_trim_backoff) {
            ...
            if (Prep_MinSpace(1)) {
                ...
                _low_space_trim_backoff = true;
                _low_space_trim_fail_ms = tnow;
            }
        }
