 ...
 *
 * Caller is responsible for freeing the returned buffer (with free()). Returned
 * buffer is nul terminated to make it easier to use as a C string. The nul
 * terminator is not included in out_len. Whitespace in the input is skipped;
 * any other character outside the alphabet is rejected.
 */
