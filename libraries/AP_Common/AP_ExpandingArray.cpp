...
bool AP_ExpandingArrayGeneric::expand(uint16_t num_chunks)
{
    ...
    if (chunk_count + num_chunks >= chunk_count_max) {
        ...
        chunk_ptr_t *chunk_ptrs_new = (chunk_ptr_t*)malloc(chunk_ptr_size * sizeof(chunk_ptr_t));
        if (chunk_ptrs_new == nullptr) {
            return false;
        }
        memcpy((void*)chunk_ptrs_new, (void*)chunk_ptrs, chunk_count * sizeof(chunk_ptr_t));
        free(chunk_ptrs);
