...
{ 
    ...
    if( dt < 1000) {    // 1 point per second
        yield(1000-dt); // don't disturbe for needed time
        return; 
    }
...
void AP_WayBack::yield(uint32_t ttw) { // no threads but just cooperative multitask

    REVOMINIScheduler::yield(ttw); // give other tasks a chance do run
}

#else // we don't should do anything if HAL has a POSIX threads

void AP_WayBack::yield(uint32_t ttw){}
#endif
