...
#include <GCS_MAVLink.h>
//BEV necessary includes to get the damn thing to compile
...
static uint8_t verify_nav_cmd_iterations_to_complete = 1;
...
void mission_complete(void)
{
    ...
    mission.start_loiter_at_home(0);
}
...
void run_mission_test()
{
	...
	/*
    if(mission.get_current_nav_cmd().index == 3) {
        hal.console->println("Pausing mission");
        mission.stop();
        hal.console->println("Starting mission");
        mission.start();
    }
	*/
}
...
void init_mission()
{
    ...
    // Command #6 : do-jump to first waypoint 3 times
