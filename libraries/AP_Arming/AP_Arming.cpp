...
bool AP_Arming::barometer_checks(bool report)
{
    if ((checks_to_perform & ARMING_CHECK_ALL) ||
        ...
        if (!barometer.all_healthy()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Barometer not healthy");
            }
            return false;
        }
    }
...
bool AP_Arming::airspeed_checks(bool report)
{
    if ((checks_to_perform & ARMING_CHECK_ALL) ||
        ...
        if (airspeed->enabled() && airspeed->use() && !airspeed->healthy()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Airspeed not healthy");
            }
            return false;
        }
    }
...
bool AP_Arming::logging_checks(bool report)
{
    if ((checks_to_perform & ARMING_CHECK_ALL) ||
        ...
        if (!logging_available) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Logging not available");
            }
            return false;
        }
    }
...
bool AP_Arming::ins_checks(bool report)
{
    if ((checks_to_perform & ARMING_CHECK_ALL) ||
        ...
        if (!ins.get_gyro_health_all()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Gyros not healthy");
            }
            return false;
        }
        if (!ins.gyro_calibrated_ok_all()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Gyros not calibrated");
            }
            return false;
        }
        if (!ins.get_accel_health_all()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Accelerometers not healthy");
            }
            return false;
        }
        if (!ins.accel_calibrated_ok_all()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: 3D accelerometers calibration needed");
            }
            return false;
        }
        ...
        if (ins.accel_cal_requires_reboot()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Accelerometers calibrated requires reboot");
            }
            return false;
        }
        ...
        if (ins.get_accel_count() > 1) {
            ...
            for(uint8_t i=0; i<ins.get_accel_count(); i++) {
                ...
                if (AP_HAL::millis() - last_accel_pass_ms[i] > 10000) {
                    if (report) {
                        gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Accelerometers inconsistent");
                    }
                    return false;
                }
            }
        ...
        if (ins.get_gyro_count() > 1) {
            ...
            for(uint8_t i=0; i<ins.get_gyro_count(); i++) {
                ...
                if (AP_HAL::millis() - last_gyro_pass_ms[i] > 10000) {
                    if (report) {
                        gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Gyros inconsistent");
                    }
                    return false;
                }
            }
...
bool AP_Arming::compass_checks(bool report)
{
    if ((checks_to_perform) & ARMING_CHECK_ALL ||
        ...
        if (!_compass.healthy()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Compass not healthy");
            }
            return false;
        }
        ...
        if (!_compass.learn_offsets_enabled() && !_compass.configured()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Compass not calibrated");
            }
            return false;
        }
        ...
        if (_compass.is_calibrating()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "Arm: Compass calibration running");
            }
            return false;
        }
        ...
        if (_compass.compass_cal_requires_reboot()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Compass calibrated requires reboot");
            }
            return false;
        }
        ...
        if (offsets.length() > AP_ARMING_COMPASS_OFFSETS_MAX) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Compass offsets too high");
            }
            return false;
        }
        ...
        if (mag_field > AP_ARMING_COMPASS_MAGFIELD_MAX || mag_field < AP_ARMING_COMPASS_MAGFIELD_MIN) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Check mag field");
            }
            return false;
        }
        ...
        if (!_compass.consistent()) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL,"PreArm: Compasses inconsistent");
    }
            return false;
        }
    }
...
bool AP_Arming::gps_checks(bool report)
{
    ...
    if ((checks_to_perform & ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_GPS)) {
        ...
        if (home_is_set == HOME_UNSET || 
            ...
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Bad GPS Position");
            }
            return false;
        }
    }
...
bool AP_Arming::battery_checks(bool report)
{
    if ((checks_to_perform & ARMING_CHECK_ALL) ||
        ...
        if (AP_Notify::flags.failsafe_battery) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Battery failsafe on");
            }
            return false;
        }
    }
...
bool AP_Arming::hardware_safety_check(bool report) 
{
    if ((checks_to_perform & ARMING_CHECK_ALL) ||
      ...
      if (hal.util->safety_switch_state() == AP_HAL::Util::SAFETY_DISARMED) {
          if (report) {
              gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Hardware safety switch");
          }
          return false;
      }
    }
...
bool AP_Arming::manual_transmitter_checks(bool report)
{
    if ((checks_to_perform & ARMING_CHECK_ALL) ||
        ...
        if (AP_Notify::flags.failsafe_radio) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL, "PreArm: Radio failsafe on");
            }
            return false;
        }
    ...
    if ((checks_to_perform & ARMING_CHECK_ALL) || (checks_to_perform & ARMING_CHECK_VOLTAGE)) {
        if(!is_zero(hal.analogin->board_voltage()) &&
           ((hal.analogin->board_voltage() < AP_ARMING_BOARD_VOLTAGE_MIN) || (hal.analogin->board_voltage() > AP_ARMING_BOARD_VOLTAGE_MAX))) {
            if (report) {
                gcs.send_text_active(MAV_SEVERITY_CRITICAL,"PreArm: Check board voltage");
            }
            return false;
        }
    }
...
bool AP_Arming::arm(uint8_t method)
{
    ...
    if (checks_to_perform == ARMING_CHECK_NONE) {
        armed = true;
        arming_method = NONE;
        gcs.send_text_active(MAV_SEVERITY_INFO, "Throttle armed");
        return true;
    }
...
bool AP_Arming::disarm() 
{
    ...
    gcs.send_text_active(MAV_SEVERITY_INFO, "Throttle disarmed");
