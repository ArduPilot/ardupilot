...
AP_Arming::AP_Arming(const AP_AHRS &ahrs_ref, const AP_Baro &baro,
       const AP_HAL::HAL &hal_ref, const bool &home_set,
       gcs_send_t_p gcs_print_func)
   : armed(false)
   , arming_checks_enabled(ARMING_CHECK_UNINITILIZED)
   , arming_method(NONE)
   , ahrs(ahrs_ref)
   , barometer(baro)
   , hal(hal_ref)
...
bool AP_Arming::enabled_checks_make_sense() {
    //has anybody ever called 'set_enabled_checks()' ?
    if ((arming_checks_enabled & ARMING_CHECK_UNINITILIZED) != 0) {
        return false;
    }
...
uint16_t AP_Arming::get_enabled_checks() {
    return arming_checks_enabled;
}

void AP_Arming::set_enabled_checks(uint16_t ap) {
    arming_checks_enabled = ap;

    enabled_checks_make_sense();
}

bool AP_Arming::barometer_checks() {
    if (arming_checks_enabled & ARMING_CHECK_ALL ||
            arming_checks_enabled & ARMING_CHECK_BARO) {
...
bool AP_Arming::compass_checks() {
    if (arming_checks_enabled & ARMING_CHECK_ALL ||
            arming_checks_enabled & ARMING_CHECK_COMPASS) {
        ...
        if (compass == NULL && (arming_checks_enabled & ARMING_CHECK_COMPASS)) {
...
bool AP_Arming::battery_checks() {
    if (arming_checks_enabled & ARMING_CHECK_ALL ||
            arming_checks_enabled & ARMING_CHECK_BATTERY) {
...
bool AP_Arming::manual_transmitter_checks() {
    if (arming_checks_enabled & ARMING_CHECK_ALL ||
            arming_checks_enabled & ARMING_CHECK_RC) {
...
bool AP_Arming::arm(uint8_t method) {
    ...
    if (arming_checks_enabled == ARMING_CHECK_NONE) {
        armed = true;
        arming_method = NONE;
