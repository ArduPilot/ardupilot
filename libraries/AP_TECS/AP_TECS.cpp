...
void AP_TECS::_update_throttle(void)
{
    ...
    else
    {
        ...
        float ff_throttle = 0;
        float nomThr = aparm.plane_throttle_cruise * 0.01f;
		const Matrix3f &rotMat = _ahrs.get_dcm_matrix();
...
void AP_TECS::_update_throttle_option(int16_t throttle_nudge)
{
    ...
    float nomThr;
     //BEV simplified throttle calculation
    nomThr = (aparm.plane_throttle_cruise + throttle_nudge)* 0.01f;
    
	//BEV more simplified calcs here
    if (_pitch_dem > 0.0 && _PITCHmaxf > 0.0)
	{
	...
	if(!(tmp+=16)) {
	    hal.console->print("Pitch deamd: ");
	    hal.console->print(_pitch_dem);
	    hal.console->print(" throttle demd: ");
	    hal.console->println(_throttle_dem);
	}
...
void AP_TECS::_initialise_states(int32_t ptchMinCO_cd, float hgt_afe) 
{
	...
	if (_DT > 1.0)
	{
		_integ6_state      = 0.0f;
		_integ7_state      = 0.0f;
		_last_throttle_dem = aparm.plane_throttle_cruise * 0.01f;
									...
									float hgt_afe) 
{
    ...
    _THRmaxf  = aparm.plane_throttle_max * 0.01f;
    _THRminf  = aparm.plane_throttle_min * 0.01f;
