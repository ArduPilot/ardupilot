...
AP_Camera::set_delay()
{
    _trigger_counter = constrain_int16(_trigger_duration*5,1,255) + constrain_int16(_noshake_duration*5,0,255) ;
}
...
//    _trigger_counter = 1;
    _trigger_counter =  constrain_int16(_noshake_duration*5,0,255) + 1;
...
{
if (_trigger_counter == _trigger_duration * 5)  // set pwm pulses "on" after noshake delay
{
        switch (_trigger_type) {
            case AP_CAMERA_TRIGGER_TYPE_SERVO:
            case AP_CAMERA_TRIGGER_TYPE_THROTTLE_OFF_TIME:
...
bool AP_Camera::do_not_shake()
{
if (_trigger_counter && _noshake_duration !=0)  //use _noshake_duration 0 for disable navigation pause 
     return true;
else
    return false;
}
