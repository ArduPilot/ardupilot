...
{
    const AP_AuxFunc::SwitchPos throttle = rc().get_channel_pos(AP::rcmap()->throttle());
    const AP_AuxFunc::SwitchPos yaw = rc().get_channel_pos(AP::rcmap()->yaw());
    const AP_AuxFunc::SwitchPos roll = rc().get_channel_pos(AP::rcmap()->roll());
    const AP_AuxFunc::SwitchPos pitch = rc().get_channel_pos(AP::rcmap()->pitch());

    Event result = Event::NONE;

    if (_button_pressed != ButtonState::NONE) {
        if (_button_pressed == ButtonState::PRESSED && yaw == AP_AuxFunc::SwitchPos::MIDDLE && pitch == AP_AuxFunc::SwitchPos::MIDDLE && roll == AP_AuxFunc::SwitchPos::MIDDLE) {
            result = Event::BUTTON_RELEASE;
        } else {
            result = Event::NONE; // still waiting to be released
        }
    } else if (throttle == AP_AuxFunc::SwitchPos::MIDDLE && yaw == AP_AuxFunc::SwitchPos::LOW
        && pitch == AP_AuxFunc::SwitchPos::MIDDLE && roll == AP_AuxFunc::SwitchPos::MIDDLE
    ...
    } else if (throttle == AP_AuxFunc::SwitchPos::MIDDLE && yaw == AP_AuxFunc::SwitchPos::HIGH
        && pitch == AP_AuxFunc::SwitchPos::MIDDLE && roll == AP_AuxFunc::SwitchPos::MIDDLE
        && (_cam_control_option & uint8_t(ControlOption::STICK_YAW_RIGHT) || is_arming_prevented())) {
        result = Event::ENTER_MENU;
    } else if (roll == AP_AuxFunc::SwitchPos::LOW) {
        result = Event::IN_MENU_EXIT;
    } else if (yaw == AP_AuxFunc::SwitchPos::MIDDLE && pitch == AP_AuxFunc::SwitchPos::MIDDLE && roll == AP_AuxFunc::SwitchPos::HIGH) {
        if (has_5_key_OSD()) {
            result = Event::IN_MENU_RIGHT;
        } else {
            result = Event::IN_MENU_ENTER;
        }
    } else if (pitch == AP_AuxFunc::SwitchPos::LOW) {
        result = Event::IN_MENU_UP;
    } else if (pitch == AP_AuxFunc::SwitchPos::HIGH) {
