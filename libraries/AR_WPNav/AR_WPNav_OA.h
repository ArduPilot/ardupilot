...
public:
    ...
    bool set_desired_location(const Location &destination, Location next_destination = Location(), Location intermediate_origin = Location()) override WARN_IF_UNUSED;
...
private:
    ...
    Location _next_destination_oabak; // backup of _desitnation so it can be restored when oa completes
