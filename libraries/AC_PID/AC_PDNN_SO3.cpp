...
//设置默认RBF网络中心矩阵 Setting the centers of RBF c=Matrix5*2
    ...
    _pdnn_output.x = 0.05f * (-_e_R.x * 20.0f - _e_Omega.x * 16.0f - 0.0f * _integrator.x - _geomrtry_output.x - 0.0f *_phi_x + 0.0f*Aug.x); 
    _pdnn_output.y = 0.05f * (-_e_R.y * 20.0f - _e_Omega.y * 16.0f - 0.0f *_integrator.y - _geomrtry_output.y- 0.0f * _phi_y + 0.0f*Aug.y);
