...
void AC_PID_OscillationDetector::update()
{
    ...
    // Caulate the filtered envelope of the difference between the filter value and actual
    target_env = target_env * (1-w_env) + fabsf(pid.target - target_filt) * w_env;
    actual_env = actual_env * (1-w_env) + fabsf(pid.actual - actual_filt) * w_env;
