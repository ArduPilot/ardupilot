...
void AP_LandingGear::init()
{
    if (_pin_deployed != -1) {
        ...
        if (!DataFlash_Class::instance()->should_log(MASK_LOG_CTUN)) {
            DataFlash_Class::instance()->Log_Write_LandingGear(gear_state_current, wow_state_current);
        }
    }
...
void AP_LandingGear::update()
{
    if (_pin_weight_on_wheels == -1) {
        ...
        if (wow_state_new != wow_state_current) {
            ...
            if (!DataFlash_Class::instance()->should_log(MASK_LOG_CTUN)) {
                DataFlash_Class::instance()->Log_Write_LandingGear(gear_state_current, wow_state_new);
            }
        }
    ...
    if (_pin_deployed == -1) {
        ...
        if (gear_state_new != gear_state_current) {
            ...
            if (!DataFlash_Class::instance()->should_log(MASK_LOG_CTUN)) {
                DataFlash_Class::instance()->Log_Write_LandingGear(gear_state_new, wow_state_current);
            }
        }
