...
{
    if (is_device_not_ready() || beacon_instance >= num_beacons) {
        return false;
    }
 ...
 *
 * @retval true  device not ready
 * @retval false device ready
 */
bool AP_Beacon::is_device_not_ready(void) const
{
    return nullptr == _driver || AP_BeaconType_None == _type;
}
