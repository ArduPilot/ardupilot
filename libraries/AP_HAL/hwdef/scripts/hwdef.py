...
class HWDef:
    ...
    def __init__(self, quiet=False, outdir=None, hwdef: list | None = None):
        ...
        # value INS_MAX_INSTANCES is defaulted to from the number of
        # IMUs; None if no default is written, in which case any value
        # comes from the hwdef itself
        self.ins_max_instances_default = None
    ...
    def write_IMU_config(self, f):
        ...
        if len(devlist) > 0:
            if len(devlist) < 3:
                ...
                self.ins_max_instances_default = len(devlist)
