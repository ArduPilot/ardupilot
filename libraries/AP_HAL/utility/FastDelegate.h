...
inline OutputClass horrible_cast(const InputClass input){
	...
	typedef int ERROR_CantUseHorrible_cast[sizeof(InputClass)==sizeof(u) 
                                               && sizeof(InputClass)==sizeof(OutputClass) ? 1 : -1] __attribute((unused));
	u.in = input;
	return u.out;
}
...
struct SimplifyMemFunc {
	...
	inline static GenericClass *Convert(X *pthis, XFuncType function_to_bind, 
		...
		typedef char ERROR_Unsupported_member_function_pointer_on_this_compiler[N-100] __attribute((unused));
		return 0; 
	}
};
...
//		m_pthis = reinterpret_cast<GenericClass *>(reinterpret_cast<long>(function_to_bind));
	...
	inline UnvoidStaticFuncPtr GetStaticFunction() const {
		...
		// and data pointers have different sizes.
		// If you get this error, you need to #undef FASTDELEGATE_USESTATICFUNCTIONHACK.
		typedef int ERROR_CantUseEvilMethod[sizeof(UnvoidStaticFuncPtr)==sizeof(this) ? 1 : -1] __attribute((unused));
...
//N=1
template<class RetType=detail::DefaultVoid, class... Params>
class FastDelegate0 {
private:
...
public:
	...
	FastDelegate0(Y *pthis, DesiredRetType (X::* function_to_bind)(Params...) ) {
	...
	template < class X, class Y >
	FastDelegate0(const Y *pthis, DesiredRetType (X::* function_to_bind)(Params...) const) {
	...
	FastDelegate0(DesiredRetType (*function_to_bind)(Params...) ) {
		bind(function_to_bind);	}
	// for efficiency, prevent creation of a temporary
	void operator = (DesiredRetType (*function_to_bind)(Params...) ) {
		bind(function_to_bind);	}
...
class FastDelegate< R ( Params... ) >
  ...
  : public FastDelegate0 < R, Params... >
{
public:
  // Make using the base type a bit easier via typedef.
  typedef FastDelegate0 < R, Params... > BaseType;
  ...
  FastDelegate(Y * pthis,
    R (X::* function_to_bind)( Params... ))
    : BaseType(pthis, function_to_bind)  { }

  template < class X, class Y >
  FastDelegate(const Y *pthis,
      R (X::* function_to_bind)( Params... ) const)
    : BaseType(pthis, function_to_bind)
  {  }

  FastDelegate(R (*function_to_bind)( Params... ))
...
FastDelegate0<FASTDLGT_RETTYPE, Params...> MakeDelegate(Y* x, RetType (X::*func)(Params...)) {
	return FastDelegate0<FASTDLGT_RETTYPE, Params...>(x, func);
}

template <class X, class Y, class RetType, class... Params>
FastDelegate0<FASTDLGT_RETTYPE, Params...> MakeDelegate(Y* x, RetType (X::*func)(Params...) const) {
	return FastDelegate0<FASTDLGT_RETTYPE, Params...>(x, func);
}
