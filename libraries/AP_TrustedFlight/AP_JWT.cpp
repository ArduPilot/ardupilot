...
AP_JWT::TokenValidationResult AP_JWT::validate(const uint8_t *new_token, uint32_t new_token_length, const uint8_t *public_key, const uint8_t *issuer, uint32_t issuer_length)
{
    ...
    // validate signature
    if (decoded_signature_len != ed25519_signature_length ||
        crypto_check(decoded_signature, public_key, token, token_length_without_sign) != 0) {
...
// exact (non-prefix) comparison of a claim string
bool AP_JWT::claim_equals(const char *value, const void *expected, size_t expected_length)
{
    return value != nullptr && strlen(value) == expected_length && memcmp(value, expected, expected_length) == 0;
}
...
bool AP_JWT::validate_type()
{
    ...
    return status == claim_status::valid ? claim_equals(json_getValue(prop), "JWT", 3) : false;
}
...
bool AP_JWT::validate_algorithm()
{
    ...
    // private alg value: EdDSA with curve25519 + Blake2b, not standard RFC 8037 EdDSA
    return status == claim_status::valid ? claim_equals(json_getValue(prop), "EdDSA-Blake2b", 13) : false;
}
...
bool AP_JWT::validate_issuer(const uint8_t *issuer, uint32_t issuer_length)
{
    ...
    return status == claim_status::valid ? claim_equals(json_getValue(prop), issuer, issuer_length) : false;
}
...
{
    // decoded buffers are malloc()ed by base64url_decode()
    free(header_section.decoded_buf);
    delete[] header_section.json_pool;
    free(payload_section.decoded_buf);
...
const uint8_t *AP_JWT::parse_json(const uint8_t *section_begin, JsonSection &section)
{
    ...
    for (uint16_t i = 0; i < current_len; ++i) {
        const uint8_t c = section.decoded_buf[i];
        if (c == '{' || c == '[' || c == ':' || c == ',') {
            pool_size++;
        }
    }
...
const uint8_t *AP_JWT::decode_section(const uint8_t *section_begin, uint8_t *&out, uint16_t &out_len, bool is_sig)
{
    ...
    const uint8_t *section_end = (next == nullptr) ? token + token_length : next;
    if (section_end < section_begin) {
        return nullptr;
    }
    const size_t section_len = section_end - section_begin;
    // base64url_decode takes a 16-bit length; refuse rather than silently truncate
    if (section_len == 0 || section_len > UINT16_MAX) {
        return nullptr;
    }
    out = base64url_decode(current, (uint16_t)section_len, &out_len);
