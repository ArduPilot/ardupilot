...
const AP_Param::GroupInfo AC_INDI_Control::var_info[] = {
    ...
    // AP_GROUPINFO("_ARM_LEN_X",                      15, AC_INDI_Control, arm_length_x, 1.0),
    // AP_GROUPINFO("_ARM_LEN_Y",                      16, AC_INDI_Control, arm_length_y, 1.0),
    ...
    AP_GROUPINFO("_RPM_FILTER",                     20, AC_INDI_Control, _rpm_filter_cutoff, 3000.0f),
...
void AC_INDI_Control::update_pos_vel(void)
{
    calculate_torque_thrust_est();

    run_pos_vel_controller();
    indi_linear_accel();
    spec_thrust_to_total_thrust();
    input_acc_des_euler_angle_yaw();
}

void AC_INDI_Control::update_att_ang_vel(void)
{
    run_attitude_angvel_controller();
    indi_angular_accel();
    calculate_mixer_input();
    write_log();
}

void AC_INDI_Control::update(void)
{
...
// run position and velocity controller in xyz 
void AC_INDI_Control::run_pos_vel_controller(void)
{
    Vector3f error_pos, error_vel;
    error_pos = _pos_target_neu_m - _inav.get_position()*0.01f;
    error_vel = _vel_target_neu_mps - _inav.get_velocity()*0.01f;

    _lin_acc_target_ned_mpss.x = _p_pos_xy.get_p(error_pos.x) +  _p_vel_xy.get_p(error_vel.x);
    _lin_acc_target_ned_mpss.y = _p_pos_xy.get_p(error_pos.y) +  _p_vel_xy.get_p(error_vel.y);
    _lin_acc_target_ned_mpss.z = -(_p_pos_z.get_p(error_pos.z) +  _p_vel_z.get_p(error_vel.z));
    
    _lin_acc_target_ned_mpss += _lin_acc_desired_ned_mpss;
}
...
*/
void AC_INDI_Control::indi_linear_accel(void)
{
    Vector3f acc_flt;
    ...
    limit_specific_thrust(_spec_thrust_cmd_ned_mpss);
}


// limit specific thrust command 
void AC_INDI_Control::limit_specific_thrust(Vector3f &spec_thrust_cmd_ned_mpss)
{
    ...
    _spec_thrust_cmd_ned_mpss.z = MIN(_spec_thrust_cmd_ned_mpss.z, -GRAVITY_MSS * 0.1f);

    // do not accelerate more than z axis or 1 g
    float scale_xy = MIN(GRAVITY_MSS, -_spec_thrust_cmd_ned_mpss.z);

    // scale xy axis command to maximum limit
    float sp_th_xy_len = norm(_spec_thrust_cmd_ned_mpss.x, _spec_thrust_cmd_ned_mpss.y);
    if ((sp_th_xy_len > scale_xy) && is_positive(sp_th_xy_len)) {
        _spec_thrust_cmd_ned_mpss.x *= (scale_xy / sp_th_xy_len);
        _spec_thrust_cmd_ned_mpss.y *= (scale_xy / sp_th_xy_len);
    }
}

// convert specific thrust command to the total thrust 
void AC_INDI_Control::spec_thrust_to_total_thrust(void)
{
...
*/
void AC_INDI_Control::input_acc_des_euler_angle_yaw(void)
{
    Vector3f e_cur_z, e_des_x, e_des_y, e_des_z, e_b_y, temp, _rot_vector;
    
    // construct a vector that lies in xy axis of inertial frame
    e_b_y = Vector3f(-sinf(_att_target_euler_angle_yaw_rad), cosf(_att_target_euler_angle_yaw_rad), 0);
    ...
    _att_target_quat.from_rotation_matrix(Matrix3f(e_des_x, e_des_y, e_des_z).transposed());
    _att_target_quat.normalize();
}
...
*/
Vector3f AC_INDI_Control::calculate_att_error(void)
{  
    Vector3f e_cur_z, e_des_z;

    Quaternion _att_cur_quat;
    _ahrs.get_quat_body_to_ned(_att_cur_quat);    
    e_cur_z = _ahrs.get_rotation_body_to_ned().colz();
    ...
    thrust_vec_correction_quat = _att_cur_quat.inverse() * thrust_vec_correction_quat * _att_cur_quat;

    // calculate the remaining rotation required after thrust vector is rotated transformed to the body frame
    Quaternion yaw_vec_correction_quat = thrust_vec_correction_quat.inverse() * _att_cur_quat.inverse() * _att_target_quat;    
...
// run attitude and angular controller
void AC_INDI_Control::run_attitude_angvel_controller(void)
{   
    Vector3f error_att, error_ang_vel;
    error_att = calculate_att_error();

    error_ang_vel = _ang_vel_target_radps - _ahrs.get_gyro_latest();

    _ang_acc_target_radpss.x = _p_angle_x.get_p(error_att.x) + _p_ang_rate_x.get_p(error_ang_vel.x);
    _ang_acc_target_radpss.y = _p_angle_y.get_p(error_att.y) + _p_ang_rate_y.get_p(error_ang_vel.y);
    _ang_acc_target_radpss.z = _p_angle_z.get_p(error_att.z) + _p_ang_rate_z.get_p(error_ang_vel.z);

    _ang_acc_target_radpss += _ang_acc_desired_radpss;
}
...
void AC_INDI_Control::calculate_mixer_input(void)
{
    float total_thrust_max;
    total_thrust_max = sq(_throttle2motor_speed) * _thrust_coefficient * 4.0f;

    _torque_cmd_scaled = _torque_cmd_body_Nm; // torque_max;
