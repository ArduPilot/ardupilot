...
TEST(COBSTest, StreamingDecoderBasic)
{
    ...
    uint8_t decoder_buf[64];
    decoder.set_buffer(decoder_buf, sizeof(decoder_buf));
...
TEST(COBSTest, AllFrameSizes)
{
    ...
    for (size_t size_idx = 0; size_idx < sizeof(edge_case_sizes)/sizeof(edge_case_sizes[0]); size_idx++) {
        ...
        // Test streaming decoder only for sizes within streaming buffer limit
...
TEST(COBSTest, LongStreamRandomFrames)
{
    ...
    AP_Networking_COBS::Decoder decoder;
    uint8_t *decoder_buf = (uint8_t*)malloc(max_frame_size);
    ASSERT_NE(decoder_buf, nullptr);
    decoder.set_buffer(decoder_buf, max_frame_size);
    size_t stream_pos = 0;
...
TEST(COBSTest, SequentialFrames)
{
    ...
    AP_Networking_COBS::Decoder decoder;
    uint8_t *decoder_buf = (uint8_t*)malloc(frame_size);
    ASSERT_NE(decoder_buf, nullptr);
    decoder.set_buffer(decoder_buf, frame_size);
