...
#if AP_NETWORKING_BACKEND_SWITCHPORT_COBS || AP_NETWORKING_BACKEND_SWITCHPORT_MAVLINK_COBS
...
/*
  ...
  - Data (bonded mode): seq[2] + ethernet_frame + CRC32(data + "BOND")
...
namespace AP_Networking_COBS_Protocol
{
    ...
    // CRC suffix for bonded mode (distinguishes from single mode)
    static constexpr uint8_t BOND_CRC_SUFFIX[4] = {'B', 'O', 'N', 'D'};
                                     ...
                                     FrameType hint = FrameType::DATA_SINGLE)
    {
        ...
        if (data_len >= MIN_ETH_FRAME) {
            ...
            if (hint == FrameType::DATA_SINGLE) {
                ...
                // Not single, try bonded
                uint32_t bond_crc = crc_crc32(single_crc, BOND_CRC_SUFFIX, 4);
                if (rx_crc == bond_crc && data_len >= 2 + MIN_ETH_FRAME) {
                    *payload_out = &data[2];
                    *payload_len_out = data_len - 2;
                    return FrameType::DATA_BONDED;
                }
            } else {
                // Try bonded first (hint == DATA_BONDED)
                uint32_t bond_crc = crc_crc32(single_crc, BOND_CRC_SUFFIX, 4);
                if (rx_crc == bond_crc && data_len >= 2 + MIN_ETH_FRAME) {
                    *payload_out = &data[2];
                    *payload_len_out = data_len - 2;
    ...
    // Extract bonded frame sequence number
    inline uint16_t get_bonded_seq(const uint8_t *data)
    {
...
#endif // AP_NETWORKING_BACKEND_SWITCHPORT_COBS || AP_NETWORKING_BACKEND_SWITCHPORT_MAVLINK_COBS
