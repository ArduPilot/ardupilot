...
const AP_Param::GroupInfo AP_Volz_Protocol::var_info[] = {
    ...
    // @Param: ENABLED
    // @DisplayName: Enable of volz servo protocol
    // @Description: outputs volz servo protocol to serial out.
    // @Values: 0:Disabled,1:Enabled
    // @User: Standard
    AP_GROUPINFO("ENABLED", 0, AP_Volz_Protocol, enabled, 0),
	...
	// @Values: 0,1....
    // @User: Standard
    AP_GROUPINFO("BITMASK", 1, AP_Volz_Protocol, chan_bitmask,  0),
	...
	// @Values: 0,1....
    // @User: Standard
    AP_GROUPINFO("MIN_PWM", 2, AP_Volz_Protocol, min_pwm,  900),
	...
	// @Values: 0,1....
    // @User: Standard
    AP_GROUPINFO("MAX_PWM", 3, AP_Volz_Protocol, max_pwm,  2100),
...
//	gcs().send_text(MAV_SEVERITY_INFO, "Volz time:%u us, %u", AP_HAL::micros64() - tnow, sent );
}

void AP_Volz_Protocol::send_command(uint8_t data[4]){
	uint8_t cmd[6];
	uint8_t i,j;
	uint16_t crc = 0xFFFF;
