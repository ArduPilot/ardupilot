...
#define SAFERTL_SIMPLIFICATION_EPSILON (SAFERTL_POSITION_DELTA * 0.5)
#define SAFERTL_MAX_PATH_LEN 110 // the amount of memory used by safe RTL will be slightly higher than 3*8*MAX_PATH_LEN bytes. Increasing this number will improve path pruning, but will use more memory, and running a path cleanup will take longer. No longer than 255.
#define SAFERTL_SIMPLIFICATION_STACK_LEN 64 // the amount of memory to be allocated for the SIMPLIFICATION algorithm to write its to do list.
...
#define SAFERTL_LOOP_BUFFER_LEN 20
...
private:
    ...
    AP_Buffer<start_finish,SAFERTL_SIMPLIFICATION_STACK_LEN> _simplification_stack;
    ...
    loop _prunable_loops[SAFERTL_LOOP_BUFFER_LEN];
