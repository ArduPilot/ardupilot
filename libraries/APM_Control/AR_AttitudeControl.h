...
public:
    ...
    // return value is normally in range -1.0 to +1.0 but can be higher or lower
    float get_steering_out_heading(float heading_rad, float rate_max_rads, bool motor_limit_left, bool motor_limit_right, float dt, bool directional = false);
    ...
    float get_turn_rate_from_heading(float heading_rad, float rate_max_rads, bool directional = false) const;
