...
float AC_AttitudeControl_Multi::get_althold_lean_angle_max() const
{
    ...
    float val = acos(constrain_value<float>(_throttle_in_filt.get()/(0.9f * thr_max), 0.0f, 1.0f));
...
float AC_AttitudeControl_Multi::get_boosted_throttle(float throttle_in)
{
    ...
    float inverted_factor = constrain_value<float>(2.0f*cos_tilt, 0.0f, 1.0f);
    float boost_factor = 1.0f/constrain_value<float>(cos_tilt, 0.5f, 1.0f);

    float throttle_out = throttle_in*inverted_factor*boost_factor;
    _angle_boost = constrain_value<float>(throttle_out - throttle_in,-1.0f,1.0f);
    return throttle_out;
}
