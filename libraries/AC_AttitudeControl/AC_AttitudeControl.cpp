...
// Command an angular step
// Used to command a step in angle without exciting the orthogonal axis during autotune
void AC_AttitudeControl::input_step_bf_roll_pitch_yaw(float roll_step_bf_cds, float pitch_step_bf_cds, float yaw_step_bf_cds)
{
    // Convert from centidegrees on public interface to radians
    float roll_step_rads = radians(roll_step_bf_cds*0.01f);
    float pitch_step_rads = radians(pitch_step_bf_cds*0.01f);
    float yaw_step_rads = radians(yaw_step_bf_cds*0.01f);
