...
void AC_PosControl::input_pos_NEU_m(const Vector3p& pos_neu_m, float pos_terrain_target_u_m, float terrain_buffer_m)
{
    ...
    update_pos_vel_accel_xy(_pos_desired_neu_m.xy(), _vel_desired_neu_ms.xy(), _accel_desired_neu_mss.xy(), _dt_s, _limit_vector_neu.xy(), _p_pos_ne.get_error(), _pid_vel_ne.get_error());

    // adjust desired altitude if motors have not hit their limits
    update_pos_vel_accel(_pos_desired_neu_m.z, _vel_desired_neu_ms.z, _accel_desired_neu_mss.z, _dt_s, _limit_vector_neu.z, _p_pos_u.get_error(), _pid_vel_u.get_error());
...
{
    _p_pos_ne.set_limits(speed_ne_ms, accel_ne_mss, 0.0f);
}
...
void AC_PosControl::init_NE_controller()
{
    ...
    _pid_vel_ne.set_integrator((_accel_target_neu_mss.xy() - _vel_target_neu_ms.xy() * _pid_vel_ne.ff()) * 100.0);
...
void AC_PosControl::update_offsets_NE()
{
    ...
    update_pos_vel_accel_xy(_pos_offset_neu_m.xy(), _vel_offset_neu_ms.xy(), _accel_offset_neu_mss.xy(), _dt_s, _limit_vector_neu.xy(), _p_pos_ne.get_error(), _pid_vel_ne.get_error());
...
void AC_PosControl::update_NE_controller()
{
    ...
    Vector2f vel_target_ne_ms = _p_pos_ne.update_all(_pos_target_neu_m.xy(), comb_pos_ne_m);
    _pos_desired_neu_m.xy() = _pos_target_neu_m.xy() - _pos_offset_neu_m.xy();
    ...
    Vector2f accel_target_ne_mss = _pid_vel_ne.update_all(_vel_target_neu_ms.xy() * 100.0, comb_vel_ne_ms * 100.0, _dt_s, _limit_vector_neu.xy()) * 0.01;
...
void AC_PosControl::set_max_speed_accel_U_m(float speed_down_ms, float speed_up_ms, float accel_mss)
{
    ...
    if (is_positive(_pid_accel_u.filt_T_hz())) {
        _jerk_max_u_msss = MIN(_jerk_max_u_msss, MIN(GRAVITY_MSS, _accel_max_u_mss) * (M_2PI * _pid_accel_u.filt_T_hz()) / 5.0);
    }
    if (is_positive(_pid_accel_u.filt_E_hz())) {
        _jerk_max_u_msss = MIN(_jerk_max_u_msss, MIN(GRAVITY_MSS, _accel_max_u_mss) * (M_2PI * _pid_accel_u.filt_E_hz()) / 5.0);
    }
}
...
void AC_PosControl::set_correction_speed_accel_U_mss(float speed_down_ms, float speed_up_ms, float accel_mss)
{
    ...
    _p_pos_u.set_limits(-fabsf(speed_down_ms), speed_up_ms, accel_mss, 0.0f);
}
...
void AC_PosControl::relax_U_controller(float throttle_setting)
{
    ...
    _pid_accel_u.relax_integrator((throttle_setting - _motors.get_throttle_hover()) * 10.0 * 100.0, _dt_s, POSCONTROL_RELAX_TC);
}
...
void AC_PosControl::init_U_controller()
{
    ...
    _pid_accel_u.set_integrator((_attitude_control.get_throttle_in() - _motors.get_throttle_hover()) * 10.0 * 100.0
        - _pid_accel_u.kP() * (_accel_target_neu_mss.z - get_measured_accel_U_mss()) * 100.0
        - _pid_accel_u.ff() * _accel_target_neu_mss.z * 100.0);
...
void AC_PosControl::update_offsets_U()
{
    ...
    update_pos_vel_accel(p_offset_u_m, _vel_offset_neu_ms.z, _accel_offset_neu_mss.z, _dt_s, MIN(_limit_vector_neu.z, 0.0f), _p_pos_u.get_error(), _pid_vel_u.get_error());
...
void AC_PosControl::update_U_controller()
{
    ...
    _vel_target_neu_ms.z = _p_pos_u.update_all(pos_target_zf, _pos_estimate_neu_m.z);
    ...
    _accel_target_neu_mss.z = _pid_vel_u.update_all(_vel_target_neu_ms.z, _vel_estimate_neu_ms.z, _dt_s, _motors.limit.throttle_lower, _motors.limit.throttle_upper);
    ...
    if (_motors.get_throttle_hover() * 10.0 * 100.0 > _pid_accel_u.imax()) {
        _pid_accel_u.set_imax(_motors.get_throttle_hover() * 10.0 * 100.0);
    }
    float thr_out;
    if (_vibe_comp_enabled) {
        ...
        thr_out = _pid_accel_u.update_all(_accel_target_neu_mss.z * 100.0, measured_accel_u_mss * 100.0, _dt_s, (_motors.limit.throttle_lower || _motors.limit.throttle_upper)) * 0.1 * 0.01;
        // Include FF contribution to reduce delay
        thr_out += _pid_accel_u.get_ff() * 0.1 * 0.01;
    }
    ...
    float error_ratio = _pid_vel_u.get_error() / _vel_max_down_ms;
...
// Used in LUA
bool AC_PosControl::get_posvelaccel_offset(Vector3f &pos_offset_NED_m, Vector3f &vel_offset_NED_m, Vector3f &accel_offset_NED_m)
{
    ...
    vel_offset_NED_m.xy() = _vel_offset_target_neu_ms.xy();
    vel_offset_NED_m.z = -_vel_offset_target_neu_ms.z;

    accel_offset_NED_m.xy() = _accel_offset_target_neu_mss.xy();
    accel_offset_NED_m.z = -_accel_offset_target_neu_mss.z;
    return true;
}
...
bool AC_PosControl::get_accel_target(Vector3f &accel_target_NED)
{
    ...
    // Convert NEU → NED by inverting Z
    accel_target_NED.xy() = _accel_target_neu_mss.xy();
    accel_target_NED.z = -_accel_target_neu_mss.z;
    return true;
}
#endif
...
void AC_PosControl::get_stopping_point_U_m(postype_t &stopping_point_u_m) const
{
    ...
    if (!is_positive(_p_pos_u.kP()) || !is_positive(_accel_max_u_mss)) {
        stopping_point_u_m = curr_pos_u_m;
        return;
    }

    // Estimate stopping point using current velocity, P gain, and max vertical acceleration
    stopping_point_u_m = curr_pos_u_m + constrain_float(stopping_distance(curr_vel_u_ms, _p_pos_u.kP(), _accel_max_u_mss), - POSCONTROL_STOPPING_DIST_DOWN_MAX_M, POSCONTROL_STOPPING_DIST_UP_MAX_M);
}
...
float AC_PosControl::get_throttle_with_vibration_override()
{
    ...
    if (!(_motors.limit.throttle_lower || _motors.limit.throttle_upper) || ((is_positive(_pid_accel_u.get_i()) && is_negative(_pid_vel_u.get_error())) || (is_negative(_pid_accel_u.get_i()) && is_positive(_pid_vel_u.get_error())))) {
        ...
        _pid_accel_u.set_integrator(_pid_accel_u.get_i() + _dt_s * thr_per_accel_u_mss * 10.0 * 100.0 * _pid_vel_u.get_error() * _pid_vel_u.kP() * POSCONTROL_VIBE_COMP_I_GAIN);
    }
    // Final throttle = P term (feedforward) + scaled I term.
    return POSCONTROL_VIBE_COMP_P_GAIN * thr_per_accel_u_mss * _accel_target_neu_mss.z + _pid_accel_u.get_i() * 0.1 * 0.01;
}
...
void AC_PosControl::update_terrain()
{
    ...
    update_pos_vel_accel(pos_terrain_u_m, _vel_terrain_u_ms, _accel_terrain_u_mss, _dt_s, MIN(_limit_vector_neu.z, 0.0f), _p_pos_u.get_error(), _pid_vel_u.get_error());
    _pos_terrain_u_m = pos_terrain_u_m;
...
void AC_PosControl::handle_ekf_NE_reset()
{
    ...
    if (reset_ms != _ekf_ne_reset_ms) {
        ...
        _pos_target_neu_m.xy() = _pos_estimate_neu_m.xy() + _p_pos_ne.get_error().topostype();
        _pos_desired_neu_m.xy() = _pos_target_neu_m.xy() - _pos_offset_neu_m.xy();

        // Reconstruct velocity targets based on PID error and velocity estimate
        _vel_target_neu_ms.xy() = _vel_estimate_neu_ms.xy() + _pid_vel_ne.get_error() * 0.01;
        _vel_desired_neu_ms.xy() = _vel_target_neu_ms.xy() - _vel_offset_neu_ms.xy();
...
void AC_PosControl::handle_ekf_U_reset()
{
    ...
    if (reset_ms != 0 && reset_ms != _ekf_u_reset_ms) {
        ...
        _pos_target_neu_m.z = _pos_estimate_neu_m.z + _p_pos_u.get_error();
        _pos_desired_neu_m.z = _pos_target_neu_m.z - (_pos_offset_neu_m.z + _pos_terrain_u_m);

        // Reconstruct vertical velocity targets from measured velocity + PID error
        _vel_target_neu_ms.z = _vel_estimate_neu_ms.z + _pid_vel_u.get_error();
