...
bool AC_Fence::check_fence_polygon()
{
    ...
    if (have_location && _poly_loader.breached(loc, _polygon_breach_distance, _polygon_breach_direction)) {
        if (!was_breached) {
...
Vector3f AC_Fence::get_breach_direction_NED(uint8_t fence_type) const
{
    ...
    if (fence_type & AC_FENCE_TYPE_POLYGON && fabsf(_polygon_breach_distance) < closest) {
        direction_to_closest = Vector3f(_polygon_breach_direction.x, _polygon_breach_direction.y, 0);
