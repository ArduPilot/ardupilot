...
bool AC_Fence::conv_alt_frame(Location::AltFrame desired_frame, float &alt_max_in_frame, float &alt_min_in_frame)
{
    int32_t new_max_alt_cm, new_min_alt_cm;
    if (fenceloc.get_spec_alt_cm(Location::AltFrame::ABOVE_HOME, desired_frame, _alt_max * 100.0f, new_max_alt_cm)) {
        alt_max_in_frame = new_max_alt_cm * 0.01;
    } else {
        gcs().send_text(MAV_SEVERITY_NOTICE, "Max alt frame change not accepted, current frame is..."); // Finish, return false
        return false;
    }
    
    if (fenceloc.get_spec_alt_cm(Location::AltFrame::ABOVE_HOME, desired_frame, _alt_min * 100.0f, new_min_alt_cm)) {
        alt_min_in_frame = new_min_alt_cm * 0.01;
    } else {
    ...
    if (conv_alt_frame(Location::AltFrame::ABSOLUTE, test_alt_max, test_alt_min)) {
        ::fprintf(stderr, "[ABSOLUTE] test_alt_max: %f test_alt_min: %f\n", test_alt_max, test_alt_min);
    }
}
