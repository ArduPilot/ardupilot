...
void AP_GPS_UAVCAN::subscribe_gps_uavcan_messages(AP_UAVCAN* ap_uavcan)
{
...
AP_GPS_Backend* AP_GPS_UAVCAN::probe(AP_GPS &_gps, AP_GPS::GPS_State &_state)
{
    ...
    for (uint8_t i = 0; i < GPS_MAX_RECEIVERS; i++) {
        if (_detected_modules[i].driver == nullptr && _detected_modules[i].ap_uavcan != nullptr) {
            ...
            if (backend == nullptr) {
                debug_gps_uavcan(2, _detected_modules[i].ap_uavcan->get_driver_index(), "Failed to register UAVCAN GPS Node %d on Bus %d\n", _detected_modules[i].node_id,
                _detected_modules[i].ap_uavcan->get_driver_index());
            } else {
                _detected_modules[i].driver = backend;
                backend->_detected_module = i;
                debug_gps_uavcan(2, _detected_modules[i].ap_uavcan->get_driver_index(), "Registered UAVCAN GPS Node %d on Bus %d\n", _detected_modules[i].node_id, _detected_modules[i].ap_uavcan->get_driver_index());
            }
            break;
        }
    }
