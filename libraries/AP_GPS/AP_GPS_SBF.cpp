...
void
AP_GPS_SBF::log_ExtEventPVTGeodetic(msg4007* temp)
{
...
AP_GPS_SBF::process_message(void)
{
    ...
    // ExtEventPVTGeodetic
    if (blockid == 4038) {
        log_ExtEventPVTGeodetic((msg4007*)&sbf_msg.data.msg4007u);
    }
    // PVTGeodetic
    if (blockid == 4007) {
        msg4007* temp = (msg4007*)&sbf_msg.data.msg4007u;
    ...
    // DOP
    if (blockid == 4001) {
        msg4001* temp = (msg4001*)&sbf_msg.data.msg4001u;
...
AP_GPS_SBF::_detect(struct SBF_detect_state &state, uint8_t temp)
{
    switch (state.sbf_state) {
        ...
        case SBF_detect_state::LENGTH2:
            ...
            state.crcbuild = crc16_ccitt((uint8_t*)&state.blockid, 2, 0);
            state.crcbuild = crc16_ccitt((uint8_t*)&state.length, 2, state.crcbuild);
            if (state.length % 4 != 0)
                state.sbf_state = SBF_detect_state::PREAMBLE1;
            break;
        case SBF_detect_state::DATA:
            state.crcbuild = crc16_ccitt((uint8_t*)&temp, 1, state.crcbuild);
            state.read++;
            if (state.read > (state.length - 8)) {
                state.sbf_state = SBF_detect_state::PREAMBLE1;

                if (state.crc == state.crcbuild) {
                    return true;
                }
            }
            break;
