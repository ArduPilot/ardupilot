...
#define CFG_NEED(KEY_CLASS, KEY, TYPE, VAL) \
    (_processing_cfg.KEY##_val != (TYPE)VAL) ||
...
bool AP_GPS_UBLOX_CFGv2::is_common_cfg_needed()
{
    ...
    if (module <= Module::F10) {
        ...
        need = UBX_CFG_COMMON_UART(CFG_NEED) false;
    } else {
        // Check constant config values
        switch (ubx_backend._ublox_port) {
        case 2: // UART1
            need = UBX_CFG_COMMON_UART1(CFG_NEED) false;
            break;
        case 3: // UART2
            need = UBX_CFG_COMMON_UART2(CFG_NEED) false;
            break;
        default:
            // should not happen
            break;
...
{
    Debug("GPS: packed push key 0x%lx value 0x%llx", (uint32_t)key, (uint64_t)value);
