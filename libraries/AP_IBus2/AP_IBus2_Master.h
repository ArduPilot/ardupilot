...
/*
  ...
  Multiple master instances are supported; this class is instantiated inside
  AP_IBus2_Masters which owns the per-instance and shared parameters.
*/
...
public:
    ...
    // instance is the 0-based index passed to find_serial(); cycle_us and
    // response_timeout_us are shared timing params owned by AP_IBus2_Masters.
    void init(uint8_t instance, const AP_Int32 &cycle_us, const AP_Int32 &response_timeout_us);
    void update();
...
private:
    ...
    // Shared timing params from AP_IBus2_Masters (valid after init())
    const AP_Int32 *_cycle_us;
    const AP_Int32 *_response_timeout_us;

    // Timestamp of the last master cycle start (µs); replaces static local
    uint32_t _last_cycle_us;
