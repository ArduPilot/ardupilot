...
const AP_Param::GroupInfo AP_IBus2_Master::var_info[] = {
    ...
    // @User: Advanced
    AP_GROUPINFO("SMASK", 2, AP_IBus2_Master, _send_mask, 3),  // default: GET_TYPE + GET_VALUE
...
void AP_IBus2_Master::init(uint8_t instance, const AP_Int32 &cycle_us, const AP_Int32 &response_timeout_us)
{
    _instance_index = instance;
    _cycle_us = &cycle_us;
    _response_timeout_us = &response_timeout_us;

    if (!_enable) {
        return;
    }
    const AP_SerialManager &serial_manager = AP::serialmanager();
    _port = serial_manager.find_serial(AP_SerialManager::SerialProtocol_IBUS2_Master, _instance_index);
    if (_port == nullptr) {
        return;
    }
...
void AP_IBus2_Master::update()
{
    ...
    if (_waiting_response && (now_us - _frame2_sent_us) > (uint32_t)_response_timeout_us->get()) {
    ...
    // Only start a new cycle every _cycle_us microseconds
    if ((now_us - _last_cycle_us) < (uint32_t)_cycle_us->get()) {
        return;
    }
    _last_cycle_us = now_us;
