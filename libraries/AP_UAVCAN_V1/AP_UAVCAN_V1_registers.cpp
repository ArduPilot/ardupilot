...
static RegisterCell registers_table[UavcanRegisters::NUMBER_OF_REGISTERS] = {
    {"uavcan.node.id"},
...
bool UavcanRegisters::init(UavcanSubscriberManager &sub_manager, CanardInstance &ins, CanardTxQueue &tx_queue)
{
    UavcanBaseSubscriber *subsriber;

    subsriber = new UavcanRegisterListRequest(ins, tx_queue, *this);
    sub_manager.add_subscriber(subsriber);

    subsriber = new UavcanRegisterAccessRequest(ins, tx_queue, *this);
    sub_manager.add_subscriber(subsriber);
...
int8_t UavcanRegisters::getRegisterIndexByRegisterName(const uint8_t register_name[], uint8_t name_length)
{
    if (name_length == 0) {
        return -1;
    }
...
uint8_t UavcanRegisters::getRegisterNameByIndex(uint8_t register_index, uint8_t register_name[])
{
...
int16_t UavcanRegisters::getPortIdByIndex(uint8_t param_idx)
{
    return (param_idx < NUMBER_OF_REGISTERS) ? _parameters_table[param_idx].get() : -1;
}

void UavcanRegisters::setPortIdByIndex(uint8_t param_idx, int16_t new_port_id)
{
 ...
 */
void UavcanRegisterAccessRequest::subscribe()
{
    subscribeOnRequest(uavcan_register_Access_Request_1_0_EXTENT_BYTES_);
}

void UavcanRegisterAccessRequest::handler(const CanardRxTransfer* transfer)
{
    auto reg_index = parseRequest(transfer);
    makeResponse(transfer, reg_index);
}

int8_t UavcanRegisterAccessRequest::parseRequest(const CanardRxTransfer* transfer)
{
...
void UavcanRegisterAccessRequest::makeResponse(const CanardRxTransfer* transfer, int8_t reg_index)
{
    ...
    if (reg_index < 0) {
        response_msg.value.natural16.value.elements[0] = UavcanRegisters::UAVCAN_INVALID_REGISTER_VALUE;
    } else {
 ...
 */
void UavcanRegisterListRequest::subscribe()
{
    subscribeOnRequest(uavcan_register_List_Request_1_0_EXTENT_BYTES_);
}

void UavcanRegisterListRequest::handler(const CanardRxTransfer* transfer)
{
    auto index = parseRequest(transfer);
    makeResponse(transfer, index);
}

uint16_t UavcanRegisterListRequest::parseRequest(const CanardRxTransfer* transfer)
{
    auto payload_len = transfer->payload_size;
    const uint8_t* payload = static_cast<const uint8_t*>(transfer->payload);
...
void UavcanRegisterListRequest::makeResponse(const CanardRxTransfer* transfer, uint16_t index)
{
    _transfer_metadata.remote_node_id = transfer->metadata.remote_node_id;
    _transfer_metadata.transfer_id = transfer->metadata.transfer_id;
