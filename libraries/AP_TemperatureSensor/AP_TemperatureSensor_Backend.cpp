...
void AP_TemperatureSensor_Backend::update_external_libraries(const float temperature)
{
    ...
    switch ((AP_TemperatureSensor_Params::Source)_params.source.get()) {
        ...
        case AP_TemperatureSensor_Params::Source::Rangefinder: {
            ...
            if (rangefinder != nullptr && _params.source_id >= 0) {
                // Get the orientation of the rangefinder instance (source_id is 0-based)
                auto *backend = rangefinder->get_backend(_params.source_id);
                if (backend != nullptr) {
                    rangefinder->set_temperature(backend->orientation(), temperature);
                }
            }
            break;
        }
