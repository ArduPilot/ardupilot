...
const AP_Param::GroupInfo AP_TemperatureSensor_Params::var_info[] = {
    ...
    // @Description: Temperature sensor ID, taking into account its bus type, bus, address and sensor type. Zero if the sensor has not been detected. For DroneCAN the address is the node ID, so sensors sharing a node share an ID
    // @ReadOnly: True
    // @User: Advanced
