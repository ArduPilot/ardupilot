...
void AP_AHRS::Write_POS() const
{
    ...
    float home_pos_down;
    if (!AP::ahrs().get_relative_position_D_home(home_pos_down)) {
        home_pos_down = nanf;
    }
    float origin_pos_down;
    if (!get_relative_position_D_origin(origin_pos_down)) {
        origin_pos_down = nanf;
    }
    float alt;
    ...
    const struct log_POS pkt{
        ...
        lng            : loc.lng,
        alt            : alt,
        rel_home_alt   : -home_pos_down,   // down->up
        rel_origin_alt : -origin_pos_down, // down->up
    };
    AP::logger().WriteBlock(&pkt, sizeof(pkt));
}
