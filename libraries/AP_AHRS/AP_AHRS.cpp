...
void AP_AHRS::set_orientation()
{
    ...
    if (orientation != ROTATION_CUSTOM) {
        ...
        float xrad = DEG_TO_RAD * _custom_roll;
        float yrad = DEG_TO_RAD * _custom_pitch;
        float zrad = DEG_TO_RAD * _custom_yaw;
        Matrix3f rotationx = Matrix3f(
                1,          0,           0,
                0, cosf(xrad), -sinf(xrad),
                0, sinf(xrad),  cosf(xrad)
        );
        Matrix3f rotationy = Matrix3f(
                cosf(yrad), 0, sinf(yrad),
                0, 1,          0,
                -sinf(yrad), 0, cosf(yrad)
        );
        Matrix3f rotationz = Matrix3f(
                cosf(zrad), -sinf(zrad), 0,
                sinf(zrad),  cosf(zrad), 0,
                0,           0, 1
        );
        _custom_rotation = rotationx * rotationy * rotationz;
