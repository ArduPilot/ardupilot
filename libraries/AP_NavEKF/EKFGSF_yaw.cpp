/*
  ...
  as a Guassian sum filter where states are velocity N,E (m/s) and yaw angle
  (rad)
...
void EKFGSF_yaw::update(const Vector3F &delAng, const Vector3F &delVel,
                        const ftype delAngDT, const ftype delVelDT, bool runEKF,
                        ftype TAS) {
  ...
  // Calculate a low pass filtered acceleration vector that will be used to keep
  // the AHRS tilt aligned The time constant of the filter is a fixed ratio
  // relative to the time constant of the AHRS tilt correction loop
  const ftype filter_coef =
      fminF(EKFGSF_accelFiltRatio * delVelDT * EKFGSF_tiltGain, 1.0f);
  const Vector3F accel = delVel / fmaxF(delVelDT, 0.001f);
  ahrs_accel = ahrs_accel * (1.0f - filter_coef) + accel * filter_coef;

  // Iniitialise states and only when acceleration is close to 1g to prevent
  // vehicle movement casuing a large initial tilt error
  if (!ahrs_tilt_aligned) {
    ...
    const bool ok_to_align =
        ((accel_norm_sq > lower_accel_limit * lower_accel_limit &&
  ...
  // Calculate AHRS acceleration fusion gain using a quadratic weighting
  // function that is unity at 1g and zero at the min and max g limits. This
  // reduces the effect of large g transients on the attitude esitmates.
  ftype EKFGSF_ahrs_ng = ahrs_accel_norm / GRAVITY_MSS;
  if (EKFGSF_ahrs_ng > 1.0f) {
    if (is_positive(true_airspeed)) {
      // When flying in fixed wing mode we need to allow for more positive g due
      // to coordinated turns Gain varies from unity at 1g to zero at 2g
  ...
  // Calculate a composite yaw as a weighted average of the states for each
  // model. To avoid issues with angle wrapping, the yaw state is converted to a
  // vector with legnth equal to the weighting value before it is summed.
  Vector2F yaw_vector;
  for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++) {
  ...
  // Example for future reference showing how a full GSF covariance matrix could
  ...
  for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++) {
      ...
      for (uint8_t row = 0; row < 3; row++) {
          for (uint8_t col = 0; col < 3; col++) {
              GSF.P[row][col] +=  GSF.weights[mdl_idx] *
  (EKF[mdl_idx].P[row][col] + delta[row] * delta[col]);
          }
      }
  }
  */
  ...
  for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++) {
    ...
    GSF.yaw_variance +=
        GSF.weights[mdl_idx] * (EKF[mdl_idx].P[2][2] + sq(yawDelta));
  }
}

void EKFGSF_yaw::fuseVelData(const Vector2F &vel, const ftype velAcc) {
  // convert reported accuracy to a variance, but limit lower value to protect
  // algorithm stability
  const ftype velObsVar = sq(fmaxF(velAcc, 0.5f));

  // The 3-state EKF models only run when flying to avoid corrupted estimates
  // due to operator handling and GPS interference
  if (run_ekf_gsf) {
    if (!vel_fuse_running) {
      // Perform in-flight alignment
      resetEKFGSF();
      for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++) {
        // Use the firstGPS  measurement to set the velocities and corresponding
        // variances
      ...
      for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++) {
        // Update states and covariances using GPS NE velocity measurements
        // fused as direct state observations
      ...
      if (!state_update_failed) {
        // Calculate weighting for each model assuming a normal error
        // distribution
        ...
        // Reset the filters if all weights have underflowed due to excessive
...
void EKFGSF_yaw::predictAHRS(const uint8_t mdl_idx) {
  // Generate attitude solution using simple complementary filter for the
  // selected model

  // Calculate 'k' unit vector of earth frame rotated into body frame
  const Vector3F k{AHRS[mdl_idx].R[2][0], AHRS[mdl_idx].R[2][1],
                   AHRS[mdl_idx].R[2][2]};

  // Calculate angular rate vector in rad/sec averaged across last sample
  // interval
  const Vector3F ang_rate_delayed_raw{delta_angle / angle_dt};

  // Perform angular rate correction using accel data and reduce correction as
  // accel magnitude moves away from 1 g (reduces drift when vehicle picked up
  // and moved). During fixed wing flight, compensate for centripetal
  // acceleration assuming coordinated turns and X axis forward
  ...
  if (accel_gain > 0.0f) {
    ...
    if (is_positive(true_airspeed)) {
      // Calculate centripetal acceleration in body frame from cross product of
      // body rate and body frame airspeed vector NOTE: this assumes X axis is
      // aligned with airspeed vector
      const Vector3F centripetal_accel_vec_bf{
          0.0f, ang_rate_delayed_raw[2] * true_airspeed,
  ...
  if (spinRate_squared < sq(0.175f)) {
    AHRS[mdl_idx].gyro_bias -=
        tilt_error_gyro_correction * (EKFGSF_gyroBiasGain * angle_dt);
    ...
    for (uint8_t i = 0; i < 3; i++) {
      AHRS[mdl_idx].gyro_bias[i] = constrain_ftype(
          AHRS[mdl_idx].gyro_bias[i], -gyro_bias_limit, gyro_bias_limit);
    }
  }

  // Calculate the corrected body frame rotation vector for the last sample
  // interval and apply to the rotation matrix
  const Vector3F ahrs_delta_angle =
      delta_angle +
      (tilt_error_gyro_correction - AHRS[mdl_idx].gyro_bias) * angle_dt;
  AHRS[mdl_idx].R = updateRotMat(AHRS[mdl_idx].R, ahrs_delta_angle);
}

void EKFGSF_yaw::alignTilt() {
  // Rotation matrix is constructed directly from acceleration measurement and
  // will be the same for all models so only need to calculate it once.
  // Assumptions are: 1) Yaw angle is zero - yaw is aligned later for each model
  // when velocity fusion commences. 2) The vehicle is not accelerating so all
  // of the measured acceleration is due to gravity.
  ...
  // Calculate earth frame North axis unit vector rotated into body frame,
  // orthogonal to 'down_in_bf'
  ...
  // Calculate earth frame East axis unit vector rotated into body frame,
  // orthogonal to 'down_in_bf' and 'north_in_bf' % operator is overloaded to
  // provide a cross product
  const Vector3F east_in_bf = down_in_bf % north_in_bf;

  // Each column in a rotation matrix from earth frame to body frame represents
  // the projection of the corresponding earth frame unit vector rotated into
  // the body frame, eg 'north_in_bf' would be the first column. We need the
  // rotation matrix from body frame to earth frame so the earth frame unit
  // vectors rotated into body frame are copied into corresponding rows instead
...
void EKFGSF_yaw::alignYaw() {
  ...
  for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++) {
    if (fabsF(AHRS[mdl_idx].R[2][0]) < fabsF(AHRS[mdl_idx].R[2][1])) {
      // get the roll, pitch, yaw estimates from the rotation matrix using a 321
      ...
      // Calculate the 312 Tait-Bryan rotation sequence that rotates from earth
      // to body frame
      Vector3F euler312 = AHRS[mdl_idx].R.to_euler312();
      euler312[2] = wrap_PI(
          EKF[mdl_idx].X[2]); // first rotation (yaw) taken from EKF model state
...
void EKFGSF_yaw::predict(const uint8_t mdl_idx) {
  ...
  // we don't start running the EKF part of the algorithm until there are
  // regular velocity observations
  if (!vel_fuse_running) {
    return;
  }

  // Calculate the yaw state using a projection onto the horizontal that avoids
  // gimbal lock
  if (fabsF(AHRS[mdl_idx].R[2][0]) < fabsF(AHRS[mdl_idx].R[2][1])) {
    ...
    EKF[mdl_idx].X[2] = atan2F(-AHRS[mdl_idx].R[0][1],
                               AHRS[mdl_idx].R[1][1]); // first rotation (yaw)
  }

  // calculate delta velocity in a horizontal front-right frame
  const Vector3F del_vel_NED = AHRS[mdl_idx].R * delta_velocity;
  const ftype dvx = del_vel_NED[0] * cosF(EKF[mdl_idx].X[2]) +
                    del_vel_NED[1] * sinF(EKF[mdl_idx].X[2]);
  const ftype dvy = -del_vel_NED[0] * sinF(EKF[mdl_idx].X[2]) +
                    del_vel_NED[1] * cosF(EKF[mdl_idx].X[2]);

  // sum delta velocities in earth frame:
  EKF[mdl_idx].X[0] += del_vel_NED[0];
  EKF[mdl_idx].X[1] += del_vel_NED[1];

  // predict covariance - autocode from
  // https://github.com/priseborough/3_state_filter/blob/flightLogReplay-wip/calcPupdate.txt
  ...
  const ftype dvxVar =
      sq(EKFGSF_accelNoise *
         velocity_dt);         // variance of forward delta velocity - (m/s)^2
  const ftype dvyVar = dvxVar; // variance of right delta velocity - (m/s)^2
  const ftype dazVar =
      sq(EKFGSF_gyroNoise * angle_dt); // variance of yaw delta angle - rad^2
  ...
  EKF[mdl_idx].P[0][0] =
      fmaxF(P00 - P20 * t6 + dvxVar * t14 + dvyVar * t13 - t6 * t7, min_var);
  ...
  EKF[mdl_idx].P[1][1] =
      fmaxF(P11 + P21 * t10 + dvxVar * t13 + dvyVar * t14 + t10 * t16, min_var);
...
// Update EKF states and covariance for specified model index using velocity
// measurement Returns false if the sttae and covariance correction failed
bool EKFGSF_yaw::correct(const uint8_t mdl_idx, const Vector2F &vel,
                         const ftype velObsVar) {
  ...
  // Perform a chi-square innovation consistency test and calculate a
  // compression scale factor that limits the magnitude of innovations to
  // 5-sigma
  ftype S_det_inv = (EKF[mdl_idx].S[0][0] * EKF[mdl_idx].S[1][1] -
                     EKF[mdl_idx].S[0][1] * EKF[mdl_idx].S[1][0]);
  ftype innov_comp_scale_factor = 1.0f;
  if (fabsF(S_det_inv) > 1E-6f) {
    // Calculate elements for innovation covariance inverse matrix assuming
    // symmetry
    ...
    // The following expression was derived symbolically from test ratio =
    // transpose(innovation) * inverse(innovation variance) * innovation = [1x2]
    // * [2,2] * [2,1] = [1,1]
    const ftype test_ratio =
        EKF[mdl_idx].innov[0] * (EKF[mdl_idx].innov[0] * S_inv_NN +
                                 EKF[mdl_idx].innov[1] * S_inv_NE) +
        EKF[mdl_idx].innov[1] * (EKF[mdl_idx].innov[0] * S_inv_NE +
                                 EKF[mdl_idx].innov[1] * S_inv_EE);

    // If the test ratio is greater than 25 (5 Sigma) then reduce the length of
    // the innovation vector to clip it at 5-Sigma This protects from large
    // measurement spikes
    if (test_ratio > 25.0f) {
  ...
  // autocode from
  // https://github.com/priseborough/3_state_filter/blob/flightLogReplay-wip/calcK.txt
  // and
  // https://github.com/priseborough/3_state_filter/blob/flightLogReplay-wip/calcPmat.txt
  ...
  for (uint8_t obs_index = 0; obs_index < 2; obs_index++) {
    ...
    for (unsigned row = 0; row < 3; row++) {
      EKF[mdl_idx].X[row] -= K[row][obs_index] * EKF[mdl_idx].innov[obs_index] *
                             innov_comp_scale_factor;
    }
  }
  const ftype yaw_delta = EKF[mdl_idx].X[2] - yaw_prev;

  // apply the change in yaw angle to the AHRS taking advantage of sparseness in
  // the yaw rotation matrix
  const ftype cos_yaw = cosF(yaw_delta);
  const ftype sin_yaw = sinF(yaw_delta);
  ftype R_prev[2][3];
  memcpy(&R_prev, &AHRS[mdl_idx].R,
         sizeof(R_prev)); // copy first two rows from 3x3
...
void EKFGSF_yaw::resetEKFGSF() {
  ...
  for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++) {
    ...
    EKF[mdl_idx].X[2] =
        -M_PI + (0.5f * yaw_increment) + ((ftype)mdl_idx * yaw_increment);

    // All filter models start with the same weight
    GSF.weights[mdl_idx] = 1.0f / (ftype)N_MODELS_EKFGSF;

    // Use half yaw interval for yaw uncertainty as that is the maximum that the
    // best model can be away from truth
    GSF.yaw_variance = sq(0.5f * yaw_increment);
    EKF[mdl_idx].P[2][2] = GSF.yaw_variance;
  }
}

// returns the probability of a selected model output assuming a gaussian error
// distribution
ftype EKFGSF_yaw::gaussianDensity(const uint8_t mdl_idx) const {
  ...
  // inv(S) * innovation
  ftype tempVec[2];
  tempVec[0] = invMat[0][0] * EKF[mdl_idx].innov[0] +
               invMat[0][1] * EKF[mdl_idx].innov[1];
  tempVec[1] = invMat[1][0] * EKF[mdl_idx].innov[0] +
               invMat[1][1] * EKF[mdl_idx].innov[1];

  // transpose(innovation) * inv(S) * innovation
  ftype normDist =
      tempVec[0] * EKF[mdl_idx].innov[0] + tempVec[1] * EKF[mdl_idx].innov[1];

  // convert from a normalised variance to a probability assuming a Gaussian
  // distribution
...
// Apply a body frame delta angle to the body to earth frame rotation matrix
...
Matrix3F EKFGSF_yaw::updateRotMat(const Matrix3F &R, const Vector3F &g) const {
  ...
  for (uint8_t r = 0; r < 3; r++) {
    rowLengthSq =
        ret[r][0] * ret[r][0] + ret[r][1] * ret[r][1] + ret[r][2] * ret[r][2];
    if (is_positive(rowLengthSq)) {
      // Use linear approximation for inverse sqrt taking advantage of the row
      // length being close to 1.0
...
bool EKFGSF_yaw::getYawData(ftype &yaw, ftype &yawVariance,
                            uint8_t *_n_clips) const {
  if (!vel_fuse_running) {
    return false;
  }
  yaw = GSF.yaw;
...
bool EKFGSF_yaw::getVelInnovLength(ftype &velInnovLength) const {
  ...
  for (uint8_t mdl_idx = 0; mdl_idx < N_MODELS_EKFGSF; mdl_idx++) {
    velInnovLength +=
        GSF.weights[mdl_idx] *
        sqrtF((sq(EKF[mdl_idx].innov[0]) + sq(EKF[mdl_idx].innov[1])));
  }
  return true;
}
