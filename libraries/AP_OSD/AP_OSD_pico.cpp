 ...
 */
/*
 * Analog OSD for RP2350. See AP_OSD_pico.h for provenance.
 */
...
volatile uint32_t AP_OSD_pico::vsync_count;
volatile uint32_t AP_OSD_pico::late_blocks;
volatile uint32_t AP_OSD_pico::desyncs;
volatile uint32_t AP_OSD_pico::blank_blocks;

/*
  The PIO1 broker decides between this and the LED driver by reading OSD_TYPE
  and OSD_TYPE2 directly, because it has to answer before AP_OSD exists -
  RCOutput asks during init_ardupilot(). It cannot include this header
  without the HAL depending on AP_OSD, so the value is duplicated there and
  pinned here.
 */
static_assert((int)AP_OSD::osd_types::OSD_PICO == 6,
              "RP2350_pio1.cpp hardcodes 6 in OSD_TYPE and OSD_TYPE2 for the PIO OSD");
...
bool AP_OSD_pico::init(void)
{
    /*
      ...
      character the MAX7456 wants; only the pixel encoding differs, and that
      is converted through a lookup table.
     */
    ...
    /*
      PAL until a camera is seen: start_scanout() counts fields and switches
      to NTSC if that is what is arriving, and flush() keeps checking while
      disarmed because the camera can still be starting when init() runs.
     */
    if (!start_scanout(true)) {
        delete fd;
        return false;
    }
    /*
      fd is deliberately not freed: glyphs are blitted straight out of it
      rather than from a converted copy, so it has to outlive init(). 13.8 KB
      of ROMFS held for the life of the boot.
     */
    set_font(fd->data);
    return true;
}
 ...
 */
void AP_OSD_pico::report_stats(void)
{
    const uint32_t now_ms = AP_HAL::millis();
    if (now_ms - last_report_ms < 10000U) {
        return;
    }
    last_report_ms = now_ms;
    const uint32_t fields = vsync_count;
    const uint32_t late = late_blocks;
    const uint32_t blank = blank_blocks;
    const uint32_t desync = desyncs;
    if (fields != 0) {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "OSD: fields=%lu late=%lu blank=%lu desync=%lu",
                      (unsigned long)(fields - last_fields),
                      (unsigned long)(late - last_late),
                      (unsigned long)(blank - last_blank),
                      (unsigned long)(desync - last_desyncs));
    }
    last_fields = fields;
    last_late = late;
    last_blank = blank;
...
bool AP_OSD_pico::claim_pio(void)
{
...
void AP_OSD_pico::configure_sm(void)
{
    PIO_TypeDef *pio = OSD_PIO;
 ...
 */
void AP_OSD_pico::build_font_lut(void)
{
 ...
 */
void AP_OSD_pico::build_blank_table(void)
{
...
void AP_OSD_pico::set_font(const uint8_t *mcm_font)
{
    font = mcm_font;
    build_font_lut();
    build_blank_table();
...
/*
  ...
  honest about it.
 */
static AP_OSD_pico *osd_instance;
...
uint16_t AP_OSD_pico::block_words(uint16_t block) const
{
    const uint32_t first = (uint32_t)block * OSD_PICO_BLOCK_LINES;
 ...
 */
void AP_OSD_pico::arm_block(uint16_t block, uint8_t buf)
{
...
void AP_OSD_pico::arm_blank(uint16_t block)
{
    dmaChannelDisableX(dma);
...
void AP_OSD_pico::signal_render(void)
{
    chSysLockFromISR();
 ...
 */
void AP_OSD_pico::advance_to(uint16_t block)
{
    dma_block = block;
 ...
 */
void AP_OSD_pico::block_complete(void)
{
...
OSAL_IRQ_HANDLER(RP_PIO1_IRQ_0_HANDLER)
{
    ...
    if ((pio->IRQ0_INTS & PIO_INTE_SM_IRQ(0)) != 0U) {
        ...
        AP_OSD_pico *osd = osd_instance;
        if (osd != nullptr) {
            osd->field_start();
        }
        AP_OSD_pico::vsync_count++;
    }

    OSAL_IRQ_EPILOGUE();
}

void AP_OSD_pico::field_start(void)
{
    if (detecting) {
 ...
 */
void AP_OSD_pico::set_standard(bool pal)
{
    PIO_TypeDef *pio = OSD_PIO;
 ...
 */
void AP_OSD_pico::apply_standard(bool pal)
{
    chSysLock();
    set_standard(pal);
    produced = 0;
...
/*
  ...
  not be.
 */
void AP_OSD_pico::core1_thread(void)
{
    PIO_TypeDef *pio = OSD_PIO;
...
bool AP_OSD_pico::start_scanout(bool pal)
{
    if (initialised) {
        return true;
    }
    ...
    if (!hal.scheduler->thread_create_pinned_to_core(
            FUNCTOR_BIND_MEMBER(&AP_OSD_pico::core1_thread, void),
...
/*
  start_scanout() measures the standard once, and a camera that is still starting
 ...
 */
void AP_OSD_pico::check_standard(void)
{
    const uint32_t now_ms = AP_HAL::millis();
