...
TEST_F(MavLogAlias, RenameBetweenTheAliasAndTheRealPath)
{
    char from[128];
    snprintf(from, sizeof(from), "@MAV_LOG/%s", name);
    const char *to = HAL_BOARD_LOG_DIRECTORY "/alias_unit_test_renamed.txt";

    // both are the same filesystem, whichever row names them
    ASSERT_EQ(fs.rename(from, to), 0);

    struct stat st {};
    EXPECT_EQ(::stat(path_under_root, &st), -1);
    EXPECT_EQ(::stat(to, &st), 0);

    ::unlink(to);
}

/*
  resolved paths held the way AP_Filesystem's own calls hold them, for the
  length of a backend call
 */
class AP_Filesystem_Test
{
public:
    /*
      another thread's alias call made while one is held waits for it, and
      does not write its path over the one still held
     */
    static void another_thread_holding_the_buffer()
    {
        std::atomic<bool> holding { false };
        std::atomic<bool> release { false };
        std::string held_path;
        std::thread holder([&]() {
            const AP_Filesystem::ResolvedPath held { fs, "@MAV_LOG/held" };
            holding = true;
            while (!release) {
        ...
        auto other = std::async(std::launch::async, []() {
            const AP_Filesystem::ResolvedPath resolved { fs, "@MAV_LOG/other" };
            return std::string(resolved.valid() ? resolved.path() : "");
        });
        const bool finished_while_held = other.wait_for(std::chrono::milliseconds(500)) == std::future_status::ready;
        release = true;
        holder.join();
        const bool finished = other.wait_for(std::chrono::seconds(5)) == std::future_status::ready;
        ASSERT_TRUE(finished);

        EXPECT_FALSE(finished_while_held);
        EXPECT_EQ(other.get(), HAL_BOARD_LOG_DIRECTORY "/other");
        EXPECT_EQ(held_path, HAL_BOARD_LOG_DIRECTORY "/held");
    }

    /*
      the same thread resolving a second alias path while it holds one: the
      shared buffer is refused rather than overwritten, and a path given a
      buffer of its own (as rename's second path is) is fine
     */
    static void this_thread_holding_the_buffer()
    {
        const AP_Filesystem::ResolvedPath first { fs, "@MAV_LOG/first" };
        errno = 0;
        const AP_Filesystem::ResolvedPath second { fs, "@MAV_LOG/second" };
        EXPECT_FALSE(second.valid());
        EXPECT_EQ(errno, EBUSY);
        const AP_Filesystem::ResolvedPath third { fs, "@MAV_LOG/third", AP_Filesystem::ResolvedPath::Buffer::OWN };
        ASSERT_TRUE(first.valid());
        ASSERT_TRUE(third.valid());
        EXPECT_STREQ(first.path(), HAL_BOARD_LOG_DIRECTORY "/first");
        EXPECT_STREQ(third.path(), HAL_BOARD_LOG_DIRECTORY "/third");
    }
};

TEST(MavLogAliasBuffer, AnotherThreadsAliasCallWaits)
{
    AP_Filesystem_Test::another_thread_holding_the_buffer();
}

TEST(MavLogAliasBuffer, ASecondAliasPathInOneThreadNeedsItsOwnBuffer)
{
    AP_Filesystem_Test::this_thread_holding_the_buffer();
}
...
// these create and stat real files, which SITL's LittleFS can't do without
// a running HAL
#if AP_FILESYSTEM_SYS_ENABLED && !AP_FILESYSTEM_LOCAL_IS_LITTLEFS
/*
  the whole-component prefix match is not particular to aliases.
  "@SYSfoo" names a local file; it is not "foo" served by @SYS
 */
TEST(BackendPrefix, RenameBetweenFilesystemsIsRefused)
{
    ASSERT_TRUE(create_directly("rename_between_filesystems.txt", "local"));

    errno = 0;
    EXPECT_EQ(fs.rename("rename_between_filesystems.txt", "@SYS/rename_between_filesystems.txt"), -1);
    EXPECT_EQ(errno, EXDEV);

    ::unlink("rename_between_filesystems.txt");
}
...
#endif  // AP_FILESYSTEM_SYS_ENABLED && !AP_FILESYSTEM_LOCAL_IS_LITTLEFS
