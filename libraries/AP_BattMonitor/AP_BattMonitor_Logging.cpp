...
{
#if HAL_SMART_BATTERY_INFO_ENABLED
#if !defined(HAL_NO_GCS)

    if (!HAVE_PAYLOAD_SPACE(chan, SMART_BATTERY_INFO)) {
        return false;
    }

    // Note: leaking stack data here from the char arrays saving flash space instead
    Smart_Batt_Info smart_batt_info;

    // Only can return false is space not available on the link thus return true here
    if (!get_smart_batt_info(smart_batt_info)) {
        return true;
    }
