...
#ifndef HAL_BATTMON_CELL_COUNT
#define HAL_BATTMON_CELL_COUNT 6
#endif
...
{
    dev = hal.i2c_mgr->get_device(HAL_BATTMON_TIBQ76952_BUS, HAL_BATTMON_TIBQ76952_ADDR);
    if (!dev) {
        return;
    }
...
void AP_BattMonitor_TIBQ76952::read(void)
{
    ...
    for (uint8_t i = 0; i < MIN(HAL_BATTMON_CELL_COUNT, AP_BATT_MONITOR_CELLS_MAX); i++) {
...
void AP_BattMonitor_TIBQ76952::mppt_set_powered_state(bool power_on)
{
    if (!configured) {
        return;
    }
...
bool AP_BattMonitor_TIBQ76952::configure()
{
    ...
    uint16_t vcell_mode = (1 << HAL_BATTMON_CELL_COUNT) - 1;
...
void AP_BattMonitor_TIBQ76952::read_voltage_current_temperature()
{
    ...
    uint16_t alarm_raw_status = direct_command_read_2bytes(TIBQ769x2_AlarmRawStatus);
    if (!BIT_IS_SET(alarm_raw_status, 7)) {
        return;
    }
    ...
    // ready stack voltage (should equal sum of cell voltages)
    ...
    for (uint8_t i = 0; i < HAL_BATTMON_CELL_COUNT; i++) {
        uint16_t cell_voltage_mv = direct_command_read_2bytes(TIBQ769x2_Cell1Voltage + i*2);
        accumulate.cell_voltages_mv[i] += cell_voltage_mv;
    }

    // read current (positive values = discharging, negative = charging)
    int16_t cc2_current = direct_command_read_2bytes(TIBQ769x2_CC2Current);
    accumulate.current += cc2_current * 0.001f; // convert to Amps

    // read temperature
    int16_t temp_internal = direct_command_read_2bytes(TIBQ769x2_IntTemperature); // 0.1K
...
// ready battert power state (e.g. charging, discharging, idle)
void AP_BattMonitor_TIBQ76952::read_power_state()
{
    AP_BattMonitor::PowerState new_state = _state.power_state;

    uint16_t mfg_status = direct_command_read_2bytes(TIBQ769x2_MfgStatusInit);
    uint8_t fet_status = direct_command_read_1byte(TIBQ769x2_FETStatus);
...
// ready safety status (e.g. check for faults)
void AP_BattMonitor_TIBQ76952::read_safety_state()
{
    uint16_t safetyA_status = direct_command_read_2bytes(TIBQ769x2_SafetyStatusA);
    if (safetyA_status > 0) {
        Debug("BQ76952: SafetyA Status: 0x%04X", safetyA_status);
    }
}
...
bool AP_BattMonitor_TIBQ76952::direct_command(uint16_t command, uint16_t data, CommandType type, uint8_t *rx_data, uint8_t rx_len) const
{
    ...
    uint8_t tx_buffer[2] = {LOWBYTE(data), HIGHBYTE(data)};
    switch (type) {
...
void AP_BattMonitor_TIBQ76952::set_register(uint16_t reg_addr, uint32_t reg_data, uint8_t len) const
{
    ...
    uint8_t tx_buffer[2];
    uint8_t tx_reg_data[6] {};

    tx_reg_data[0] = LOWBYTE(reg_addr);
    tx_reg_data[1] = HIGHBYTE(reg_addr);
    tx_reg_data[2] = LOWBYTE(reg_data);
...
{
    uint8_t tx_buffer[2] = {LOWBYTE(command), HIGHBYTE(command)};
...
bool AP_BattMonitor_TIBQ76952::sub_command(uint16_t command, uint16_t data, CommandType type, uint8_t *rx_data, uint8_t rx_len) const
{
    ...
    uint8_t tx_reg[4] = {};
    tx_reg[0] = LOWBYTE(command);
    tx_reg[1] = HIGHBYTE(command);
...
{
    // calculate checksum using sum of all bytes
    uint8_t sum = 0;
    for (uint8_t i = 0; i < len; i++) {
        sum += data[i];
    }
