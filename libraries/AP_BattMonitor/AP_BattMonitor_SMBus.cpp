...
bool AP_BattMonitor_SMBus::read_full_charge_capacity(void)
{
    ...
    if (_full_charge_capacity_mah != 0) {
        ...
        _full_charge_capacity_mah = data * get_capacity_scaler();
        return true;
    }
    return false;
}
...
// returns true if the read was successful, which will only be read if
// we know the full charge capacity (accounting for battery degradation)
