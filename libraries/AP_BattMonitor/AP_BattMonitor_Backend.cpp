...
bool AP_BattMonitor_Backend::capacity_remaining_pct(int8_t &percentage) const
{
    // anything under 10 mAh is an invalid capacity and so will be our measurement of remaining capacity
    if ( _params._pack_capacity <= 10) {
        return false;
    }
    float mah_remaining = _params._pack_capacity - _state.consumed_mah;
    percentage = MIN(MAX((100 * (mah_remaining) / _params._pack_capacity), 0), UINT8_MAX);
    return true;
}
