...
void AP_BattMonitor_UAVCAN::subscribe_battmon_uavcan_messages(AP_UAVCAN* ap_uavcan)
{
    ...
    const int battinfo_listener_res = battinfo_listener->start(BattInfoCb(ap_uavcan, &handle_battery_info_trampoline));     //Backend Msg Handler
...
AP_BattMonitor_UAVCAN* AP_BattMonitor_UAVCAN::get_uavcan_backend(AP_UAVCAN* ap_uavcan, uint8_t node_id)
{
    ...
    for (uint8_t i = 0; i < AP::battery()._num_instances; i++) {
        if (AP::battery().drivers[i] == nullptr || AP::battery().get_type(i) != AP_BattMonitor_Params::BattMonitor_TYPE_UAVCAN_BatteryInfo) {
            continue;
        }
    ...
    for (uint8_t i = 0; i < AP::battery()._num_instances; i++) {
        if (AP::battery().drivers[i] != nullptr && AP::battery().get_type(i) == AP_BattMonitor_Params::BattMonitor_TYPE_UAVCAN_BatteryInfo) {
            ...
            batmon->init();
            debug_bm_uavcan(2, ap_uavcan->get_driver_index(),"Registered BattMonitor Node %d on Bus %d\n", node_id, ap_uavcan->get_driver_index());
            return batmon;
        }
    }
    return nullptr;
