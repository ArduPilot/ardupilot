...
const AP_Param::GroupInfo AP_Airspeed::var_info[] = {
    ...
    // @Description: This parameter allows you to control whether the order in which the tubes are attached to your pitot tube matters. If you set this to 0 then the first (often the top) connector on the sensor needs to be the dynamic pressure. If set to 1 then the second (often the bottom) connector needs to be the dynamic pressure. If set to 2 (the default) then the airspeed driver will accept either order. The reason you may wish to specify the order is it will allow your airspeed sensor to detect if the aircraft is receiving excessive pressure on the static port compared to dynamic such as during a stall, which would otherwise be seen as a positive airspeed.
    // @User: Advanced
