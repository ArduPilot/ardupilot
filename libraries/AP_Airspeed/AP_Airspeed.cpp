...
// slots 1-9, 11-20 were previously used by params before being refactored into AP_Airspeed_Params
    ...
    AP_SUBGROUPINFO(param[0], "1_", 25, AP_Airspeed, AP_Airspeed_Params),

#if AIRSPEED_MAX_SENSORS > 1
    // @Group: 2_
    // @Path: AP_Airspeed_Params.cpp
    AP_SUBGROUPINFO(param[1], "2_", 26, AP_Airspeed, AP_Airspeed_Params),
#endif
...
void AP_Airspeed::update()
{
    ...
    for (uint8_t i=1; i<AIRSPEED_MAX_SENSORS; i++) {
        if (enabled(i)) {
            char msg_label[4];
            hal.util->snprintf((char *)msg_label, sizeof(msg_label), "AS%i", i+1);
            gcs().send_named_float(msg_label, get_airspeed(i));
        }
    }
