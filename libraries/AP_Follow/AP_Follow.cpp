...
const AP_Param::GroupInfo AP_Follow::var_info[] = {
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("_OFS", 7, AP_Follow, _offset_XXD_m, 0),
...
// May add option to turn on input shaping, turn on acceleration due to rotation of offset
...
bool AP_Follow::update_estimate()
{
    ...
    Vector3f delta_pos = _target_vel_ned_ms * dt + _target_accel_ned_mss * 0.5f * sq(dt);
    Vector3f delta_vel = _target_accel_ned_mss * dt;
    ...
    if (_estimate_valid) {
        ...
        shape_pos_vel_accel_xy(_target_pos_ned_m.xy() + delta_pos.xy().topostype(), _target_vel_ned_ms.xy() + delta_vel.xy(), _target_accel_ned_mss.xy(),
        ...
        shape_angle_vel_accel(radians(_target_heading_deg), radians(_target_heading_rate_degs), 0.0,
        ...
        _estimate_pos_ned_m = _target_pos_ned_m + delta_pos.topostype();
        _estimate_vel_ned_ms = _target_vel_ned_ms + delta_vel;
        _estimate_accel_ned_mss = _target_accel_ned_mss;
        _estimate_valid = true;
    }

    Vector3f offset_m = _offset_XXD_m.get();
...
{
    if (_automatic_sysid) {
        // timeout check: if no position update received within timeout, clear automatic sysid
    ...
    if (updated) {
        // message provided updated position information
        Vector3f offset_m = _offset_XXD_m.get();

        // if using relative frame, rotate offsets based on estimated heading
        if (_offset_type == AP_FOLLOW_OFFSET_TYPE_RELATIVE) {
            offset_m.xy().rotate(_target_heading_deg);
        }
        ...
        if (!AP::ahrs().get_relative_position_NED_origin(current_position_ned_m)) {
            ...
            const Vector3p ofs_dist_vec = dist_vec_ned_m + offset_m.topostype();

            // initialise offsets dynamically if needed
            init_offsets_if_required(dist_vec_ned_m.tofloat());
...
bool AP_Follow::estimate_error_too_large() const
{
    ...
    const float half_timeout_sec = timeout_sec * 0.5f;

    // Calculate position thresholds
    const float pos_thresh_horiz_m = _accel_max_ne_mss.get() * sq(half_timeout_sec);
    const float pos_thresh_vert_m  = _accel_max_d_mss.get()  * sq(half_timeout_sec);
    ...
    Vector3f pos_error = _estimate_pos_ned_m.tofloat() - _target_pos_ned_m.tofloat();
    Vector3f vel_error = _estimate_vel_ned_ms - _target_vel_ned_ms;

    Vector2f pos_error_xy = pos_error.xy();
    float pos_error_z = pos_error.z;
    Vector2f vel_error_xy = vel_error.xy();
    float vel_error_z = vel_error.z;
