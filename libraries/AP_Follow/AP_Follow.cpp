...
bool AP_Follow::handle_global_position_int_message(const mavlink_message_t &msg)
{
    ...
    Vector3p target_pos_ned_cm;
    if (!target_location.get_vector_from_origin_NEU(target_pos_ned_cm)) {
        return false;
    }
    ...
    // convert NEU cm -> NED m
    _target_pos_ned_m.xy() = target_pos_ned_cm.xy() * 0.01; 
    _target_pos_ned_m.z = -target_pos_ned_cm.z * 0.01;
...
bool AP_Follow::handle_follow_target_message(const mavlink_message_t &msg)
{
    ...
    _target_pos_ned_m = target_pos_ned_cm * 0.01; // convert from cm to meters
