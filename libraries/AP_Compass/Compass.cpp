...
const AP_Param::GroupInfo Compass::var_info[] PROGMEM = {
    ...
    // @User: Advanced
    AP_GROUPINFO("EXTERNAL", 9, Compass, _external, 1),
...
bool Compass::configured(uint8_t i)
{
    ...
    if (i > get_count()) {
        /*
        hal.console->print("Compass ");
        hal.console->print(i);
        hal.console->println(" is greater than count.");
        */
        return false;
    }
    ...
    if (get_offsets(i).length() == 0.0f) {
        ...
        hal.console->print("Compass ");
        hal.console->print(i);
        hal.console->println(" is not calibrated.");
        */
        return false;
    }
    ...
    if (_dev_id[i] != dev_id_orig) {
        ...
        hal.console->print("Compass ");
        hal.console->print(i);
        hal.console->print(" device id is ");
        hal.console->print(dev_id_orig);
        hal.console->print(" which does not match read id of ");
        hal.console->println(_dev_id[i]);
        */
        // restore device id
        _dev_id[i].set_and_save_ifchanged(dev_id_orig);
        // return failure
        return false;
    }
#endif
