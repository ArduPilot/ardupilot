...
#define MICRO_SECONDS_BETWEEN_MEASUREMENTS 10000
...
void AP_Compass_BMM150::_load_trim_values()
{
    uint8_t data[21];

    _dev->read_registers(DIG_X1_REG, data, sizeof(data));

    _dig_x1 = (int8_t)data[DIG_X1_REG - DIG_X1_REG];
    _dig_y1 = (int8_t)data[DIG_Y1_REG - DIG_X1_REG];
    _dig_x2 = (int8_t)data[DIG_X2_REG - DIG_X1_REG];
    _dig_y2 = (int8_t)data[DIG_Y2_REG - DIG_X1_REG];
    _dig_xy1 = data[DIG_XY1_REG - DIG_X1_REG];
    _dig_xy2 = (int8_t)data[DIG_XY2_REG - DIG_X1_REG];
    _dig_z1 = (uint16_t)le16toh(data[DIG_Z1_LSB_REG - DIG_X1_REG] | data[DIG_Z1_MSB_REG - DIG_X1_REG] << 8);
    _dig_z2 = (int16_t)le16toh(data[DIG_Z2_LSB_REG - DIG_X1_REG] | data[DIG_Z2_MSB_REG - DIG_X1_REG] << 8);
    _dig_z3 = (int16_t)le16toh(data[DIG_Z3_LSB_REG - DIG_X1_REG] | data[DIG_Z3_MSB_REG - DIG_X1_REG] << 8);
    _dig_z4 = (int16_t)le16toh(data[DIG_Z4_LSB_REG - DIG_X1_REG] | data[DIG_Z4_MSB_REG - DIG_X1_REG] << 8);
    _dig_xyz1 = (uint16_t)le16toh(data[DIG_XYZ1_LSB_REG - DIG_X1_REG] | data[DIG_XYZ1_MSB_REG - DIG_X1_REG] << 8);
}
...
bool AP_Compass_BMM150::init()
{
    ...
    _dev->write_register(POWER_AND_OPERATIONS_REG, POWER_CONTROL_VAL);
    hal.scheduler->delay(2);

    uint8_t val;
    _dev->read_registers(CHIP_ID_REG, &val, 1);
    if (val != CHIP_ID_VAL) {
     ...
     */
    _dev->write_register(REPETITIONS_XY_REG, (47 - 1) / 2);
    _dev->write_register(REPETITIONS_Z_REG, 83 - 1);
    /* Change operation mode from sleep to normal and set ODR */
    _dev->write_register(OP_MODE_SELF_TEST_ODR_REG, NORMAL_MODE | ODR_30HZ);
 ...
 */
int16_t AP_Compass_BMM150::_compensate_xy(int16_t xy, uint16_t rhall, bool is_x)
{
    int8_t txy1, txy2;
    int32_t val;
    ...
    /* TODO: make compensation calculations works on big-endian CPUs */
    val = ((int16_t)(((uint16_t)((((int32_t)_dig_xyz1) << 14) / rhall)) -
          ((uint16_t)0x4000)));
    val = ((int16_t)((((int32_t)xy) * ((((((((int32_t)_dig_xy2) *
          ((((int32_t)val) * ((int32_t)val)) >> 7)) + (((int32_t)val) *
          ((int32_t)(((int16_t)_dig_xy1) << 7)))) >> 9) +
...
int16_t AP_Compass_BMM150::_compensate_z(int16_t z, uint16_t rhall)
{
    int32_t val;

    /* TODO: make compensation calculations works on big-endian CPUs */
    val = (((((int32_t)(z - _dig_z4)) << 15) - ((((int32_t)_dig_z3) *
          ((int32_t)(((int16_t)rhall) - ((int16_t)_dig_xyz1)))) >> 2)) /
          (_dig_z2 + ((int16_t)(((((int32_t)_dig_z1) *
...
void AP_Compass_BMM150::_update()
{
    ...
    if (time_us - _last_update_timestamp < MICRO_SECONDS_BETWEEN_MEASUREMENTS) {
        return;
    }
    ...
    uint16_t data[4];
    _dev->read_registers(DATA_X_LSB_REG, (uint8_t *) &data, sizeof(data));
    _dev->get_semaphore()->give();
    ...
    val = _compensate_xy(val, rhall, true);
    raw_field.x = val;

    val = le16toh(((int16_t)data[1]) >> 3);
    val = _compensate_xy(val, rhall, false);
    raw_field.y = val;
