...
bool AP_Compass_AK09940A::_configure()
{
    if (!_reset()) {
        return false;
    }
    ...
    if (!_setup_mode(AK09940A_POWERDOWN_MODE)) {
        return false;
    }
    hal.scheduler->delay_microseconds(100);

    if (!_setup_mode(AK09940A_CONTINUOUS_MODE4)) {
        return false;
    }

    return _set_sensordrive(AK09940A_SENSORDRIVE_LN1);
}

int32_t AP_Compass_AK09940A::combine_axis_bytes(uint8_t hl, uint8_t hm, uint8_t hh)
{
...
void AP_Compass_AK09940A::_update()
{
    ...
    Vector3f raw_field{
        (float) combine_axis_bytes(data.magx[0], data.magx[1], data.magx[2]) * AK09940A_LSB_TO_MILLIGAUSS_SCALE,
        (float) combine_axis_bytes(data.magy[0], data.magy[1], data.magy[2]) * AK09940A_LSB_TO_MILLIGAUSS_SCALE,
        (float) combine_axis_bytes(data.magz[0], data.magz[1], data.magz[2]) * AK09940A_LSB_TO_MILLIGAUSS_SCALE
    };
...
bool AP_Compass_AK09940A::_check_id()
{
    for (int i = 0; i < 5; i++) {
        uint8_t deviceid = 0;
        uint8_t mfgid = 0;
        if (_dev->read_registers(AK09940A_WIA1, &mfgid, 1) &&
            _dev->read_registers(AK09940A_WIA2, &deviceid, 1) &&
            deviceid == AK09940A_DEV_ID && mfgid == AK09940A_MFG_ID) {
            return true;
        }
    }
    return false;
...
{
    uint8_t reg_ctrl3 = 0;
    if (!_dev->read_registers(AK09940A_CNTL3, &reg_ctrl3, 1)) {
        return false;
    }
    reg_ctrl3 = (reg_ctrl3 & ~0x60) | ((sensordrive << 5) & 0x60);
