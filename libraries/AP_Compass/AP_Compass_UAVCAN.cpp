    ...
    for (uint8_t i = 0; i < COMPASS_MAX_BACKEND; i++) {
        if (!_detected_modules[i].driver && _detected_modules[i].ap_uavcan) {
            ...
            if (driver) {
                ...
                driver->init();
                debug_mag_uavcan(2, _detected_modules[i].ap_uavcan->get_driver_index(),"Found Mag Node %d on Bus %d Sensor ID %d\n", _detected_modules[i].node_id, _detected_modules[i].ap_uavcan->get_driver_index(), _detected_modules[i].sensor_id);
            }
            break;
        }
    }
