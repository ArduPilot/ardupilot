...
Compass::compass_cal_update()
{
    ...
    for (uint8_t i=0; i<COMPASS_MAX_INSTANCES; i++) {
        bool failure;
        _s_instance._calibrator[i].update(failure);
        if (failure) {
            AP_Notify::events.compass_cal_failed = 1;
        }

        if (_s_instance._calibrator[i].check_for_timeout()) {
            AP_Notify::events.compass_cal_failed = 1;
            cancel_calibration_all();
        }

        if (_s_instance._calibrator[i].running()) {
            running = true;
        }
    }
    ...
    if (is_calibrating()) {
        _s_instance._cal_has_run = true;
        return;
    } else if (_s_instance._cal_has_run && auto_reboot()) {
...
Compass::start_calibration(uint8_t i, bool retry, bool autosave, float delay, bool autoreboot)
{
    ...
    memset(_s_instance._reports_sent,0,sizeof(_s_instance._reports_sent));
    if (!is_calibrating() && delay > 0.5f) {
        AP_Notify::events.initiated_compass_cal = 1;
    }
    if (i == get_primary()) {
        _s_instance._calibrator[i].set_tolerance(_s_instance._calibration_threshold);
    } else {
        _s_instance._calibrator[i].set_tolerance(_s_instance._calibration_threshold*2);
    }
    _s_instance._calibrator[i].start(retry, autosave, delay);
    _s_instance._compass_cal_autoreboot = autoreboot;

    // disable compass learning both for calibration and after completion
    _s_instance._learn.set_and_save(0);
...
Compass::cancel_calibration(uint8_t i)
{
    ...
    if (_s_instance._calibrator[i].running() || _s_instance._calibrator[i].get_status() == COMPASS_CAL_WAITING_TO_START) {
        AP_Notify::events.compass_cal_canceled = 1;
    }
    _s_instance._calibrator[i].clear();
}
...
{
    CompassCalibrator& cal = _s_instance._calibrator[i];
    uint8_t cal_status = cal.get_status();

    if (cal_status == COMPASS_CAL_SUCCESS) {
        _s_instance._cal_complete_requires_reboot = true;
...
Compass::send_mag_cal_progress(mavlink_channel_t chan)
{
    ...
    for (uint8_t compass_id=0; compass_id<COMPASS_MAX_INSTANCES; compass_id++) {
        uint8_t cal_status = _s_instance._calibrator[compass_id].get_status();

        if (cal_status == COMPASS_CAL_WAITING_TO_START  ||
            cal_status == COMPASS_CAL_RUNNING_STEP_ONE ||
            cal_status == COMPASS_CAL_RUNNING_STEP_TWO) {
            uint8_t completion_pct = _s_instance._calibrator[compass_id].get_completion_percent();
            uint8_t completion_mask[10];
            Vector3f direction(0.0f,0.0f,0.0f);
            uint8_t attempt = _s_instance._calibrator[compass_id].get_attempt();
...
void Compass::send_mag_cal_report(mavlink_channel_t chan)
{
    ...
    for (uint8_t compass_id=0; compass_id<COMPASS_MAX_INSTANCES; compass_id++) {
        ...
        if ((cal_status == COMPASS_CAL_SUCCESS ||
            cal_status == COMPASS_CAL_FAILED) && ((_s_instance._reports_sent[compass_id] < MAX_CAL_REPORTS) || CONTINUOUS_REPORTS)) {
            float fitness = _s_instance._calibrator[compass_id].get_fitness();
            Vector3f ofs, diag, offdiag;
            _s_instance._calibrator[compass_id].get_calibration(ofs, diag, offdiag);
            uint8_t autosaved = _s_instance._calibrator[compass_id].get_autosave();
            ...
            _s_instance._reports_sent[compass_id]++;
        }

        if (cal_status == COMPASS_CAL_SUCCESS && _s_instance._calibrator[compass_id].get_autosave()) {
...
Compass::get_cal_mask()
{
    uint8_t cal_mask = 0;
    for (uint8_t i=0; i<COMPASS_MAX_INSTANCES; i++) {
        if (_s_instance._calibrator[i].get_status() != COMPASS_CAL_NOT_STARTED) {
            cal_mask |= 1 << i;
        }
    }
    return cal_mask;
