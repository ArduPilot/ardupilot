...
Compass::learn_offsets(void)
{
    ...
    if (!_s_instance._null_init_done) {
        // first time through
        _s_instance._null_init_done = true;
        for (uint8_t k=0; k<COMPASS_MAX_INSTANCES; k++) {
            const Vector3f &field = _s_instance._state[k].field;
            const Vector3f &ofs = _s_instance._state[k].offset.get();
            for (uint8_t i=0; i<_mag_history_size; i++) {
                // fill the history buffer with the current mag vector,
                // with the offset removed
                _s_instance._state[k].mag_history[i] = Vector3i(roundf(field.x) - ofs.x, 
                                                    roundf(field.y) - ofs.y, 
                                                    roundf(field.z) - ofs.z);
            }
            _s_instance._state[k].mag_history_index = 0;
        }
        return;
    }
    ...
    for (uint8_t k=0; k<COMPASS_MAX_INSTANCES; k++) {
        ...
        if (ofs.is_nan()) {
            ...
            _s_instance._state[k].offset.set(Vector3f());
        }

        // get a past element
        b1 = Vector3f(_s_instance._state[k].mag_history[_s_instance._state[k].mag_history_index].x,
                      _s_instance._state[k].mag_history[_s_instance._state[k].mag_history_index].y,
                      _s_instance._state[k].mag_history[_s_instance._state[k].mag_history_index].z);
        ...
        if (length < min_diff) {
            ...
            // rotation.
            _s_instance._state[k].mag_history_index = (_s_instance._state[k].mag_history_index + 1) % _s_instance._mag_history_size;
            continue;
        }

        // put the vector in the history
        _s_instance._state[k].mag_history[_s_instance._state[k].mag_history_index] = Vector3i(roundf(field.x) - ofs.x, 
                                                                      roundf(field.y) - ofs.y, 
                                                                      roundf(field.z) - ofs.z);
        _s_instance._state[k].mag_history_index = (_s_instance._state[k].mag_history_index + 1) % _mag_history_size;
        ...
        Vector3f new_offsets = _s_instance._state[k].offset.get() - diff;
        ...
        // set the new offsets
        _s_instance._state[k].offset.set(new_offsets);
    }
}
