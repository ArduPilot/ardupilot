...
void AP_Mount_Topotek::update()
{
    ...
    case 0:
        // get gimbal version
        if (!_got_gimbal_firmware) {
            request_gimbal_firmware();
        }
        break;
    case 1:
    case 6:
...
bool AP_Mount_Topotek::set_tracking(TrackingType tracking_type, const Vector2f& p1, const Vector2f& p2)
{
    ...
    if (send_tracking_cmd) {
        ...
        if (_last_tracking_state == TrackingStatus::STOP_TRACKING) {
        ...
        databuff[8] = 0;
        // enable the fuzzy click function of the gimbal when in point selection mode
        // disable the fuzzy click function of the gimbal when in box selection mode
        if (tracking_type == TrackingType::TRK_POINT) {
            // enable gimbal fuzzy click function
            databuff[9] = 9;
        } else {
            // disable gimbal fuzzy click function
            databuff[9] = 1;
        }
...
bool AP_Mount_Topotek::cancel_tracking()
{
    ...
    // put the gimbal in the stop tracking state
    uint8_t track_set = 0;
    // when the gimbal tracking status is tracking, send a command to position the gimbal in the ready tracking state
    if (_last_tracking_state == TrackingStatus::TRACKING_IN_PROGRESS) {
        // put the gimbal in the ready tracking state
        track_set = 1;
    } else {
        // when the tracking status is in the waiting tracking state, set the tracking status manually to stop,
        // because at this time, the gimbal cannot receive the tracking status feedback
        _last_tracking_state = TrackingStatus::STOP_TRACKING;
        _is_tracking = false;
    }

    // send tracking command
    if (send_fixedlen_packet(AddressByte::SYSTEM_AND_IMAGE, AP_MOUNT_TOPOTEK_ID3CHAR_TRACKING, true, track_set)) {
        return true;
    }
    return false;
}
...
// request gimbal firmware version number
void AP_Mount_Topotek::request_gimbal_firmware()
{
...
void AP_Mount_Topotek::gimbal_track_analyse()
{
    ...
    case TrackingStatus::STOP_TRACKING:
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s %s stopped", send_message_prefix, tracking_str);
        _is_tracking = false;
        break;
    case TrackingStatus::WAITING_FOR_TRACKING:
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s %s waited", send_message_prefix, tracking_str);
        _is_tracking = false;
        break;
    case TrackingStatus::TRACKING_IN_PROGRESS:
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s %s started", send_message_prefix, tracking_str);
        _is_tracking_info_recv = true;
        break;
    }
}

// gimbal firmware version message analysis
void AP_Mount_Topotek::gimbal_firmware_ver_analyse()
{
    // version[0] is the major version number, version[1] is the minor version number, and version[2] is the revision version number
    uint8_t version[3] = "";
    ...
    if (data_buf_len <= 7) {
        ...
        for (int i = 0; i < data_buf_len; i++) {
            ...
            } else if (_msg_buff[10 + i] >= '0' && _msg_buff[10 + i] <= '9') {
                num = num * 10 + (_msg_buff[10 + i] - '0');
            }
        }
        version[count] = num;
    }
...
void AP_Mount_Topotek::gimbal_model_name_analyse()
{
    ...
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Mount: Topotek model name %s", _model_name);
