...
void AP_Mount_Topotek::send_target_rates(const MountRateTarget& rate_rads)
{
    ...
    const uint8_t roll_angle_speed = constrain_float(degrees(rate_rads.roll) * ANGULAR_VELOCITY_CONVERSION, -99, 99);
    const uint8_t pitch_angle_speed = constrain_float(-degrees(rate_rads.pitch) * ANGULAR_VELOCITY_CONVERSION, -99, 99);
    const uint8_t yaw_angle_speed = constrain_float(degrees(rate_rads.yaw) * ANGULAR_VELOCITY_CONVERSION, -99, 99);
