...
void AP_Mount_Topotek::send_angle_target(float roll_rad, float pitch_rad, float yaw_rad, bool yaw_is_ef)
{
    ...
    uint8_t speed = 99;
    int16_t roll_angle_cd = constrain_int16(degrees(roll_rad) * 100, -18000, 18000);
    int16_t pitch_angle_cd = constrain_int16(-degrees(pitch_rad) * 100, -9000, 9000);
    int16_t yaw_angle_cd = constrain_int16(degrees(yaw_rad) * 100, -18000, 18000);

    // control yaw using a gyroscope or magnetic encoding
    _set_yaw_angle_cmd[8] = yaw_is_ef ? 'I' : 'A';

    _set_yaw_angle_cmd[10] = hex2char((yaw_angle_cd >> 12) & 0x0F);
    _set_yaw_angle_cmd[11] = hex2char((yaw_angle_cd >> 8) & 0x0F);
    _set_yaw_angle_cmd[12] = hex2char((yaw_angle_cd >> 4) & 0x0F);
    _set_yaw_angle_cmd[13] = hex2char((yaw_angle_cd) & 0x0F);
    ...
    _set_pitch_angle_cmd[10] = hex2char((pitch_angle_cd >> 12) & 0x0F);
    _set_pitch_angle_cmd[11] = hex2char((pitch_angle_cd >> 8) & 0x0F);
    _set_pitch_angle_cmd[12] = hex2char((pitch_angle_cd >> 4) & 0x0F);
    _set_pitch_angle_cmd[13] = hex2char((pitch_angle_cd) & 0x0F);
    ...
    _set_roll_angle_cmd[10] = hex2char((roll_angle_cd >> 12) & 0x0F);
    _set_roll_angle_cmd[11] = hex2char((roll_angle_cd >> 8) & 0x0F);
    _set_roll_angle_cmd[12] = hex2char((roll_angle_cd >> 4) & 0x0F);
    _set_roll_angle_cmd[13] = hex2char((roll_angle_cd) & 0x0F);
...
void AP_Mount_Topotek::send_rate_target(float roll_rads, float pitch_rads, float yaw_rads, bool yaw_is_ef)
{
    ...
    const uint8_t roll_angle_speed = constrain_int16(degrees(roll_rads) * ANGULAR_VELOCITY_CONVERSION, -99, 99);
    const uint8_t pitch_angle_speed = constrain_int16(degrees(pitch_rads) * ANGULAR_VELOCITY_CONVERSION, -99, 99);
    const uint8_t yaw_angle_speed = constrain_int16(degrees(yaw_rads) * ANGULAR_VELOCITY_CONVERSION, -99, 99);
...
void AP_Mount_Topotek::gimbal_angle_analyse(void)
{
    ...
    uint16_t yaw_angle_cd = wrap_180_cd(char_to_number(_msg_buff[10])<<12 | char_to_number(_msg_buff[11])<<8 | char_to_number(_msg_buff[12])<<4 | char_to_number(_msg_buff[13]));
    uint16_t pitch_angle_cd = char_to_number(_msg_buff[14])<<12 | char_to_number(_msg_buff[15])<<8 | char_to_number(_msg_buff[16])<<4 | char_to_number(_msg_buff[17]);
    uint16_t roll_angle_cd = char_to_number(_msg_buff[18])<<12 | char_to_number(_msg_buff[19])<<8 | char_to_number(_msg_buff[20])<<4 | char_to_number(_msg_buff[21]);
...
void AP_Mount_Topotek::gimbal_track_analyse(void)
{
    switch (_msg_buff[11]) {
    case '0':
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "%s tracking error", send_message_prefix);
        break;
    case '1':
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s stopped tracking", send_message_prefix);
        break;
    case '2':
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s started tracking", send_message_prefix);
        break;
    }
}
...
void AP_Mount_Topotek::gimbal_basic_info_analyse(void)
{
    ...
    for (uint8_t count = 0; count < char_to_number(_msg_buff[5]); count++) {
        if (count == 0) {
    ...
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Mount: Topotek %s v%u.%u.%u",
        _model_name,
        major_ver,
        minor_ver,
        patch_ver);
