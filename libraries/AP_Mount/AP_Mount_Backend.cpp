...
#define rc_ch(i) RC_Channel::rc_channel(i-1)
    ...
    if (_frontend._joystick_speed) {
        ...
        if (roll_rc_in && rc_ch(roll_rc_in)) {
            ...
            _angle_ef_target_rad.x = constrain_value<float>(_angle_ef_target_rad.x, radians(_state._roll_angle_min*0.01f), radians(_state._roll_angle_max*0.01f));
        }
        if (tilt_rc_in && (rc_ch(tilt_rc_in))) {
            _angle_ef_target_rad.y += rc_ch(tilt_rc_in)->norm_input_dz() * 0.0001f * _frontend._joystick_speed;
            _angle_ef_target_rad.y = constrain_value<float>(_angle_ef_target_rad.y, radians(_state._tilt_angle_min*0.01f), radians(_state._tilt_angle_max*0.01f));
        }
        if (pan_rc_in && (rc_ch(pan_rc_in))) {
            _angle_ef_target_rad.z += rc_ch(pan_rc_in)->norm_input_dz() * 0.0001f * _frontend._joystick_speed;
            _angle_ef_target_rad.z = constrain_value<float>(_angle_ef_target_rad.z, radians(_state._pan_angle_min*0.01f), radians(_state._pan_angle_max*0.01f));
        }
    } else {
...
{
    float GPS_vector_x = (target.lng-_frontend._current_loc.lng)*cosf(radians((_frontend._current_loc.lat+target.lat)*0.00000005f))*0.01113195f;
