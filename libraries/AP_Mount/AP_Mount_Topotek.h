...
private:
    ...
    enum class TrackingStatus : uint8_t {
        STOP_TRACKING = 0x30,                  // not tracking
    ...
    void gimbal_firmware_ver_analyse();
    ...
    bool _is_tracking_info_recv;                                // status of receiving tracking information flag
    TrackingStatus _last_tracking_state = TrackingStatus::STOP_TRACKING;  // last tracking state received from gimbal
    ...
    bool _got_gimbal_firmware;                                  // true if gimbal's firmware version has been received
    ...
    uint8_t _model_name[16];                                    // gimbal type
    ...
    UartCmdFunctionHandler uart_recv_cmd_compare_list[AP_MOUNT_RECV_GIMBAL_CMD_CATEGORIES_NUM] = {
        ...
        {{"VSN"}, &AP_Mount_Topotek::gimbal_firmware_ver_analyse},
