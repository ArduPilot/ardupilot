...
const AP_Param::GroupInfo AP_Mount_Params::var_info[] = {
    ...
    // @Description: Mount options bitmask, note bit 2 and 3 only impacts RC targeting mode
    // @Bitmask: 0:RC lock state from previous mode, 1:Return to neutral angles on RC failsafe, 2:Force FPV (bf) lock on roll and pitch, 3:Maintain yaw lock entry direction
    // @User: Standard
