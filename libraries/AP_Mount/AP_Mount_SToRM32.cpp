...
void AP_Mount_SToRM32::init(const AP_SerialManager& serial_manager)
{
    ...
    if ((_port = serial_manager.find_serial(AP_SerialManager::SerialProtocol_SToRM32, 0))) {
        _initialised = true;
...
bool AP_Mount_SToRM32::can_send(bool with_control) {
...
void AP_Mount_SToRM32::get_angles() {
    // exit immediately if not initialised
    if (!_initialised) {
        return;
    }
...
void AP_Mount_SToRM32::read_incoming() {
    uint8_t data;
    int16_t numc;
...
void AP_Mount_SToRM32::parse_reply() {
    uint16_t crc;
    bool crc_ok;
