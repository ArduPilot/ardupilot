...
bool AP_Mount::has_pan_control(uint8_t instance_index) const
{
    const auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
    }
...
MAV_MOUNT_MODE AP_Mount::get_mode(uint8_t instance_index) const
{
    const auto *device = get_instance(instance_index);
    if (device == nullptr) {
...
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
    }
    device->set_mode((enum MAV_MOUNT_MODE)_params[instance_index].default_mode.get());
}

// set_mode - sets mount's mode
void AP_Mount::set_mode(uint8_t instance_index, enum MAV_MOUNT_MODE mode)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::set_yaw_lock(uint8_t instance_index, bool yaw_lock)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::set_roll_lock(uint8_t instance_index, bool roll_lock)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::set_pitch_lock(uint8_t instance_index, bool pitch_lock)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::set_angle_target(uint8_t instance_index, float roll_deg, float pitch_deg, float yaw_deg, bool yaw_is_earth_frame)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::set_rate_target(uint8_t instance_index, float roll_degs, float pitch_degs, float yaw_degs, bool yaw_lock)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
{
    auto *device = mount_device_from_mavlink_gimbal_id(packet.z);
...
void AP_Mount::handle_gimbal_manager_set_attitude(const mavlink_message_t &msg)
{
    ...
    auto *device = mount_device_from_mavlink_gimbal_id(packet.gimbal_device_id);
...
bool AP_Mount::get_poi(uint8_t instance_index, Quaternion &quat, Location &loc, Location &poi_loc) const
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
void AP_Mount::set_poi_lock(uint8_t instance_index)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::clear_poi_lock(uint8_t instance_index)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::suspend_poi_lock(uint8_t instance_index)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
bool AP_Mount::get_attitude_quaternion(uint8_t instance_index, Quaternion& att_quat)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
bool AP_Mount::get_attitude_euler(uint8_t instance_index, float& roll_deg, float& pitch_deg, float& yaw_bf_deg)
{
    // re-use get_attitude_quaternion and convert to Euler angles
    Quaternion att_quat;
    if (!get_attitude_quaternion(instance_index, att_quat)) {
        return false;
    }
...
bool AP_Mount::get_rate_target(uint8_t instance_index, float& roll_degs, float& pitch_degs, float& yaw_degs, bool& yaw_is_earth_frame)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
bool AP_Mount::get_angle_target(uint8_t instance_index, float& roll_deg, float& pitch_deg, float& yaw_deg, bool& yaw_is_earth_frame)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
bool AP_Mount::get_location_target(uint8_t instance_index, Location& target_loc)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
void AP_Mount::set_attitude_euler(uint8_t instance_index, float roll_deg, float pitch_deg, float yaw_bf_deg)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::write_log(uint8_t instance_index, uint64_t timestamp_us)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::set_target_sysid(uint8_t instance_index, uint8_t sysid)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::set_roi_target(uint8_t instance_index, const Location &target_loc)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::clear_roi_target(uint8_t instance_index)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
bool AP_Mount::take_picture(uint8_t instance_index)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
bool AP_Mount::record_video(uint8_t instance_index, bool start_recording)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
bool AP_Mount::set_zoom(uint8_t instance_index, ZoomType zoom_type, float zoom_value)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
SetFocusResult AP_Mount::set_focus(uint8_t instance_index, FocusType focus_type, float focus_value)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
...
bool AP_Mount::set_tracking(uint8_t instance_index, TrackingType tracking_type, const Vector2f& p1, const Vector2f& p2)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
bool AP_Mount::set_lens(uint8_t instance_index, uint8_t lens)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
bool AP_Mount::set_camera_source(uint8_t instance_index, uint8_t primary_source, uint8_t secondary_source)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
void AP_Mount::send_camera_information(uint8_t instance_index, mavlink_channel_t chan) const
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::send_camera_settings(uint8_t instance_index, mavlink_channel_t chan) const
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::send_camera_capture_status(uint8_t instance_index, mavlink_channel_t chan) const
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
void AP_Mount::send_camera_thermal_range(uint8_t instance_index, mavlink_channel_t chan) const
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return;
...
bool AP_Mount::change_setting(uint8_t instance_index, CameraSetting setting, float value)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
bool AP_Mount::get_rangefinder_distance(uint8_t instance_index, float& distance_m) const
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
...
bool AP_Mount::set_rangefinder_enable(uint8_t instance_index, bool enable)
{
    auto *device = get_instance(instance_index);
    if (device == nullptr) {
        return false;
