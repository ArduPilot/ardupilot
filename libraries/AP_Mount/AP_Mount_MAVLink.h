...
public:
    ...
    uint8_t get_mavlink_device_id() const override
    {
        return _instance == 0 ? MAV_COMP_ID_GIMBAL : MAV_COMP_ID_GIMBAL2 + (_instance - 1);
    }

    // Preserve the native device identity and attitude frame in cached status.
...
private:
    ...
    uint32_t _last_target_msgid;
    union {
        mavlink_command_int_t location;
