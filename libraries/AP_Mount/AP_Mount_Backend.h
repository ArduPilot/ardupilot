...
protected:
    ...
    public:
        ...
        float pitch;
        float yaw;
        bool pitch_is_ef; //only used by CADDX currently
        bool roll_is_ef; //only used by CADDX currently
        bool yaw_is_ef;
    ...
    enum class Options : uint8_t {
        ...
        FPV_RP_LOCK                     = (1U << 2), // if mount supports, force body frame FPV lock on roll/pitch unconditionally
    };
    ...
    bool _pitch_lock;                // pitch_lock used in RC_TARGETING mode. True if the gimbal's tilt target is maintained in earth-frame, if false (aka "follow") it is maintained in body-frame
    bool _roll_lock;                // roll_lock used in RC_TARGETING mode. True if the gimbal's roll target is maintained in earth-frame, if false (aka "follow") it is maintained in body-frame
