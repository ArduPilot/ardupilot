...
#define AP_SERIALMANAGER_LIMBCH_BUFSIZE_RX        64
#define AP_SERIALMANAGER_LIMBCH_BUFSIZE_TX        64

#include "AP_LIMBCH_Query1.h"
#include <../libraries/AP_HAL/utility/functor.h>
#include <../ArduCopter/GCS_Copter.h>
extern const AP_HAL::HAL& hal;

// constructor
AP_LIMBCH_Query1::AP_LIMBCH_Query1(void)
{
    _port = NULL;
}

// init - perform require initialisation including detecting which protocol to use
void AP_LIMBCH_Query1::init(const AP_SerialManager& serial_manager)
{
    gcs().send_text(MAV_SEVERITY_CRITICAL,
        "port = serial_manager.find_serial(111");
    // check for DEVO_DPort
    if ((_port = serial_manager.find_serial(AP_SerialManager::SerialProtocol_LIMBCH_TEST, 0))) {
        gcs().send_text(MAV_SEVERITY_CRITICAL,
            "port = serial_manager.find_serial(");
        _port->begin(AP_SERIALMANAGER_OPEN_BAUD, AP_SERIALMANAGER_LIMBCH_BUFSIZE_RX, AP_SERIALMANAGER_LIMBCH_BUFSIZE_TX);
        
    }
}

bool AP_LIMBCH_Query1::send_request()
{

    if (_port == NULL){
        return false;
    }
    // gcs().send_text(MAV_SEVERITY_INFO, "send_request1_");
    ...
    // 发送查询指令
    for (int i = 0; i < 3; ++i) {
        gcs().send_text(MAV_SEVERITY_INFO, "send_request1_target_yaw=%u", static_cast<int>(query[i]));
        _port->write(query[i]);
    }
    return true;
...
Query1Msg AP_LIMBCH_Query1::accepting_request(){
...
// 字节 1: 识别码
uint8_t char_length = 0x12;           // 通信协议字符长度（典型CAN总线帧长度）
uint8_t id_code =0x13;
// 字节 2-3: 预留位
uint16_t reserved_2_3 = 1;         // 预留字段值（通常为0，给定为1作为示例）

// 字节 4-5: 存储定义
uint16_t store_def = 2;            // 存储定义（假设为2）

// 字节 6-9: 预留位
uint32_t reserved_6_9 = 5;         // 预留字段值（通常为0，给定为5作为示例）

// 字节 10-11: 发动机运行状态
uint16_t engine_state = 1;         // 发动机状态（假设为1表示正常）

// 字节 12-13: 发动机转速
uint16_t engine_rpm = 1500;        // 发动机转速（假设为1500RPM）

// 字节 14-17: 预留位
uint32_t reserved_14_17 = 7;       // 预留字段值（通常为0，给定为7作为示例）

// 字节 18-19: 距暖车结束时间
uint16_t warmup_time_remaining = 10; // 暖车剩余时间（单位：秒，假设为10秒）

// 字节 20-23: 预留位
uint32_t reserved_20_23 = 3;       // 预留字段值（通常为0，给定为3作为示例）

// 字节 24-25: 怠速位置设置
uint16_t idle_position = 15;       // 怠速位置设置（假设为15）

// 字节 26-27: 干扰脉冲计数
uint16_t interference_count = 2;   // 干扰脉冲计数（假设为2）

// 字节 28-29: 进气量
uint16_t air_intake = 200;         // 进气量（单位：cc，假设为200cc）

// 字节 30-31: 进气压力
uint16_t air_pressure = 1000;      // 进气压力（单位：Pa，假设为1000Pa）

// 字节 32-33: 转速出错计数
uint16_t rpm_error_count = 1;     // 转速出错计数（假设为1）

// 字节 34-35: 喷射时间
uint16_t injection_time = 10;     // 喷射时间（单位：ms，假设为10ms）

// 字节 36-37: 点火提前角
uint16_t ignition_angle = 5;      // 点火提前角（单位：度，假设为5度）

// 字节 38-39: 预留位
uint16_t reserved_38_39 = 0;      // 预留字段值（通常为0）

// 字节 40-41: 节气门位置传感器电压
uint16_t throttle_position_voltage = 500; // 节气门位置传感器电压（单位：mV，假设为500mV）

// 字节 42-43: 电池电量传感器电压
uint16_t battery_voltage = 1200;  // 电池电压（单位：mV，假设为1200mV）

// 字节 44-45: 空燃比传感器电压
uint16_t afr_sensor_voltage = 1100; // 空燃比传感器电压（单位：mV，假设为1100mV）

// 字节 46-47: 发动机温度传感器电压
uint16_t engine_temp_voltage = 1500; // 发动机温度传感器电压（单位：mV，假设为1500mV）

// 字节 48-49: 进气温度传感器电压
uint16_t intake_temp_voltage = 1300; // 进气温度传感器电压（单位：mV，假设为1300mV）

// 字节 50-51: 外界气压传感器电压
uint16_t external_pressure_voltage = 1400; // 外界气压传感器电压（单位：mV，假设为1400mV）

// 字节 52-53: 进气压力传感器电压
uint16_t intake_pressure_voltage = 1250; // 进气压力传感器电压（单位：mV，假设为1250mV）

// 字节 54-55: 进气量传感器电压
uint16_t intake_quantity_voltage = 1450; // 进气量传感器电压（单位：mV，假设为1450mV）

// 字节 56-57: 空燃比
int16_t afr_value = 14;            // 空燃比（假设为14）

// 字节 58-73: 预留位
uint16_t reserved_58_73 = 0;      // 预留字段值（通常为0）

// 字节 74-75: 节气门位置
uint16_t throttle_position = 20;  // 节气门位置（假设为20%）

// 字节 76-77: 发动机温度（冷却液温度 1）
uint16_t engine_cooling_temp = 80; // 发动机冷却液温度（单位：℃，假设为80℃）

// 字节 78-79: 蓄电池电压（ECU 工作电压）
uint16_t ecu_battery_voltage = 1300; // 蓄电池电压（单位：mV，假设为1300mV）

// 字节 80-81: 进气温度
uint16_t intake_temp = 35;        // 进气温度（单位：℃，假设为35℃）

// 字节 82-83: 进气压力
uint16_t intake_pressure = 100;   // 进气压力（单位：kPa，假设为100kPa）

// 字节 84-85: 传感器状况
uint16_t sensor_status = 1;      // 传感器状态（假设为1，表示正常）

// 字节 86: 检验字节
uint8_t checksum = 12;           // 检验字节（可由 CRC 或其他方式动态计算）
