...
#include "AP_LIMBCH_Query2.h"
#include <../libraries/AP_HAL/utility/functor.h>
#include <../ArduCopter/GCS_Copter.h>
extern const AP_HAL::HAL& hal;

// constructor
AP_LIMBCH_Query2::AP_LIMBCH_Query2(void)
{
    _port = NULL;
}

// init - perform require initialisation including detecting which protocol to use
void AP_LIMBCH_Query2::init(const AP_SerialManager& serial_manager)
{
    gcs().send_text(MAV_SEVERITY_CRITICAL,
        "port = serial_manager.find_serial(222");
...
bool AP_LIMBCH_Query2::send_request()
{

    if (_port == NULL){
        return false;
    }
    gcs().send_text(MAV_SEVERITY_INFO, "send_request2_");
    ...
    // 发送查询指令
    for (int i = 0; i < 3; ++i) {
        gcs().send_text(MAV_SEVERITY_INFO, "send_request2_target_yaw=%u", static_cast<int>(query[i]));
        _port->write(query[i]);
    }
    return true;
...
Query2Msg AP_LIMBCH_Query2::accepting_request(){
