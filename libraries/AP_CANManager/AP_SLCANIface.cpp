...
inline void SLCAN::CANIface::addByte(const uint8_t data)
{
    if (_port == nullptr) {
        return;
    }
    if ((data >= 32 && data <= 126)) {  // Normal printable ASCII character
    ...
    } else if (data == '\r') {  // End of command (SLCAN)
    ...
    } else if (data == 8 || data == 127) {  // DEL or BS (backspace)
        if (pos_ > 0) {
            pos_ -= 1;
        }
