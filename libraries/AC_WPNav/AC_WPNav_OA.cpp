...
float AC_WPNav_OA::get_wp_distance_to_destination_cm() const
{
    ...
    return get_horizontal_distance_cm(_pos_control.get_pos_estimate_NEU_cm().tofloat().xy(), _destination_oabak_neu_cm.xy());
}
...
int32_t AC_WPNav_OA::get_wp_bearing_to_destination_cd() const
{
    ...
    return get_bearing_cd(_pos_control.get_pos_estimate_NEU_cm().tofloat().xy(), _destination_oabak_neu_cm.xy());
}
