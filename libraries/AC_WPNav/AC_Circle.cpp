...
void AC_Circle::calc_velocities(bool init_velocity)
{
    ...
    if (_radius <= 0) {
        _angular_vel_max = radians(_rate.get());
        _angular_accel = MAX(fabsf(_angular_vel_max),radians(AC_CIRCLE_ANGULAR_ACCEL_MIN));  // reach maximum yaw velocity in 1 second
    }else{
