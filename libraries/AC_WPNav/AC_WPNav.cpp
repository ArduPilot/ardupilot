...
bool AC_WPNav::advance_wp_target_along_track(float dt)
{
    ...
    if (is_positive(curr_target_vel_neu_cms.length_squared())) {
        ...
        track_dt_scaler = constrain_float(0.05f + (track_velocity_neu_cms - _pos_control.get_pos_NE_p().kP() * track_error_neu_cm) / curr_target_vel_neu_cms.length(), 0.0f, 1.0f);
    }

    // Use vel_dt_scaler to slow down the trajectory time
    // vel_dt_scaler scales the velocity and acceleration to be kinematically consistent
    float vel_dt_scaler = 1.0;
    if (is_positive(_wp_desired_speed_ne_cms)) {
        ...
        vel_dt_scaler = _offset_vel_cms / _wp_desired_speed_ne_cms;
    }
    ...
    _track_dt_scalar += (track_dt_scaler - _track_dt_scalar) * (dt / track_dt_scalar_tc);
    ...
    if (!_this_leg_is_spline) {
        ...
        s_finished = _scurve_this_leg.advance_target_along_track(_scurve_prev_leg, _scurve_next_leg, _wp_radius_cm, get_corner_acceleration_cmss(), _flags.fast_waypoint, _track_dt_scalar * vel_dt_scaler * dt, target_pos_neu_cm, target_vel_neu_cms, target_accel_neu_cmss);
    } else {
        // splinetarget_vel
        target_vel_neu_cms = curr_target_vel_neu_cms;
        _spline_this_leg.advance_target_along_track(_track_dt_scalar * vel_dt_scaler * dt, target_pos_neu_cm, target_vel_neu_cms);
    ...
    target_vel_neu_cms *= vel_dt_scaler;
    target_accel_neu_cmss *= sq(vel_dt_scaler);
...
/// get_wp_bearing_to_destination_cd - get bearing to next waypoint in centi-degrees
float AC_WPNav::get_bearing_to_target_rad() const
{
