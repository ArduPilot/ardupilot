    ...
    if (!_flags.reached_destination) {
        if (s_finished) {
            ...
            if (_flags.fast_waypoint) {
                ...
                const Vector3f dist_to_dest_m = curr_pos_neu_cm - _destination_neu_cm;
                if (dist_to_dest_m.length_squared() <= sq(_wp_radius_cm)) {
                    _flags.reached_destination = true;
                }
            }
        }
...
bool AC_WPNav::get_vector_NEU_cm(const Location &loc, Vector3f &pos_from_origin_neu_cm, bool &is_terrain_alt)
{
    ...
    if (loc.get_alt_frame() == Location::AltFrame::ABOVE_TERRAIN) {
        int32_t temp_alt_cm;
        if (!loc.get_alt_cm(Location::AltFrame::ABOVE_TERRAIN, temp_alt_cm)) {
            return false;
        }
        pos_from_origin_neu_cm.z = temp_alt_cm;
        is_terrain_alt = true;
    } else {
        is_terrain_alt = false;
        int32_t temp_alt_cm;
        if (!loc.get_alt_cm(Location::AltFrame::ABOVE_ORIGIN, temp_alt_cm)) {
            return false;
        }
        pos_from_origin_neu_cm.z = temp_alt_cm;
        is_terrain_alt = false;
