...
public:
    ...
    // Parse from the main thread too: stamps are set at parse time, so with
    // only the reader thread parsing, healthy() measures its scheduling, not
    // data freshness. Same as other backends; no-op where reads are owner-gated.
...
private:
    ...
    struct AeronDeferredMsgs {
        bool     crc_fail;
        bool     bad_length;    // recognised ID but payload too short for it
        bool     unknown_pkt;   // unrecognised packet ID (unknown_id is valid)
        bool     ext_mag_checked;
        bool     ext_asp_checked;
        bool     stale_data;
        uint16_t unknown_id;    // only meaningful when unknown_pkt is set
    };
    ...
    // accessors get_filter_status()/report_hw_status()/get_variances() on
    // other threads).
    //
    // Lock ordering: the frontend `state` struct has its own lock
    // (state.sem, owned by the base class). These two locks CAN nest - the
    // frontend holds state.sem while calling get_filter_status(), which
    // then takes arn_sem. To avoid an AB-BA deadlock the order is fixed:
    //   state.sem is the OUTER lock, arn_sem is the INNER lock.
    // i.e. code may take arn_sem while holding state.sem, but must never
    // take state.sem while holding arn_sem. The publish_* handlers respect
    ...
    // beneath it. The main-thread caller via AP_AHRS::update() already holds
    // AP_AHRS::_rsem; the one via AP_InertialSensor::wait_for_sample() holds
    // nothing, so this is the outermost lock only on that path.
    ...
    // Single source of truth for decoding the PLX GPS status word into an
    // ArduPilot fix type, so the AP_GPS publication path and
    // get_filter_status() cannot reach different conclusions from the same
    // status word.
    static AP_GPS_FixType fix_type_from_status(uint32_t status);

    // EXTD_GNSS freshness. Caller must hold arn_sem.
    bool extd_fresh_locked() const;

    // utility
    void send_deferred_messages(const AeronDeferredMsgs &msgs);
    // Sends only the requested queries. Returns false if a request could
    // not be written to the UART in full.
    bool query_ext_sensors(bool want_mag, bool want_asp);
    ...
    // Bounded retry for the external-sensor presence queries. The PLX answers
    // whether or not the sensor is fitted, so an unanswered query means the
    // request or its reply was lost rather than that the sensor is absent.
    // Retry the outstanding ones a few times, then stop: firmware that does
    // not implement the query would otherwise retry forever. Giving up leaves
    // the *_checked flags false, which keeps EXT_MAG/EXT_ASP unreportable in
    // report_hw_status() - the safe default.
    static constexpr uint32_t EXT_QUERY_RETRY_MS  = 2000;
    static constexpr uint8_t  EXT_QUERY_MAX_TRIES = 5;
    uint32_t last_ext_query_ms;
    uint8_t  ext_query_tries;
    ...
    // last_warned_ms is indexed by table ROW (0..HW_STATUS_COUNT-1), so its
    // size tracks the number of table entries and does not depend on the
    // enum bit values. A static_assert in the .cpp keeps this in sync with
    // ARRAY_SIZE(hw_status_table).
    static constexpr uint8_t HW_STATUS_COUNT = 12;
    uint32_t last_warned_stat;
    uint32_t last_warned_ms[HW_STATUS_COUNT];

    // hw_status bits that mean the device cannot be trusted to navigate.
    // Manual 7.1.2.9 Table 15 rates all of these High severity and PBIT:
    // they are power-on self-test failures with no recovery path, which is
    // why hw_status_table gives the first three no clear_msg. Config-memory
    // failure in particular leaves the device streaming fresh packets from a
    // wrong configuration, which no staleness check can catch.
    //
    // ACC and GYR are included because the PLX's own fused solution is built
    // on them: a unit reporting a non-functional accelerometer or gyro is no
    // more trustworthy fused than raw. Being PBIT, they are set at power-on
    // rather than mid-flight, so this fails pre-arm rather than dropping the
    // AHRS in the air. MAG and BARO are deliberately absent - the PLX
    // navigates without them, so those are gated at the publication site
    // instead (see publish_sens_para).
    static constexpr uint32_t HW_STATUS_FATAL_MASK =
          (1U << uint8_t(AeronHwStatus::RAM))
        | (1U << uint8_t(AeronHwStatus::FIRMWARE))
        | (1U << uint8_t(AeronHwStatus::CONFIG))
        | (1U << uint8_t(AeronHwStatus::ACC))
        | (1U << uint8_t(AeronHwStatus::GYR));
    ...
    static constexpr uint32_t DEVICE_RESET_GAP_MS = 500;
    ...
    // bits 0-9 the visible-satellite count and bits 11-14 the fix type
    // code (manual 7.1.2.10, Table 16).
    static constexpr uint32_t GPS_STATUS_FIX_BIT       = 0x00000400U;
    static constexpr uint32_t GPS_STATUS_NUMSAT_MASK   = 0x000003FFU;
    static constexpr uint32_t GPS_STATUS_FIXTYPE_MASK  = 0x00007800U;
    static constexpr uint8_t  GPS_STATUS_FIXTYPE_SHIFT = 11;
};
