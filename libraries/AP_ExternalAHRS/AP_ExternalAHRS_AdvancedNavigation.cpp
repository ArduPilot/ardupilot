...
#define AN_AIRSPEED_AIDING_FREQUENCY 20           // Hz
...
struct PACKED AN_RawGnss
{
    ...
    void get_gps_message(AP_ExternalAHRS::gps_data_message_t &packet, const AN_Satellites *satellites, const AN_VelocityStandardDeviation *standard_deviation)
    {
        ...
        {
            packet.has_yaw = true;
            packet.yaw = degrees(this->heading);
            packet.yaw = degrees(this->heading_sd);
        }
    }
...
/*
    ANPP External Air Data Packet
    
    https://docs.advancednavigation.com/certus/ANPP/ExtAirDataPacket.htm#External_Air_Data_Packet
*/
struct PACKED AN_ExternalAirData
{
    float baro_delay;
    ...
    enum AN_ExternalAirDataFlags
    {
        BAROMETRIC_ALTITUDE_VALID           = 1 << 0,   // 1 bit (barometric_altitude_set_valid)
        AIRSPEED_VALID                      = 1 << 1,   // 1 bit (airspeed_set_valid)
        BAROMETRIC_ALTITUDE_REFERENCE_RESET = 1 << 2,   // 1 bit (barometric_altitude_reference_reset)
    };

    AN_ExternalAirData() {};
    
    AN_ExternalAirData(AP_DAL &dal, float airspeed_error) 
    {
        generate(dal, airspeed_error);
    }

    bool generate(AP_DAL &dal, float airspeed_error)
    {
        auto now = AP_HAL::millis();
        auto airspeed = dal.airspeed();
        auto barometer = dal.baro();
        ...
        if(!airspeed->healthy() || !barometer.healthy(barometer.get_primary()))
        {
            return false;
        }
        
        auto speed = airspeed->get_airspeed();
        
        // estimte of pressure error from the airspeed error at 20m/s
        auto pressure_error = (0.5 * (sq(20 + airspeed_error))) - (0.5 * sq(20));
        
        // estimate of airspeed error based on the pressure error at 20m/s
        float nominal_pressure = 0.5 * sq(speed);
        float adjusted_airspeed_error = sqrt(2 * nominal_pressure + pressure_error) - speed;
        
        this->airspeed_standard_deviation = adjusted_airspeed_error;
        this->barometric_standard_deviation = pressure_error;
        this->true_airspeed = speed * dal.get_EAS2TAS();
        this->airspeed_delay = (now - airspeed->last_update_ms()) / 1000;
        this->barometric_altitude = barometer.get_altitude();
        this->baro_delay = (now - barometer.get_last_update()) / 1000;
        this->flags = BAROMETRIC_ALTITUDE_VALID & AIRSPEED_VALID;
...
static const char *get_device_name(uint8_t id)
{
    return get_device_name(static_cast<AN_DeviceInfo::AN_DeviceId>(id));
}

static const char* get_packet_title(AN_PacketId id)
{
    switch (id)
    {
        case AN_ACKNOWLEDGE_ID: return "Acknowledge";
        case AN_REQUEST_ID: return "Request";
        case AN_BOOT_MODE_ID: return "Boot Mode";
        case AN_DEVICE_INFORMATION_ID: return "Device Information";
        case AN_RESTORE_FACTORY_SETTINGS_ID: return "Restore Factory Settings";
        case AN_RESET_ID: return "Reset";
        case AN_RESERVED_6_ID: return "Reserved 6";
        case AN_FILE_TRANSFER_REQUEST_ID: return "File Transfer Request";
        case AN_FILE_TRANSFER_ACKNOWLEDGE_ID: return "File Transfer Acknowledge";
        case AN_FILE_TRANSFER_ID: return "File Transfer";
        case AN_SERIAL_PORT_PASSTHROUGH_ID: return "Serial Port Passthrough";
        case AN_IP_CONFIGURATION_ID: return "IP Configuration";
        case AN_RESERVED_12_ID: return "Reserved 12";
        case AN_EXTENDED_DEVICE_INFORMATION_ID: return "Extended Device Information";
        case AN_SUBCOMPONENT_INFORMATION_ID: return "Subcomponent Information";
        case AN_END_SYSTEM_PACKETS: return "End System Packets";

        case AN_SYSTEM_STATE_ID: return "System State";
        case AN_UNIX_TIME_ID: return "Unix Time";
        case AN_FORMATTED_TIME_ID: return "Formatted Time";
        case AN_STATUS_ID: return "Status";
        case AN_POSITION_STANDARD_DEVIATION_ID: return "Position Standard Deviation";
        case AN_VELOCITY_STANDARD_DEVIATION_ID: return "Velocity Standard Deviation";
        case AN_EULER_ORIENTATION_STANDARD_DEVIATION_ID: return "Euler Orientation Standard Deviation";
        case AN_QUATERNION_ORIENTATION_STANDARD_DEVIATION_ID: return "Quaternion Orientation Standard Deviation";
        case AN_RAW_SENSORS_ID: return "Raw Sensors";
        case AN_RAW_GNSS_ID: return "Raw GNSS";
        case AN_SATELLITES_ID: return "Satellites";
        case AN_SATELLITES_DETAILED_ID: return "Satellites Detailed";
        case AN_GEODETIC_POSITION_ID: return "Geodetic Position";
        case AN_ECEF_POSITION_ID: return "ECEF Position";
        case AN_UTM_POSITION_ID: return "UTM Position";
        case AN_NED_VELOCITY_ID: return "NED Velocity";
        case AN_BODY_VELOCITY_ID: return "Body Velocity";
        case AN_ACCELERATION_ID: return "Acceleration";
        case AN_BODY_ACCELERATION_ID: return "Body Acceleration";
        case AN_EULER_ORIENTATION_ID: return "Euler Orientation";
        case AN_QUATERNION_ORIENTATION_ID: return "Quaternion Orientation";
        case AN_DCM_ORIENTATION_ID: return "DCM Orientation";
        case AN_ANGULAR_VELOCITY_ID: return "Angular Velocity";
        case AN_ANGULAR_ACCELERATION_ID: return "Angular Acceleration";
        case AN_EXTERNAL_POSITION_VELOCITY_ID: return "External Position Velocity";
        case AN_EXTERNAL_POSITION_ID: return "External Position";
        case AN_EXTERNAL_VELOCITY_ID: return "External Velocity";
        case AN_EXTERNAL_BODY_VELOCITY_ID: return "External Body Velocity";
        case AN_EXTERNAL_HEADING_ID: return "External Heading";
        case AN_RUNNING_TIME_ID: return "Running Time";
        case AN_LOCAL_MAGNETICS_ID: return "Local Magnetics";
        case AN_ODOMETER_STATE_ID: return "Odometer State";
        case AN_EXTERNAL_TIME_ID: return "External Time";
        case AN_EXTERNAL_DEPTH_ID: return "External Depth";
        case AN_GEOID_HEIGHT_ID: return "Geoid Height";
        case AN_RTCM_CORRECTIONS_ID: return "RTCM Corrections";
        case AN_RESERVED_56_ID: return "Reserved 56";
        case AN_WIND_ID: return "Wind";
        case AN_HEAVE_ID: return "Heave";
        case AN_RESERVED_59_ID: return "Reserved 59";
        case AN_RAW_SATELLITE_DATA_ID: return "Raw Satellite Data";
        case AN_RAW_SATELLITE_EPHEMERIS_ID: return "Raw Satellite Ephemeris";
        case AN_RESERVED_62_ID: return "Reserved 62";
        case AN_RESERVED_63_ID: return "Reserved 63";
        case AN_RESERVED_64_ID: return "Reserved 64";
        case AN_RESERVED_65_ID: return "Reserved 65";
        case AN_GNSS_SUMMARY_ID: return "GNSS Summary";
        case AN_EXTERNAL_ODOMETER_ID: return "External Odometer";
        case AN_EXTERNAL_AIR_DATA_ID: return "External Air Data";
        case AN_GNSS_RECEIVER_INFORMATION_ID: return "GNSS Receiver Information";
        case AN_RAW_DVL_DATA_ID: return "Raw DVL Data";
        case AN_NORTH_SEEKING_STATUS_ID: return "North Seeking Status";
        case AN_GIMBAL_STATE_ID: return "Gimbal State";
        case AN_AUTOMOTIVE_ID: return "Automotive";
        case AN_RESERVED_74_ID: return "Reserved 74";
        case AN_EXTERNAL_MAGNETOMETERS_ID: return "External Magnetometers";
        case AN_RESERVED_76_ID: return "Reserved 76";
        case AN_RESERVED_77_ID: return "Reserved 77";
        case AN_RESERVED_78_ID: return "Reserved 78";
        case AN_RESERVED_79_ID: return "Reserved 79";
        case AN_BASESTATION_ID: return "Basestation";
        case AN_RESERVED_81_ID: return "Reserved 81";
        case AN_RESERVED_82_ID: return "Reserved 82";
        case AN_ZERO_ANGULAR_VELOCITY_ID: return "Zero Angular Velocity";
        case AN_EXTENDED_SATELLITES_ID: return "Extended Satellites";
        case AN_SENSOR_TEMPERATURES_ID: return "Sensor Temperatures";
        case AN_SYSTEM_TEMPERATURE_ID: return "System Temperature";
        case AN_RESERVED_87_ID: return "Reserved 87";
        case AN_END_STATE_PACKETS: return "End State Packets";

        case AN_PACKET_TIMER_PERIOD_ID: return "Packet Timer Period";
        case AN_PACKET_PERIODS_ID: return "Packet Periods";
        case AN_BAUD_RATES_ID: return "Baud Rates";
        case AN_RESERVED_183_ID: return "Reserved 183";
        case AN_SENSOR_RANGES_ID: return "Sensor Ranges";
        case AN_INSTALLATION_ALIGNMENT_ID: return "Installation Alignment";
        case AN_FILTER_OPTIONS_ID: return "Filter Options";
        case AN_RESERVED_187_ID: return "Reserved 187";
        case AN_GPIO_CONFIGURATION_ID: return "GPIO Configuration";
        case AN_MAGNETIC_CALIBRATION_VALUES_ID: return "Magnetic Calibration Values";
        case AN_MAGNETIC_CALIBRATION_CONFIGURATION_ID: return "Magnetic Calibration Configuration";
        case AN_MAGNETIC_CALIBRATION_STATUS_ID: return "Magnetic Calibration Status";
        case AN_ODOMETER_CONFIGURATION_ID: return "Odometer Configuration";
        case AN_ZERO_ALIGNMENT_ID: return "Zero Alignment";
        case AN_REFERENCE_OFFSETS_ID: return "Reference Offsets";
        case AN_GPIO_OUTPUT_CONFIGURATION_ID: return "GPIO Output Configuration";
        case AN_DUAL_ANTENNA_CONFIGURATION_ID: return "Dual Antenna Configuration";
        case AN_GNSS_CONFIGURATION_ID: return "GNSS Configuration";
        case AN_USER_DATA_ID: return "User Data";
        case AN_GPIO_INPUT_CONFIGURATION_ID: return "GPIO Input Configuration";
        case AN_RESERVED_200_ID: return "Reserved 200";
        case AN_RESERVED_201_ID: return "Reserved 201";
        case AN_IP_DATAPORTS_CONFIGURATION_ID: return "IP Dataports Configuration";
        case AN_CAN_CONFIGURATION_ID: return "CAN Configuration";
        case AN_DEVICE_NAME_ID: return "Device Name";
        case AN_END_CONFIGURATION_PACKETS: return "End Configuration Packets";

        default: return "Unknown Packet ID";
    }
}
...
// return the Advanced Navigation device name
const char *AP_ExternalAHRS_AdvancedNavigation::get_name() const
{
    ...
    return get_device_name(_device_id);
}
...
void AP_ExternalAHRS_AdvancedNavigation::update_thread(void)
{
    ...
    while (true)
    {
        ...
        if (init && option_is_set(AP_ExternalAHRS::OPTIONS::ARSP_AID))
        {
...
bool AP_ExternalAHRS_AdvancedNavigation::configure_device(void)
{
    ...
    if ((!_rates_set || _current_rate != get_rate()) && (!_last_pkt_rate_message_sent_ms || now - _last_pkt_rate_message_sent_ms > AN_DEVICE_PACKET_RATE_UPDATE_PERIOD))
    {
        ...
        if(!send_packet_period_request(5 
                            , AN_SYSTEM_STATE_ID, (uint32_t)1.0e3 / _current_rate 
                            , AN_VELOCITY_STANDARD_DEVIATION_ID, (uint32_t)1.0e3 / _current_rate
                            , AN_RAW_SENSORS_ID, (uint32_t)1.0e3 / _current_rate
                            , AN_RAW_GNSS_ID , (uint32_t)1.0e3 / AN_GNSS_PACKET_RATE
                            , AN_SATELLITES_ID, (uint32_t)1.0e3 / AN_GNSS_PACKET_RATE
                            ))
        {
...
// transmit the airspeed aiding packet at the required rate
bool AP_ExternalAHRS_AdvancedNavigation::send_airspeed_aiding()
{

    uint32_t now = AP_HAL::millis();
    
    if (now < _last_ext_air_data_sent_ms + 1000 / AN_AIRSPEED_AIDING_FREQUENCY)
    {
        return true;
    }
    
    AN_ExternalAirData airdata_packet;
    
    if(!airdata_packet.generate(dal, airspeed_err_20ms()))
    {
        return false;
    }
    
    AN_Packet message(AN_EXTERNAL_AIR_DATA_ID, (uint8_t *)&airdata_packet, sizeof(AN_ExternalAirData));
...
{
    auto now = AP_HAL::millis();
    const auto *ptr = reinterpret_cast<const AN_SystemState *>(&packet.payload);
    auto system_state = *ptr;
...
void AP_ExternalAHRS_AdvancedNavigation::handle_acknowledge_packet()
{
    ...
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, 
        "External AHRS: Acknowledged packet %s [%d] - Result: %s", 
        get_packet_title(acknowledge->id_acknowledged),
        acknowledge->id_acknowledged,
        acknowledge->get_result_pretty());
...
void AP_ExternalAHRS_AdvancedNavigation::handle_device_info_packet()
{
    ...
    const auto *device_info = reinterpret_cast<const AN_DeviceInfo *>(&packet.payload);
...
void AP_ExternalAHRS_AdvancedNavigation::handle_raw_sensors_packet()
{
    ...
    const auto *ptr = reinterpret_cast<const AN_RawSensors *>(&packet.payload);
    auto raw_sensors = *ptr;
...
void AP_ExternalAHRS_AdvancedNavigation::handle_raw_gnss_packet()
{
    ...
    const auto *satellites = reinterpret_cast<const AN_Satellites *>(&last_satellites_packet.payload);
