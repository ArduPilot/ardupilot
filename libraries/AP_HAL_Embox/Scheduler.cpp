...
#define APM_EMBOX_MAX_PRIORITY 20
#define APM_EMBOX_TIMER_PRIORITY 15
#define APM_EMBOX_UART_PRIORITY 14
#define APM_EMBOX_NET_PRIORITY 14
#define APM_EMBOX_RCIN_PRIORITY 13
#define APM_EMBOX_MAIN_PRIORITY 12
#define APM_EMBOX_IO_PRIORITY 10
#define APM_EMBOX_SCRIPTING_PRIORITY 1

#define APM_EMBOX_TIMER_RATE 1000
#define APM_EMBOX_UART_RATE 100

#define APM_EMBOX_RCIN_RATE 100
#define APM_EMBOX_IO_RATE 50
...
#define SCHED_THREAD(name_, UPPER_NAME_)            \
    {                                               \
        ...
        .policy = SCHED_FIFO,                       \
        .prio = APM_EMBOX_##UPPER_NAME_##_PRIORITY, \
        .rate = APM_EMBOX_##UPPER_NAME_##_RATE,     \
    }
...
void Scheduler::init_realtime() {
    ...
    struct sched_param param = {.sched_priority = APM_EMBOX_MAIN_PRIORITY};
...
void Scheduler::register_timer_process(AP_HAL::MemberProc proc) {
    ...
    if (_num_timer_procs >= EMBOX_SCHEDULER_MAX_TIMER_PROCS) {
...
void Scheduler::register_io_process(AP_HAL::MemberProc proc) {
    ...
    if (_num_io_procs < EMBOX_SCHEDULER_MAX_IO_PROCS) {
...
uint8_t Scheduler::calculate_thread_priority(priority_base base, int8_t priority) const {
    uint8_t thread_priority = APM_EMBOX_IO_PRIORITY;
    static const struct {
        priority_base base;
        uint8_t p;
    } priority_map[] = {
        {PRIORITY_BOOST, APM_EMBOX_MAIN_PRIORITY},
        {PRIORITY_MAIN, APM_EMBOX_MAIN_PRIORITY},
        {PRIORITY_SPI, AP_EMBOX_SENSORS_SCHED_PRIO},
        {PRIORITY_I2C, AP_EMBOX_SENSORS_SCHED_PRIO},
        {PRIORITY_CAN, APM_EMBOX_TIMER_PRIORITY},
        {PRIORITY_TIMER, APM_EMBOX_TIMER_PRIORITY},
        {PRIORITY_RCIN, APM_EMBOX_RCIN_PRIORITY},
        {PRIORITY_IO, APM_EMBOX_IO_PRIORITY},
        {PRIORITY_UART, APM_EMBOX_UART_PRIORITY},
        {PRIORITY_STORAGE, APM_EMBOX_IO_PRIORITY},
        {PRIORITY_SCRIPTING, APM_EMBOX_SCRIPTING_PRIORITY},
        {PRIORITY_NET, APM_EMBOX_NET_PRIORITY},
    };
    for (uint8_t i = 0; i < ARRAY_SIZE(priority_map); i++) {
        if (priority_map[i].base == base) {
            thread_priority = constrain_int16(priority_map[i].p + priority, 1, APM_EMBOX_MAX_PRIORITY);
            break;
        }
    }
