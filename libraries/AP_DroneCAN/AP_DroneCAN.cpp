...
void AP_DroneCAN::SRV_push_servos()
{
    ...
    for (uint8_t i = 0; i < DRONECAN_SRV_NUMBER; i++) {
        ...
        if (SRV_Channels::channel_function(i) >= SRV_Channel::k_none) {
            ...
            // Flag channels which are none zero
            if (_SRV_conf[i].pulse > 0) {
                none_zero_channels |= 1U << i;
            }
        }
    }

    uint32_t servo_armed_mask = _servo_bm & none_zero_channels;
    uint32_t esc_armed_mask = _esc_bm & none_zero_channels;
