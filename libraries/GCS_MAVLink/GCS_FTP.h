...
private:
    ...
    ObjectBuffer<Request> requests{AP_MAVLINK_FTP_MAX_SESSIONS};
    ...
    public:
        ...
        bool check_name_len(const Request &request);
        ...
        void list_dir(Request &request, Request &response);
        void push_reply(Request &reply);
        bool handle_request(Request &request, Request &reply);
    ...
    static bool send_reply(const Request &reply);
    static void error(Request &response, FTP_ERROR error);

    /*
      setup reply packet to reply to the request
     */
    void setup_reply(const Request &request, Request &reply);
