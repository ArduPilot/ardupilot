...
void GCS_MAVLINK::handle_follow_target(const mavlink_message_t &msg)
{
    ...
    Location loc {
        packet.lat,
        packet.lon,
        int32_t(packet.alt * 100), // m to cm
        Location::AltFrame::ABSOLUTE
    };
    ...
    const uint8_t populated_fields = (uint8_t)AP_LocationDB_Item::DataField::POS |
        ((uint8_t)AP_LocationDB_Item::DataField::VEL & (packet.est_capabilities & (1<<1))) |
        ((uint8_t)AP_LocationDB_Item::DataField::HEADING & (packet.est_capabilities & (1<<3)));

    AP_LocationDB_Item item(key, AP_HAL::millis(), pos, vel, Vector3f{}, heading, 0, populated_fields);
    db->add_item(item);
}
#endif // AP_LOCATIONDB_KEYDOMAIN_MAVLINK_ENABLED
