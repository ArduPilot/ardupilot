...
void GCS::setup_uarts()
{
    ...
    for (uint16_t i = 1; i <= UINT8_MAX; i++) {  // we really can't have this many serial connections, right?!
        AP_HAL::UARTDriver *uart = AP::serialmanager().find_serial(AP_SerialManager::SerialProtocol_MAVLink, uint8_t(i));
        if (uart == nullptr) {
