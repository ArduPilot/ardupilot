...
typedef struct __mavlink_gps_accuracy_t
{
 ...
 uint16_t ekf_check_mask; /*< Which fields pass EKF checks*/
 uint8_t instance; /*< Which instance of GPS we're reporting on*/
} mavlink_gps_accuracy_t;

#define MAVLINK_MSG_ID_GPS_ACCURACY_LEN 23
#define MAVLINK_MSG_ID_225_LEN 23

#define MAVLINK_MSG_ID_GPS_ACCURACY_CRC 211
#define MAVLINK_MSG_ID_225_CRC 211
...
#define MAVLINK_MESSAGE_INFO_GPS_ACCURACY { \
	...
	{  { "h_acc", NULL, MAVLINK_TYPE_FLOAT, 0, 0, offsetof(mavlink_gps_accuracy_t, h_acc) }, \
         ...
         { "ekf_check_mask", NULL, MAVLINK_TYPE_UINT16_T, 0, 20, offsetof(mavlink_gps_accuracy_t, ekf_check_mask) }, \
         { "instance", NULL, MAVLINK_TYPE_UINT8_T, 0, 22, offsetof(mavlink_gps_accuracy_t, instance) }, \
         } \
}
...
static inline uint16_t mavlink_msg_gps_accuracy_pack(uint8_t system_id, uint8_t component_id, mavlink_message_t* msg,
						       uint8_t instance, float h_acc, float s_acc, float h_vel_filt, float v_vel_filt, float p_drift, uint16_t ekf_check_mask)
{
	...
	_mav_put_uint16_t(buf, 20, ekf_check_mask);
	_mav_put_uint8_t(buf, 22, instance);
...
static inline void mavlink_msg_gps_accuracy_send(mavlink_channel_t chan, uint8_t instance, float h_acc, float s_acc, float h_vel_filt, float v_vel_filt, float p_drift, uint16_t ekf_check_mask)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
	char buf[MAVLINK_MSG_ID_GPS_ACCURACY_LEN];
	_mav_put_float(buf, 0, h_acc);
 ...
 */
static inline void mavlink_msg_gps_accuracy_send_buf(mavlink_message_t *msgbuf, mavlink_channel_t chan,  uint8_t instance, float h_acc, float s_acc, float h_vel_filt, float v_vel_filt, float p_drift, uint16_t ekf_check_mask)
{
#if MAVLINK_NEED_BYTE_SWAP || !MAVLINK_ALIGNED_FIELDS
 ...
 */
static inline uint16_t mavlink_msg_gps_accuracy_get_ekf_check_mask(const mavlink_message_t* msg)
{
