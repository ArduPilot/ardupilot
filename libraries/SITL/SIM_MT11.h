...
protected:
    friend class MT11_RTSPServer;
    static uint64_t next_frame_time_us(uint64_t previous_us, uint64_t now_us);
...
private:
    ...
    void get_camera_definition(mavlink_camera_information_t &info) const override;

    void handle_ftp(const mavlink_message_t &msg);
    struct {
        bool open;
        uint8_t session;
        uint32_t sysid;
