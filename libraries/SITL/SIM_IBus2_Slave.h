...
// Concrete subclass: responds to Frame 2 queries with synthetic telemetry (Frame 3)
class IBus2SlaveDevice : public IBus2Slave {
public:
    // dev_idx_default sets the default value of the DEV param so that each
    // array element in SITL.h has a distinct default without runtime config.
    explicit IBus2SlaveDevice(uint8_t dev_idx_default = 0) : _dev_idx_default(dev_idx_default) {}

    static const AP_Param::GroupInfo var_info[];

    AP_Int8 _dev_idx;  // device-name suffix; create_serial_sim matches "ibus2slaveN"

private:
    const float _dev_idx_default;
