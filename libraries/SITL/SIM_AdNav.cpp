...
void AdNav::update()
{
    ...
    if (now - _last_geoid_sent_us >= 1000000) {
        _last_geoid_sent_us = now;
        send_geoid_height_pkt(); // Send ANPP.54
    }
}
...
void AdNav::send_state_pkt()
{
    ...
    system_state_packet.height = fdm.altitude + AN_SIM_GEOID_HEIGHT;
...
void AdNav::send_raw_gnss_pkt()
{
    ...
    raw_gnss.position[2] = fdm.altitude + AN_SIM_GEOID_HEIGHT;
...
/*
    Function to send a Geoid Height Packet.
 */
void AdNav::send_geoid_height_pkt()
{
    geoid_height_packet_t geoid_height_pkt;
    memset(&geoid_height_pkt, 0, sizeof(geoid_height_pkt));

    geoid_height_pkt.geoid_height = AN_SIM_GEOID_HEIGHT;

    send_packet(encode_geoid_height_packet(&geoid_height_pkt));
}
...
/*
    Function to encode a Geoid Height Packet into an an_packet structure.
 */
an_packet_t* AdNav::encode_geoid_height_packet(geoid_height_packet_t* geoid_height_packet)
{
    an_packet_t* an_packet = an_packet_allocate(4, AN_PACKET_ID_GEOID_HEIGHT);
    if (an_packet != NULL) {
        memcpy(&an_packet->data[0], &geoid_height_packet->geoid_height, sizeof(float));
    }
    return an_packet;
}
    ...
    if (an_packet->id == AN_PACKET_ID_PACKET_PERIODS && (an_packet->length - 2) % 5 == 0) {
        ...
        for (i = 0; i < AN_MAXIMUM_PACKET_PERIODS; i++) {
            if (i < packet_periods_count) {
                ...
                memset(&packet_periods_packet->packet_periods[i], 0, sizeof(packet_period_t));
            }
        }
        return 0;
    } else {
