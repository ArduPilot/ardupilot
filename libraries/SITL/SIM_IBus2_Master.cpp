...
// Encoder full-scale.  SES_UnpackChannels uses raw_mag = sign_bit/2 = 512 as the
// reference for ±512 µs full deflection (not mag_mask = 1023).
// SIM_MAX_SES = (512 × SIM_FACTOR + 0x8000) >> 16 = 16777.
static const uint32_t SIM_MAX_SES  = 16777U;
...
#else
    const bool have_udp = false;
#endif  // AP_RCPROTOCOL_UDP_ENABLED

    const uint16_t *src = have_udp ? udp_channels : sim_channels;
    const uint8_t   n   = have_udp ? udp_count    : (uint8_t)AP_IBUS2_MAX_CHANNELS;

    // Deliver each resource the slave's latest GET_TYPE response still
    // requests; a conforming device clears the request bit once delivered,
    // so these stop after a cycle or two (a non-conforming device keeps
    // getting them forever, matching real receiver behaviour).
    if (_have_dev_resources) {
        if (_dev_resources & 0x01) {
            send_frame1_subtype1(src, n);
        }
        if (_dev_resources & 0x02) {
            send_frame1_subtype2(n);
        }
    }
...
void IBus2Master::send_frame1_subtype2(uint8_t n)
{
    // Failsafe (runaway protection) values: SES-encoded like subtype=0.
    // Encode the keep-last-output sentinel (raw == sign bit) per channel.
    ...
    uint16_t bit_pos = 0;
    for (uint8_t i = 0; i < n; i++) {
        ses_write_bits(payload, bit_pos, 1U << (SIM_NB_BITS - 1), SIM_NB_BITS);
    }
...
{
    // Real receivers open with a burst of RESET commands at power-up
    // (a TMr sends ~10 in the first 25ms); devices must ignore them.
    // Then, before we have a GET_TYPE response, keep sending GET_TYPE so
    // the slave can tell us what data it needs (channels_types, failsafe).
    // Once connected, cycle GET_VALUE for telemetry.
    IBUS2Cmd cmd;
    if (_boot_resets_sent < BOOT_RESET_COUNT) {
        _boot_resets_sent++;
        cmd = IBUS2Cmd::RESET;
    } else if (!_have_dev_resources || _dev_resources != 0) {
        ...
        if (_rx_len == IBUS2_FRAME3_SIZE) {
            if (sitl_ibus2_crc8_ok(_rx_buf, IBUS2_FRAME3_SIZE)) {
                ...
                case IBUS2Cmd::GET_TYPE: {
                    ...
                    // Track the outstanding resources from every response:
                    // sensor polling begins only when the device reports zero.
                    _have_dev_resources = true;
