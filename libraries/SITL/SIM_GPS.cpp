...
void GPS::simulate_jamming(struct GPS_Data &d)
{
    ...
    if (now_ms - jam.jam_start_ms < jam.initial_loss_duration_ms) {
        ...
        if (now_ms >= jam.last_sats_change_ms) {
            ...
            jam.last_sats_change_ms = now_ms + rand_delay_ms(sats_change_hz);
        }
        if (now_ms >= jam.last_vz_change_ms) {
            d.speedD = rand_float() * 400;
            jam.last_vz_change_ms = now_ms + rand_delay_ms(vz_change_hz);
        }
        if (now_ms >= jam.last_vel_change_ms) {
            d.speedN = rand_float() * 400;
            d.speedE = rand_float() * 400;
            jam.last_vel_change_ms = now_ms + rand_delay_ms(vel_change_hz);
        }
        if (now_ms >= jam.last_pos_change_ms) {
            jam.latitude += rand_float()*200 * LATLON_TO_M_INV * 1e-7;
            jam.longitude += rand_float()*200 * LATLON_TO_M_INV * 1e-7;
            jam.last_pos_change_ms = now_ms + rand_delay_ms(pos_change_hz);
        }
        if (now_ms >= jam.last_acc_change_ms) {
            d.vertical_acc = fabsf(rand_float())*300;
            d.horizontal_acc = fabsf(rand_float())*300;
            d.speed_acc = fabsf(rand_float())*50;
            jam.last_acc_change_ms = now_ms + rand_delay_ms(acc_change_hz);
        }
    }
