...
void Battery::consume_energy(float current_amps, uint64_t now_us)
{
    ...
    update_temperature(current_amps, dt);
}

void Battery::update_temperature(float current_amps, float dt)
{
    ...
    const float temp_increase = (current_amps * current_amps) * resistance * inverse_of_thermal_capacity * dt;
