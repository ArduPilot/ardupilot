...
/*
 * Generics for all types but double
 */
template<class T>
float hz_to_nsec(const T &freq) {
    return !is_zero(static_cast<float>(freq)) ? static_cast<float>(NSEC_PER_SEC) / freq : 0;
}

template<class T>
float nsec_to_hz(const T &nsec) {
    return !is_zero(static_cast<float>(nsec)) ? static_cast<float>(NSEC_PER_SEC) / nsec : 0;
}

template<class T>
float usec_to_nsec(const T &usec) {
    return !is_zero(static_cast<float>(usec)) ? static_cast<float>(usec) * NSEC_PER_USEC : 0;
}

template<class T>
float nsec_to_usec(const T &nsec) {
    return !is_zero(static_cast<float>(nsec)) ? static_cast<float>(nsec) / NSEC_PER_USEC : 0;
}

template<class T>
float hz_to_usec(const T &freq) {
    return !is_zero(static_cast<float>(freq)) ? static_cast<float>(USEC_PER_SEC) / freq : 0;
}

template<class T>
float usec_to_hz(const T &usec) {
    return !is_zero(static_cast<float>(usec)) ? static_cast<float>(USEC_PER_SEC) / usec : 0;
}
