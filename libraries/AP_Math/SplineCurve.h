...
public:
    ...
    void set_origin_and_destination(const Vector3p &origin, const Vector3p &destination, const Vector3f &origin_vel, const Vector3f &destination_vel);
...
private:
    ...
    void calc_target_pos_vel(float time, Vector3p &position, Vector3f &velocity, Vector3f &acceleration, Vector3f &jerk);

    // interval variables
    Vector3p    _origin;                // origin offset (in NED frame) from EKF
    Vector3p    _destination;           // destination offset (in NED frame) from EKF
    Vector3f    _origin_vel;            // the target velocity vector in NED frame at the origin of the spline segment
    Vector3f    _destination_vel;       // the target velocity vector in NED frame at the destination point of the spline segment
    Vector3p    _hermite_solution[4];   // array describing path between origin and destination
