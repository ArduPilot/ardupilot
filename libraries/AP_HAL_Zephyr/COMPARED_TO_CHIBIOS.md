...
| `mem_info()`                                     | yes                                   | verified CubeOrange                                | ChibiOS's `MemInfoV1` format, all six H743 RAM banks at ChibiOS's addresses and `TYPE` codes. Captures from the same board under both HALs: `chibios-cubeorange-memory.txt`, `zephyr-cubeorange-memory.txt` in the repo root. |
| `uart_info()`, `timer_info()`, `dma_info()`      | yes                                   | verified CubeOrange                                | `@SYS/uarts.txt`, `timers.txt`, `dma.txt` in ChibiOS's `UARTV1`/`TIMERV1`/`DMAV1` formats, column for column. `dma.txt`'s lock-contention columns are structurally zero on Zephyr (streams are bound in the devicetree, nothing arbitrates); see `zephyr-sys-file-gaps.md`. |
