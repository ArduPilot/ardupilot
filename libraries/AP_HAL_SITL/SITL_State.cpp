...
void SITL_State::_output_to_flightgear(void)
{
    ...
    if (_vehicle == ArduCopter) {
        ...
        for (uint8_t i=0; i<4; i++) {
            fdm.rpm[i] = constrain_value<float>((pwm_output[i]-1000), 0, 1000);
        }
    } else {
        fdm.num_engines = 4;
        fdm.rpm[0] = constrain_value<float>((pwm_output[2]-1000)*3, 0, 3000);
        // for quadplane
        fdm.rpm[1] = constrain_value<float>((pwm_output[5]-1000)*12, 0, 12000);
        fdm.rpm[2] = constrain_value<float>((pwm_output[6]-1000)*12, 0, 12000);
        fdm.rpm[3] = constrain_value<float>((pwm_output[7]-1000)*12, 0, 12000);
    }
    fdm.ByteSwap();
