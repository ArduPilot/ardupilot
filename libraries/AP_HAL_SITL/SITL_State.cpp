...
void SITL_State::_simulator_servos(SITL::Aircraft::sitl_input &input)
{
        ...
        if (_sitl->state.battery_voltage <= 0) {
            if (_vehicle == ArduSub) {
                ...
                for (uint8_t i = 0; i < 6; i++) {
                    ...
                    //printf("i: %d, pwm: %.2f\n", i, pwm);
                    const float fraction = fabsf((pwm - 1500.0f) / 500.0f);
    ...
    _voltage_pin_value = ((voltage / 10.1f) / 5.0f) * 1024;
    _current_pin_value = ((current / 17.0f) / 5.0f) * 1024;
}
...
void SITL_State::set_height_agl()
{
    if (_home_alt == -1.0f && _sitl->state.altitude > 0.0) {
        ...
        _home_alt = _sitl->state.altitude;
    }
    ...
    _sitl->height_agl = _sitl->state.altitude - _home_alt;
}
