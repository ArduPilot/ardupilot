...
LowPassFilter<Eigen::Vector3f> low_pass_filter_Eigen;
...
void loop()
{
    ...
    low_pass_filter_Eigen.reset(Eigen::Vector3f(0,0,0));
    ...
    Eigen::Vector3f filtered_value_Eigen;
    Eigen::Vector3f new_vec_Eigen;
    for( i=0; i<5000000; i++ ) {
        // new data value
        new_value = sinf((float)i*2*PI*5/50.0f);  // 5hz
        new_vec_Eigen = Eigen::Vector3f(new_value,new_value,new_value);
