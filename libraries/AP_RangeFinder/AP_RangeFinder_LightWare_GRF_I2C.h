...
private:
    ...
    // Distance readings collected by timer() between update() calls; update()
    // averages them, publishes to state.distance_m, and resets the counters.
    struct {
        float sum_m;
        uint16_t count;
    } _accum;
    ...
    ConfigStep _config_step;
    uint8_t    _config_retry;
};
