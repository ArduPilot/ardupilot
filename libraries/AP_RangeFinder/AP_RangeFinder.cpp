    ...
    float max_att_deviation_deg, float max_mean_abs_deviation_m) const
{
    ...
    if (oldest_sample_time_ms > time_threshold_ms) {
        gcs().send_text(MAV_SEVERITY_NOTICE,
                                     ...
                                     float &average_distance_m) const
{
    ...
    float corrected_distances[RANGEFINDER_SAMPLE_HISTORY_SIZE];
    ...
    for (uint16_t i = 0; i < sample_count; ++i) {
        ...
        corrected_distances[valid_samples] =
        ...
        sum += corrected_distances[valid_samples] * sample_weight;
    ...
    if (valid_samples == 0) {
        ...
        gcs().send_text(MAV_SEVERITY_DEBUG, "Rangefinder: %u out of range",
                        status_rejected_count);
        gcs().send_text(MAV_SEVERITY_DEBUG,
                        "Rangefinder: %u over max. att. deviation",
                        attitude_rejected_count);
    ...
    if (max_mean_abs_deviation_m >= 0.0f) {
        ...
        for (uint16_t i = 0; i < valid_samples; ++i) {
            sum_abs_deviation += fabsf(corrected_distances[i] - mean);
        }
