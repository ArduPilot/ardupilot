...
void AP_RangeFinder_Backend::update_pre_arm_check()
{
    ...
    if (state.status == RangeFinder::RangeFinder_NotConnected || state.status == RangeFinder::RangeFinder_NoData) {
        state.pre_arm_check = false;
        return;
    }
    state.pre_arm_check = true;
    if (!state.pre_arm_check_opflow) {
        ...
        if (((state.pre_arm_distance_max - state.pre_arm_distance_min) >= RANGEFINDER_PREARM_REQUIRED_CHANGE_CM) &&
            ...
            state.pre_arm_check_opflow = true;
        }
    }
}
