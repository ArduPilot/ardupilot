 ...
 */
#define STREAM1_VAL stream_the_median_distance_to_the_first_return
...
AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN(RangeFinder::RangeFinder_State &_state,
...
/*
   ...
   there.
*/
AP_RangeFinder_Backend *AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::detect(RangeFinder::RangeFinder_State &_state,
        AP_RangeFinder_Params &_params,
        AP_HAL::OwnPtr<AP_HAL::I2CDevice> dev)
{
    if (!dev) {
    ...
    AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN *sensor
        = NEW_NOTHROW AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN(_state, _params, std::move(dev));
 ...
 */
bool AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::write_bytes(uint8_t *write_buf_u8, uint32_t len_u8)
{
 ...
 */
void AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::sf20_disable_address_tagging()
{
 ...
 */
bool AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::sf20_send_and_expect(const char* send_msg, const char* expected_reply)
{
...
/*
  ...
  version string
 */
void AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::sf20_get_version(const char* send_msg, const char *reply_prefix, char reply[15])
{
 ...
 */
bool AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::init()
{
    if (sf20_init()) {
 ...
 */
bool AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::legacy_init()
{
    union {
        be16_t be16_val;
        uint8_t bytes[2];
    ...
    _dev->register_periodic_callback(50000,
                                     FUNCTOR_BIND_MEMBER(&AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::legacy_timer, void));
 ...
 */
bool AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::sf20_init()
{
    ...
    */
    ...
    // call timer() at 20Hz
    _dev->register_periodic_callback(50000,
                                     FUNCTOR_BIND_MEMBER(&AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::sf20_timer, void));

    return true;
}

// read - return last value measured by sensor
bool AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::legacy_get_reading(float &reading_m)
{
    be16_t val;
    ...
    if (_dev->transfer(&read_reg, 1, (uint8_t *)&val, sizeof(val))) {
        ...
        if (signed_val < 0) {
            ...
            reading_m = uint16_t(max_distance() + LIGHTWARE_OUT_OF_RANGE_ADD_CM) * 0.01f;
        } else {
...
bool AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::sf20_get_reading(float &reading_m)
{
...
bool AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::sf20_parse_stream(uint8_t *stream_buf,
        ...
        uint16_t &val)
{
    ...
    if (strncmp((const char *)&stream_buf[*p_num_processed_chars], "-1.00", 5) == 0) {
        val = uint16_t(max_distance() + LIGHTWARE_OUT_OF_RANGE_ADD_CM);
...
*/
void AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::update(void)
{
    // nothing to do - its all done in the timer()
}

void AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::legacy_timer(void)
{
...
void AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::sf20_timer(void)
{
...
bool AP_RangeFinder_LightWareI2CFILTEREDFIRSTRETURN::sf20_wait_on_reply(uint8_t *rx_two_byte)
{
