...
bool NavEKF2::InitialiseFilter(void)
{
    ...
    if (_enable == 0) {
        if (AP::dal().ekf_type_parameter_value() == 2) {
...
bool NavEKF2::getLLH(AbsAltLocation &loc) const
{
    if (!core) {
        return false;
    }
...
bool NavEKF2::getOriginLLH(AbsAltLocation &loc) const
{
    if (!core) {
        return false;
    }
...
bool NavEKF2::setOriginLLH(const AbsAltLocation &loc)
{
