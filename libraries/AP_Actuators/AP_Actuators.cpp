...
void AP_Actuators::initialize_actuators()
{
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(aux_actuator_value); i++) {
...
{
    if (actuator_num >= ARRAY_SIZE(aux_actuator_value)) {
        return;
    }
...
void AP_Actuators::set_actuator(uint8_t actuator_num, float value)
{
    ...
    // maps value from external input range [-1,1] to internally used proportion [0,1]
