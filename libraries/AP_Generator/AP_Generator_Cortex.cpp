...
/**
 * Run a pre-arm check on the generator.
 * We do not want these checks to prevent the system from arming,
 * as there are valid use-cases where the generator is not connected or not working but the vehicle still needs to be armed
 * Instead, we return a failure message that can be displayed to the user, but allow arming to proceed.
 */
bool AP_Generator_Cortex::pre_arm_check(char *failmsg, uint8_t failmsg_len) const
{
    ...
    if (!is_connected()) {
        ...
        // Connected, not inhibited. Check ready state if in standby
        const Cortex_StatusBits_t &status = telemetry.status.status;

        if (status.mode == CORTEX_MODE_STANDBY && !is_ready()) {
