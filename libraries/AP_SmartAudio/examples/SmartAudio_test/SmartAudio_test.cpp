    ...
    bool SmartAudioTest::assert_request_last_inserted(uint8_t *expectedRequest,uint8_t const size){
        ...
        for(uint8_t i=0;i<size;i++){
             if(reinterpret_cast<uint8_t*>(&commands[AP_SmartAudio::get_singleton()->requests_queue.available()-1].frame.u8RequestFrame)[i]!=expectedRequest[i]){
                 ...
                 printf("Result: ");
                 AP_SmartAudio::get_singleton()->print_bytes_to_hex_string(reinterpret_cast<uint8_t*>(&commands[AP_SmartAudio::get_singleton()->requests_queue.available()-1].frame.u8RequestFrame),size);
                 printf("Expected: ");
                 AP_SmartAudio::get_singleton()->print_bytes_to_hex_string(expectedRequest,size);
