...
void NavEKF3_core::SelectFlowFusion()
{
    ...
    // Update the IMU-aided AGL KF unconditionally when the option is enabled.
...
void NavEKF3_core::UpdateAglKf()
{
    ...
    // At the 5 s validity timeout, |v| is at most exp(-5/2) ~ 8% of its
    // value at last RF fusion, so the hard reset finds v near zero.
    if (!rangeDataToFuse) {
        const ftype tau_v = 2.0f;
        aglKfV *= expf(-imuDt / tau_v);
    }
    ...
    const ftype Qvel = sq(frontend->_accNoise.get() * imuDt);   // matches CovariancePrediction: sq(imuDt*accNoise)
    ...
    if (!rangeDataToFuse) {
        if (imuSampleTime_ms - lastAglRngFuseTime_ms > 5000) {
            aglKfValid = false;
        }
        return;
    }
    ...
    // After > 5 s of IMU-only propagation, vertical velocity drift makes the
    ...
    const ftype measNoiseVar = sq(frontend->_rngNoise.get());
