...
void NavEKF3_core::SelectFlowFusion()
{
    ...
    // Scoped with onGround rather than inFlight: the fly-forward branch of detectFlight()
    // sets inFlight only from GPS ground speed, which a flow navigating vehicle need not
    // have, and there it would never authorise the assumption at all
    if (onGround) {
...
void NavEKF3_core::FuseOptFlow(const of_elements &ofDataDelayed, bool really_fuse)
{
    ...
    // A vertical datum reset moves pd and carries terrainState with it, but terrain_srtm_alt
    // stays referenced to the origin, so this route is offset by any such reset and is only
    // taken while the terrain estimator has nothing fresher
