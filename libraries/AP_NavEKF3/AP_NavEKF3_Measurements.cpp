...
void NavEKF3_core::readGpsData()
{
    ...
    if (frontend->option_is_enabled(NavEKF3::Option::JammingExpected) &&
        ...
        bool canDeadReckon = ((doingFlowNav && gndOffsetValid) || canDoWindRelNav || doingBodyVelNav);
#if EK3_FEATURE_EXTERNAL_POSITION_FUSION
        canDeadReckon = canDeadReckon || useSetLatLngAsMeasurement;
...
void NavEKF3_core::writeExtNavData(const Vector3f &pos, const Quaternion &quat, float posErr, float angErr, uint32_t timeStamp_ms, uint16_t delay_ms, uint32_t resetTime_ms)
{
    ...
    // data sources.
    if ((lastSetlatLngPassTime_ms != 0) && (imuSampleTime_ms - lastSetlatLngPassTime_ms < 5000)) {
        return;
    }
#endif // EK3_FEATURE_EXTERNAL_POSITION_FUSION
