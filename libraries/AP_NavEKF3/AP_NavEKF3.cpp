...
const AP_Param::GroupInfo NavEKF3::var_info2[] = {
    ...
    // @Description: EKF optional behaviour. Bit 0 (JammingExpected): Setting JammingExpected will change the EKF behaviour such that if dead reckoning navigation is possible it will require the preflight alignment GPS quality checks controlled by EK3_GPS_CHECK and EK3_CHECK_SCALE to pass before resuming GPS use if GPS lock is lost for more than 2 seconds to prevent bad position estimate. Bit 1 (Manual lane switching): DANGEROUS – If enabled, this disables automatic lane switching. If the active lane becomes unhealthy, no automatic switching will occur. Users must manually set EK3_PRIMARY to change lanes. No health checks will be performed on the selected lane. Use with extreme caution.  Bit 2 (Optflow may use terrain alt): Terrain SRTM data will be used if the vehicle climbs above the rangefinder's range allowing optical flow to be used at higher altitudes. Bit 3 (AGL KF for optflow scaling): Use a 2-state IMU-aided AGL Kalman filter (height + vertical velocity, fused with rangefinder) to compute the height-above-ground used for optical flow velocity scaling, instead of terrainState-pd. This decouples optical flow scaling from errors in the main filter's vertical position state.  Bit 4 (SetLatLngFusion) will enable frequent position setting via MAV_CMD_EXTERNAL_POSITION_ESTIMATE to be used as a GPS alternative when the GPS fails quality checks or is unavailable. The advantage of this method over using MAV_CMD_EXTERNAL_POSITION_ESTIMATE messages only to reset the position, is that it allows for update of wind drift estimates and provided estimates are received at 1Hz or better, also allows continued navigation during flight of Copters and Planes in VTOL flight modes. Use of this option will block receipt of data from the visual odometry library to prevent data sources fighting, so do NOT set this bit if a non zero value for the VISO_TYPE parameter is set. Bit 5 (SetLatLngOffset) when set in combination with SetLatLngFusion, causes a horizontal position offset for the GLOBAL_POSITION_SENSOR or MAV_CMD_EXTERNAL_POSITION_ESTIMATE message to be learned when GPS quality is sufficient to pass the same checks used for EKF alignment. This offset is then applied to the GLOBAL_POSITION_SENSOR or MAV_CMD_EXTERNAL_POSITION_ESTIMATE data when used as specified by the SetLatLngFusion option. This option is useful if the GLOBAL_POSITION_SENSOR or MAV_CMD_EXTERNAL_POSITION_ESTIMATE message is derived from an odometry technique that drifts over time.
    // @Bitmask: 0:JammingExpected, 1:ManualLaneSwitching, 2:Optflow may use terrain alt, 3:AGL KF for optflow scaling, 4:SetLatLngFusion, 5:SetLatLngOffset
    // @User: Advanced
