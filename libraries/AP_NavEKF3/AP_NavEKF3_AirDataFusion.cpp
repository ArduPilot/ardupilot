...
void NavEKF3_core::FuseAirspeed()
{
    ...
    if (VtasPred > 1.0f)
    {
        ...
        if (tasDataDelayed.allowFusion && !accelBiasLearningInhibited() && !airDataFusionWindOnly) {
...
void NavEKF3_core::FuseSideslip()
{
    ...
    if (vel_rel_wind.x > 5.0f)
    {
        ...
        if (!accelBiasLearningInhibited() && !airDataFusionWindOnly) {
            for (uint8_t index = 0; index < 3; index++) {
                const uint8_t stateIndex = index + 13;
                if (!dvelBiasAxisInhibit[index]) {
                    kalman_mask |= (1<<stateIndex);
...
void NavEKF3_core::FuseDragForces()
{
    ...
    for (uint8_t axis_index = 0; axis_index < 2; axis_index++) {
        ...
        for (unsigned i = 0; i<=stateIndexLim; i++) {
            ...
            for (unsigned j = 0; j<=stateIndexLim; j++) {
                ftype res = 0;
                res += (Kfusion[i] * Hfusion[0]) * Pmut[0][j];
                res += (Kfusion[i] * Hfusion[1]) * Pmut[1][j];
                res += (Kfusion[i] * Hfusion[2]) * Pmut[2][j];
                res += (Kfusion[i] * Hfusion[3]) * Pmut[3][j];
                res += (Kfusion[i] * Hfusion[4]) * Pmut[4][j];
                res += (Kfusion[i] * Hfusion[5]) * Pmut[5][j];
                res += (Kfusion[i] * Hfusion[6]) * Pmut[6][j];
                res += (Kfusion[i] * Hfusion[22]) * Pmut[22][j];
                res += (Kfusion[i] * Hfusion[23]) * Pmut[23][j];
                KHP[i][j] = res;
            }
        }
