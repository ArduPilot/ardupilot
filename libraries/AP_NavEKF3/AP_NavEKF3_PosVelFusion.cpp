...
void NavEKF3_core::SelectVelPosFusion()
{
    ...
    if (gpsDataToFuse && (PV_AidingMode == AID_ABSOLUTE) && (posxy_source == AP_NavEKF_Source::SourceXY::GPS)) {
        ...
        fuseVelVertData = frontend->sources.useVelZSource(AP_NavEKF_Source::SourceZ::GPS);
        fusePosData = true;
#if EK3_FEATURE_EXTERNAL_NAV
...
void NavEKF3_core::FuseVelPosNED()
{
    ...
    if (fuseVelData || fuseVelVertData || fusePosData || fuseHgtData) {
        ...
        if (fuseVelData) {
            fuseData[0] = true;
            fuseData[1] = true;
            if (useGpsVertVel && frontend->sources.useVelZSource(AP_NavEKF_Source::SourceZ::GPS)) {
                fuseData[2] = true;
            } else if (useExtNavVel && frontend->sources.useVelZSource(AP_NavEKF_Source::SourceZ::EXTNAV)) {
                fuseData[2] = true;
            }
        }
        if (fusePosData) {
