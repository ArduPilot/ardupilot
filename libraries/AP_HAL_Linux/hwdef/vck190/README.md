# AMD VCK190 Linux Board Target

This board target supports the AMD VCK190 Evaluation Kit running PetaLinux.
It uses the Linux HAL with the toolchain and CPU tuning for the Cortex-A72
application processing unit in the VCK190's XCVC1902 device.

This profile provides build configuration and runtime filesystem defaults. It
does not yet define a flight sensor or actuator suite; those interfaces must be
configured and validated before using the VCK190 as an autopilot.
...
python3 ./waf configure --board vck190 --toolchain aarch64-linux-gnu
...
/usr/bin/python3 ./waf configure --board vck190 --toolchain aarch64-amd-linux
/usr/bin/python3 ./waf copter -j"$(nproc)"
```

Build artifacts are created under `build/vck190/bin/`.
...
For cross-builds, this compiles test binaries; execute them on the VCK190 target.
...
```bash
scp build/vck190/bin/arducopter root@<target-ip>:/opt/ardupilot/bin/
...
4. Run ArduCopter for process bring-up only. Do not arm until physical sensors,
   actuator outputs, and failsafes have been configured and validated:
