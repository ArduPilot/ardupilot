...
void RCOutput_PCA9685::push()
{
    ...
    size_t len = (max_ch - min_ch) * 4 + 1;
    uint8_t transfer_data[len];
    transfer_data[0] = PCA9685_RA_LED0_ON_L + 4 * (_channel_offset + min_ch);
    memcpy(&transfer_data[1], &data[min_ch * 4], (len - 1) * sizeof(uint8_t));

    _dev->transfer(transfer_data, sizeof(transfer_data), nullptr, 0);
    _dev->get_semaphore()->give();
