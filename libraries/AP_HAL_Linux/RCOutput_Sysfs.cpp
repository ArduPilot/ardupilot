...
RCOutput_Sysfs::RCOutput_Sysfs(uint8_t chip, uint8_t channel_count, uint16_t freq) :
    _chip(chip),
...
RCOutput_Sysfs::RCOutput_Sysfs(uint8_t chip, uint8_t channel_count) :
    RCOutput_Sysfs(chip, channel_count, 20000)
{
}
...
void RCOutput_Sysfs::init(void* machtnichts)
{
    for (uint8_t i = 0; i < _channel_count; i++) {
        ...
        if (!_pwm_channels[i]) {
            hal.scheduler->panic(PSTR("RCOutput_Sysfs_PWM: Unable to setup PWM pin."));
        }
