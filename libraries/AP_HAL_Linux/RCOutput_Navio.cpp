...
{    
    ...
    uint8_t prescale = round(24576000.0f / 4096.0f / freq_hz)  - 1;
...
{   
    ...
    if (period_us == 0)
        ...
        length = round((period_us * 4096) / (1000000.0f / _frequency)) - 1;
...
uint16_t LinuxRCOutput_Navio::read(uint8_t ch)
{
    ...
    uint16_t period_us = (length + 1) * (1000000.0f / _frequency) / 4096.0f;
