...
void AP_InertialSensor_SITL::timer_update(void)
{
    ...
    const bool running_in_lockstep = frame_num > 0;

    if (running_in_lockstep && frame_num == sitl->state.frame_num) {
        return;
    }

    frame_num = sitl->state.frame_num;

    if (now >= next_accel_sample || running_in_lockstep) {
    ...
    if (now >= next_gyro_sample || running_in_lockstep) {
