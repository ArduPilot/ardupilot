...
#define BMI160_POWERUP_DELAY_MS 10
#define BMI160_SOFTRESET_DELAY_MS 1
 ...
 * occasions. */
#define BMI160_ACCEL_NORMAL_POWER_MODE_DELAY_MS 4
#define BMI160_GYRO_NORMAL_POWER_MODE_DELAY_MS 81
...
/* Consider number maximum of samples as double of the expected number of
...
#define BMI160_MAX_FIFO_SAMPLES \
    ((2 * BMI160_ODR_TO_HZ(BMI160_MAX_ODR) + 999) / 1000)
...
void AP_InertialSensor_BMI160::start()
{
    ...
    if (_sampling_config.mode == DATA_READY) {
        accel_cfg.odr = ODR_800Hz;
        gyro_cfg.odr = ODR_800Hz;

        if (_sampling_config.drdy_pin >= 0) {
            r = _dev->write_register(BMI160_REG_INT_EN_1,
                                     BMI160_INT_DRDY_EN,
                                     1);
            if (!r) {
                AP_HAL::panic("BMI160: Unable to enable data ready interrupt");
            }

            r = _dev->write_register(BMI160_REG_INT_MAP_1,
                                     BMI160_INT_MAP_INT1_DRDY,
                                     1);
            if (!r) {
                AP_HAL::panic("BMI160: Unable to configure interrupt mapping");
            }
            ...
            r = _dev->write_register(BMI160_REG_INT_OUT_CTRL,
                                     ...
                                     1);
            if (!r) {
                AP_HAL::panic("BMI160: Unable to configure interrupt output");
            }

            _drdy_pin = hal.gpio->channel((uint16_t)_sampling_config.drdy_pin);
            if (_drdy_pin == nullptr) {
                AP_HAL::panic("BMI160: Couldn't request data ready GPIO channel");
            }
    ...
    if (_sampling_config.mode == FIFO) {
        ...
        if (!r) {
            AP_HAL::panic("BMI160: Unable to flush FIFO");
        }
    }
...
bool AP_InertialSensor_BMI160::_configure_accel(AccelConfig &cfg)
{
    ...
    r = _dev->write_register(BMI160_REG_ACC_CONF, cfg.osr << 4 | cfg.odr, 1);
    if (!r) {
        return false;
    }
    ...
    r = _dev->write_register(BMI160_REG_ACC_RANGE, range_bits, 1);
    if (!r) {
        return false;
    }
...
bool AP_InertialSensor_BMI160::_configure_gyro(GyroConfig &cfg)
{
    ...
    r = _dev->write_register(BMI160_REG_GYR_CONF, cfg.osr << 4 | cfg.odr, 1);
    if (!r) {
        return false;
    }
    ...
    r = _dev->write_register(BMI160_REG_GYR_RANGE, cfg.range, 1);
    if (!r) {
        return false;
    }
...
bool AP_InertialSensor_BMI160::_hardware_init()
{
    ...
    hal.scheduler->delay(BMI160_POWERUP_DELAY_MS);
    ...
    for (unsigned i = 0; i < BMI160_HARDWARE_INIT_MAX_TRIES; i++) {
        ...
        ret = _dev->write_register(BMI160_REG_CMD,
                                   ...
                                   BMI160_ACCEL_NORMAL_POWER_MODE_DELAY_MS);
        if (!ret) {
            continue;
        }
        ...
        ret = _dev->write_register(BMI160_REG_CMD,
                                   ...
                                   BMI160_GYRO_NORMAL_POWER_MODE_DELAY_MS);
        if (!ret) {
            continue;
        }
