...
// accelerometer output.  Bandwidth is set by LSM6DSO_ACCEL_LPF2_BW.
#ifndef LSM6DSO_ACCEL_LPF2_ENABLED
...
#define LSM6DSO_REG_FIFO_CTRL1              0x07  // FIFO watermark threshold [7:0]
#define LSM6DSO_REG_FIFO_CTRL2              0x08  // FIFO watermark threshold [8], compression
#define LSM6DSO_REG_FIFO_CTRL3              0x09  // FIFO gyro/accel batch data rate
#define LSM6DSO_REG_FIFO_CTRL4              0x0A  // FIFO mode selection

#define LSM6DSO_FIFO_CTRL3_BDR_DISABLED     0x00
#define LSM6DSO_FIFO_CTRL4_MODE_BYPASS       0x00
#define LSM6DSO_FIFO_CTRL4_MODE_CONTINUOUS   0x06
...
#define LSM6DSO_CTRL1_FS_XL_4G              0x08
#define LSM6DSO_CTRL1_FS_XL_8G              0x0C
...
#define LSM6DSO_CTRL2_FS_G_500DPS           0x04
#define LSM6DSO_CTRL2_FS_G_1000DPS          0x08
#define LSM6DSO_CTRL2_FS_G_2000DPS          0x0C

// ---- Control register 3 (R/W) ----
#define LSM6DSO_REG_CTRL3                   0x12
#define LSM6DSO_CTRL3_BDU                   (1U << 6)  // block data update
...
#define LSM6DSO_STATUS_XLDA                 (1U << 0)  // accel data available
#define LSM6DSO_STATUS_GDA                  (1U << 1)  // gyro data available
#define LSM6DSO_STATUS_TDA                  (1U << 2)  // temperature data available

// ---- Data output registers (R) ----
#define LSM6DSO_REG_OUT_TEMP_L              0x20  // temperature output (16-bit)
...
#define LSM6DSO_ODR_833HZ                   0x70
#define LSM6DSO_ODR_1660HZ                  0x80
#define LSM6DSO_ODR_3330HZ                  0x90
#define LSM6DSO_ODR_6660HZ                  0xA0

// ---- FIFO data output registers (R) ----
#define LSM6DSO_REG_FIFO_DATA_OUT_TAG       0x78  // FIFO tag byte
#define LSM6DSO_REG_FIFO_DATA_OUT_X_L       0x79  // FIFO data start

// ---- SPI protocol ----
#define LSM6DSO_SPI_READ_FLAG               0x80

// ---- Driver timing constants ----
#define LSM6DSO_DEFAULT_BACKEND_RATE_HZ     833
#define LSM6DSO_INIT_MAX_TRIES              5
#define LSM6DSO_RESET_TIMEOUT_MS            100
#define LSM6DSO_DATA_READY_TIMEOUT_MS       20
#define LSM6DSO_POWERUP_DELAY_MS            5

// ---- FIFO sizing ----
#define LSM6DSO_PRIMARY_FIFO_WATERMARK_WORDS 2
#define LSM6DSO_FIFO_MAX_DRAIN_WORDS        32
#define LSM6DSO_FIFO_BURST_WORDS            16

// temperature update interval in milliseconds
#define LSM6DSO_TEMPERATURE_UPDATE_MS       100

// ---- Temperature conversion ----
#define LSM6DSO_TEMPERATURE_ZERO_C          25.0f
#define LSM6DSO_TEMPERATURE_SENSITIVITY     256.0f  // LSB/°C

// ---- Accelerometer sensitivity (mg/LSB → m/s²) ----
#define LSM6DSO_ACCEL_SCALE_2G              (GRAVITY_MSS *  2.0f / 32768.0f)
#define LSM6DSO_ACCEL_SCALE_4G              (GRAVITY_MSS *  4.0f / 32768.0f)
#define LSM6DSO_ACCEL_SCALE_8G              (GRAVITY_MSS *  8.0f / 32768.0f)
#define LSM6DSO_ACCEL_SCALE_16G             (GRAVITY_MSS * 16.0f / 32768.0f)

// ---- Gyroscope sensitivity per ST datasheet (mdps/LSB → rad/s) ----
#define LSM6DSO_GYRO_SCALE_125DPS           radians(4.375f   / 1000.0f)
#define LSM6DSO_GYRO_SCALE_250DPS           radians(8.75f  / 1000.0f)
#define LSM6DSO_GYRO_SCALE_500DPS           radians(17.5f   / 1000.0f)
#define LSM6DSO_GYRO_SCALE_1000DPS          radians(35.0f   / 1000.0f)
#define LSM6DSO_GYRO_SCALE_2000DPS          radians(70.0f  / 1000.0f)
...
constexpr uint16_t LSM6DSO_FIFO_BURST_BUFFER_SIZE = LSM6DSO_FIFO_BURST_WORDS * sizeof(RawFifoWord) + 1;
...
AP_InertialSensor_LSM6DSO::AP_InertialSensor_LSM6DSO(AP_InertialSensor &imu,
    ...
    , _dev(std::move(dev))
    , _rotation(rotation)
    , _accel_scale(LSM6DSO_ACCEL_SCALE_16G)
    , _gyro_scale(LSM6DSO_GYRO_SCALE_2000DPS)
{
}
...
AP_InertialSensor_LSM6DSO::~AP_InertialSensor_LSM6DSO()
{
    if (_fifo_buffer != nullptr) {
        hal.util->free_type(_fifo_buffer, LSM6DSO_FIFO_BURST_BUFFER_SIZE,
...
void AP_InertialSensor_LSM6DSO::start()
{
    ...
    _backend_rate_hz = LSM6DSO_DEFAULT_BACKEND_RATE_HZ;
    if (enable_fast_sampling(accel_instance) && get_fast_sampling_rate() > 1) {
        _fast_sampling = (_dev->bus_type() == AP_HAL::Device::BUS_TYPE_SPI);
    }
    if (_fast_sampling) {
        _backend_rate_hz = calculate_backend_rate(LSM6DSO_DEFAULT_BACKEND_RATE_HZ);
    }
    _backend_period_us = 1000000UL / _backend_rate_hz;

    if (!_imu.register_accel(accel_instance, _backend_rate_hz, _dev->get_bus_id_devtype(DEVTYPE_INS_LSM6DSO)) ||
        !_imu.register_gyro(gyro_instance, _backend_rate_hz, _dev->get_bus_id_devtype(DEVTYPE_INS_LSM6DSO))) {
        return;
    }

    {
        WITH_SEMAPHORE(_dev->get_semaphore());
        if (!write_register(LSM6DSO_REG_FIFO_CTRL1, 0x00, true) ||
            !write_register(LSM6DSO_REG_FIFO_CTRL2, 0x00, true) ||
            !write_register(LSM6DSO_REG_FIFO_CTRL3, LSM6DSO_FIFO_CTRL3_BDR_DISABLED, true) ||
            !write_register(LSM6DSO_REG_FIFO_CTRL4, LSM6DSO_FIFO_CTRL4_MODE_BYPASS, true)) {
            return;
        }
        ...
        const uint8_t odr = odr_code_for_rate(_backend_rate_hz);
               ...
               write_register(LSM6DSO_REG_CTRL1,
               odr | LSM6DSO_CTRL1_FS_XL_16G | LSM6DSO_CTRL1_LPF2_XL_EN,
               true);

               write_register(LSM6DSO_REG_CTRL2,
               odr | LSM6DSO_CTRL2_FS_G_2000DPS,
               true);
    ...
    _fifo_buffer = static_cast<uint8_t *>(hal.util->malloc_type(LSM6DSO_FIFO_BURST_BUFFER_SIZE,
                                                                 AP_HAL::Util::MEM_DMA_SAFE));
    if (_fifo_buffer == nullptr) {
...
{
    snprintf(banner, banner_len, "IMU%u: LSM6DSO16X %s sampling %.1fkHz",
             gyro_instance,
...
bool AP_InertialSensor_LSM6DSO::init()
{
    _dev->set_read_flag(LSM6DSO_SPI_READ_FLAG);
    return hardware_init();
}

bool AP_InertialSensor_LSM6DSO::hardware_init()
{
    hal.scheduler->delay(LSM6DSO_POWERUP_DELAY_MS);
    ...
    for (uint8_t attempt = 0; attempt < LSM6DSO_INIT_MAX_TRIES; attempt++) {
        if (!check_whoami()) {
            hal.scheduler->delay(10);
            continue;
        }

        switch (_lsm6dso_type) {
        case LSM6DSO_Type::LSM6DSOXTR:
            _gyro_scale = LSM6DSO_GYRO_SCALE_2000DPS;
            _accel_scale = LSM6DSO_ACCEL_SCALE_16G;
            break;
        }
...
bool AP_InertialSensor_LSM6DSO::check_whoami()
{
    if (!read_registers(LSM6DSO_REG_WHO_AM_I, &_whoami, 1)) {
        return false;
    }
    switch (_whoami) {
    case LSM6DSO_ID_LSM6DSOXTR:
        _lsm6dso_type = LSM6DSO_Type::LSM6DSOXTR;
        return true;
    }
...
bool AP_InertialSensor_LSM6DSO::reset_device()
{
    if (!write_register(LSM6DSO_REG_CTRL3, LSM6DSO_CTRL3_SW_RESET)) {
        return false;
    }

    const uint32_t start_ms = AP_HAL::millis();
    while (AP_HAL::millis() - start_ms < LSM6DSO_RESET_TIMEOUT_MS) {
        uint8_t ctrl3 = 0;
        hal.scheduler->delay(1);
        if (!read_registers(LSM6DSO_REG_CTRL3, &ctrl3, 1)) {
            continue;
        }
        if ((ctrl3 & LSM6DSO_CTRL3_SW_RESET) == 0) {
            return true;
        }
    }
...
bool AP_InertialSensor_LSM6DSO::configure_gyro()
{
    return write_register(LSM6DSO_REG_CTRL2,
                          LSM6DSO_ODR_833HZ | LSM6DSO_CTRL2_FS_G_2000DPS,
                          true);
}

bool AP_InertialSensor_LSM6DSO::configure_accel()
{
    return write_register(LSM6DSO_REG_CTRL1,
                          LSM6DSO_ODR_833HZ |
                          LSM6DSO_CTRL1_FS_XL_16G |
                          LSM6DSO_CTRL1_LPF2_XL_EN,
                          true);
}

bool AP_InertialSensor_LSM6DSO::configure_primary_fifo()
{
    const uint8_t bdr = fifo_bdr_code_for_rate(_backend_rate_hz);
    const uint8_t fifo_ctrl3 = uint8_t((bdr << 4) | bdr);

    return write_register(LSM6DSO_REG_FIFO_CTRL1,
                          uint8_t(LSM6DSO_PRIMARY_FIFO_WATERMARK_WORDS & 0xFFU),
                          true) &&
           write_register(LSM6DSO_REG_FIFO_CTRL2, 0x00, true) &&
           write_register(LSM6DSO_REG_FIFO_CTRL3, fifo_ctrl3, true) &&
           write_register(LSM6DSO_REG_FIFO_CTRL4, LSM6DSO_FIFO_CTRL4_MODE_CONTINUOUS, true);
}

uint8_t AP_InertialSensor_LSM6DSO::odr_code_for_rate(uint16_t rate_hz) const
{
    switch (rate_hz) {
    case 6660: return LSM6DSO_ODR_6660HZ;
    case 3330: return LSM6DSO_ODR_3330HZ;
    case 1660: return LSM6DSO_ODR_1660HZ;
    case 833:
    default:
        return LSM6DSO_ODR_833HZ;
    }
}

uint8_t AP_InertialSensor_LSM6DSO::fifo_bdr_code_for_rate(uint16_t rate_hz) const
{
    switch (rate_hz) {
    case 6660: return 0x0A;
    case 3330: return 0x09;
    case 1660: return 0x08;
    case 833:
    default:
        return 0x07;
    }
...
uint16_t AP_InertialSensor_LSM6DSO::calculate_backend_rate(uint16_t base_rate_hz) const
{
...
AP_InertialSensor_LSM6DSO::FifoTag AP_InertialSensor_LSM6DSO::decode_fifo_tag(const uint8_t raw_tag)
{
...
uint8_t AP_InertialSensor_LSM6DSO::decode_fifo_tag_count(const uint8_t raw_tag)
{
    return (raw_tag >> 1) & 0x03U;
}

bool AP_InertialSensor_LSM6DSO::wait_for_data_ready()
{
    for (uint8_t i = 0; i < LSM6DSO_DATA_READY_TIMEOUT_MS; i++) {
        uint8_t status = 0;
        hal.scheduler->delay(1);
        if (!read_registers(LSM6DSO_REG_STATUS, &status, 1)) {
            continue;
        }
        if ((status & LSM6DSO_STATUS_GDA) != 0 &&
            (status & LSM6DSO_STATUS_XLDA) != 0) {
            return true;
        }
    }
...
bool AP_InertialSensor_LSM6DSO::read_registers(uint8_t reg, uint8_t *data, uint8_t len)
{
    return _dev->read_registers(reg, data, len);
}

bool AP_InertialSensor_LSM6DSO::write_register(uint8_t reg, uint8_t value, bool checked)
{
    return _dev->write_register(reg, value, checked);
}

bool AP_InertialSensor_LSM6DSO::read_fifo_status(FifoFrame &frame, uint32_t now_us)
{
    uint8_t fifo_status[2] {};
    if (!read_registers(LSM6DSO_REG_FIFO_STATUS1, fifo_status, sizeof(fifo_status))) {
...
bool AP_InertialSensor_LSM6DSO::read_fifo_words_block(const uint16_t n_words, uint32_t now_us)
{
    if (_fifo_buffer == nullptr || n_words == 0 || n_words > LSM6DSO_FIFO_BURST_WORDS) {
        return false;
    }

    _fifo_buffer[0] = LSM6DSO_REG_FIFO_DATA_OUT_TAG | LSM6DSO_SPI_READ_FLAG;
...
bool AP_InertialSensor_LSM6DSO::consume_fifo_word(FifoFrame &frame, SampleFrame &sample, const uint8_t *raw_word)
{
    RawFifoWord raw;
...
uint16_t AP_InertialSensor_LSM6DSO::drain_fifo(uint32_t now_us)
{
    FifoFrame frame{};
    ...
    while (drained < frame.unread_words && drained < LSM6DSO_FIFO_MAX_DRAIN_WORDS) {
        const uint16_t remaining = MIN(uint16_t(frame.unread_words - drained),
                                       uint16_t(LSM6DSO_FIFO_MAX_DRAIN_WORDS - drained));
        const uint16_t block_words = MIN(remaining, LSM6DSO_FIFO_BURST_WORDS);
...
void AP_InertialSensor_LSM6DSO::publish_gyro_sample(SampleFrame &sample)
{
    _rotate_and_correct_gyro(gyro_instance, sample.gyro);
    _notify_new_gyro_raw_sample(gyro_instance, sample.gyro);
}

void AP_InertialSensor_LSM6DSO::publish_accel_sample(SampleFrame &sample)
{
...
void AP_InertialSensor_LSM6DSO::check_register_monitor()
{
    _dev->set_speed(AP_HAL::Device::SPEED_LOW);
...
void AP_InertialSensor_LSM6DSO::update_temperature()
{
    const uint32_t now_ms = AP_HAL::millis();
    if (now_ms - _temperature_last_ms < LSM6DSO_TEMPERATURE_UPDATE_MS) {
        return;
    }
    _temperature_last_ms = now_ms;

    uint8_t tbuf[2];
    if (!read_registers(LSM6DSO_REG_OUT_TEMP_L, tbuf, sizeof(tbuf))) {
        _inc_accel_error_count(accel_instance);
        return;
    }
    const int16_t temperature_raw = int16_t(uint16_t(tbuf[0] | (tbuf[1] << 8)));
    const float temp_degc = LSM6DSO_TEMPERATURE_ZERO_C + temperature_raw / LSM6DSO_TEMPERATURE_SENSITIVITY;
