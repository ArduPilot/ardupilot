...
static constexpr uint16_t FILTER_68HZ = (0x0000000000000);       // 68 Hz default filter
static constexpr uint16_t FILTER_30HZ = (0b0000001001001);       // 30 Hz  filter
static constexpr uint16_t FILTER_BYPASS = (0b0000111111111);     // No filtering
                                   ...
                                   enum Rotation rotation)
{
    ...
    auto sensor = NEW_NOTHROW AP_InertialSensor_ZeroOnex(imu, std::move(dev), rotation);
...
void AP_InertialSensor_ZeroOnex::seneor_ctrl(uint8_t fifo_rst) 
{
    ...
    dev->transfer_fullduplex(wbuf, ptr);
}
...
uint8_t AP_InertialSensor_ZeroOnex::gen_crc8(uint8_t* data) {
    uint16_t crc = 0xff;
    uint8_t byte_value;
...
void AP_InertialSensor_ZeroOnex::_register_write(uint16_t addr, uint32_t value, uint8_t* buff) {
    buff[0] = (addr >> 2) & 0xff;
    buff[1] = ((addr & 3) << 6);
    buff[1] |= 1 << 5;
    buff[1] |= 1 << 3;
...
void AP_InertialSensor_ZeroOnex::_register_read(uint16_t addr, uint8_t* buff) {
    buff[0] = (addr >> 2) & 0xff;
    buff[1] = ((addr & 3) << 6);
    buff[1] |= 1 << 3;
    buff[2] = 0;
...
void AP_InertialSensor_ZeroOnex::sch16t_gen_cmd(uint32_t addr, uint8_t* buff, uint8_t read_valid, uint8_t is_sensor, uint8_t offset) {
...
bool AP_InertialSensor_ZeroOnex::register_read(uint16_t addr, uint32_t* value) {
    uint8_t tbuf[8];
    uint8_t rbuf[8];
