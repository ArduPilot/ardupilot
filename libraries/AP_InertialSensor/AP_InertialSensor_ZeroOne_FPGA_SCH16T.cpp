...
#define BACKEND_SAMPLE_RATE       1475
...
static constexpr uint16_t RATE_300DPS_1475HZ = 0b0001001011011011; // Gyro XYZ range 300 deg/s @ 1518Hz
static constexpr uint16_t ACC12_8G_1475HZ = 0b0001001011011011;  // Acc XYZ range 8 G and 1518 update rate
...
AP_InertialSensor_ZeroOne_FPGA_SCH16T::AP_InertialSensor_ZeroOne_FPGA_SCH16T(AP_InertialSensor &imu,
    ...
    , rotation(_rotation)
{
    ...
    _registers[3] = RegisterConfig(CTRL_RATE,RATE_300DPS_1475HZ); // +/- 300 deg/s, 1600 LSB/(deg/s) -- default, Decimation 8, 1475Hz RATE_300DPS_1475HZ
    _registers[4] = RegisterConfig(CTRL_ACC12,ACC12_8G_1475HZ);    // +/- 80 m/s^2, 3200 LSB/(m/s^2) -- default, Decimation 8, 1475Hz ACC12_8G_1475HZ
    _registers[5] = RegisterConfig(CTRL_ACC3,ACC3_26G);           // +/- 260 m/s^2, 1600 LSB/(m/s^2) -- default ACC3_26G
}

AP_InertialSensor_Backend *
AP_InertialSensor_ZeroOne_FPGA_SCH16T::probe(AP_InertialSensor &imu,
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::start()
{
    // set backend rate
    backend_rate_hz = BACKEND_SAMPLE_RATE;

    if (!_imu.register_accel(accel_instance, backend_rate_hz, dev->get_bus_id_devtype(DEVTYPE_INS_ZEROONE_FPGA_SCH16T)) ||
        !_imu.register_gyro(gyro_instance, backend_rate_hz,   dev->get_bus_id_devtype(DEVTYPE_INS_ZEROONE_FPGA_SCH16T))) {
        return;
    }
    ...
    //calculate backend rate
    uint32_t period_us = 1000000UL / backend_rate_hz;
    // setup callbacks
    periodic_handle = dev->register_periodic_callback(period_us, FUNCTOR_BIND_MEMBER(&AP_InertialSensor_ZeroOne_FPGA_SCH16T::run_state_machine, void));
}

void AP_InertialSensor_ZeroOne_FPGA_SCH16T::sensor_ctrl(uint8_t fifo_rst)
{
...
bool AP_InertialSensor_ZeroOne_FPGA_SCH16T::init()
{
    ...
    uint8_t tries = 2;
    do {
        hal.scheduler->delay(250); // wait for the FPGA to power on
        reset_chip();
        hal.scheduler->delay(250); // wait for the FPGA to reset
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::init_fpga()
{
    ...
    fpga_read_config();//configure which parameters of the fpga will be read
...
uint8_t AP_InertialSensor_ZeroOne_FPGA_SCH16T::gen_crc8(uint8_t* data)
{
    uint16_t crc = 0xff;
    uint8_t byte_value;
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::_register_write(uint16_t addr, uint32_t value, uint8_t* buff)
{
    RegCtl reg_ctl{};
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::_register_read(uint16_t addr, uint8_t* buff)
{
    RegCtl reg_ctl{};
    reg_ctl.reg.cmd_reg_addr_l = (addr & 3) << 6;
    reg_ctl.reg.reg_addr_h = (addr >> 2) & 0xff;
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::fpga_read_config(void)
{
    uint8_t ptr = 0;
    /*configure which parameters of the fpga will be read */
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::sch16t_gen_cmd(uint32_t addr, uint8_t* buff, uint8_t read_valid, uint8_t is_sensor, uint8_t offset)
{
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::fpga_write_cmd(uint32_t addr, uint8_t ptr, uint8_t read_valid, uint8_t is_sensor, uint8_t offset)
{
    uint8_t buff[16];
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::run_state_machine()
{
    ...
    case State::Reset: {
        ...
        dev->adjust_periodic_callback(periodic_handle, 250000UL);//wait 250ms reset
        break;
    }
    ...
    case State::Validate: {
        read_status_registers();//the FPGA doesn't process the first instruction,it only starts processing after receiving the second instruction,resulting in the same command being sent twice.
...
bool AP_InertialSensor_ZeroOne_FPGA_SCH16T::read_data()
{
    ...
    for (; pkt_num>1; pkt_num--) {
        ...
        if (buff[0] & 0x80) {
            data.gyro_x = SPI32BITCONVERT2_20BIT(((buff[1] & 0xf) << 16) | (buff[2] << 8) | buff[3]);
        }
        if (buff[0] & 0x40) {
            data.gyro_y = -SPI32BITCONVERT2_20BIT(((buff[4] & 0xf) << 16) | (buff[5] << 8) | buff[6]);
        }
        if (buff[0] & 0x20) {
            data.gyro_z = -SPI32BITCONVERT2_20BIT(((buff[7] & 0xf) << 16) | (buff[8] << 8) | buff[9]);
        }
        if (buff[0] & 0x10) {
            data.acc_x = SPI32BITCONVERT2_20BIT(((buff[10] & 0xf) << 16) | (buff[11] << 8) | buff[12]);
        }
        if (buff[0] & 0x08) {
            data.acc_y = -SPI32BITCONVERT2_20BIT(((buff[13] & 0xf) << 16) | (buff[14] << 8) | buff[15]);
        }
        if (buff[0] & 0x04) {
            data.acc_z = -SPI32BITCONVERT2_20BIT(((buff[16] & 0xf) << 16) | (buff[17] << 8) | buff[18]);
        }
        if (buff[0] & 0x02) {
            data.temp = SPI32BITCONVERT2_20BIT(((buff[19] & 0xf) << 16) | (buff[20] << 8) | buff[21])>>4;
        }
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::reset_chip()
{
#if defined(HAL_GPIO_PIN_nSPI6_RESET_EXTERNAL1)
...
bool AP_InertialSensor_ZeroOne_FPGA_SCH16T::validate_sensor_status()
{
    auto &s = _sensor_status;
    const uint16_t values[] = { s.summary, s.saturation, s.common, s.rate_common, s.rate_x, s.rate_y, s.rate_z, s.acc_x, s.acc_y, s.acc_z };
...
uint8_t AP_InertialSensor_ZeroOne_FPGA_SCH16T::wait_direct_busy()
{
    ...
    while (timeout) {
        if (!(fpga_read_reg16(Addr_RW16_DirectSpiCtrl) & 1)) {
            return 1;
        }
        timeout--;
    }
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::read_status_registers()
{
    _sch16t_read_status(STAT_SUM);//the FPGA doesn't process the first instruction,it only starts processing after receiving the second instruction,resulting in the same command being sent twice.
...
bool AP_InertialSensor_ZeroOne_FPGA_SCH16T::register_read(uint16_t addr, uint32_t* value)
{
    uint8_t tbuf[8];
    uint8_t rbuf[8];
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::register_write(uint16_t addr, uint32_t value)
{
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::fpga_read_fifo8(uint16_t reg, uint8_t* value, uint8_t size)
{
    FpgaFrame frame{};
    if (value ==nullptr) {
        return;
    ...
    dev->transfer_fullduplex(frame.raw,size+5);
    memcpy(value, frame.fifo.data+1, size);
}

void AP_InertialSensor_ZeroOne_FPGA_SCH16T::fpga_write_reg8(uint16_t reg, uint8_t value)
{
    FpgaFrame frame{};

    fill_fpga_frame(frame, FuncBit_Write | FuncBit_Reg | FuncBit_Bit8, reg, 0);
    frame.reg.data[0] = value;
    dev->transfer_fullduplex(frame.raw,4);
}
uint8_t AP_InertialSensor_ZeroOne_FPGA_SCH16T::fpga_read_reg8(uint16_t reg)
{
    FpgaFrame frame{};
    ...
    dev->transfer_fullduplex(frame.raw,5);
    return frame.reg.data[1];
}

uint16_t AP_InertialSensor_ZeroOne_FPGA_SCH16T::fpga_read_reg16(uint16_t reg)
{
    FpgaFrame frame{};
    ...
    dev->transfer_fullduplex(frame.raw,6);
    return frame.reg.data[1] | (frame.reg.data[2] << 8);
}

void AP_InertialSensor_ZeroOne_FPGA_SCH16T::fpga_write_reg16(uint16_t reg, uint16_t value)
{
    FpgaFrame frame{};
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::fpga_write_ram8(uint16_t reg, uint16_t ram_addr, uint8_t* value, uint16_t size)
{
    FpgaFrame frame{};
    if (size> sizeof(frame.fifo.data)-1) {
        return;
...
void AP_InertialSensor_ZeroOne_FPGA_SCH16T::fpga_read_ram8(uint16_t reg, uint16_t ram_addr, uint8_t* value, uint16_t size)
{
    FpgaFrame frame{};
    ...
    dev->transfer_fullduplex(frame.raw,size+6);
    memcpy(value, frame.ram.data+1, size);
}

void AP_InertialSensor_ZeroOne_FPGA_SCH16T::fill_fpga_frame(FpgaFrame& frame, uint8_t cmd, uint16_t reg_addr, uint16_t ram_addr)
{
...
bool AP_InertialSensor_ZeroOne_FPGA_SCH16T::update()
{
