...
struct PACKED adis_burst_data
{
    ...
    uint16_t temp;
    uint16_t data_cntr;
    uint16_t timestamp_upr; // 0x0000 when scaled sync disabled (included in checksum)
    uint16_t spi_chksum;    // 16-bit byte-wise sum of all words except DIAG_STAT
};

class AP_InertialSensor_ADIS1657x : public AP_InertialSensor_Backend {
public:
    virtual ~AP_InertialSensor_ADIS1657x();
...
private: 
    AP_InertialSensor_ADIS1657x(AP_InertialSensor &imu,
                                AP_HAL::OwnPtr<AP_HAL::Device> _dev,
                                enum Rotation _rotation,
                                uint8_t drdy_gpio,
                                uint8_t rst_gpio);
    ...
    void accumulate_samples(const adis_burst_data *data);
