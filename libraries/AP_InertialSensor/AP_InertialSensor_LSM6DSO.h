...
#ifndef LSM6DSO_DEFAULT_ROTATION
#define LSM6DSO_DEFAULT_ROTATION ROTATION_NONE
#endif

class AP_InertialSensor_LSM6DSO : public AP_InertialSensor_Backend {
public:
    enum class LSM6DSO_Type: uint8_t {
        LSM6DSOXTR,
    };

    static AP_InertialSensor_Backend *probe(AP_InertialSensor &imu,
                                            AP_HAL::OwnPtr<AP_HAL::SPIDevice> dev,
                                            enum Rotation rotation=LSM6DSO_DEFAULT_ROTATION);

    ~AP_InertialSensor_LSM6DSO() override;
...
private:
    ...
    uint8_t fifo_bdr_code_for_rate(uint16_t rate_hz) const;
