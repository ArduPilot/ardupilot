...
/*
   ...
   along with this program.  If not, see <http://www.gnu.org/licenses/>.
    ...
    -- Coded by Victor Mayoral Vilches <victor@erlerobot.com> --
*/
...
#define MPU9255_ACCEL_SCALE_1G    (GRAVITY_MSS / 4096.0f)
...
// MPU6000 & MPU9255 registers
// not sure if present in MPU9255
...
// Configuration bits MPU 3000, MPU 6000 and MPU9255
...
AP_InertialSensor_MPU9255::AP_InertialSensor_MPU9255(AP_InertialSensor &imu) :
...
/*
  detect the sensor
 */
AP_InertialSensor_Backend *AP_InertialSensor_MPU9255::detect(AP_InertialSensor &_imu)
{
    AP_InertialSensor_MPU9255 *sensor = new AP_InertialSensor_MPU9255(_imu);
    if (sensor == NULL) {
        return NULL;
    }
 ...
 */
bool AP_InertialSensor_MPU9255::_init_sensor(void)
{
    _spi = hal.spi->device(AP_HAL::SPIDevice_MPU9255);
    ...
    if (whoami != 0x73) {
        ...
        hal.console->printf("MPU9255: unexpected WHOAMI 0x%x\n", (unsigned)whoami);
        return false;
    }
    ...
    do {
        ...
        if (success) {
            ...
            if (!_spi_sem->take(100)) {
                hal.console->printf("MPU9255: Unable to get semaphore");
                return false;
            }
 ...
 */
bool AP_InertialSensor_MPU9255::update( void )
{
...
/**
 * Timer process to poll for new data from the MPU9255.
 */
void AP_InertialSensor_MPU9255::_poll_data(void)
{
 ...
 */
void AP_InertialSensor_MPU9255::_read_data_transaction() 
{
 ...
 */
uint8_t AP_InertialSensor_MPU9255::_register_read( uint8_t reg )
{
 ...
 */
void AP_InertialSensor_MPU9255::_register_write(uint8_t reg, uint8_t val)
{
    uint8_t tx[2];
    uint8_t rx[2];
 ...
 */
void AP_InertialSensor_MPU9255::_set_filter(uint8_t filter_hz)
{
    if (filter_hz == 0) {
 ...
 */
bool AP_InertialSensor_MPU9255::_hardware_init(void)
{
    if (!_spi_sem->take(100)) {
        hal.console->printf("MPU9255: Unable to get semaphore");
        return false;
    ...
    if (tries == 5) {
        hal.console->println_P(PSTR("Failed to boot MPU9255 5 times"));
        _spi_sem->give();
        return false;
    }
...
void AP_InertialSensor_MPU9255::_dump_registers(void)
{
    hal.console->println_P(PSTR("MPU9255 registers"));
