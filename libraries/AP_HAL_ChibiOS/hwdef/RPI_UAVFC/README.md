...
There is no SPI OSD chip on this revision. Instead the analog overlay is
generated from PIO1 on the `OSD_W`, `OSD_EN` and `OSD_SYNC` lines (GPIO21-23),
genlocked to the camera, and selected with `OSD_TYPE 6`. It has been run with an
NTSC camera; PAL is untested. See `OSD.md`.
