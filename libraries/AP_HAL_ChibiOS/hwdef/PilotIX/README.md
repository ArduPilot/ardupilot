...
|------|---------|--------------------|--------|--------|
| 0    | OTG1    | MAVLink2 (USB)     | ✘      | ✘      |
| 1    | UART7   | MAVLink2 (Telem1)  | ✔      | ✔      |
| 2    | USART1  | MAVLink2 (Telem2)  | ✔      | ✔      |
| 3    | USART2  | GPS1               | ✔      | ✔      |
| 4    | USART3  | GPS2               | ✔      | ✔      |
| 5    | UART4   | MAVLink2 (spare)   | ✔      | ✔      |
| 6    | USART6  | RCin               | ✘      | ✘      |
| 7    | UART8   | MAVLink2 (spare)   | ✔      | ✔      |
...
WS2812 LED-strip output (S13). Configured by default for a quadcopter,
X frame (`FRAME_CLASS` 1, `FRAME_TYPE` 1): S1-S4 drive the motors,
S5-S12 are free for servos/gimbal/lighting.
...
Note that S3/S4 (motors M3, M4) share Group 2 with S5/S6 (spare outputs) —
if S3/S4 run DShot for ESCs, S5/S6 must also run DShot rather than a
standard 50Hz servo signal.

## OSD Support

Onboard analog OSD using an AT7456E (MAX7456-compatible driver) is enabled
by default (`OSD_TYPE` 1).

## VTX / Camera Switch

An onboard analog video multiplexer selects between two camera/VTX video
inputs. The select line is controlled via PINIO2 (GPIO 82). By default this
is not pre-bound to a RELAY; assign a RELAY or script to PINIO2 to switch
sources in flight.

PINIO1 (GPIO 81) switches peripheral ("Vsw") power and defaults LOW at boot.
...
## RSSI / Analog Airspeed

If an analog RSSI signal is used, set `RSSI_ANA_PIN` to 8 and `RSSI_TYPE`
to 1 (or to 3 if the RC protocol itself provides RSSI data). The pin is
labelled ADC_RSSI1 (PC5) on the schematic.
...
- `BATT_VOLT_PIN` = 10
- `BATT_CURR_PIN` = 11
- `BATT_VOLT_MULT` = 11.21 (verified against a multimeter and a known 16.8V 4S source)
- `BATT_AMP_PERVLT` = 15.49 (calibrated against a current clamp at 50% throttle)
...
- `BATT2_VOLT_PIN` = 18
- `BATT2_CURR_PIN` = 7
- `BATT2_VOLT_MULT` = 21.46 (verified against a multimeter and a known 16.8V 4S source)
- `BATT2_AMP_PERVLT` = 15.58 (calibrated against a current clamp at 50% throttle)
...
## Verification checklist before first flash / first power-up

- [x] Confirm BOARD_ID 6120 does not collide with another board (already
      reserved as `AP_HW_PILOTIX` in `Tools/AP_Bootloader/board_types.txt`).
- [x] Confirm GYRO_CS1 (PC15) / GYRO_CS2 (PC13) / OSD_CS1 (PB12) with a
      continuity check.
- [x] Confirm S1-S4 motor order matches ESC wiring (props off first test).
- [x] Confirm BATT_VOLT_MULT / BATT2_VOLT_MULT with a multimeter.
- [x] Confirm BATT_AMP_PERVLT / BATT2_AMP_PERVLT with a known current load.
- [x] Confirm which physical UART connector is which before plugging in a
      GPS/telemetry radio.
- [x] Build with the bootloader first, flash it over SWD/ST-Link, then
      flash the main firmware over USB/DFU.
