...
- Interfaces
  ...
  - 11 PWM outputs
  - SBUS input via the `RC` and `SBUS` pads
- Power
...
The `ADC` connector provides two analog inputs:

| Label | ArduPilot analog input |
|---|---|
| ADC6V6 | 12 |
| ADC3V3 | 13 |
...
The IIC2 connector maps to `I2C2` and is intended for an external compass or GPS/compass module. The `GPS2` connector also carries the `I2C2` bus.

## UART Mapping

| Port | Connector | UART | Protocol | TX DMA | RX DMA |
|---|---|---|---|---|---|
| SERIAL0 | USB | OTG1 | MAVLink2 | No | No |
| SERIAL1 | TELEM1 | UART7 | MAVLink2 | Yes | Yes |
| SERIAL2 | TELEM2 | UART5 | MAVLink2 | Yes | Yes |
| SERIAL3 | GPS&SAFETY | USART1 | GPS | Yes | Yes |
| SERIAL4 | GPS2 | UART8 | GPS | Yes | Yes |
| SERIAL5 | TELEM3 | USART2 | MAVLink2 | Yes | Yes |
| SERIAL6 | UART4 | UART4 | None | Yes | Yes |
| SERIAL7 | DEBUG | USART3 | None | Yes | Yes |
| SERIAL8 | SBUS | USART6 | RCIN | n/a | Yes |

All configured UARTs have DMA capability. TELEM1, TELEM2, and TELEM3 also provide hardware flow control (RTS/CTS). SERIAL7 is available on the `DEBUG` connector, and SERIAL4 is available on the `GPS2` connector. USART6 is RX only.

## RC Input

The `RC` and `SBUS` pads are connected to USART6 RX, which maps to SERIAL8. SERIAL8 is configured for RC input by default (`SERIAL8_PROTOCOL` = `23`), so a receiver connected to either pad works out of the box. PPM and DSM input are not supported.
...
For each PWM output connector, the pinout image legend shows the pins from left to right as `GND` (`-`), `5V` (`+`), and `Signal` (`S`).
...
The board provides an `nARMED` status signal on the pin labeled `nARMED` at the end of the PWM header, to drive an external LED or relay, indicating the current flight controller state.
