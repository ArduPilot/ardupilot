...
- MCU - STM32H757 dual-core (Cortex-M7 / M4) running the M7 core at 400 MHz, 2 MB flash
...
- Voltage and current monitoring on the battery rail; rail-health sensing on 3V3 (analog pin 1), the 10 V / 12 V rail (analog pin 4, reported as servo rail voltage) and 5V (analog pin 9, reported as board voltage)
- 30 x 30 mm mounting
...
| SERIAL2 | USART2 | VTX J9 pin 5 (RX only) | None -- user picks (RX-only protocols) | Yes |
...
`SERIAL2_PROTOCOL` is left at None because J9 pin 5 (USART2 RX) can be used for several different camera/VTX backchannels -- set it to match the device you connect. Only the RX pin is brought out, so this port suits protocols the flight controller only listens to (for example ESC telemetry or a GPS); protocols that need to talk back, such as RunCam control or MSP, will not work here.
...
The on-board LEDs and the power-good monitor inputs are also GPIOs, although
the LEDs are driven by the notify system by default:
