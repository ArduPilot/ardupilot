...
All ArduPilot supported unidirectional RC protocols can be input here including PPM. For bi-directional or half-duplex protocols, such as CRSF/ELRS a full UART will have to be used.
...
Channels within the same group need to use the same output rate. If any channel in a group uses DShot then all channels in the group need to use DShot. Outputs 1-8 support BDShot.

1-8 PWM outputs of CUAV-X25-EVO flight controller support switching between 3.3V voltage and 5V voltage output. It can be switched to 5V by setting GPIO 80 high by setting up a Voltage-Level Translator to control it.

## RSSI

CUAV-X25-EVO has an analog RSSI voltage monitoring. Set parameter `RSSI_TYPE` to 1 and `RSSI_ANA_PIN` to 10 for use.

## Compass

The CUAV-X25-EVO has a RM3100 builtin compass, you can also attach external compass using I2C port.
...
The CUAV-X25-EVO has 5 analog inputs.

- ADC Pin5 -> Vdd 5V supply sense
- ADC Pin6 -> Servo rail voltage sense
