...
- 9V 3A BEC for VTX, GPIO controlled
- 5V 3A BEC
- MicroSD Card Slot
...
The 9V VTX BEC output can be switched on and off by an RC channel. The VTX power switch (Vsw) is on GPIO 81 (PINIO1) and is held on during bootloading so the VTX is not power-cycled by a firmware update. Set the parameters below (RC 7 used in the example):
