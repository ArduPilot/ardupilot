...
- SERIAL8 -> UART8 (VTX control, TX8 on the analog VTX connector)
...
SPI3 is brought out to an external SPI port with two chip-select lines (EXT_CS1 and EXT_CS2). Support for a Pixart optical flow sensor on EXT_CS1 is pre-configured.
...
The pad is pre-configured as Relay 1: RELAY1_PIN defaults to 81 and the relay function is enabled automatically. The pad idles low at boot, and the bootloader holds it low too, so its level does not change when the firmware starts. If your external switch is active-low (enabled when PD10 is low), set RELAY1_INVERTED = 1 so a relay command of "on" drives the pad low.
