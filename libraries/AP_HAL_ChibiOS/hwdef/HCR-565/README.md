...
The HCR-565 is available from the [HC Robotics flight controller store](https://www.hcrobo.com/flight-controller/).
...
|SERIAL3|UART4|GPS|GPS (5)|General purpose|
|SERIAL4|UART5|General purpose|None (-1)|Protocol explicitly disabled by default|
|SERIAL5|UART7|sBUS output|SBUS Out (15)|TX only|
|SERIAL6|USART1|RC input|RCIN (23)|RX only; requires `BRD_ALT_CONFIG = 1`|
|SERIAL7|OTG2 USB|SLCAN|SLCAN (22)|Routed to CAN1 via `CAN_SLCAN_CPORT = 1`|

Only TELEM1 (USART2) and TELEM2 (USART3) provide hardware RTS/CTS flow control. The other UARTs do not.

USART1 RX is shared with the RC input pin (see [RC Input](#rc-input)) and is only available as SERIAL6 when `BRD_ALT_CONFIG = 1`.

## RC Input

RC input is on the dedicated RC IN pin (PB15), which is wired in two modes selected by the `BRD_ALT_CONFIG` parameter:

* `BRD_ALT_CONFIG = 0` (default): PB15 is connected to TIM12 in capture mode, supporting all unidirectional RC protocols (PPM, SBUS, DSM, ST24, SUMD, etc.).
* `BRD_ALT_CONFIG = 1`: PB15 is connected to `USART1_RX` and exposed as SERIAL6. This is an RX-only connection with no USART1 TX pad. FPort is supported on this H7 UART by setting `SERIAL6_PROTOCOL = 23` and `SERIAL6_OPTIONS = 15`.

CRSF/ELRS, GHST, and SRXL2 cannot use the RX-only SERIAL6 connection because they require a TX connection. Use one of the full TX/RX ports: SERIAL1 (USART2), SERIAL2 (USART3), SERIAL3 (UART4), or SERIAL4 (UART5). Configure the selected port as follows:

* CRSF/ELRS: `SERIALx_PROTOCOL = 23` and `SERIALx_OPTIONS = 0`. Reliable operation requires a DMA-capable UART.
* GHST: `SERIALx_PROTOCOL = 23`, connect the receiver signal to the UART TX pin, and set `SERIALx_OPTIONS = 4`.
* SRXL2: `SERIALx_PROTOCOL = 23`, connect the receiver signal to the UART TX pin, and set `SERIALx_OPTIONS = 4`.

See [RC systems](https://ardupilot.org/copter/docs/common-rc-systems.html) for additional protocol details.

## sBUS Output

The HCR-565 provides a dedicated sBUS output on UART7 TX (SERIAL5), preconfigured with `SERIAL5_PROTOCOL = 15`. This is intended for driving sBUS-compatible servos or external receivers/PWM expanders.
...
The board has two power inputs, each of which supports both analog voltage sensing and CAN-based power monitor inputs. Analog monitoring is enabled by default, and the dedicated channels are scaled for high-voltage battery sensing (3.3 V × 11 ≈ 36.3 V max, covering up to 8S LiPo):

* Power 1 voltage -> analog input 1 (`BATT_MONITOR = 4`, `BATT_VOLT_PIN = 1`, `BATT_VOLT_MULT = 11`)
* Power 2 voltage -> analog input 2 (`BATT2_MONITOR = 4`, `BATT2_VOLT_PIN = 2`, `BATT2_VOLT_MULT = 11`)

For digital battery monitoring, DroneCAN/UAVCAN power monitors can be attached to either CAN bus. Both CAN drivers are enabled by default (`CAN_P1_DRIVER = 1`, `CAN_P2_DRIVER = 2`).
...
The onboard RGB status LED uses GPIOs 90–92. GPIOs 91 and 92 are exceptions to the external GPIO range above and are reserved for the green and blue LEDs.
...
The HCR-565 has two independent FDCAN buses, each rated up to 8 Mbit. Both drivers are enabled by default (`CAN_P1_DRIVER = 1`, `CAN_P2_DRIVER = 2`), and CAN1 is preconfigured as the SLCAN port (`CAN_SLCAN_CPORT = 1`) so that the OTG2 USB virtual serial port can be used as a SLCAN bridge.
...
Dataflash logging is provided by the onboard 2 Gbit W25N01 NAND flash, accessed through LittleFS. `LOG_BACKEND_TYPE` defaults to 1.

## IMU Orientation and Position

The two ICM42688 IMUs are mounted with different rotations to decorrelate vibration:

* IMU1 (SPI3, ICM42688_A): `ROTATION_ROLL_180_YAW_90`
* IMU2 (SPI2, ICM42688_B): `ROTATION_ROLL_180_YAW_45`

The physical offsets of each IMU from the board origin are preset in firmware:

* IMU1: `INS_POS1_X = 0.00375`, `INS_POS1_Y = 0.0075`
* IMU2: `INS_POS2_X = 0.00775`, `INS_POS2_Y = 0.0075`

Fast sampling is enabled on both IMUs by default (`HAL_DEFAULT_INS_FAST_SAMPLE = 3`).
