...
RC input is on the **RX4** pad. The pad has two operating modes selected by `BRD_ALT_CONFIG`:

**Default (`BRD_ALT_CONFIG = 0`)** — RX4 is connected to a timer interrupt and will support all uni-directional RC protocols, including PPM.

**Set `BRD_ALT_CONFIG = 1`** to use RX4 as input to UART4 for bi-directional RC protocols like CRSF/ELRS. UART4 protocol is set by default to accept bi-directional RC inputs. The `SERIAL4_OPTIONS` may need adjustment for half-duplex protocols. See [RC Systems](https://ardupilot.org/plane/docs/common-rc-systems.html) for more information.

## OSD Support

The SimpliFly H7 has a built-in AT7456E (MAX7456) OSD chip on SPI2. The OSD is enabled by default (`OSD_TYPE = 1`).

MSP DisplayPort OSD (`OSD_TYPE2 = 5`) is also enabled by default, allowing simultaneous use of an external DisplayPort OSD device. Configure a free UART for MSP or DisplayPort protocol (`SERIALx_PROTOCOL` parameter) to use this feature.
...
An analog RSSI input is available on the **RSSI** pad.
