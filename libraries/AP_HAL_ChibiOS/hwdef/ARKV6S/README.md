...
_**Note:** HW_VER_SENSE, HW_VER_REV_DRIVE, and PD15 are unused by Ardupilot._
...
By default, RC input is configured on the USART6 RX pin (labeled `USART6_RX_FROM_IO__RC_INPUT`). It supports all unidirectional RC protocols except PPM. Bi-directional protocols (CRSF/ELRS, etc.) also require connection to the USART6 TX pin (labeled `USART6_TX_TO_IO__NC`). See [Radio Control Systems](https://ardupilot.org/copter/docs/common-rc-systems.html) for details on a specific RC system.

A separate timer input (labeled `FMU_PPM_INPUT`) is also available for unidirectional RC protocols, including PPM.
...
PWM outputs are provided through the module connector (labeled `FMU_CHx`). PWM 1-6 are capable of PWM and DShot (including bi-directional DShot). PWM 7-8 are PWM-only since they have no DMA. Outputs in the same timer group must use the same protocol:
...
The ARKV6S ships with the PX4 bootloader pre-installed, which is ArduPilot compatible and allows loading of \*.apj firmware files with any ArduPilot compatible ground station. Firmware for this board can be found at the [ArduPilot firmware server](https://firmware.ardupilot.org) in sub-folders labeled "ARKV6S".

If desired, the ArduPilot bootloader can be installed after ArduPilot firmware has been loaded by having the ground station update the bootloader (in Mission Planner: SETUP -> Install Firmware -> Bootloader Update, or `flashbootloader` in MAVProxy). Alternatively, the ArduPilot bootloader can be flashed directly using an ST-LINK. Pre-built bootloader binaries can be found at the [ArduPilot firmware server](https://firmware.ardupilot.org/Tools/Bootloaders/).
