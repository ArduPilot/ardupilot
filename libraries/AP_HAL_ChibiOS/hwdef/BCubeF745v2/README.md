...
- **Interfaces**
  - USB Type-C
  - 6 PWM Outputs (5 with DShot and 4 with Bi-Dir DShot support)
  - 6 UARTs / USARTs
  - 1 I2C bus (I2C1)
  - Current, Voltage
  - Buzzer output
...
- **Power**
  - Input voltage: 3-8S LiPo (11.1V - 33.6V)
...
| 0      | OTG1   | MAVLink2   | ✘      | ✘      | USB |
| 1      | USART1 | MAVLink2   | ✘      | ✘      | |
| 2      | USART2 | RCIN       | ✔      | ✔      | Default RC input |
| 3      | USART3 | NONE       | ✘      | ✘      | |
| 4      | UART4  | GPS1       | ✘      | ✘      | |
| 6      | USART6 | Display    | ✔      | ✔      | |
...
By default, the RC input is mapped to USART2 (RX pad). This interface supports all ArduPilot compatible RC systems, except PPM, including SBUS, CRSF, and FPort. USART2 has DMA enabled, satisfying the requirements for protocols like CRSF/ELRS and SRXL2.

## PWM Outputs

The B-CUBE F745 V2 controller supports 6 PWM outputs. The outputs support normal PWM output formats.
Outputs 3-4 support Bi-Directional DShot. Output 6 supports DShot, but not Bi-Directional DShot.
Output 5 does not have DMA assigned, so it does not support DShot.
Output 7 is defaulted to serial LED, but can be used for PWM or DShot by changing its `SERVO7_FUNCTION <SERVO7_FUNCTION >`
...
On-board analog OSD (using the AT7456E) is supported and enabled by default. Simultaneous HD VTX (DisplayPort) operation on SERIAL6 is also enabled.

## Compass

The B-CUBE F745 V2 does not have a built-in compass. However, an external compass can be connected using the SCL and SDA pads.
