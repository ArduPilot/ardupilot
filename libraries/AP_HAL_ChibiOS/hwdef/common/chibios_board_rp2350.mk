# Shared ChibiOS board makefile for the RP2350 (Cortex-M33, dual core) targets.
#
# chibios.py picks hwdef/<BOARD>/chibios_board.mk by directory name and falls
# back to common/chibios_board.mk, which builds the ARMv7-M port. RP2350 needs
# the ARMv8-M-ML-ALT SMP port, ArduPilot's board.c and the core1 startup
# trampoline, so every RP2350 target routes through this file instead. A board
# with no chibios_board.mk of its own silently gets the ARMv7-M fallback and
# fails with "unknown ARMv7-M variant".
#
...
ifeq ($(USE_OPT),)
  USE_OPT = -O3 -ggdb -fomit-frame-pointer -falign-functions=16 -DCRT0_AREAS_NUMBER=$(RP2350_CRT0_AREAS_NUMBER)
endif
...
# in hwdef/common/malloc.c. Drop fatfs_syscall.c to avoid duplicate symbols.
...
UDEFS = $(ENV_UDEFS) $(FATFS_FLAGS) -DHAL_BOARD_NAME=\"$(HAL_BOARD_NAME)\" \
        ...
        -DCRT0_EXTRA_CORES_NUMBER=1 $(RP2350_EXTRA_UDEFS)
