 ...
 * Both sides move the lock by compare-and-swap: Core1 parks only on 1->2, and
 * on timeout Core0 withdraws 1->0 and the flash operation fails, so Core1 is
 * never left parked with nobody to release it, nor running while XIP is off.
...
bool rpEflBeforeXipOff(void)
{
    ...
    while (c1_xip_lock != 2U) {
        if ((int32_t)(TIMER0->TIMERAWL - deadline) >= 0) {
            /* timeout -- withdraw the request, so a late Core1 does not park
               with nobody left to release it, and fail the flash operation,
               since Core1 is still running from XIP. If Core1 parked in the
               meantime the swap fails and it is released as usual. */
            uint32_t requested = 1U;
            if (__atomic_compare_exchange_n(&c1_xip_lock, &requested, 0U, false,
