...
- Onboard SPI SD NAND (CSNP1GCR01-BOW) for log storage (FAT filesystem)
- 2 I2C buses
...
- SERIAL0 -> USB (OTG1)
- SERIAL1 -> USART1 (MAVLink2)
- SERIAL2 -> USART2 (GPS)
- SERIAL3 -> USART3 (MAVLink2)
- SERIAL4 -> UART4 (GPS2, can be changed to ESC telemetry if protocol is changed)
- SERIAL5 -> UART5 (User)
- SERIAL6 -> USART6 (optional, shared with PWM1/PWM2, not assigned by default)
- SERIAL7 -> UART7 (not assigned by default)
- SERIAL8 -> UART8 (not assigned by default)
...
2. Set `NTF_LED_TYPES` = 257 (NeoPixel + other options) in Mission Planner.
3. Set `NTF_LED_LEN` to the number of LEDs.
4. Set `SERVO11_FUNCTION` = 120 (NeoPixel).
...
| SD NAND  | SPI3 | PD7     | Onboard SPI SD NAND (CSNP1GCR01-BOW), FAT filesystem for logs |
...
The board includes an onboard SPI SD NAND chip (CSNP1GCR01-BOW) for data logging.

- The chip is accessed via SPI3 (CS: PD7).
...
- **Firmware folder**: The compiled firmware binary will be located in `/ardupilot/build/AET_H743_Air/bin/arducopter.apj`
