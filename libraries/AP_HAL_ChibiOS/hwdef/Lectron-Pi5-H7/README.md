...
Firmware for the Lectron Pi5-H7 can be found [here](<https://firmware.ardupilot.org>)

## Loading Firmware

Boards ship with the ArduPilot bootloader pre-installed. Firmware for the
Lectron Pi5-H7 is available on the ArduPilot firmware server at [ardupilot](https://firmware.ardupilot.org).

Because the bootloader is already present, firmware can be loaded directly
through a ground station (Mission Planner, QGroundControl, etc.) as an
*.apj file over USB - no additional tools required. Subsequent firmware
