...
- Interfaces
  ...
  - 6 UARTs plus USB; UART1 is shared with the USB extender connector
  - I2C port for external compass, digital airspeed, and other I2C peripherals
  - Analog airspeed input
  - microSD card slot
  - USB-C port
  - USB extender connector for the optional BLE/WiFi wireless module
...
SERIAL1 is wired to the USB extender connector, which is where the optional BLE/WiFi wireless module plugs in, and is also brought out to the `T1` and `R1` solder pads. Only one of the two can be used at a time. `SERIAL1_BAUD` defaults to 115200 to match the wireless module, so a device wired to the `T1`/`R1` pads must run at that rate or have `SERIAL1_BAUD` changed.

SERIAL2 RX is tied to the inverted SBUS RC input. TX2 and RX2 are also present on the `TELEM2/4` connector, but they only work as a normal UART if `BRD_ALT_CONFIG = 1` and no signal drives the SBUS pin.

SERIAL3 is shared between the analog VTX connector (`VTX/TX3/GND/9V`) and the HD VTX connector (`SBUS/GND/RX3/TX3/GND/9V`). Only one video transmitter can be connected at a time, and the protocol has to match it. See OSD Support and VTX Control below.

SERIAL4 is the `TELEM2/4` connector, a GH1.25 6P carrying `GND RX2 TX2 RX4 TX4 4V5`. The same RX4/TX4 signals are on the `5V T4 R4 G` telemetry solder pads. It defaults to MAVLink2 for a telemetry radio.

The GPS solder pads and the GPS connector do not use the same pin order. Check the pinout images before wiring a GPS module.
...
- CRSF / ELRS / TBS Crossfire: connect to TX6 and RX6
...
The optional CoreWing wireless module plugs into the USB extender connector and supports BLE and WiFi AP/STA modes. The [CoreWing app](https://www.corewing.com/en/app/) can be used for wireless parameter setup, wireless firmware flashing, and wireless board settings. Mission Planner can connect through WiFi using TCP or UDP.
...
The CoreWing F405 Wing V2 supports using its internal analog OSD using OSD_TYPE 1 (MAX7456 driver), which is set by default.

An HD VTX using MSP DisplayPort connects to the HD VTX connector, which uses UART3. Set `SERIAL3_PROTOCOL = 42` and `OSD_TYPE2 = 5`. See [MSP OSD](https://ardupilot.org/copter/docs/common-msp-osd-overview-4.2.html) for more information.

UART3 is shared with the analog VTX connector, so DisplayPort OSD and analog VTX control cannot both be used.

## VTX Control

TX3 is present on both the analog VTX connector and the HD VTX connector, so only one of them can be used at a time and the protocol has to match the VTX that is actually plugged in:

- analog VTX with SmartAudio: `SERIAL3_PROTOCOL = 37`
- analog VTX with IRC Tramp: `SERIAL3_PROTOCOL = 44`
- HD VTX with MSP DisplayPort: `SERIAL3_PROTOCOL = 42` and `OSD_TYPE2 = 5`

See [VTX support](https://ardupilot.org/plane/docs/common-vtx.html) for more information.

## VTX Power Control

GPIO 81 switches the VTX/CAM power output through Relay 1. The output is high-side switched, so `RELAY1_INVERTED = 1` is set by default and Relay 1 on means VTX/CAM power on. `RELAY1_DEFAULT = 1` is also set by default, so the VTX/CAM supply is powered at boot.

To switch the supply from a transmitter, assign an RC channel to Relay 1, for example on Channel 7:

- RC7_OPTION = 28

Set `RELAY1_DEFAULT = 0` if you would rather have the VTX/CAM supply off until it is switched on.
...
The board is not shipped with an ArduPilot bootloader, so the first load has to be done over DFU. There is no boot button on the flight controller itself: pull the `BOOT0` pin on the USB extender connector low while powering the board over USB, or hold the BOOT button on the wireless USB extender module if one is fitted. Then use the STM32CubeProgrammer app to load the `***_with_bl.hex` file.

Subsequently, a GCS such as Mission Planner, or the CoreWing app, can update firmware using the `*.apj` files.
