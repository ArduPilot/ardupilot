...
> **Notes:**
>
> - The **SBUS** pin on the DJIO3 connector is internally routed to the **RX6 (RCIN)** pin.
...
> **Note**: The SBUS pin on the DJIO3 connector is internally routed to RX6 and can be used as an alternative RC input for SBUS receivers. Because it is the same pin as RX6 on the RCIN connector, only one of the two may be driven at a time — a DJI air unit outputting SBUS on DJIO3 will conflict with a receiver plugged into RCIN.
...
The motor order follows the standard ArduPilot/PX4 quad-X convention:
...
> **Note**: `BATT_AMP_PERVLT` will need to be adjusted to match the current sensor on your ESC, since the current sense input (the `BAT I` pin on the ESC1 connector) comes from an external sensor rather than from the board.
...
The ZenFC H743 includes a CAN transceiver and a dedicated CAN connector. This port is enabled by default (CAN_P1_DRIVER = 1) to support DroneCAN peripherals such as GPS, Compasses, and ESCs.
