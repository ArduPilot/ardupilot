...
/*
  OSD_TYPE or OSD_TYPE2 selecting the RP2350 PIO scan-out. AP_OSD's enum runs 0-5 and gains
...
PIO1Owner pio1_intended_owner(void)
{
    ...
    const bool got2 = AP_Param::get("OSD_TYPE2", osd_type2);
    /*
      ...
      OSD_TYPE2 is the same, for the PIO overlay run as the second OSD beside
      an MSP one.
     */
    if ((got && is_equal(osd_type, OSD_TYPE_PICO)) ||
        (got2 && is_equal(osd_type2, OSD_TYPE_PICO))) {
