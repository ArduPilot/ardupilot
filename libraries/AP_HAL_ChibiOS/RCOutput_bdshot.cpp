    ...
    for (uint8_t i = 0; i < 4; i++) {
        ...
        if (group.chan[i] != CHAN_DISABLED && _bdshot.mask & group.ch_mask) {
            ...
            /*
              ...
              re-apply the same mode.  We are only here when
              is_bidir_dshot_enabled() is true at runtime, so a pin
              merely declared BIDIR keeps its hold; one whose group has
              bidirectional DShot enabled gets no protection.  The hwdef
              allows HOLD_HIGH and BIDIR together deliberately.
             */
