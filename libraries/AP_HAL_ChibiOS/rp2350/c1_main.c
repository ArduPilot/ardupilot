...
static void c1_xip_lockout_handler(void)
{
    ...
    /* Signal Core0: Core1 is parked, XIP-off is now safe. Only while the
       request stands: if Core0 has timed out and withdrawn it, parking now
       would spin here for good, since nothing would clear the lock. */
    uint32_t requested = 1U;
    if (!__atomic_compare_exchange_n(&c1_xip_lock, &requested, 2U, false,
