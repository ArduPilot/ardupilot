...
Core0  rpEflBeforeXipOff():
         ...
         on timeout: CAS 1 → 0, and if that succeeds return false,
           so the flash operation fails with FLASH_ERROR_HW_FAILURE
...
Core1  c1_xip_lockout_handler()  [.ramtext, runs from SRAM]:
         ...
         CAS c1_xip_lock 1 → 2       ← ack: Core1 parked
           (if Core0 already withdrew: restore NVIC and return)
