...
public:
    ...
    // bind the LED chain on an output channel to a GPIO, giving it a state
    // machine of its own. neopixel_init() must have been called first.
    static bool neopixel_add_channel(uint8_t chan, uint8_t gpio);
     ...
     */
    static bool neopixel_send_begin(uint8_t chan);
    static bool neopixel_send_word(uint8_t chan, uint32_t word);
    static void neopixel_send_end(uint8_t chan);
...
private:
    ...
    // state machine bound to an output channel, or -1
    static int8_t neopixel_sm_for(uint8_t chan);

    static bool     _neop_initialised;
    // state machines in use, and the output channel each one drives
    static uint32_t _neop_chan_mask;
    static uint8_t  _neop_chan[MAX_CHANNELS];
