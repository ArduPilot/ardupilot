...
uint8_t  RCOutput_pico::_neop_chan[RCOutput_pico::MAX_CHANNELS];
...
int8_t RCOutput_pico::neopixel_sm_for(uint8_t chan)
{
    for (uint8_t sm = 0; sm < MAX_CHANNELS; sm++) {
        if ((_neop_chan_mask & (1U << sm)) && _neop_chan[sm] == chan) {
            return sm;
        }
    }
    return -1;
...
bool RCOutput_pico::neopixel_add_channel(uint8_t chan, uint8_t gpio)
{
    if (!_neop_initialised) {
        return false;
    }
    int8_t idx = neopixel_sm_for(chan);
    for (uint8_t sm = 0; idx < 0 && sm < MAX_CHANNELS; sm++) {
        if (!(_neop_chan_mask & (1U << sm))) {
            idx = sm;
        }
    }
    if (idx < 0) {
...
bool RCOutput_pico::neopixel_send_begin(uint8_t chan)
{
    const int8_t idx = neopixel_sm_for(chan);
    if (!_neop_initialised || idx < 0) {
        return false;
    }
...
bool RCOutput_pico::neopixel_send_word(uint8_t chan, uint32_t word)
{
    PIO_TypeDef *pio = NEOP_PIO;
    const int8_t sm = neopixel_sm_for(chan);
    if (sm < 0) {
        return false;
    }
...
void RCOutput_pico::neopixel_send_end(uint8_t chan)
{
    const int8_t idx = neopixel_sm_for(chan);
    if (idx < 0) {
        return;
...
bool RCOutput::serial_led_send(pwm_group &group)
{
     ...
     */
    {
        ...
        for (uint8_t i = 0; i < HAL_PWM_GROUP_CHANNELS; i++) {
            ...
            if (!RCOutput_pico::neopixel_send_begin(chan)) {
                continue;
            }
            for (uint8_t led = 0; led < group.serial_nleds; led++) {
                ...
                if (!RCOutput_pico::neopixel_send_word(chan, w)) {
                    break;
                }
            }
            RCOutput_pico::neopixel_send_end(chan);
        }
    }
    return true;
}
