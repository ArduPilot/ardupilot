...
# AP_FLAKE8_CLEAN
"""
Decentralized GSoC-to-CoSG morph with onboard CBF avoidance over AP_SwarmMesh.

The leader publishes only which word is active and where it is anchored, in its
coordination basket. Every other vehicle runs swarm_speller_morph.lua, derives its
...
Usage:
    python3 swarm_spell_morph_test.py --work-dir ~/morph_run --csv ~/morph_run/track.csv
"""
...
COHORT_SIZE = 2
COHORTS_PER_WAVE = 4
APPLET = (Path(__file__).resolve().parents[2] / "AP_Scripting" / "applets" / "swarm_speller_morph.lua")
...
def parse_args():
    ...
    ap.add_argument("--word", default="GSoC", help="word to spell (must be in the Lua WORDS list)")
    ap.add_argument("--then", default="CoSG", help="second word to morph into (default: CoSG)")
    ...
    # asserted, and the Lua script switches those rules on whenever position data is available.
    ...
    ap.add_argument("--form-timeout", type=float, default=1500.0,
                    help="per-phase timeout for the scheduled cohort waves, s (default: 1500)")
    ...
    ap.add_argument("--work-dir", default="./swarm_spell_morph_run")
    ap.add_argument("--csv", default="morph_track.csv")
    ap.add_argument("--glyph-csv", default=None, help="write the target cells here (default: alongside --csv)")
    ap.add_argument("--task-csv", default=None, help="write task time windows here (default: alongside --csv)")
...
def main():
    ...
    task_csv = Path(args.task_csv) if args.task_csv else Path(args.csv).with_name(
        Path(args.csv).stem + "_tasks.csv")
    ...
    # cell it has no claim to, which effects the presence rules and lets nearest free cell
    ...
    with open(task_csv, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["word", "start_s", "end_s"])
        for name, start_s, end_s in task_windows:
            w.writerow([name, f"{start_s:.2f}", f"{end_s:.2f}"])
    print(f"    wrote {task_csv} (task windows)")
