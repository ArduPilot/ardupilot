 ...
 */
static bool test_byte_protocol(const char *name, const AP_RCProtocol::SerialConfig& config,
                               ...
                               uint8_t pause_at)
{
    ...
    for (uint8_t repeat=0; repeat<repeats+4; repeat++) {
        for (uint8_t i=0; i<nbytes; i++) {
            ...
            rcprot->process_byte(bytes[i], config);
        }
        hal.scheduler->delay(10);
        if (repeat > repeats) {
...
/*
  test a byte protocol handler
 */
static bool test_pulse_protocol(const char *name, const AP_RCProtocol::SerialConfig& config,
                                const uint8_t *bytes, uint8_t nbytes,
                                const uint16_t *values, uint8_t nvalues,
                                uint8_t repeats, uint8_t pause_at)
{
    bool ret = true;
    for (uint8_t repeat=0; repeat<repeats+4; repeat++) {
        send_pause(1, config.baud, 6000, config.invert_rx);
        for (uint8_t i=0; i<nbytes; i++) {
            if (pause_at > 0 && i > 0 && ((i % pause_at) == 0)) {
                send_pause(1, config.baud, 10000, config.invert_rx);
            }
            send_byte(bytes[i], config.baud, config.invert_rx);
        }
        send_pause(1, config.baud, 6000, config.invert_rx);
        if (repeat > repeats) {
 ...
 */
static bool test_protocol(const char *name, const AP_RCProtocol::SerialConfig& config,
                          const uint8_t *bytes, uint8_t nbytes,
                          const uint16_t *values, uint8_t nvalues,
                          uint8_t repeats=1,
                          int8_t pause_at=0)
{
    ...
    ret &= test_byte_protocol(name, config, bytes, nbytes, values, nvalues, repeats, pause_at);
    delete rcprot;

    rcprot = new AP_RCProtocol();
    rcprot->init();
    ret &= test_pulse_protocol(name, config, bytes, nbytes, values, nvalues, repeats, pause_at);
    delete rcprot;
...
void loop()
{
    ...
    test_protocol("SRXL", AP_RCProtocol::DEFAULT_SR_CONFIG, srxl_bytes, sizeof(srxl_bytes), srxl_output, ARRAY_SIZE(srxl_output), 1);
    test_protocol("SUMD", AP_RCProtocol::DEFAULT_SR_CONFIG, sumd_bytes, sizeof(sumd_bytes), sumd_output, ARRAY_SIZE(sumd_output), 1);
    test_protocol("SUMD2", AP_RCProtocol::DEFAULT_SR_CONFIG, sumd_bytes2, sizeof(sumd_bytes2), sumd_output2, ARRAY_SIZE(sumd_output2), 1);
    test_protocol("SUMD3", AP_RCProtocol::DEFAULT_SR_CONFIG, sumd_bytes3, sizeof(sumd_bytes3), sumd_output3, ARRAY_SIZE(sumd_output3), 1);
    test_protocol("IBUS", AP_RCProtocol::DEFAULT_SR_CONFIG, ibus_bytes, sizeof(ibus_bytes), ibus_output, ARRAY_SIZE(ibus_output), 1);

    // SBUS needs 3 repeats to pass the RCProtocol 3 frames test
    test_protocol("SBUS", AP_RCProtocol::SBUS_SR_CONFIG, sbus_bytes, sizeof(sbus_bytes), sbus_output, ARRAY_SIZE(sbus_output), 3, 0);

    // DSM needs 8 repeats, 5 to guess the format, then 3 to pass the RCProtocol 3 frames test
    test_protocol("DSM1", AP_RCProtocol::DEFAULT_SR_CONFIG, dsm_bytes,  sizeof(dsm_bytes),  dsm_output,  ARRAY_SIZE(dsm_output), 9);
    test_protocol("DSM2", AP_RCProtocol::DEFAULT_SR_CONFIG, dsm_bytes2, sizeof(dsm_bytes2), dsm_output2, ARRAY_SIZE(dsm_output2), 9, 16);
    test_protocol("DSM3", AP_RCProtocol::DEFAULT_SR_CONFIG, dsm_bytes3, sizeof(dsm_bytes3), dsm_output3, ARRAY_SIZE(dsm_output3), 9, 16);
    test_protocol("DSM4", AP_RCProtocol::DEFAULT_SR_CONFIG, dsm_bytes4, sizeof(dsm_bytes4), dsm_output4, ARRAY_SIZE(dsm_output4), 9, 16);
    test_protocol("DSM5", AP_RCProtocol::DEFAULT_SR_CONFIG, dsm_bytes5, sizeof(dsm_bytes5), dsm_output5, ARRAY_SIZE(dsm_output5), 9);
    test_protocol("DSMX22", AP_RCProtocol::DEFAULT_SR_CONFIG, dsmx22ms_bytes, sizeof(dsmx22ms_bytes), dsmx22ms_output, ARRAY_SIZE(dsmx22ms_output), 9, 16);
    test_protocol("DSMX22_VTX", AP_RCProtocol::DEFAULT_SR_CONFIG, dsmx22ms_vtx_bytes, sizeof(dsmx22ms_vtx_bytes), dsmx22ms_vtx_output, ARRAY_SIZE(dsmx22ms_vtx_output), 9, 16);
    test_protocol("DSMX11", AP_RCProtocol::DEFAULT_SR_CONFIG, dsmx11ms_bytes, sizeof(dsmx11ms_bytes), dsmx11ms_output, ARRAY_SIZE(dsmx11ms_output), 9, 16);
    test_protocol("DSMX11_VTX", AP_RCProtocol::DEFAULT_SR_CONFIG, dsmx11ms_vtx_bytes, sizeof(dsmx11ms_vtx_bytes), dsmx11ms_vtx_output, ARRAY_SIZE(dsmx11ms_vtx_output), 9, 16);

    test_protocol("FPORT", AP_RCProtocol::FPORT_SR_CONFIG, fport_bytes, sizeof(fport_bytes), fport_output, ARRAY_SIZE(fport_output), 3, 0);
    test_protocol("FPORT2_16CH", AP_RCProtocol::FPORT_SR_CONFIG, fport2_16ch_bytes, sizeof(fport2_16ch_bytes), fport2_16ch_output, ARRAY_SIZE(fport2_16ch_output), 3, 0);
    test_protocol("FPORT2_24CH", AP_RCProtocol::FPORT_SR_CONFIG, fport2_24ch_bytes, sizeof(fport2_24ch_bytes), fport2_24ch_output, ARRAY_SIZE(fport2_24ch_output), 3, 0);
}
