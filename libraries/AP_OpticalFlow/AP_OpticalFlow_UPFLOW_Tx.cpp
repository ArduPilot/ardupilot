 ...
 */
 ...
 AP_OpticalFlow_UPFLOW_Tx *AP_OpticalFlow_UPFLOW_Tx::_singleton = nullptr;
 ...
 {
    _singleton = this;
    _tof_data.distance_mm = 0;
    _tof_data.quality = 0;
    _tof_data.last_update_ms = 0;
    _tof_data.valid = false;
 }
 ...
 void AP_OpticalFlow_UPFLOW_Tx::update(void)
 {
     ...
     //Update the TOF data regardless of whether the optical flow is valid or not
     update_tof_data();
     ...
     state.surface_quality = updata.opt_quality;
 ...
 void AP_OpticalFlow_UPFLOW_Tx::update_tof_data()
{
    WITH_SEMAPHORE(_tof_sem);
    
    // Update TOF data
    _tof_data.distance_mm = updata.ground_distance; 
    _tof_data.quality = updata.tof_quality;
    _tof_data.last_update_ms = AP_HAL::millis();
    _tof_data.valid = (updata.tof_quality > 0);
}

 // Static interface function - For RangeFinder to call
bool AP_OpticalFlow_UPFLOW_Tx::get_tof_data(Uplx_TOF &tof_data)
{
    if (_singleton == nullptr) {
        return false;
    }
    
    WITH_SEMAPHORE(_singleton->_tof_sem);
    
    // Check whether the data is outdated
    const uint32_t now = AP_HAL::millis();
    if (now - _singleton->_tof_data.last_update_ms > 300) {
        return false;
    }
    
    // Copy data
    tof_data = _singleton->_tof_data;
    return _singleton->_tof_data.valid;
}
