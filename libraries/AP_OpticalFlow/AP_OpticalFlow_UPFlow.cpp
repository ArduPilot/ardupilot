...
/*
   driver for UPIXELS optical flow sensor
   UPFlow serial packet description
   from https://discuss.ardupilot.org/t/cheerson-cx-of-low-cost-optical-flow-sensor-testing/35598/90
   ...
   Code almost entirely based on the CXOF driver by Randy Mackay
 */
...
void AP_OpticalFlow_UPFlow::update(void)
{
    ...
    while (nbytes-- > 0) {
        ...
        if (buf_len == 0) {
            ...
            // if buffer has 14 items try to decode it
            if (buf_len >= UPFLOW_FRAME_LENGTH) {
                // check last character matches footer
                if (buf[buf_len-1] != UPFLOW_FOOTER) {
                    buf_len = 0;
                    continue;
                }

                // decode package
                int16_t x_raw = (int16_t)((uint16_t)buf[2] << 8) | buf[3];
                int16_t y_raw = (int16_t)((uint16_t)buf[4] << 8) | buf[5];
    ...
    // calculate dt
    uint64_t this_frame_us = uart->receive_time_constraint_us(UPFLOW_FRAME_LENGTH);
    if (this_frame_us == 0) {
