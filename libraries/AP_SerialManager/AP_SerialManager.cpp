...
#elif HAL_SENSORHUB_ENABLED
#define SERIAL5_PROTOCOL SerialProtocol_SENSORHUB
#define SERIAL5_BAUD AP_SERIALMANAGER_SENSORHUB_BAUD/1000
#else
    ...
    for (uint8_t i=1; i<SERIALMANAGER_NUM_PORTS; i++) {
        if (state[i].uart != nullptr) {
            switch (state[i].protocol) {
                ...
                case SerialProtocol_SENSORHUB:
                    state[i].baud = AP_SERIALMANAGER_SENSORHUB_BAUD / 1000;
                    state[i].uart->begin(map_baudrate(state[i].baud),
                                         AP_SERIALMANAGER_SENSORHUB_BUFSIZE_RX,
                                         AP_SERIALMANAGER_SENSORHUB_BUFSIZE_TX);
                    break;
#endif
