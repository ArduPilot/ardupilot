/*
  ...
  Trimmed to AES-256 ECB single-block operation only. See tiny_aes.h.
...
#define Nb 4
// The number of 32 bit words in a key (AES-256).
#define Nk 8
// The number of rounds in AES Cipher (AES-256).
#define Nr 14

// state - array holding the intermediate results during decryption.
typedef uint8_t state_t[4][4];

// The lookup-tables are marked const so they can be placed in read-only memory.
...
static void KeyExpansion(uint8_t* RoundKey, const uint8_t* Key)
{
    ...
    for (i = Nk; i < Nb * (Nr + 1); ++i) {
        ...
        // AES-256 applies SubWord to the word at offset Nk/2 of each key block.
        if (i % Nk == 4) {
            {
                tempa[0] = getSBoxValue(tempa[0]);
                tempa[1] = getSBoxValue(tempa[1]);
...
// The SubBytes Function Substitutes the values in the
// state matrix with values in an inverse S-box.
