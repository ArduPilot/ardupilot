...
setup_radio(uint8_t argc, const Menu::arg *argv)
{
    ...
    if(g.channel_azimuth.radio_in < 500) {
        while(1) {
    ...
    g.channel_azimuth.radio_min                = g.channel_azimuth.radio_in;
    g.channel_elevation.radio_min              = g.channel_elevation.radio_in;
    g.rc_3.radio_min = g.rc_3.radio_in;
    g.rc_4.radio_min = g.rc_4.radio_in;

    g.channel_azimuth.radio_max                = g.channel_azimuth.radio_in;
    g.channel_elevation.radio_max              = g.channel_elevation.radio_in;
    ...
    g.rc_3.radio_trim = g.rc_3.radio_in;
    ...
    while(1) {
        ...
        g.channel_azimuth.update_min_max();
        g.channel_elevation.update_min_max();
...
setup_flightmodes(uint8_t argc, const Menu::arg *argv)
{
    ...
    cliSerial->printf_P(PSTR("\nMove RC toggle switch to each position to edit, move azimuth stick to select modes."));
    print_hit_enter();
    trim_radio();
...
print_radio_values()
{
    cliSerial->printf_P(PSTR("CH1: %d | %d | %d\n"), (int)g.channel_azimuth.radio_min, (int)g.channel_azimuth.radio_trim, (int)g.channel_azimuth.radio_max);
    cliSerial->printf_P(PSTR("CH2: %d | %d | %d\n"), (int)g.channel_elevation.radio_min, (int)g.channel_elevation.radio_trim, (int)g.channel_elevation.radio_max);
...
radio_input_switch(void)
{
    ...
    if (int16_t(g.channel_azimuth.radio_in - g.channel_azimuth.radio_trim) > 100) {
        bouncer = 10;
    }
    if (int16_t(g.channel_azimuth.radio_in - g.channel_azimuth.radio_trim) < -100) {
        bouncer = -10;
