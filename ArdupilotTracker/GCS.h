...
//    void        init(AP_HAL::BetterStream *port);
	...
	// int32_t numMessagesReceived;
	bool detected;
};

// This class can distribute data streams to multiple interpreters.
// It is also responsible for the switch to CLI at beginning.
class Datacomm_Class_Multiple: public Datacomm_Class {
public:
//    void        init(AP_HAL::BetterStream *port);
	void update(void);
	// call to reset the timeout window for entering the cli
	// void 		reset_cli_timeout();
	void addInterpreter(Datacomm_Interpreter* interpreter) {
		interpreters[numInterpreters++] = interpreter;
	}
private:
	uint8_t numInterpreters;
	Datacomm_Interpreter* interpreters[4];
#if CLI_ENABLED == ENABLED
    // this allows us to detect the user wanting the CLI to start
    uint8_t crlf_count;
#endif
...
///
class GCS_MAVLINK: public Datacomm_Interpreter {
public:
	GCS_MAVLINK();
	void update(void);
...
class ArduTrackerDataInterpreter: public Datacomm_Interpreter {
public:
	// Format is :
	// !!!PAN:XXXX,TLT:YYYY
	enum ParseState {
		PANTTILT_PARSE_EXC0 = 0,
		PANTTILT_PARSE_LIT0 = 3,
		PANTTILT_PARSE_COL0 = 6,
		PANTTILT_PARSE_PAN0 = 7,
		PANTTILT_PARSE_COMMA0 = 11,
		PANTTILT_PARSE_LIT1 = 12,
		PANTTILT_PARSE_COL1 = 15,
		PANTTILT_PARSE_TLT0 = 16,
		PANTTILT_PARSE_END = 20
	};
