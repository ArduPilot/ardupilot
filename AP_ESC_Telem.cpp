...
const AP_Param::GroupInfo AP_ESC_Telem::var_info[] = {
    ...
    // @Param: _MAX_ERR
    // @DisplayName: ESC Telemetry max available error count
    // @Description: If esc error count larger than this valu ,will trigger alarm ,0:disable.
    // @Units: count
    // @Increment: 1
    // @Range: 0 100
    // @User: Standard
    AP_GROUPINFO("_MAX_ERR", 2, AP_ESC_Telem, error_count_max, 30),

    // @Param: _SRV2ESC
    // @DisplayName: Servo info send by ESC Telemetry.
    // @Description: If have servo info, send info by esc info msg.
    // @Values: 0:Disabled, 1:Enabled(for A6/2), -1:Avoid(for copter)
    // @User: Standard
    //AP_GROUPINFO("_SRV2ESC", 3, AP_ESC_Telem, servo2esc, -1),

    // @Param: _MAX_TEMP
    // @DisplayName: ESC Telemetry max available temperature
    // @Description: If esc temperature over this valu ,will trigger alarm ,0:disable.
    // @Units: C
    // @Increment: 1
    // @Range: 1 200
    // @User: Standard
    AP_GROUPINFO("_MAX_TEMP", 3, AP_ESC_Telem, max_acceptable_temperature, 150),
    ...
    // @Range: 100 10000
    // @User: Standard
    AP_GROUPINFO("_TIMEOUT", 4, AP_ESC_Telem, esc_timeout, 300),
...
void AP_ESC_Telem::send_esc_telemetry_mavlink(uint8_t mav_chan)
{
    ...
    for (uint8_t idx = 0; idx < num_idx; idx++) {
        ...
        for (uint8_t j = 0; j < 4; j++) {
            ...
            if(_telem_data[esc_id].motor_temp_cdeg == 426 /*&& servo2esc == 1*/){//@yu
                s.temperature[j] = _telem_data[esc_id].temperature_cdeg;//回傳就是度了
                s.voltage[j] = _telem_data[esc_id].voltage * 100;
                s.current[j] = _telem_data[esc_id].current * 100; 
                float rpmf = 0.0f;
                if (get_rpm(esc_id, rpmf)) {
                    s.rpm[j] = constrain_float(rpmf, 0, UINT16_MAX);//@yu 由servo position帶入
                }
                ...
                float rpmf = 0.0f;
                if (get_rpm(esc_id, rpmf)) {
                    s.rpm[j] = constrain_float(fabs(rpmf), 0, UINT16_MAX);//@yu rpm取絕對值
                }
                s.count[j] = _telem_data[esc_id].count;
            }

            if(s.temperature[j] > (int16_t)max_acceptable_temperature){//過熱時紀錄；因紀錄速度可能不同，所以用加的
                over_heat_esc_index[esc_id]++;
                _have_over_heat = true;
            }
        }
    ...
    if(_have_over_heat && AP_HAL::millis() - over_heat_alarm_delay_count > 1000){//1秒1次
        char status_msg[150]; // 用來存儲狀態訊息
        char temp[50]; // 暫時字串

        snprintf(status_msg, sizeof(status_msg), "ESC ");
        
        for(int w = 0;w < 16;w++){
            if (over_heat_esc_index[w] > 0) {
                snprintf(temp, sizeof(temp), "%i ,",w+1);//從1開始
                strncat(status_msg, temp, sizeof(status_msg) - strlen(status_msg) - 1);
                over_heat_esc_index[w] = 0;//歸零
            }
        }

        if (status_msg[strlen(status_msg) - 1] == ',') {//消去最後面的','
        status_msg[strlen(status_msg) - 1] = ' ';
        }

        snprintf(temp, sizeof(temp), " OVER HEAT");
        strncat(status_msg, temp, sizeof(status_msg) - strlen(status_msg) - 1);

        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "%s",status_msg);
        over_heat_alarm_delay_count = AP_HAL::millis();
        _have_over_heat = false;
    }
...
void AP_ESC_Telem::update_rpm(const uint8_t esc_index, const float new_rpm, const float error_rate)
{
    ...
    rpmdata.rpm = abs(new_rpm);//MP無法顯示負值
    ...
    if(error_count_max > 0 && error_rate > error_count_max){//@yu to alarm user esc maybe not healthy
        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "ESC[%u] error count too large, up to: %.1f time", (esc_index + 1),error_rate);
    }
    ...
    for (uint8_t i = 0; i < ESC_TELEM_MAX_ESCS; i++) {
        ...
        if (logger && logger->logging_enabled()) {
            if (_telem_data[i].last_update_ms != _last_telem_log_ms[i]
                ...
                uint32_t esc_update_duty = _telem_data[i].last_update_ms - _last_telem_log_ms[i];//@yu esc data 更新的週期
                if(esc_update_duty > 100000){//超過100秒的就給0
                    esc_update_duty = 0;
                }
                //if(!(esc_log_neglect_mask & (1 << i))){//跳過開機後第一筆，應～該～可以避掉出現奇怪的數值
                //    esc_update_duty = 100;
                //    esc_lost_need_alarm_mask |= (1 << i);
                //}
                ...
                //   new:ecs update duty         //origin:(current_tot is in milli-Ampere hours)
                ...
                const struct log_Esc pkt{
                    ...
                    current_tot : (float)esc_update_duty,//@yu 將更新間隔植入這個值
        ...
        if(esc_timeout > 0){
            if(AP_HAL::millis() - _telem_data[i].last_update_ms > (uint32_t)esc_timeout && _telem_data[i].last_update_ms != 0){//啟動條件
                esc_lost_need_alarm_mask |= (1 << i);//用遮罩儲存有問題的編號
            }
        }else{
            esc_lost_need_alarm_mask = 0;//esc_timeout <= 0則不啟用
        }
    }

    if(AP_HAL::millis() - esc_lost_alarm_ms > 1000 && esc_lost_need_alarm_mask != 0){//警告1秒1次
        char status_msg[150]; // 用來存儲狀態訊息
        char temp[50]; // 暫時字串
        ...
        for(uint8_t i = 0; i < ESC_TELEM_MAX_ESCS; i++){
            if (esc_lost_need_alarm_mask & (1 << i)) {
                snprintf(temp, sizeof(temp), "%i ,",i+1);//從1開始
                strncat(status_msg, temp, sizeof(status_msg) - strlen(status_msg) - 1);
            }
        }
        ...
        snprintf(temp, sizeof(temp), " is Timeout");
        strncat(status_msg, temp, sizeof(status_msg) - strlen(status_msg) - 1);

        GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "%s",status_msg);
        esc_lost_alarm_ms = AP_HAL::millis();
        esc_lost_need_alarm_mask = 0;//通報完才歸0
    }
}
