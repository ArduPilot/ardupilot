...
bool Tracker::servo_test_set_servo(uint8_t servo_num, uint16_t pwm)
{
    ...
    if (servo_num == CH_YAW) {
        channel_yaw.set_radio_out(constrain_value<int16_t>(pwm, channel_yaw.get_radio_min(), channel_yaw.get_radio_max()));
        channel_yaw.output();
    }

    // set pitch servo pwm and send output to servo
    if (servo_num == CH_PITCH) {
        channel_pitch.set_radio_out(constrain_value<int16_t>(pwm, channel_pitch.get_radio_min(), channel_pitch.get_radio_max()));
