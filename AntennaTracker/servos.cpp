...
void Tracker::init_servos()
{
	...
	g.channel_mode.set_range(0,1000);
...
{
	//pitch limits in full degrees
	int32_t pitch_limit = g.pitch_range/2;

	float constrained_pitch;

	if ((enum PitchServoType)g.pitch_servo_type.get() == PITCH_SERVO_TYPE_CR)
		constrained_pitch = constrain_float(pitch, g.cr_pitch_min, g.cr_pitch_max);
	else
		constrained_pitch = constrain_float(pitch, -pitch_limit, pitch_limit);

	switch ((enum PitchServoType)g.pitch_servo_type.get()) {
	case PITCH_SERVO_TYPE_ONOFF:
		update_pitch_onoff_servo(constrained_pitch);
		break;
	case PITCH_SERVO_TYPE_CR:
		update_pitch_cr_servo(constrained_pitch);
		break;
	case PITCH_SERVO_TYPE_POSITION:
	default:
		update_pitch_position_servo(pitch);	//Position servo will be limited by position settings.
		break;
	}
...
void Tracker::update_pitch_cr_servo(float pitch)
{
	...
	//find pitch correction error in centi-degrees
...
{
	switch ((enum YawServoType)g.yaw_servo_type.get()) {
	case YAW_SERVO_TYPE_ONOFF:
		update_yaw_onoff_servo(yaw);
		break;

	case YAW_SERVO_TYPE_CR:
		update_yaw_cr_servo(yaw);
		break;

	case YAW_SERVO_TYPE_POSITION:
	default:
