...
#define AP_FOLLOW_TIMEOUT_MS    5000    // position estimate timeout after 1 second //@yu 3000 -> 5000
...
const AP_Param::GroupInfo AP_Follow::var_info[] = {
    ...
    // @Range: 1 1000
    // @User: Standard
    AP_GROUPINFO("_DIST_MAX", 5, AP_Follow, _dist_max, 0),//@yu 0 for neglect
    ...
    // @Param: _RARSID
    // @DisplayName: RARS's system mavlink id
    // @Description: RARS's system mavlink id
    // @Values: 1 255 ,0:disable
    // @User: Standard
    AP_GROUPINFO("_RARS_ID", 12, AP_Follow, _rarsid, 0),

    // @Param: _RARS_LACT
    // @DisplayName: RARS lost action
    // @Description: The action that if RARS is lost
    // @Values: 0:change to origin sysid ,1:wait for rars back
    // @User: Standard
    AP_GROUPINFO("_RARS_LACT", 13, AP_Follow, _rars_lost_act, 0),
    ...
    // @Values: 0:change to origin sysid ,1:wait for rars back
    // @User: Standard
    AP_GROUPINFO("_RARS_RADIUS", 14, AP_Follow, _rars_work_radius, 0),

    // @Param: _TIMEOUT
    // @DisplayName: Follow timeout
    // @Description: The follow's timeout set by user, if zero, use defult 5000ms
    // @Units: s
    // @Range: 0 100
    // @User: Standard
    AP_GROUPINFO("_TIMEOUT", 15, AP_Follow, _custom_timeout, 0),
...
bool AP_Follow::get_target_location_and_velocity(Location &loc, Vector3f &vel_ned) const
{
    ...
    // check for timeout
    uint32_t new_timeout;//use custom timeout
    if(_custom_timeout > 0){
        new_timeout = _custom_timeout * 1000;
    }else{
        new_timeout = AP_FOLLOW_TIMEOUT_MS;
    }
    if ((_last_location_update_ms == 0) || (AP_HAL::millis() - _last_location_update_ms > new_timeout)) {
        return false;
    }
...
bool heart_msg = 0;
bool gps_fix_msg = 0;
int last_mode_update_ms = 0;
int last_gps_fix_type_update_ms = 0;
int last_heartbeat_update_ms = 0;
int last_gps_update_ms = 0;
...
void AP_Follow::handle_msg(const mavlink_message_t &msg)
{
    ...
    switch (msg.msgid) {
    case MAVLINK_MSG_ID_HEARTBEAT: {//MAVlink MSG id:0
        mavlink_heartbeat_t packet;
        mavlink_msg_heartbeat_decode(&msg, &packet);
        heart_msg = true;
        if(packet.custom_mode ==10){//in plane/quadplane, mode id:10 is auto mode
            _is_target_in_auto = true;
            last_mode_update_ms = AP_HAL::millis();
        }
        last_heartbeat_update_ms = AP_HAL::millis();
        break;
    }/**/
    case MAVLINK_MSG_ID_GPS_RAW_INT: {//MAVlink MSG id:24
        mavlink_gps_raw_int_t packet;
        mavlink_msg_gps_raw_int_decode(&msg, &packet);
        gps_fix_msg = true;
        _target_fix_type = packet.fix_type;
        if(packet.fix_type > 2){//fix type >= 3, gps is good
            _is_target_gps_fix = true;
            last_gps_fix_type_update_ms = AP_HAL::millis();
        }
        last_gps_update_ms = AP_HAL::millis();
        break;
    }
    case MAVLINK_MSG_ID_GLOBAL_POSITION_INT: {//MAVlink MSG id:33
        // decode message
        ...
        _target_alt = packet.alt/1000;//mm to meter
    ...
    case MAVLINK_MSG_ID_FOLLOW_TARGET: {//MAVlink MSG id:144
        // decode message
...
//@yu return true if target is auto mode
bool AP_Follow::is_target_in_auto(void) const
{
    if(AP_HAL::millis() - last_mode_update_ms > AP_FOLLOW_SYSID_TIMEOUT_MS){
        return false;
    }
    return true;
}
//@yu return true if have target's mode
bool AP_Follow::have_heart_msg(void) const
{
    if(AP_HAL::millis() - last_heartbeat_update_ms > 2000){
        return false;
    }
    return true;
}
//@yu return true if have target's gps fix type
bool AP_Follow::have_gps_msg(void) const
{
    if(AP_HAL::millis() - last_gps_update_ms > 10000){
        return false;
    }
    return true;
}
//@yu return true if target's gps is good
bool AP_Follow::is_target_gps_fix_good(void) const
{
    if(AP_HAL::millis() - last_gps_fix_type_update_ms > AP_FOLLOW_SYSID_TIMEOUT_MS){
        return false;
    }
    return true;
}
//
bool AP_Follow::rars_is_use(void) const
{
    if(_rarsid <= 0){
        return false;
    }
