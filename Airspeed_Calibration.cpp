...
//int ratio_range_type = 0;//@yu 變化範圍的類型, 0:not define,1:range < 4,2:range < 9
/*
...
void AP_Airspeed::update_calibration(uint8_t i, const Vector3f &vground, int16_t max_airspeed_allowed_during_cal)
{
    ...
    if(state[i].last_saved_ratio > 4.0f && state[i].ratio_range_type == 0){
        state[i].ratio_range_type = 2;
    }else{
        state[i].ratio_range_type = 1;
    }
    float ratio;
    if(state[i].ratio_range_type == 2){
        ratio = constrain_float(param[i].ratio, 1.0f, 9.0f);//@yu 若起始值大於4，則最大值由 4 -> 9
    }else{
        ratio = constrain_float(param[i].ratio, 1.0f, 4.0f);
    }
    ...
    if(state[i].ratio_range_type == 2){
        zratio = constrain_float(zratio, 0.3f, 1.0f);// 1.0 ~ 9.0
    }else{
        zratio = constrain_float(zratio, 0.5f, 1.0f);
    }
    param[i].ratio.set(1/sq(zratio));
    if (state[i].counter > 60) {
        /*if(is_over_range){//啟用大範圍時僅通知，不儲存
            if (state[i].last_saved_ratio > 1.05f*param[i].ratio ||
                state[i].last_saved_ratio < 0.95f*param[i].ratio){
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Airspeed %u ratio reset: %f", i + 1 , static_cast<double> (param[i].ratio));
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Please check");
                param[i].ratio.save();
