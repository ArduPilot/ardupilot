...
public:
    ...
    mutable uint32_t follow_time_ms;//@yu for jerry test
    ...
    // @yu ti function 
    // return true if target is auto mode
    bool is_target_in_auto() const;
    // return true if have target's heartbeat msg
    bool have_heart_msg() const;
    // return  true if target's gps fix type >= 3
    bool is_target_gps_fix_good() const;
    // return true if have target's gps msg
    bool have_gps_msg() const;
    // return target altitude
    int32_t get_target_alt() const { return _target_alt; }
    // return target gps fix time
    uint8_t get_target_gps_fix_type() const { return _target_fix_type; }
    //function for RARS
    // return true if rars is use (rarsid > 0)
    bool rars_is_use() const;
    // get rars's system id
    uint8_t get_rars_sysid() const { return _rarsid.get(); }
    // get rars's lost actn
    uint8_t get_rars_lost_act() const { return _rars_lost_act.get(); }
    // get radius rars change system ID
    float get_rars_work_radius() const { return _rars_work_radius.get(); }
...
private:
    ...
    AP_Int16    _rarsid;            // RARS's mavlink system id (0 not use)
    AP_Int16    _rars_lost_act;     // the action if rars signal is lost
    AP_Float    _rars_work_radius;  // the radius that rars will be work
    AP_Int32    _custom_timeout;    // the follow timeout set by user, if some system's response isn't immediate, this might be useful 
    ...
    bool _is_target_in_auto;        // @yu true if target is in auto mode(10)
    bool _is_target_gps_fix;        // @yu true if target gps fix is good
    int32_t _target_alt;            // @yu target altitude
    uint8_t _target_fix_type;       // @yu target gps fix  type
