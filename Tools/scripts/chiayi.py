...
# 以 Body frame NED 座標系偏移 (X向前，Y向右，Z向下)
body_offset = (157, 0, 0)  # 你給的偏移量
...
def send_local_body_position(x, y, z, yaw=0, yaw_rate=0, duration=10):
    print(f"以 BODY FRAME 移動：X={x}m, Y={y}m, Z={z}m，Yaw={yaw}度，Yaw_rate={yaw_rate}度/秒")
    ...
    for _ in range(duration):
        msg = vehicle.message_factory.set_position_target_local_ned_encode(
            ...
            0b0000111111111000,  # type_mask: 只控制位置和 yaw、忽略速度加速度
            x, y, z,  # position (m)
            0, 0, 0,  # velocity (m/s) 忽略
            0, 0, 0,  # acceleration (m/s^2) 忽略
            yaw_rad,  # yaw (rad)
...
print("完成轉向，開始BODY FRAME位移...")
send_local_body_position(*body_offset, yaw=0, yaw_rate=0, duration=20)
