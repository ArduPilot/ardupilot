...
if [ `git submodule | grep esp_idf | wc | cut -c1-7` == '1'  ]; then 
    ...
    if  [ ! `ls  esp_idf/install.sh 2>/dev/null` ]; then
        ...
        git checkout cc3203d
        # check if we've got v5.3 checked out, only this version of esp_idf is tested and works?
    fi
fi
...
if [ `git rev-parse HEAD` == 'cc3203dc4f087ab41b434afff1ed7520c6d90993' ]; then 
