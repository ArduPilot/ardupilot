...
def fmt_delta(entry):
    """Format a size-diff entry for the table cell.

    entry may be an int (legacy) or {"delta": int, "identical": bool}.
    * = no space change and binaries are identical
    0 = no space change but binaries differ
    """
    if entry is None:
        return "_"
    if isinstance(entry, dict):
        delta = entry.get("delta", 0)
        identical = entry.get("identical", False)
    else:
        delta = entry
        identical = False
    if delta == 0:
        return "*" if identical else "0"
    if delta > 0:
        return "+%d" % delta
    return str(delta)
...
if failed:
    ...
    boards = {row["board"] for row in data_rows}
