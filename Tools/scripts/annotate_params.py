...
def get_xml_data(base_url: str, directory: str, filename: str) -> ET.Element:
    ...
    if os.path.isfile(file_path):
        ...
        try:
            ...
            logging.error(
                "Please install it by running 'pip install requests' in your terminal."
            )
...
def create_doc_dict(root: ET.Element, vehicle_type: str) -> Dict[str, Any]:
    ...
    for param in root.findall(".//param"):
        ...
        values = {
            value.get("code"): value.text for value in param.findall("values/value")
        }
...
def format_columns(
    values: Dict[str, Any], max_width: int = 105, max_columns: int = 4
) -> List[str]:
    """
...
def extract_parameter_name_and_validate(line: str, filename: str, line_nr: int) -> str:
    ...
    param_sep = line[
        param_len
    ]  # the character following the parameter name must be a separator
    if param_sep not in {",", " ", "\t"}:
        logging.error(
            "Invalid parameter name %s on line %d in file %s",
            param_name,
            line_nr,
            filename,
        )
    ...
    if param_len > PARAM_NAME_MAX_LEN:
        logging.error(
            "Too long parameter name on line %d in file %s", line_nr, filename
        )
...
def update_parameter_documentation(
    doc: Dict[str, Any], target: str = ".", sort_type: str = "none"
) -> None:
    """
    ...
    if os.path.isfile(target):
        ...
        param_files = glob.glob(os.path.join(target, "*.param")) + glob.glob(
            os.path.join(target, "*.parm")
        )
    else:
    ...
    for param_file in param_files:
        ...
        if total_params == documented_params:
            logging.info(
                "Read file %s with %d parameters, all got documented",
                param_file,
                total_params,
            )
        else:
            logging.warning(
                "Read file %s with %d parameters, but only %s of which got documented",
                param_file,
                total_params,
                documented_params,
            )
            logging.warning(
                "No documentation found for: %s", ", ".join(undocumented_params)
            )
...
def main():
    ...
    try:
        xml_dir = (
            ...
            else os.path.dirname(os.path.realpath(args.target))
        )
        xml_root = get_xml_data(
            BASE_URL + args.vehicle_type + "/", xml_dir, PARAM_DEFINITION_XML_FILE
        )
