...
1. Conceptual flow (Source: [Rhythm Chopra / LinkedIn](https://www.linkedin.com/posts/rhythm-chopra-923667102_authentication-security-opensource-activity-7035557389210644480-Hs3B))
   ...
   ![Overall Conceptual Flow](images/basic_sequence.jpeg)

2. Basic flow with the management server

   ![Aerobridge Trusted Flight basic flow](images/aerobridge_trusted_flight_basic_flow.png)
...
4. Flow with management server
   ...
   ![Trusted Flight flow with Aeribridge Management Server](images/aerobridge_trusted_flight.png)

## Pre-requisites

The protection provided by the Trusted Flight layer relies entirely on the security of the hardware. **_The firmware needs to be trusted and signed_** to prevent any tampering with the Trusted Flight validation workflow and to ensure the root of trust.
...
1. Build and upload firmware to CubeOrange
   ...
   ```bash
   ./Tools/scripts/TrustedFlight/generate_key_and_token.py /tmp/trusted_flight
   ./waf clean
   ./waf configure --board CubeOrange --trusted-flight-issuer=test.cname --trusted-flight-key=/tmp/trusted_flight/key.pub
   ./waf --targets bin/arducopter --upload
   ```

2. Upload `/tmp/trusted_flight/token` to `/APM/trusted_flight` directory on CubeOrange
3. Arm the vehicle.
...
- For technical introduction see these two blog posts:
  - [A journey to offline JWT authentication](https://medium.com/@rhythm8/a-journey-to-offline-jwt-authentication-ebc7859f0246)
  - [Using certificate chain of trust to verify JWT offline](https://medium.com/@rhythm8/using-certificate-chain-of-trust-to-verify-jwt-offline-ab3d4c3f0322)
