...
from utils import aerobridgetools
...
parser.add_argument("-o", "--operation_id", type=str, help="Specify a Aerobridge Flight Operation ID,"
                    "e.g. try 3408bce9-dbab-4665-abfc-8ea03b0ad871")
...
async def run(operation_id):
    # This is the main function to execute the preflight checks, for loading the operation that is stored in Aerobridge.
    # Aerobridge issues permissions as a JWT token that is downloaded.
    my_authorization_helper = aerobridgetools.AuthorityCredentialsGetter(
        client_id=env.get('AEROBRIDGE_CLIENT_ID', None),
        client_secret=env.get('AEROBRIDGE_CLIENT_SECRET', None),
        audience=env.get('AEROBRIDGE_AUDIENCE', None),
        base_url=env.get('PASSPORT_URL'),
        token_endpoint=env.get('PASSPORT_TOKEN_ENDPOINT'),
        jwks_endpoint=env.get('PASSPORT_JWKS_ENDPOINT'))
    ...
    # Get the Auth Server related data: Public Key of the Auth server and Full Chain PEM details of the
    # Auth server that is sent to the vehicle

    # Created Trusted Flight Directory on the vehicle
    logging.info("Creating 'trusted_flight' directory on the board..")
    trusted_flight_directory = "/APM/trusted_flight"
    try:
        await vehicle.ftp.create_directory(trusted_flight_directory)
        logging.info("Trusted flight directory successfully created!")
    except FtpError as e:
        if e._result.result != FtpResult.Result.FILE_EXISTS:
            raise
        logging.info("Trusted flight directory already exists, continuing")
    ...
    try:
        ...
        async for _ in vehicle.ftp.upload(pem_file.name, trusted_flight_directory):
            pass
        logging.info("PEM File uploaded successfully")
    finally:
        pem_file.close()
        os.unlink(pem_file.name)
        logging.debug("PEM file successfully deleted")

    # Create a Aerobridge Client
    my_aerobridge_client = aerobridgetools.AerobridgeClient(aerobridge_url=env.get('AEROBRIDGE_URL'),
    ...
    if auth_server_full_chain_query.status_code == 200:
        # For more information see: https://redocly.github.io/redoc/?url=https://raw.githubusercontent.com/openskies-sh
        # /aerobridge/master/api/aerobridge-1.0.0.resolved.yaml#tag/Credentials/operation/listAuthServerFullChain
        auth_server_full_chain_url = auth_server_full_chain_query.json()['binary_file_url']
    else:
    ...
    if full_chain_download_query.status_code == 200:
        try:
            ...
            # send file to drone
            async for _ in vehicle.ftp.upload(full_chain_file.name, trusted_flight_directory):
                pass
        finally:
            full_chain_file.close()
            os.unlink(full_chain_file.name)
    else:
        logging.error("Error in writing full chain data to file")
        exit()
    ...
    # Download flight permission, its basically a JWT token
    ...
    if operation_permission.status_code == 200:
        ...
        print("Flight permission for operation %s could not be retrieved (HTTP %s)" %
              (operation_id, operation_permission.status_code))
        exit()
    ...
    # Create a file to hold permission token.
    permission_token = permission_data['token']
    if not isinstance(permission_token, str):
        logging.error("Unexpected permission token type %s" % type(permission_token).__name__)
        exit()
    auth_token_dir = tempfile.mkdtemp(prefix='aerobridge_trusted_flight')
    auth_token_path = os.path.join(auth_token_dir, TOKEN_FILE)
    try:
        with open(auth_token_path, 'w') as f:
            f.write(permission_token)
    except OSError as e:
        logging.error("Error in writing auth token to file %s" % e)
        exit()
    else:
        # send file to drone
        async for _ in vehicle.ftp.upload(auth_token_path, trusted_flight_directory):
            pass
    finally:
        shutil.rmtree(auth_token_dir, ignore_errors=True)
...
if __name__ == '__main__':
    ...
    if not operation_id:
        print("A valid Operation ID (UUID) from your Aerobridge instance must be provided before arming the drone with a"
              "mission, e.g. try -o 3408bce9-dbab-4665-abfc-8ea03b0ad871")
        exit()
    ...
    except (Exception, KeyboardInterrupt) as e:  # noqa: BLE001  top-level handler: report, don't traceback
        print('ERROR', str(e))
        exit()
