...
class AuthorityCredentialsGetter():
    """
    All calls to your Aerobridge instance requires credentials from a oauth server,
    in this case this is Flight Passport OAUTH server, this class gets the token and the associated public key.

    Attributes:
        client_id : str
            OAUTH client id for the client credentials token
        client_secret : str
            OAUTH client secret for the client credentials token client as is set in the OAUTH server
        audience : str
    ...
    Methods:
        get_credentials():
            Executes a client credentials grant request to the auth server to get a JWT token
        get_public_key():
    ...
    def __init__(self, client_id: str, client_secret: str, audience: str, base_url: str,
                 token_endpoint: str, jwks_endpoint: str):
    ...
    def get_credentials(self):
        """
        Executes the Client Credentials Grant

        If the argument 'additional' is passed, then it is appended after the main info.

        Returns: JSON with access token if the request is successful,
                 if it is unsuccessful the JSON returned a error message is displayed
        """
        payload = {
            ...
            "scope": 'aerobridge.read aerobridge.write'
            }
        url = self.token_url
    ...
    def get_public_key(self):
        ...
        Returns: JSON in the JWKS format
        """
        url = self.jwks_url
...
class AerobridgeClient():
    """
    This a a Python client that make calls to the Aerobridge API and returns data.
    It requires the requests package and the json module.
    ...
    def ping_aerobridge(self):
        ...
        Returns:
            hearbeat_response (str): Pong if the server is running properly
            https://redocly.github.io/redoc/?url=https://raw.githubusercontent.com/openskies-sh/aerobridge
            /master/api/aerobridge-1.0.0.resolved.yaml#/paths/~1ping/get
        '''
    ...
    def get_auth_server_fullchain_url(self):
        '''
        This method returns full chain for the auth server

        Returns:
            full_chain_url_details (json): A link with the full chain PEM file of the domain of the auth server
            that issues the permission tokens for more information see:
            https://redocly.github.io/redoc/?url=https://raw.githubusercontent.com/openskies-sh/aerobridge
            /master/api/aerobridge-1.0.0.resolved.yaml#tag/Credentials/operation/listAuthServerFullChain
        '''
    ...
    def download_flight_permission(self, operation_id: UUID):
        ...
        Returns:
            plan_details (json): Details of a flight plan, see
            https://redocly.github.io/redoc/?url=https://raw.githubusercontent.com/openskies-sh/aerobridge/
            master/api/aerobridge-1.0.0.resolved.yaml#operation/retrieveFlightPlan
        '''
    ...
    def upload_flight_log(self, operation_id: UUID, raw_log: str):
        ...
        Returns:
            log_details (json): Details of the newly created flight log, see
            https://redocly.github.io/redoc/?url=https://raw.githubusercontent.com/openskies-sh/aerobridge
            /master/api/aerobridge-1.0.0.resolved.yaml#operation/createFlightLog
        '''
    ...
    def get_aircraft_by_flight_controller_id(self, registered_flight_module_id: str):
        ...
        Returns:
            aircraft_detail (json): Details of an aircraft, see
            https://redocly.github.io/redoc/?url=https://raw.githubusercontent.com/openskies-sh/aerobridge
            /master/api/aerobridge-1.0.0.resolved.yaml#operation/Get%20Single%20Aircraft%20(RFM%20ID)
        '''
    ...
    def get_firmware_by_flight_controller_id(self, registered_flight_module_id: str):
        ...
        Returns:
            firmware_detail (json): Details of an aircraft firmware, see
            https://redocly.github.io/redoc/?url=https://raw.githubusercontent.com/openskies-sh/aerobridge
            /master/api/aerobridge-1.0.0.resolved.yaml#operation/Get%20Aircraft%20firmware%20by%20RFM%20ID
        '''
