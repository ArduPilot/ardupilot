...
for t in $CI_BUILD_TARGET; do
    ...
    if [ "$t" == "CubeOrange-TrustedFlight" ]; then
        ...
        sudo apt-get install -y python3-dev
        python3 -m pip install pymonocypher==3.1.3.2
        # generate a throwaway key pair and use during build
        ...
        $waf configure --board CubeOrange --trusted-flight-issuer=test.cname --trusted-flight-key=${BUILDROOT}/key.pub
        $waf clean
        $waf copter
        $waf plane
        continue
