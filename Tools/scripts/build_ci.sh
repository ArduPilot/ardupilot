...
for t in $CI_BUILD_TARGET; do
    ...
    if [ "$t" == "CubeOrange-TrustedFlight" ]; then
        echo "Building CubeOrange-TrustedFlight"
        sudo apt-get update
        sudo apt-get install -y python3-dev
        python3 -m pip install pymonocypher==3.1.3.2
        # generate a throwaway key pair and use during build (also creates ${BUILDROOT})
        Tools/scripts/TrustedFlight/generate_key_and_token.py ${BUILDROOT}
        # generate a throwaway firmware signing key: trusted flight is only meaningful with signed firmware
        ./Tools/scripts/signing/generate_keys.py ${BUILDROOT}/testkey

        # key path and token issuer defined in Tools/scripts/TrustedFlight/utils/constants.py
        $waf configure --board CubeOrange --trusted-flight-issuer=test.cname --trusted-flight-key=${BUILDROOT}/key.pub --signed-fw --private-key ${BUILDROOT}/testkey_private_key.dat
        $waf clean
        $waf copter
        $waf plane
        continue
