...
VEHICLE_TYPES = ["Copter", "Plane", "Rover", "Sub", "Tracker"]
TAG_VEHICLES = {'Sub': ('Sub', 'ArduSub')}
PARAMETER_VEHICLES = {'Sub': 'ArduSub'}
...
def get_vehicle_tags(repository: Path, vehicle_type: str):
    ...
    tag_vehicles = TAG_VEHICLES.get(vehicle_type, (vehicle_type,))
    tags_output = '\n'.join(
        subprocess.check_output(['git', '-C', repository, 'tag', '--list', f'{tag_vehicle}-*'], text=True)
        for tag_vehicle in tag_vehicles
    )
    releases = {}
    for tag in tags_output.splitlines():
        match = VERSION_RE.fullmatch(tag)
        if match is None or match.group('vehicle') not in tag_vehicles:
            continue
        version = match.group('version')
        # Sub releases may have both Sub-* and ArduSub-* tags.  Prefer the
        # first tag family listed in TAG_VEHICLES and process each version once.
        releases.setdefault(version, (version_key(version), tag))
    return [(tag, version) for version, (_, tag) in sorted(releases.items(), key=lambda item: item[1][0])]
...
def publish_file(source: Path, destination: Path):
    ...
    # Keep staging files outside the published vehicle tree so an interrupted
    # run cannot leave files that are subsequently uploaded by rsync.
    staging_dir = destination.parent.parent.parent
    fd, temporary_name = tempfile.mkstemp(prefix='.apm.pdef-', dir=staging_dir)
    try:
...
def create_one_pdef_xml_file(repository: Path, vehicle_type: str, destination: Path, git_tag: str):
    ...
    with tempfile.TemporaryDirectory(prefix='pdef-metadata-') as temporary_dir:
        ...
        try:
            ...
            parameter_vehicle = PARAMETER_VEHICLES.get(vehicle_type, vehicle_type)
            command = [sys.executable, parser_path, '--vehicle', parameter_vehicle, '--format', 'xml']
            if supports_git_sha:
...
def sync_to_remote(vehicle_dir: Path, password_file: Path, username: str):
    ...
    rsync_command = [
        ...
        f'{vehicle_dir}/',
        f'{username}@firmware.ardupilot.org::param_versioned/{vehicle_dir.name}/',
    ]
...
def main():
    ...
    parser.add_argument('--rsync-username',
                        help='Username used with --upload')
    ...
    if args.upload and args.rsync_username is None:
        parser.error('--rsync-username is required with --upload')
    ...
    for vehicle_type in vehicle_types:
        ...
        for git_tag, version in get_vehicle_tags(repository, vehicle_type):
            ...
            if major == 3:
                continue
            if major == 4 and minor == 0 and vehicle_type != 'Sub':
                continue
            ...
            generated_any = True
        if args.upload and generated_any:
            sync_to_remote(destination_vehicle_dir, args.rsync_password_file.resolve(), args.rsync_username)
