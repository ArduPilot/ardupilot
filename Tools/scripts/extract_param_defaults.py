...
def parse_arguments(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Extracts parameter default values from an ArduPilot .bin log file."
    )
    parser.add_argument(
        "-f",
        "--format",
...
def sort_params(
    defaults: Dict[str, float], sort_type: str = "none"
) -> Dict[str, float]:
    """
    ...
    if sort_type == "missionplanner":
        defaults = dict(
            sorted(defaults.items(), key=lambda x: missionplanner_sort(x[0]))
        )
...
def output_params(
    ...
    if format_type == "qgcs":
        ...
        if sysid < 0:
            raise SystemExit(
                "Invalid system ID parameter %i must not be negative" % sysid
            )
        if sysid > MAVLINK_SYSID_MAX - 1:
            raise SystemExit(
                "Invalid system ID parameter %i must be smaller than %i"
                % (sysid, MAVLINK_SYSID_MAX)
            )
        if compid < 0:
            raise SystemExit(
                "Invalid component ID parameter %i must not be negative" % compid
            )
        if compid > MAVLINK_COMPID_MAX - 1:
            raise SystemExit(
                "Invalid component ID parameter %i must be smaller than %i"
                % (compid, MAVLINK_COMPID_MAX)
            )
