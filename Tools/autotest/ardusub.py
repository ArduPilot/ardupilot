...
class AutoTestSub(vehicle_test_suite.TestSuite):
    ...
    def TemperatureSensorDevID(self):
        ...
        # TEMP2_PIN only exists once the analog backend has been created
        self.reboot_sitl()
        ...
        self.assert_parameter_value('TEMP2_DEV_ID', (1 << 8) | (5 << 16))

        self.progress("Moving sensor to an address nothing responds on")
        self.set_parameters({
            'TEMP1_ADDR': 0x45,
            'TEMP2_PIN': -1,      # disabled, so no reading is possible
        })
        self.reboot_sitl()
