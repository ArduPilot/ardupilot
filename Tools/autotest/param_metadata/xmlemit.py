...
'''
        ...
        self.firmware_element = None
        if self.git_sha is not None or self.git_tag is not None:
            self.firmware_element = etree.SubElement(self.paramfile, 'firmware')
    ...
    def close(self):
        # Populate firmware element with optional build metadata
        if self.firmware_element is None and (self.git_sha is not None or self.git_tag is not None):
            self.firmware_element = etree.Element('firmware')
            self.paramfile.insert(0, self.firmware_element)
        if self.firmware_element is not None:
            if self.git_sha is not None:
                self.firmware_element.set('git_sha', self.git_sha)
            if self.git_tag is not None:
                self.firmware_element.set('git_tag', self.git_tag)
