...
#            lastlog = int(mavproxy.match.group(3))
            ...
            if numlogs != expected_numlogs:
                raise NotAchievedException("Expected numlogs want=%u got=%u" %
                                           (expected_numlogs, numlogs))
            if log_num != expected_log_num:
                raise NotAchievedException("Unexpected log_num want=%u got=%u" %
                                           (expected_log_num, log_num))
            if size <= 0:
                raise NotAchievedException("Expected positive log size got=%u" %
                                           (size,))
