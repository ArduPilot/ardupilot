...
#            self.remove_bin_logs() # can't do this as one of the binlogs is probably open for writing by the SITL process.  If we force a rotate before running tests then we can do this.  # noqa
    ...
    def SITLCompassCalibration(self, compass_count=3, timeout=1000):
        ...
        def do_test_mag_cal(mavproxy, params, compass_tnumber):
            ...
            # MAG_CAL_BAD_FITNESS.
            # TODO(mavlink-bump): drop getattr fallback after MAG_CAL_BAD_FITNESS
            # lands in pymavlink.
            MAG_CAL_BAD_FITNESS = getattr(mavutil.mavlink, 'MAG_CAL_BAD_FITNESS', 10)
            while True:
                if self.get_sim_time_cached() - tstart > timeout:
                    raise NotAchievedException("Cannot receive enough MAG_CAL_PROGRESS")
                ...
                if m.get_type() == "MAG_CAL_REPORT":
                    if report_get[m.compass_id] == 0:
                        ...
                        if m.cal_status == MAG_CAL_BAD_FITNESS:
                            report_get[m.compass_id] = 1
                        else:
                            raise NotAchievedException("Expected MAG_CAL_BAD_FITNESS (10), got %u" % m.cal_status)
            ...
            if compass_tnumber > 1 and target_mask == 0:
                ...
                try:
                    ...
                    # calibrator sets BAD_OFFSETS (8) instead of FAILED (5).
                    # TODO(mavlink-bump): drop getattr fallback after
                    # MAG_CAL_BAD_OFFSETS lands in pymavlink.
                    MAG_CAL_BAD_OFFSETS = getattr(mavutil.mavlink, 'MAG_CAL_BAD_OFFSETS', 8)
                    if report_status[0] != MAG_CAL_BAD_OFFSETS:
                        raise NotAchievedException(
                            "Expected compass 0 to report MAG_CAL_BAD_OFFSETS (8), got %u" %
                            report_status[0]
                        )
