...
from utils.constants import JWT_ALG  # noqa:E402
from utils.constants import JWT_TYPE  # noqa:E402
from utils.constants import PUBLIC_KEY_FILE  # noqa:E402
from utils.constants import TOKEN_FILE  # noqa:E402
from utils.constants import TOKEN_ISSUER  # noqa:E402
...
class TokenNameMappings(str, Enum):
    ...
    token_with_short_signature = 'short_signature'
    ...
    token_with_array_claim = 'array_claim'
...
class TrustedFlightTestsWrapper:
    ...
    def __call__(self, func):
        ...
        def wrapper(*args, **kwargs):
            token_path = f'{self.token_storage_path()}/{TOKEN_FILE}'
            try:
                if self._token_file is not None:
                    # copy token for trusted flight test
                    shutil.copy(f'{self.test_artifacts_directory()}/{self._token_file.value}', token_path)
                func(*args, **kwargs)
            finally:
...
class AutoTestTrustedFlight(AutoTestCopter):
    ...
    def __init__(self, *args, **kwargs):
        if importlib.util.find_spec("monocypher") is None:
            raise ImportError(
                "Trusted flight tests need monocypher package.\n"
                "Try: python3 -m pip install pymonocypher==3.1.3.2"
            )
        from generate_key_and_token import create_invalid_token
        ...
        def _prepare_test_artifacts(artifacts_node):
            with open(os.devnull, 'w') as null, contextlib.redirect_stdout(null), contextlib.redirect_stderr(null):
                ...
                with open(f'{artifacts_node}/valid', 'rb') as f:
                    header, payload, signature = f.read().split(b'.')
                with open(f'{artifacts_node}/short_signature', 'wb') as f:
                    f.write(b'.'.join([header, payload, signature[:4]]))
                ...
                # multi-element array claims need a JSON node per element
                create_token(pk, f'{artifacts_node}/array_claim', payload={
                    'iss': TOKEN_ISSUER,
                    'iat': datetime.now(timezone.utc),
                    'exp': datetime.now(timezone.utc) + timedelta(minutes=5),
                    'aud': ['a', 'b', 'c', 'd'],
                    'nested': {'roles': ['pilot', 'observer'], 'level': 2},
                })
                ...
                # token larger than AP_TrustedFlight::max_file_size (2048 bytes)
                with open(f'{artifacts_node}/oversized', 'wb') as f:
                    f.write(b'a' * (16 * 1024))
        ...
        shutil.copy(f'{TrustedFlightTestsWrapper.test_artifacts_directory()}/valid.key',
                    f'{TrustedFlightTestsWrapper.token_storage_path()}/{PUBLIC_KEY_FILE}')
    ...
    def arm_fail(self, fail_msg, force=False):
        self.context_push()
        try:
            self.wait_ready_to_arm(check_prearm_bit=False)
            self.assert_arm_failure(f'Arm: TrustedFlight: {fail_msg}', force=force)
        finally:
    ...
    def MissingPublicKey(self):
        '''Trusted Flight init fails when public key is missing'''
        key_path = f'{TrustedFlightTestsWrapper.token_storage_path()}/{PUBLIC_KEY_FILE}'
        if os.path.exists(key_path):
            os.unlink(key_path)
        try:
            self.reboot_sitl()
            self.prearm_fail('Not initialised')
        finally:
            shutil.copy(f'{TrustedFlightTestsWrapper.test_artifacts_directory()}/valid.key', key_path)
            self.reboot_sitl()

    def MissingTokenIssuer(self):
        '''Trusted Flight init fails when token issuer is missing'''
        issuer_path = f'{TrustedFlightTestsWrapper.token_storage_path()}/token_issuer'
    ...
    def MissingToken(self):
        ...
        self.prearm_fail('Token validation pending')

    @TrustedFlightTestsWrapper(token_file=TokenNameMappings.oversized_token)
    def OversizedToken(self):
        '''Token file larger than the maximum accepted size is rejected'''
        self.prearm_fail('Token validation pending')
    ...
    @TrustedFlightTestsWrapper(token_file=TokenNameMappings.token_with_short_signature)
    def TokenWithShortSignature(self):
        '''Token whose signature decodes to fewer than 64 bytes'''
    ...
    def TokenSignedWithDifferentKeyForceArm(self):
        ...
        self.arm_fail('Invalid token signature', force=True)

    @TrustedFlightTestsWrapper(token_file=TokenNameMappings.valid_token)
    def StaleValidationResult(self):
        '''Stale token validation status after expiration'''
        self.wait_ready_to_arm()
        # simulate artificial delay when updating validation status
        self.set_parameter("SIM_TF_PAUSE", 1)
        try:
            # stale threshold is two validation periods (2s)
            self.delay_sim_time(5, reason="cached validation result to go stale")
            self.prearm_fail('Validation result is stale')
        finally:
            self.set_parameter("SIM_TF_PAUSE", 0)

    @TrustedFlightTestsWrapper(token_file=TokenNameMappings.valid_token)
    def TokenSwapUpdatesValidationResult(self):
        '''Validation result updated after token changes'''
        self.wait_ready_to_arm()
        # copy invalid token after successful validation
        shutil.copy(f'{TrustedFlightTestsWrapper.test_artifacts_directory()}/{TokenNameMappings.invalid_token.value}',
                    f'{TrustedFlightTestsWrapper.token_storage_path()}/{TOKEN_FILE}')
        self.prearm_fail('Invalid token signature')

    @TrustedFlightTestsWrapper(token_file=TokenNameMappings.valid_token)
    def TokenWithRtcUnavailable(self):
        '''Valid token but RTC unavailable'''
        old_rtc_types = self.get_parameter("BRD_RTC_TYPES")
        self.set_parameter("BRD_RTC_TYPES", 0)
        try:
            self.reboot_sitl()
            self.prearm_fail('RTC not available')
        finally:
            self.set_parameter("BRD_RTC_TYPES", old_rtc_types)
            self.reboot_sitl()

    @TrustedFlightTestsWrapper(token_file=TokenNameMappings.token_with_array_claim)
    def TokenWithArrayClaim(self):
        '''Valid token carrying multi-element array and nested object claims'''
        self.arming_successful()
    ...
    @TrustedFlightTestsWrapper(token_file=TokenNameMappings.invalid_token)
    def ArmingRequireZero(self):
        '''Arm with ARMING_REQUIRE=0 and an invalid token'''
        self.set_parameter("ARMING_REQUIRE", 0)
        self.arm_fail('Invalid token signature')

    def tests(self):
        return [
            self.MissingPublicKey,
            self.MissingTokenIssuer,
            self.MissingToken,
            self.OversizedToken,
            ...
            self.StaleValidationResult,
            self.TokenSwapUpdatesValidationResult,
            self.TokenWithRtcUnavailable,
            self.TokenWithArrayClaim,
            self.ArmingRequireZero,
            self.ValidToken
        ]
