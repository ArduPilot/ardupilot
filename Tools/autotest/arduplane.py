...
#            self.context_clear_collection("STATUSTEXT")
    ...
    def ExternalAHRSWindEstimate(self):
        ...
        self.wait_ready_to_arm()
        # the estimate must not be reported valid on the ground: the wind
        # triangle is only meaningful once the vehicle is moving through
        # the air, so a stationary vehicle - even one rotated during
        # accelcal - produces no estimate.  Regression: it used to latch
        # a meaningless near-zero wind as valid before any real flight:
        self.wait_message_field_values('NAMED_VALUE_FLOAT', {
            "name": "WINDVALID",
            "value": 0,
        }, timeout=20, minimum_duration=10)
        self.takeoff(70)  # default wind sim wind is a sqrt function up to 60m
        self.change_mode('LOITER')
