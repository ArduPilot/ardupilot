...
class AutoTestRover(vehicle_test_suite.TestSuite):
    ...
    def PARAM_ERROR(self):
        ...
        self.start_subtest("Non-existant parameter (get)")
        self.context_push()
        ...
        self.start_subtest("Non-existant parameter (get-by-id)")
        self.context_push()
        self.context_collect('PARAM_ERROR')
        self.install_messageprinter_handlers_context(['PARAM_ERROR'])
        non_existant_param_index = 32764  # hopefully, anyway....
        self.mav.mav.param_request_read_send(
            self.sysid_thismav(),
            1,
            bytes("", 'ascii'),
            non_existant_param_index
        )
        self.assert_received_message_field_values('PARAM_ERROR', {
            "target_system": 250,
            "target_component": 250,
            "param_id": '',
            "param_index": non_existant_param_index,
            "error": mavutil.mavlink.MAV_PARAM_ERROR_DOES_NOT_EXIST,
        }, check_context=True, very_verbose=True)
        self.context_pop()

        self.start_subtest("Non-existant parameter (set)")
        self.context_push()
        self.context_collect('PARAM_ERROR')
