...
optparse.OptionParser.parse_args when a parse error occurs; raise a
detailed exception which can be caught"""
...
#        progress("pids for (%s): %s" % (victim,",".join([ str(p) for
#        p in pids])))
        for apid in pids:
...
def kill_tasks():
    """Clean up stray processes by name.  This is a the shotgun approach"""
