    ...
    for key, value in supplementary_test_binary_map.items():
        if step.startswith(key):
            ...
            for supplementary_test_binary in value:
                ...
                # note that the instance-number field is ignored: an
                # instance number allocates real machine resources, so
                # the test framework derives each supplementary
                # peripheral's instance from the suite's own instance
