...
#        self.wait_statustext("SIM Hit ground", timeout=timeout)
    ...
    def WPArcs(self):
        ...
        while True:
            ...
            if delta > timeout:
                raise AutoTestTimeoutException("Failed to achieve position within %fs" %
                                               (timeout,))
            if now - last_print_time > 1:
                self.progress("Waiting for current long (%f) > desired long (%f) (%.2fs remaining before timeout)" % (current_long, desired_long, timeout - delta))
                last_print_time = now

            if current_wp > 16:
                raise NotAchievedException("mission progressed beyond WP 16 before current longitude > desired long (%f)" % desired_long )

            if (current_long > desired_long):
                 self.progress("Success: Copter traveled far enough east whilst traveling to WP 16 that we are likely flying a WP Arc")
                 break
