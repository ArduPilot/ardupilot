...
#        self.takeoffAndMoveAway()
    ...
    def LoggerFreeSpaceAtStartup(self):
        ...
        # AP_Logger's MB_to_B. Leave ~100 MB free: below the 400 MB
        # LOG_FILE_MB_FREE the boot-time Prep_MinSpace trims to, but
        # above the in-flight trigger (10% of that, 40 MB), so only
        # Init() on the following boot can remove anything. Nothing
        # is written with LOG_DISARMED=0, so free space stays there.
        log_file_mb_free = 400
        sim_disk_max_mb = int(seed_bytes / 1e6) + 100
        self.set_parameters({
