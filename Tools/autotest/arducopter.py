...
class AutotestCopter(Autotest):
    ...
    def fly_battery_failsafe(self, timeout=30):
        ...
        self.mavproxy.send("param set FS_BATT_ENABLE 1\n")

        # trigger low voltage
        self.mavproxy.send('param set SIM_BATT_VOLTAGE 10\n')

        # wait for LAND mode
        new_mode = self.wait_mode('LAND')
        if new_mode == 'LAND':
            success = True

        # disable battery failsafe
        self.mavproxy.send('param set FS_BATT_ENABLE 0\n')
