...
#        self.takeoffAndMoveAway()
    ...
    def MountAuxFunctionAtBoot(self):
        ...
        losing it against a backend which does not exist yet"""
        self.setup_servo_mount()
        self.set_parameters({
    ...
    def CameraAuxFunctionAtBoot(self):
        """test the camera applies its aux switch position once its backends
        exist, rather than losing it against a backend which does not yet.

        The vehicle boots in throttle failsafe so the pending one-shot is held
        until RC is recovered - well after the log file opens.  That makes the
        initialisation AUXF deterministic, where a normal boot can dispatch it
        before AP_Logger will accept writes and drop it, and it exercises the
        camera side of the failsafe gate at the same time."""
        self.set_parameters({
    ...
    def MountAuxFunctionAtBootRCFailsafe(self):
        """test a receiver sitting in failsafe does not have its failsafe
        switch positions applied at boot.

        A receiver in failsafe still streams frames with in-range PWM, so the
        switch reads cleanly; only RC_Channels::has_valid_input() knows the
        difference.  Applying the boot position is therefore held pending
        until that is true, rather than taken at the backend's first update."""
        self.setup_servo_mount()
        self.set_parameters({
            "RC7_OPTION": 27,       # RETRACT_MOUNT1
            "RC7_REVERSED": 1,
        ...
        self.set_rc(3, 1500)    # leave the failsafe for the context to unwind

    def AuxFunctionAtBootWithoutRC(self):
        """test that a switch which cannot be read is not applied to a backend
        from an invented position, and is still picked up once RC comes up"""
        self.setup_servo_mount()
        self.set_parameters({
            "RC7_OPTION": 27,       # RETRACT_MOUNT1
            "RC7_REVERSED": 1,
