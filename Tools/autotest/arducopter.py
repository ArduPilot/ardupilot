...
#        self.takeoffAndMoveAway()
    ...
    def MountTopotekNetwork(self):
        ...
        # the simulated gimbal listens on this port on the host, so it
        # must differ between concurrently-running instances or they
        # bind each other's gimbal
        port = self.topotek_gimbal_port()
        self.set_parameters({
            "MNT1_TYPE": 12,      # Topotek
            "CAM1_TYPE": 4,       # Mount
            "NET_ENABLE": 1,
...
""")
    ...
    def PeriphMultiUARTTunnel(self):
        ...
        try:
            ...
            # periph_tunnel_mcast_port().  The serial link to it is UDP
            # multicast, shared machine-wide by default, so it needs a
            # port of its own too or other suites' traffic appears on it.
