...
ESC_APD_Telem::ESC_APD_Telem(void) { }

void ESC_APD_Telem::init(AP_HAL::UARTDriver *_uart, float num_poles) {
    uart = _uart;
    uart->begin(115200);
        ...
        if ((size_t)len == sizeof(received.packet)) {
            ...
            if (received.packet.stop == 65535) {
                ...
                if (crc_fletcher16(received.bytes, 18) == received.packet.checksum) {
                    ...
                    decoded.current = le16toh(received.packet.bus_current) * (1 / 12.5);
...
float ESC_APD_Telem::convert_temperature(uint16_t raw) const {
    ...
    temperature += 1 / (nominal_temperature + C_TO_KELVIN); // + (1/To)
