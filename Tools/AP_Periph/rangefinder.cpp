...
void AP_Periph_FW::can_rangefinder_update(void)
{
    ...
    // We only allow continuos probing for rangefinders while vehicle is disarmed. Probing is currently inefficient and leads to longer loop times.
    ...
    for (uint8_t i = 0; i <= rangefinder.num_sensors(); i++) {
        ...
        AP_RangeFinder_Backend *backend = rangefinder.get_backend(instance);
        if (backend == nullptr) {
            continue;
        }
        ...
        if (last_rangefinder_sample_ms[instance] == sample_ms) {
            // don't same the same reading again
            continue;
        }
        last_rangefinder_sample_ms[instance] = sample_ms;
