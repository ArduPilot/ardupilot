...
/* Fast-reboot signature in a register that survives a warm reset, which is what
 * lets reboot-to-bootloader work without BOOT0.
 *
 * The register is SoC-specific and MUST be guarded. These addresses are the
 * RT1176's SNVS block; on an STM32H743 nothing is mapped at 0x40C90000, and
 * because AP_Bootloader.cpp calls check_fast_reboot() as the first thing it
 * does, an unguarded read takes a precise data bus fault before the console
 * exists. Measured on CubeOrangeZephyr silicon: BFAR 0x40C9010C, fatal reason
 * 25 (K_ERR_ARM_BUS_PRECISE_DATA_BUS), dead board with no output.
 */
#if defined(CONFIG_SOC_SERIES_IMXRT11XX)
...
/* No warm-reset-persistent store wired up for this SoC yet.
 *
 * Returning zero means check_fast_reboot() reports RTC_BOOT_OFF, so the
 * bootloader behaves exactly as it does after a cold boot: it waits its normal
 * window for an upload and then jumps to the application. What is lost is
 * reboot-to-bootloader on request - a missing feature, not a fault.
 *
 * The STM32 equivalent is an RTC backup register (AP_HAL_ChibiOS uses
 * RTC->BKP0R). Implementing it here needs the backup domain clocked and write
 * protection lifted first, which the bootloader does not currently do, so it
 * is left for a change that can be tested on the board rather than guessed at.
 */
uint32_t board_get_rtc_signature(void)
{
    return 0;
...
#endif  // CONFIG_SOC_SERIES_IMXRT11XX
