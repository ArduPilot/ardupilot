    ...
    if [ "$PACKAGE" == "wxpython" ]; then
        ...
        case ${RELEASE_CODENAME} in
            focal)
                ...
                time $PIP install $PIP_USER_ARGUMENT -U -f $WXPYTHON_WHEEL_REPO $PACKAGE -c $PIP_CONSTRAINTS_FILE
                ;;
            jammy)
                echo "##### Adding wxpython wheel repository for faster installation"
            ...
            *)
                ...
                time $PIP install $PIP_USER_ARGUMENT -U $PACKAGE -c $PIP_CONSTRAINTS_FILE
                ;;
        esac
    else
        time $PIP install $PIP_USER_ARGUMENT -U $PACKAGE -c $PIP_CONSTRAINTS_FILE
