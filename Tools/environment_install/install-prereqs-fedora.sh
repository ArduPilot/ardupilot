...
DNF="sudo dnf install --setopt=install_weak_deps=False"
if $ASSUME_YES; then
...
#Don't install python3, Fedora already has it anyway
#$DNF python3 || echo "Check dnf output for errors"

#As Fedora can have many python versions, we need to know which one is currently active
PYPKGVER=python$(python3 --version | cut -d' ' -f2 | awk -F. '{printf"%s.%s",$1,$2}')

BASE_PKGS="make automake gcc kernel-devel ccache git axel valgrind screen gcc-c++ xterm free-ttf-fonts SFML-devel zip glibc-static rsync"
#SITL_PKGS_FEDORA below is the only python packages here for fedora that does not follow naming convention. we should to improve the script 
#integration for these packages
SITL_PKGS_FEDORA="python3-pyyaml python3-wxpython4"
SITL_PKGS="${PYPKGVER}-pip ${PYPKGVER}-devel ${PYPKGVER}-setuptools ${PYPKGVER}-wheel ${PYPKGVER}-lxml ${PYPKGVER}-pyparsing ${PYPKGVER}-opencv ${PYPKGVER}-numpy ${PYPKGVER}-scipy ${PYPKGVER}-matplotlib"
...
$DNF $BASE_PKGS $SITL_PKGS $SITL_PKGS_FEDORA || echo "Check dnf output for errors"
