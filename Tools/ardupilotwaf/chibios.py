...
def chibios_firmware(self):
    ...
    if self.env.EMBED_SECURE_BOOTLOADER:
...
def generate_hwdef_h(env):
    ...
    if env.EMBED_SECURE_BOOTLOADER:
        print("WARNING:Embedding Secure Bootloader")
        env.EMBED_SECURE_BOOTLOADER_OPTION="--embed-secure-bootloader"
    else:
        env.EMBED_SECURE_BOOTLOADER_OPTION=""
    ...
    cmd = "{0} '{1}' -D '{2}' '{3}' {4} --params '{5}' {6} {7}".format(python, hwdef_script, hwdef_out, env.HWDEF, env.BOOTLOADER_OPTION, env.DEFAULT_PARAMETERS, env.SECURE_OPTION,env.EMBED_SECURE_BOOTLOADER_OPTION)
...
def build(bld):
    ...
    bld(
        ...
        rule="%s '${AP_HAL_ROOT}/hwdef/scripts/chibios_hwdef.py' -D '${BUILDROOT}' '%s' %s --params '%s' %s %s" % (
            bld.env.get_flat('PYTHON'), bld.env.HWDEF, bld.env.BOOTLOADER_OPTION, bld.env.default_parameters, 
            bld.env.SECURE_OPTION, bld.env.EMBED_SECURE_BOOTLOADER_OPTION),
