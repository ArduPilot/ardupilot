...
def pre_build(self):
    ...
    esp_idf = self.cmake(
            ...
            cmake_bld='esp-idf_build_s3',
            )

    esp_idf_showinc = esp_idf.build('showinc', target='esp-idf_build_s3/includes.list')
    ...
    class load_generated_includes(Task.Task):
        ...
        def run(tsk):
            ...
            includes = bld.bldnode.find_or_declare('esp-idf_build_s3/includes.list').read().split()
            #print(includes)
            bld.env.prepend_value('INCLUDES', includes)

    tsk = load_generated_includes(env=self.env)
    tsk.set_inputs(self.path.find_resource('esp-idf_build_s3/includes.list'))
...
def esp32_firmware(self):
    ...
    build = esp_idf.build('all', target='esp-idf_build_s3/ardupilot.bin')
    build.post()
    ...
    #                                          src='esp-idf_build_s3/ardupilot.bin')
