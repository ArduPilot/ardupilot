...
class sitl_embox(Board):
    ...
    def configure_env(self, cfg, env):
        super(sitl_embox, self).configure_env(cfg, env)
        cfg.env.STATIC_LINKING = True
        env.DEFINES.update(
            CONFIG_HAL_BOARD = 'HAL_BOARD_SITL',
            CONFIG_HAL_BOARD_SUBTYPE = 'HAL_BOARD_SUBTYPE_NONE',
        ...
        env.CFLAGS.remove('-Werror=shadow')
        env.CFLAGS.remove('-Werror=undef')
        env.CFLAGS += ['-Wno-error=shadow',
                         '-Wno-error=undef'
        ]

        env.CXXFLAGS.remove('-Werror=shadow')
        env.CXXFLAGS.remove('-Werror=undef')
        env.CXXFLAGS += ['-Wno-error=shadow',
                         '-Wno-error=undef'
        ]
        ...
        env.CFLAGS += [f'-I{embox_root_dir}/project/ardupilot/sitl/include/']
