...
class Board:
    ...
    def disable_buggy_compiler_warnings(self, cfg, env):
        ...
        This is called after configure_env() so that the -Werror= flags
        which the board classes append - and which the ChibiOS board
        copies from CFLAGS into CXXFLAGS - can not promote them back;
        the compiler honours whichever comes last.
        '''
        ...
        # -Wformat-truncation is deliberately not in this list; the only
        # report of it was a genuine bug rather than a compiler problem
        buggy_warnings = [
            '-Wno-error=maybe-uninitialized',
            '-Wno-error=array-bounds',
        ]
        env.CFLAGS += buggy_warnings
        env.CXXFLAGS += buggy_warnings
