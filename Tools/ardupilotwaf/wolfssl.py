...
def configure(cfg):
    ...
    env.WOLFSSL_USER_HEADERS = cfg.path.find_node('libraries/AP_Security/wolfssl/').abspath()
...
def build(bld):
    ...
    if bld.env.TOOLCHAIN == 'native':
        wolfssl_source = [bld.path.find_node('libraries/AP_Security/wolfssl/user_settings.h')]
        wolfssl_task = bld(
            #build libwolfssl.a from WolfSSL sources
            rule="cd ${WOLFSSL_ROOT} && ./configure --prefix=${WOLFSSL_BUILDDIR} CFLAGS=\"${EXTRA_CFLAGS} -DWOLFSSL_USER_SETTINGS -I${WOLFSSL_USER_HEADERS} -DWOLFSSL_SITL -I${BUILDROOT} ${HAL_INCLUDE_CFLAGS} -Wno-error=unused-parameter\" --disable-filesystem --disable-shared --enable-sha --enable-keygen --enable-rsa --disable-oldnames && make clean && make && make install",
        ...
        wolfssl_task = bld(
            #build libwolfssl.a from WolfSSL sources
            rule="cd ${WOLFSSL_ROOT} && ./configure --host=arm-none-eabi CC=arm-none-eabi-gcc AR=arm-none-eabi-ar STRIP=arm-none-eabi-strip RANLIB=arm-none-eabi-ranlib --prefix=${WOLFSSL_BUILDDIR} CFLAGS=\"--specs=nosys.specs -mthumb ${EXTRA_CFLAGS} -DWOLFSSL_CHIBIOS -DHAL_DIGITAL_SKY_RFM -DWOLFSSL_USER_SETTINGS -I${WOLFSSL_USER_HEADERS} -I${BUILDROOT}/libraries -I${BUILDROOT} ${HAL_INCLUDE_CFLAGS} ${STM32_HASH} -Wno-error=unused-parameter\" --disable-filesystem --disable-shared --enable-sha --enable-keygen --enable-rsa --disable-oldnames && make clean && make && make install",
            group='dynamic_sources',
            source=wolfssl_source,
            target=bld.bldnode.find_or_declare('lib/libwolfssl.a')
        )
