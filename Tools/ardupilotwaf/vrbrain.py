...
@feature('vrbrain_ap_stlib', 'vrbrain_ap_program')
@before_method('process_source')
def vrbrain_dynamic_env(self):
...
@feature('vrbrain_ap_program')
@after_method('apply_link')
@before_method('process_use')
def vrbrain_import_objects_from_use(self):
...
class vrbrain_copy(Task.Task):
    run_str = '${CP} ${SRC} ${TGT}'
    color = 'CYAN'

    def keyword(self):
        return "VRBRAIN: Copying %s to" % self.inputs[0].name

    def __str__(self):
        return self.outputs[0].path_from(self.generator.bld.bldnode)

class vrbrain_add_git_hashes(Task.Task):
    ...
    def keyword(self):
        return "VRBRAIN: Copying firmware and adding git hashes"
...
def vrbrain_firmware(self):
    global _firmware_semaphorish_tasks, _upload_task
    version = self.env.get_flat('VRBRAIN_VERSION')

    vrbrain = self.bld.cmake('vrbrain')
    vrbrain.vars['APM_PROGRAM_LIB'] = self.link_task.outputs[0].abspath()

    fw_task = self.create_cmake_build_task(
        'vrbrain',
        'build_firmware_vrbrain-v%s' % version,
    )
    ...
    firmware = vrbrain.bldnode.make_node(
        'src/firmware/nuttx/nuttx-vrbrain-v%s-apm.px4' % version,
    )
    fw_elf = vrbrain.bldnode.make_node(
    ...
    git_hashes = self.create_task('vrbrain_add_git_hashes', firmware, fw_dest)
    ...
    cp_elf = self.create_task('vrbrain_copy', fw_elf, fw_elf_dest)
    ...
    if self.bld.options.upload:
        if _upload_task:
            Logs.warn('VRBRAIN: upload for %s ignored' % self.name)
            return
        _upload_task = self.create_cmake_build_task('vrbrain', 'upload')
        _upload_task.set_run_after(fw_task)
        _firmware_semaphorish_tasks.append(_upload_task)

def _vrbrain_taskgen(bld, **kw):
    if 'cls_keyword' in kw and not callable(kw['cls_keyword']):
        cls_keyword = str(kw['cls_keyword'])
        kw['cls_keyword'] = lambda tsk: 'VRBRAIN: ' + cls_keyword
...
def _process_romfs(self):
    ...
    for item in file_list:
        ...
        dst.parent.mkdir()
        self.create_task('vrbrain_copy', src, dst)
...
def configure(cfg):
    ...
    env.AP_PROGRAM_FEATURES += ['vrbrain_ap_program']
    env.AP_STLIB_FEATURES += ['vrbrain_ap_stlib']
    ...
    env.PX4_CMAKE_VARS = dict(
        CONFIG='nuttx_vrbrain-v%s_apm' % env.get_flat('VRBRAIN_VERSION'),
...
def build(bld):
    version = bld.env.get_flat('VRBRAIN_VERSION')
    vrbrain = bld.cmake(
        name='vrbrain',
    ...
    vrbrain.build(
        'prebuild_targets',
        group='dynamic_sources',
        cmake_output_patterns='vrbrain-v%s/NuttX/nuttx-export/**/*.h' % version,
    )

    bld(
        name='vrbrain_romfs_static_files',
        group='dynamic_sources',
        features='_vrbrain_romfs',
    )
...
''' % build_summary.header_text['target'])
    ...
    if not bld.options.upload:
        build_summary.text('')
        build_summary.text('', '''
You can use the option --upload to upload the firmware to the VRBRAIN board if you
have one connected.''')
