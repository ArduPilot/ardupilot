...
class Device():
    ...
    def wait_for_supervisor_state(self, targets_super_state="success", target_app_state="applied"):
        ...
        logger.info(f"Achieved {current_super_status} ==> {targets_super_state} and {current_app_state} ==> {target_app_state}")

    def wait_for_service_status(self, service_name, target_service_status="Running", timeout=10.0, sleep=1.0):
        logger.debug(f"Waiting for {service_name}[{target_service_status}]")
        service = self.get_service(service_name)
        if service is None:
            return False

        current_service_status = service["status"]
        time_begin = time.time()

        while current_service_status != target_service_status:
            time_now = time.time()
            if (time_now - time_begin) > timeout:
                logger.warning(f"Timeout expired while waiting for {service_name}: {current_service_status} ==> {target_service_status}")
                return False
        ...
        logger.info(f"Achieved for {service_name}[{target_service_status}]")
        return True
    ...
    def try_stop_service(self, service_name):
        ...
        return self.wait_for_service_status(service_name, target_service_status="exited")
    ...
    def try_start_service(self, service_name):
        ...
        self.balena.models.device.start_service(None, service["imageId"])
        return self.wait_for_service_status(service_name, target_service_status="Running")
    ...
    def get_service(self, service_name):
        ...
        res = [image for image in status["images"] if image["serviceName"] == service_name]
        if len(res) == 0:
            logger.error(f"Service {service_name} was not found in super state images. Typo in service name?")
            return None
    ...
    def leave_me_alone(self):  # Deprecated?
        status = self.get_supervisor_state_status()
