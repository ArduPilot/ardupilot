...
const AP_Param::GroupInfo QuadPlane::var_info2[] = {
    ...
    // @Param: LAND_REPOS_G
    // @DisplayName: Landing reposition speed gain
    // @Description: If VTOL Landing reposition is use, this will increase moving speed rate
    // @Range: 1.0 5.0
    // @Increment: 0.1
    // @User: Advanced
    AP_GROUPINFO("LAND_REPOS_G", 62, QuadPlane, q_land_repos_gain, 1.0f),
...
void QuadPlane::update_land_positioning(void)
{
    ...
    q_land_repos_gain.set(constrain_value((float)q_land_repos_gain,1.0f,5.0f));//@yu 將移動速度乘上倍率，1.0～5.0
    poscontrol.xy_correction += poscontrol.target_vel_cms.xy() * dt * 0.01*(float)q_land_repos_gain;
...
float QuadPlane::get_land_airspeed(void)
{
    ...
    if (qstate == QPOS_AIRBRAKE) {
        ...
        return plane.aparm.airspeed_min;//@yu 待測試
    }
