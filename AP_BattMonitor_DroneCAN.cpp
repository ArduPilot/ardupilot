...
const AP_Param::GroupInfo AP_BattMonitor_DroneCAN::var_info[] = {
    ...
    // @Param: CURR_REV
    // @DisplayName: Reverse the battery current that FC receive
    // @Description: FC treats positive current as consumption and is enabled if the battery logic is reversed
    // @Values: 0:Disabled, 1:Enabled
    // @User: Advanced
    AP_GROUPINFO("CURR_REV", 31, AP_BattMonitor_DroneCAN, _curr_reverse, 1),
...
{
    if(msg.state_of_charge_pct == 0 && !is_zero(msg.remaining_capacity_wh) && !is_zero(msg.full_charge_capacity_wh)){//@yu 沒soc但有容量的話自己算
        int cal_soc = static_cast<int>((msg.remaining_capacity_wh/msg.full_charge_capacity_wh)*100);//*100%
        update_interim_state(msg.voltage, msg.current, msg.temperature, cal_soc, msg.state_of_health_pct);
    }else{
...
void AP_BattMonitor_DroneCAN::update_interim_state(const float voltage, const float current, const float temperature_K, const uint8_t soc, uint8_t soh_pct)
{
    ...
    if(_curr_reverse){//@yu 啟用時正負對調，將電池放電視為正電流
        _interim_state.current_amps = -1*(_curr_mult * current);
    }else{
...
void AP_BattMonitor_DroneCAN::read()
{
    ...
    if(!_interim_state.healthy && !is_zero(_interim_state.voltage)){//@yu 拔掉後電壓/流歸零
        _interim_state.voltage = 0;
        _interim_state.current_amps = 0;
        _interim_state.temperature = 0;
        _interim_state.state_of_health_pct = 0;
    }
}
